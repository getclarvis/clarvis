...
usable composer, full application hydration, Extension Profile resolution, kernel readiness, and the
...
1. Read `AGENTS.md`, `packages/code/README.md`, `packages/kernel/README.md`,
   ...
   `specs/hosts/code-bootstrap.md`, `specs/hosts/extension-profiles.md`, `specs/hosts/plugins.md`,
...
5. Put marketplace state, global Clarvis state, workspaces, and any copied credentials in explicit
   temporary directories. Keep the user's normal Clarvis state untouched and remove the copies after the
   run.
...
governor or power profile, load, Bun version, artifact, polling interval, and machine are compatible.
...
Create two isolated Extension Profiles with the same artifact and workspace:

- a clean Extension Profile using only `builtin:default`;
- a marketplace-heavy Extension Profile containing every plugin in the selected pinned marketplace revision.
...
Measure and correlate these boundaries in both Extension Profiles:

1. first startup composer and complete app;
2. each plugin snapshot independently;
3. the combined Extension Profile snapshot;
4. kernel readiness;
5. run admission;
...
In an isolated fixture, mutate one manifest, sidecar, and resource after Extension Profile resolution.
...
handle closes and prove it reacquires the same Extension Profile lease. These guard against optimizations
...
3. Verify the exact draft or submission survives the root handoff and starts without a second
   Enter.
4. Repeat with no runnable provider/Agent Profile and verify an accepted startup submission becomes the
...
Record timestamps for composer readiness, Extension Profile resolution, kernel readiness, full app,
...
Include at least one OAuth-backed MCP with no valid token in the isolated Extension Profile. Permit the app to
...
| only plugin-heavy Extension Profile/kernel regresses | contribution discovery, repeated parsing, hashing, qualified projection reuse |
...
canonical snapshot hashing, lazy Extension Profile drift, memory run leases, MCP retained admission, and
...
- clean versus plugin-heavy Extension Profile, kernel, full-app, and real-run timings;
...
- every temporary Extension Profile/root, browser flow, process, and credential copy cleaned up;
