...
| `SAFE-09` | Container Kernel connection         | Destination selection precedes Kernel creation; Docker/Podman serves the public Kernel protocol through attached stdio, proves the same workspace bind and never falls back to Host/Sandbox                                                                                               |
| `SAFE-10` | Native Container domain             | Plans, Memory/indexing, Workflows/leaders and Goals execute and persist in the guest; Tasks, plugins, skills, hooks, MCP and external capability providers fail before side effects                                                                                                         |
| `SAFE-11` | Container recovery and lifecycle    | Steer/cancel use public run controls; close revokes inference and confirms exact process death; reconnect creates a new generation in the same namespace without replaying unknown work; state survives while only the disposable Container is removed                                      |
| `SAFE-12` | Independent base and artifact       | The operator recipe customizes only the pinned base; product changes rebuild the compiled archive, immutable artifact cache hits are revalidated, and the guest cannot invoke or mutate recipe, artifact or engine authority                                                               |
...
| `OPS-05`    | Remote Clarvis over SSH          | Paired flags select the remote workspace; verified key/agent/certificate login carries the full wire with port/agent/X11 forwarding disabled; remote settings/OAuth/state stay remote; reconnect owns one fresh SSH process and no mutation replay; `/background` refuses a handoff that cannot outlive the SSH channel |
