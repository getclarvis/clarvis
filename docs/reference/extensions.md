...
| `marketplaces`   | Git URL array                       | Marketplace catalogs shown by `/extensions/market` |
...
`plugin.json` is tolerant: Clarvis reports unknown fields but does not act on them. Only `name` is
required, and it must equal the plugin directory name.
...
| `mcpServers`            | MCP server map                                  | Plugin-provided servers                                    |
...
Conventional contribution directories are `agents/` and `skills/`. If the manifest contributes no
hooks, Clarvis also reads `hooks/hooks.json`.
