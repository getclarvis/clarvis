...
tools. Sandbox can read host-visible files but writes only to admitted roots. A shell guard uses `"guard": { "type": "shell", "mode": "on" }`; other valid modes are `auto` and `off`. A saved provider or isolation change may need reconnect before it affects a run.
