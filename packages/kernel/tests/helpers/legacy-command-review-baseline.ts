import { callFacts } from "../../src/guard/command-facts.ts";
...
import { GUARD_REVIEW_AGENT_INSTANCE_ID } from "./legacy-reviewer-policy.ts";
    ...
    const messages: Message[] = [
      ...
      ...(cfg.guidance
        ? [
            {
              role: "user" as const,
              content: JSON.stringify({ guidance: cfg.guidance }),
            },
          ]
        : []),
      {
    ...
    try {
      const result = await callReviewerWithTrace(
        deps.llm,
        {
          ...
          timeoutMs: cfg.timeout_ms ?? JUDGE_DEFAULTS.timeoutMs,
          maxRetries: cfg.max_retries ?? JUDGE_DEFAULTS.maxRetries,
