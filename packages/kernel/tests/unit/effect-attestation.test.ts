...
function fixture(
  runChange: Record<string, unknown> = {},
  prChange: Record<string, unknown> = {},
...
describe("closed effect attestation", () => {
  portable.each([
    ...
    const deps = fixture({}, {}, { [name]: "unattested-override" });
  ...
  portable(
    ...
    async () => {
      ...
      try {
        ...
        delete deps.environment.GIT_DIR;
      }
    },
  );
