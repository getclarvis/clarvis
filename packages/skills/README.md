...
The root also exports `enumerateResources` and the bounded skill file/resource byte and character
limits. The kernel uses those exact primitives when hashing a plugin skill snapshot, so identity and
later disclosure walk the same admitted resource surface instead of maintaining a second scanner.
