...
`createMemoryTools`'s test-observed name order (packages/memory/tests/component/tools.test.ts) is
...
(packages/memory/src/tools.ts), and a dedicated test pins `query_memories` ahead of `grep_memories` in that
order (packages/memory/tests/component/tools.test.ts, "ranked search before literal search").

`createMemoryTools(args: CreateMemoryToolsArgs)`'s exported parameter surface (packages/memory/src/tools.ts)
...
`source?: MemoryRevisionSource` (packages/memory/src/tools.ts), defaulting to `{ kind: "tool", tool: <wire name> }`
(an agent editing memory in its own run), versus the per-run indexer's own `{ kind: "indexer",
run_id }` (packages/memory/src/tools.ts); and `intent?: MemoryWriteIntent` (packages/memory/src/tools.ts), defaulting to
`"agent_tool"` (packages/memory/src/tools.ts) — this is exactly what `checkWrite` (§4.5) receives as `intent`,
so it decides which branch of the write policy a caller is judged under. `mutationFence?:
MemoryMutationFence` (packages/memory/src/tools.ts) is the background indexer's queue-claim lease, covered in §4.4.
...
```
`packages/memory/src/toolset.ts`. `defs` maps each `MemoryToolDef` to a `NamespacedTool` with
`fullName === wireName === toolName === t.name` and `mcpName: ""` (packages/memory/src/toolset.ts) — bare wire
names, no MCP namespacing, "same convention as `load_skill`" (packages/memory/src/toolset.ts). `dispatch` on an
unknown name returns `{ isError: true, text: "unknown memory tool '<name>'" }`
(packages/memory/src/toolset.ts) rather than throwing.

### 2.3 `buildMemoryToolsHandler` (`packages/memory/src/handler.ts`)
...
```
Returns a `ToolHandler` whose `matches` is `toolset.names.has(call.name)` (packages/memory/src/handler.ts) and whose
`handle` enforces `toolset.callLimit` with a **per-handler-instance** closure counter `calls`
(packages/memory/src/handler.ts) — confirmed distinct per instance even over a *shared* toolset by
`packages/memory/tests/component/toolset-handler.test.ts`.

`onMutation?(m: Mutation)` (packages/memory/src/handler.ts, importing `isMutatingTool`/`Mutation` from
`./indexer/pyramid.ts`, packages/memory/src/handler.ts) fires **only** when a dispatched call both succeeds
...
| --- | --- | --- |
| `MEMORY_CAPABILITY_NAME` | `"memory"` | packages/memory/src/settings.ts |
| `MEMORY_INGEST_EVENT` | `"ingest"` | packages/memory/src/settings.ts |
| `MEMORY_SETTINGS_FIELDS.memory` | `memoryConfigSchema.optional()` | packages/memory/src/settings.ts |
| `MEMORY_REQUEST_PARAMS.memory` | `z.enum(["on","off"]).optional()`, custom error `"memory must be 'on' or 'off'"` | packages/memory/src/settings.ts |
| `MemorySettingsBlock` | `z.input<typeof memoryConfigSchema>` (pre-defaults; `enabled` optional) | packages/memory/src/settings.ts |
| `memorySettingsSpec` | `{ key: "memory", schema: memoryConfigSchema, merge: "lastWins", pluginContributable: false, requestParams: MEMORY_REQUEST_PARAMS }` | packages/memory/src/settings.ts |

The `memory:` settings block (`memoryConfigSchema`, packages/memory/src/schemas.ts, `.strict()`):
...
`MEMORY_DEFAULTS.history` (packages/memory/src/config.ts) is a sibling default block in the same `as const`
...
Path schemas gating `write_memory`/`edit_memory`/`delete_memory` (packages/memory/src/schemas.ts):
...
`MemoryService` (`packages/kernel/src/memory/memory-service.ts`), implementing the protocol
interface at `packages/protocol/src/memory.ts`:
...
| `jobs(filter?)` | `{ jobs: MemoryJob[]; counts }` | `limit` clamped to `MAX_LIMIT = 100` (packages/kernel/src/memory/memory-service.ts) |
...
absent or resolves nothing (packages/kernel/src/memory/memory-service.ts). `mapMemoryFailure` (packages/kernel/src/memory/memory-errors.ts)
...
(`invalid_request`) respectively (packages/kernel/src/memory/memory-errors.ts); anything else falls through to
`toKernelError`.

Two mechanisms are common to every method rather than incidental to one. `guard(context, fn)`
(packages/kernel/src/memory/memory-service.ts) wraps every method body in a `try/catch`, resolving `mem()` and funnelling
any thrown error through `mapMemoryFailure` — it is the one place a package error becomes a tagged
kernel exception. `toJob(j: MemoryIndexJob)` (packages/kernel/src/memory/memory-service.ts) is the wire projection every
...
### 3.1 `MemoryToolResult` (`packages/memory/src/types.ts`), `MemoryToolDef` (`packages/memory/src/types.ts`)
...
```
Every tool body is built by `tool()` (packages/memory/src/tools.ts), which parses `args` against the tool's zod
schema first (`fail("Invalid arguments: <path>: <message>")` on the first issue, packages/memory/src/tools.ts)
and wraps the run in `try/catch` (`fail("Tool failed: <message>")`, packages/memory/src/tools.ts) — so a
`MemoryToolDef.execute` can never throw regardless of what its body does. Successes and failures
are both passed through `sanitizeText` (`ok`/`fail`, packages/memory/src/tools.ts) so a document that happened to
...
(packages/memory/src/seed.ts). `wrapMemorySeed(raw, maxChars = SEED_MAX_CHARS)` (packages/memory/src/seed.ts) is the single
place that: trims, escapes any `<memory>`/`</memory>` substring the provider's own content
contained (`escapeSeedTags`, packages/memory/src/seed.ts, into `&lt;`/`&gt;`), runs `sanitizeText`, truncates to
...
```
(preamble text at packages/memory/src/seed.ts; wrapping in `createMemoryRunCapability` via the
prepared `seed(...)` then
...
`buildSeed(args: BuildSeedArgs)` (`packages/memory/src/seed.ts`) is what actually produces the raw content the
...
`args.store.read` when it does not (packages/memory/src/seed.ts). It returns `null` — no seed at all — when
`PROFILE.md` is absent, when the trimmed body is empty (packages/memory/src/seed.ts), and, as a distinct edge case,
when the bounded read was truncated **and** the truncated bytes fail to parse as frontmatter
(`if (bounded?.truncated === true && parsed.unparsable) return null;`, packages/memory/src/seed.ts) — a bounded read
that cut off mid-frontmatter is discarded rather than served garbled. On success it returns
`${PREAMBLE}\n\n${profile}` (packages/memory/src/seed.ts). `BuildSeedArgs.task` is accepted but "presently unused
(the seed is always the whole PROFILE index)" (packages/memory/src/seed.ts) — retained for future task-aware
seeding.

### 3.3 `DocFrontmatter` (`packages/memory/src/types.ts`)
...
### 3.4 `PolicyDecision` (`packages/memory/src/policy.ts`)
...
`MemoryHealthReport` (packages/protocol/src/memory.ts): `generated_at`, `totals {documents, topics, memories,
pending_jobs, failed_jobs}`, `counts` by severity, `findings[]` (each `{code, severity, path,
message, suggested_action}`), `truncated`, `skipped_codes`. `MemoryJob` (packages/protocol/src/memory.ts): `run_id,
state ("pending"|"running"|"retry_wait"|"completed"|"failed"), attempts, enqueued_at, updated_at,
next_attempt_at?, last_error? {phase, message, at}, note?`. `MemoryIngestDetail` (packages/protocol/src/memory.ts)
...
### 3.6 Storage caps referenced by the tool layer (`packages/memory/src/storage-limits.ts`, delegated in full to
...
`reviewDigest(docs)` (packages/memory/src/review.ts) is the cheap, LLM-free tree overview a host UI renders; it
is pure over an already-fetched listing so the facade owns the one `list()` call. It assembles
three fields: `totals` from `summarizeTotals` (packages/memory/src/review.ts), which counts `topics` and
...
- Builds two `MemoryToolset`s: `readToolset` over `provider.readTools` (call limit
  ...
  `packages/capability/src/env.ts`) and `writeToolset` over `provider.writeTools ?? []` with an
...
| `grep_memories` | Formats hits as `path:line: text`, one per line; no hits returns `"No matches. Memory may simply not cover this yet."` | packages/memory/src/tools.ts |
...
(packages/memory/src/tools.ts): if a `mutationFence` is supplied (the background indexer's queue-claim lease),
...
edit to a pinned document allowed under `checkWrite` (the `tools.ts` `checkWrite` doc comment and
the policy rule in §4.5 below).

### 4.5 `checkWrite` (`packages/memory/src/policy.ts`)

| `intent` | Result |
| --- | --- |
| `"owner"` or `"reindex"` | `ALLOW` unconditionally, first line (packages/memory/src/policy.ts) |
...
`next` is only consulted when the caller supplies it (packages/memory/src/policy.ts): a caller that cannot
...
`authority_escalation` check only fires on a rise to `confirmed` (packages/memory/src/policy.ts).

## 5. Invariants

**INV-090.** The package's public facade (`src/index.ts`) re-exports `sanitizeText` (by identity,
from `@clarvis/capability`, packages/memory/src/index.ts) and deliberately does **not** re-export `sanitizeDeep`.
Production: `packages/memory/src/index.ts`. Test:
`packages/memory/tests/architecture/barrel.test.ts`. Prevents "restores a weaker default
under memory's name" (packages/memory/tests/architecture/barrel.test.ts comment).
...
`packages/memory/tests/architecture/capability-flag-surface.test.ts` (`wireSurfaceOf`
equality) (`Object.hasOwn(...,"onRunEnd")` differs and only that).
...
'on' or 'off'"`. Production: `packages/memory/src/settings.ts`. Test:
`packages/memory/tests/architecture/settings-ownership.test.ts`.
...
`packages/memory/src/settings.ts` (settings imports). Test:
...
authority check). Test: `packages/memory/tests/unit/write-policy.test.ts`.

**INV-100.** The `reindex` intent is allowed to `replace` a pinned index file. Production:
`packages/memory/src/policy.ts` (same early return covers `"reindex"`). Test: `packages/memory/tests/unit/write-policy.test.ts`.
...
`packages/memory/src/policy.ts`. Test: `packages/memory/tests/unit/write-policy.test.ts`, plus component-level confirmation at
`packages/memory/tests/component/tools.test.ts` (write refused with "pin"/"confirmed" in
...
removes the pin or grants authority. Production: `packages/memory/src/policy.ts` (the pin
block tests only `"delete"`/`"replace"`, while the subsequent checks inspect `next`).
Test: `packages/memory/tests/unit/write-policy.test.ts`; component evidence at `packages/memory/tests/component/tools.test.ts` ("a surgical edit
...
`packages/memory/src/policy.ts`. Test: `packages/memory/tests/unit/write-policy.test.ts`; component-level at
`packages/memory/tests/component/tools.test.ts` (both direct `write_memory` self-pinning and a smuggled marker through
...
"unpin via edit, then replace freely". Production: `packages/memory/src/policy.ts`. Test:
`packages/memory/tests/unit/write-policy.test.ts`; component-level at `packages/memory/tests/component/tools.test.ts` (an edit that removes the
...
retain authority a document already had. Production: `packages/memory/src/policy.ts` (both checks only fire on
a *change* — `next.pinned !== true` / `next.authority !== "confirmed"` compared against the
opposite existing value). Test: `packages/memory/tests/unit/write-policy.test.ts` (pin retained → allowed) (`confirmed → confirmed` → allowed); component confirmation at `packages/memory/tests/component/tools.test.ts`
...
`packages/memory/src/policy.ts` (the escalation check fires only when `next.authority === "confirmed"` and the
existing value is not; a lowering `next` never matches). Test: `packages/memory/tests/unit/write-policy.test.ts`.
...
`packages/memory/tests/architecture/write-policy.test.ts`.
...
returns before the write paragraph); `packages/memory/src/seed.ts`
(`PREAMBLE` names only `list_memories`/`read_memory`/`grep_memories`). Test:
`packages/memory/tests/architecture/write-policy.test.ts` (subagent system-section half) (seed-block half).
...
inside the tool body both resolve to a `fail(...)` result. Production: `packages/memory/src/tools.ts`. Test:
`packages/memory/tests/component/tools.test.ts` (a path-traversal argument comes back as an
...
`packages/memory/src/handler.ts` (`calls` is a closure-local `let`, one per call to `buildMemoryToolsHandler`).
Test: `packages/memory/tests/component/toolset-handler.test.ts`.
...
`packages/capability/src/env.ts`. Unpinned by a
component test specific to this pairing (the budget-exhaustion test in
`packages/memory/tests/component/toolset-handler.test.ts` exercises the mechanism generically, not this specific
...
| Container configuration selects a local `wiki` or `file` provider | The native Container Kernel runs Memory against its private content/state volumes; external providers remain `unsupported` before provider startup or inference | `projectContainerConfiguration` in packages/kernel/src/config/container-projection.ts and `createNativeKernel` in packages/kernel/src/native-kernel.ts |
...
- `./indexer/pyramid.ts` (`isMutatingTool`, `Mutation` type) — owned by [capabilities/memory-indexer.md](memory-indexer.md);
  `packages/memory/src/handler.ts` is this document's one reach into that sibling's module, used only to decide whether a
...
- `packages/kernel/src/memory/memory-service.ts` imports `MemoryFactory` from
  `@clarvis/memory/capability`, `Memory`/`MemoryIndexJob` from `@clarvis/memory`, and
  `MemoryHealthReport`/`MemoryJob`/`MemoryJobFilter`/`MemoryJobState`/`MemoryReindexResult`/
  `MemoryService` from `@clarvis/protocol` — the kernel is the sole
...
- `packages/memory/tests/architecture/settings-ownership.test.ts` fails the build if `settings.ts` ever value-imports
  ...
  `capability-registry.ts` import of `memorySettingsSpec`
...
- `packages/kernel/src/kernel.ts` constructs `createMemoryService({ factory: opts.memoryFactory,
  owner: scope.owner })` per connection scope, and `packages/kernel/src/kernel.ts` reports `memory:
...
- **Read/write call-budget asymmetry is unpinned.** `CLARVIS_MEMORY_TOOL_CALL_LIMIT` (default 12)
  ...
  budget-exhaustion mechanism is tested at `packages/memory/tests/component/toolset-handler.test.ts`, but always against a
