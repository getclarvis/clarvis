...
(`packages/memory/src/types.ts:4`–`:7`). Every document is a small frontmatter block plus freeform
markdown; nothing else about the knowledge is persisted — no vectors, no embeddings, no derived
index (`packages/memory/src/query.ts:9`).
...
committed as a unit (`packages/memory/src/journal.ts:1`). Second, a human is expected to hand-edit
...
edit (`packages/memory/src/frontmatter.ts:6`, `packages/memory/src/reindex.ts:6`,
`packages/memory/src/revisions.ts:49`).
...
`src/worker.ts`, and conformance cases at `packages/memory/src/testing.ts:773`–`:1011`). The write
...
| --- | --- | --- |
| `createFileMemoryStore` | `(opts: CreateFileMemoryStoreOptions) => MemoryStore` | `packages/memory/src/file-store.ts:125` |
| `createInMemoryMemoryStore` | `(opts?: {clock?}) => MemoryStore` | `packages/memory/src/testing.ts:70` |
| `memoryStoreConformance` | `() => readonly ConformanceCase[]` | `packages/memory/src/testing.ts:639` |
| `createTestClock` | `(start?) => TestClock` | `packages/memory/src/testing.ts:1202` |
| `createMemory` | `(opts: CreateMemoryOptions) => Memory` | `packages/memory/src/memory.ts:18` |
| `createFileMemory` | `(opts & {root}) => Memory` | `packages/memory/src/memory.ts:126` |
| `normalizeMemoryPath` | `(relPath: string) => string` | `packages/memory/src/paths.ts:36` |
| `compareMemoryPaths` | `(a, b) => number` | `packages/memory/src/paths.ts:62` |
| `memoryDocKind` | `(relPath) => DocKind` | `packages/memory/src/paths.ts:73` |
| `parseFrontmatter` | `(markdown) => ParsedDoc` | `packages/memory/src/frontmatter.ts:112` |
| `serializeDoc` | `(Partial<DocFrontmatter>, body) => string` | `packages/memory/src/frontmatter.ts:204` |
| `readDescription` | `(markdown) => string` | `packages/memory/src/frontmatter.ts:184` |
| `reindex` | `(store: TreeStore, logger?) => Promise<string[]>` | `packages/memory/src/reindex.ts:311` |
| `planReindex` | `(store: TreeReader, logger?) => Promise<ReindexChange[]>` | `packages/memory/src/reindex.ts:203` |
| `reindexView` | `(bx: MemoryBatch) => Pick<MemoryTx,"read"\|"write"\|"list">` | `packages/memory/src/batch.ts:303` |
| `runBatch` | `(prims, input, fn) => Promise<T>` (internal; not barrel-exported) | `packages/memory/src/batch.ts:110` |
| `decideRecovery` | `({markers, record, current}) => {outcome, reason?}` | `packages/memory/src/journal.ts:123` |
| `digestBody` | `(content) => string` (sha256 hex) | `packages/memory/src/revisions.ts:65` |
| `newRevisionId` | `(at) => string` | `packages/memory/src/revisions.ts:78` |
| `compareRevisionsNewestFirst` | `(a, b) => number` | `packages/memory/src/revisions.ts:90` |
| `queryMemory` | `({tx, input, config?}) => Promise<MemoryQueryResult>` | `packages/memory/src/query.ts:270` |
| `scoreDoc` / `buildStats` | BM25F scoring primitives | `packages/memory/src/text/bm25.ts:89`, `:119` |
| `tokenList` / `tokenCounts` / `tokenize` / `tokenizeQuery` / `overlapScore` | `packages/memory/src/text/tokenize.ts:126`, `:148`, `:164`, `:201`, `:177` |
| `isBoundedPattern` / `createGrepScanner` / `matchWindowed` / `grepHitText` | `packages/memory/src/text/grep.ts:144`, `:246`, `:309`, `:329` |
| `rankByOverlap` | `(items, query, keyOf, limit) => T[]` — **no production caller** | `packages/memory/src/similar.ts:35` |
| `buildDigest` / `renderDigest` | `packages/memory/src/digest.ts:112`, `:179` |
| `extractTitle` / `isIndexFile` | `packages/memory/src/tree.ts:183`, `:96` |
| `captureWorkspaceState` | `(cwd, logger?) => Promise<WorkspaceState \| undefined>` | `packages/memory/src/workspace-state.ts:37` |
| `readUtf8FileBounded` / `readUtf8PrefixSync` / `scanDirectoryBounded` | `packages/memory/src/bounded-io.ts:40`, `:106`, `:134` |
| `assertMemoryPayloadBytes` / `assertMemoryStorageCount` | `packages/memory/src/storage-limits.ts:53`, `:67` |
| `bestEffortFileStore` / `detachFileStoreTask` | `packages/memory/src/file-store/tasks.ts:20`, `:29` |
| `truncate` | `(text: string, max: number) => string` (internal; not barrel-exported) — prefix + trailing `…` when over `max`, unchanged otherwise | `packages/memory/src/text.ts:9` |

### 2.3 `CreateFileMemoryStoreOptions`

| Field | Type | Default | Source |
| --- | --- | --- | --- |
| `root` | `string` | required — "the wiki's Markdown, and nothing else" | `packages/memory/src/file-store.ts:48` |
| `machineryRoot` | `string?` | defaults to `root` | `packages/memory/src/file-store.ts:65`, `packages/memory/src/file-store/layout.ts:19` |
| `workspaceRoot` | `string?` | when given, root is created via `ensureWorkspaceSubdir` so the `.gitignore` seed has one owner | `packages/memory/src/file-store.ts:75`, `packages/memory/src/file-store/layout.ts:54` |
| `clock` | `() => number` | `Date.now` | `packages/memory/src/file-store.ts:130` |
| `lock` | `{staleMs?, heartbeatMs?, timeoutMs?, warnMs?}` | `60_000 / 15_000 / 180_000 / 5_000` | `packages/memory/src/file-store.ts:35`–`:37`, `packages/memory/src/file-store/lock.ts:34` |
| `logger` | `Logger?` | `NOOP_LOGGER` | `packages/memory/src/file-store.ts:129` |

### 2.4 Errors

| Class | `code` | Thrown when | Source |
| --- | --- | --- | --- |
| `MemoryPathError` | `memory_path_invalid` | path absolute, escapes root, or not `.md` | `packages/memory/src/paths.ts:18` |
| `MemoryStorageLimitError` | `memory_storage_limit` | a byte or count budget is exceeded; carries `kind`, `identifier`, `actual`, `maximum` | `packages/memory/src/storage-limits.ts:27` |
| `MemoryRecoveryRequiredError` | `memory_recovery_required` | an interrupted batch awaits an operator decision; carries `batchId` | `packages/memory/src/journal.ts:164` |

### 2.5 Search input/output

`MemoryQueryInput` = `{ query, limit?, prefix?, kinds?, include_snippets? }`
(`packages/memory/src/query.ts:35`). `MemoryQueryHit` carries `path`, `kind`, `title`, `description`,
`tags`, `score`, `matched_fields`, `snippet`, `snippet_line`, `updated_at`, `pinned`, `authority`
(`packages/memory/src/query.ts:49`). `MemoryQueryResult` adds `scanned`, `listed`, `terms`, `truncated`, and an
optional `truncation_reasons` (`packages/memory/src/query.ts:75`). The `score` doc comment states it
is comparable only within one result set and must never be rendered as a percentage
(`packages/memory/src/query.ts:59`).
...
against; nothing in it executes. `CreateMemoryOptions` (`:18`) is `{ store: MemoryStore; indexer?:
...
before it existed" (`:24`–`:29`). `IndexReport` (`:35`) is `{ run_id, skipped, note?, written: string[],
deleted: string[], reindexed, indexer_run_id?, continuation_blocker?: string | null }` — the return
shape of `Memory.index()`, produced by `packages/memory/src/memory.ts:94`–`:131` (see §4.11); its
`indexer_run_id`/`continuation_blocker` fields are the sibling **memory-indexer-and-jobs** document's
(`indexRun`, `packages/memory/src/indexer/run.ts`). `ReviewDigest` (`:47`) is `{ totals: {documents,
topics, memories}; recent: {path, description, updated_at}[]; undescribed: string[] }`, produced by
`reviewDigest` (§4.11).

The `Memory` interface itself (`:65`–`:84`) is the complete host-facing surface — every member
`createMemory` (`packages/memory/src/memory.ts:42`–`:132`) must implement:
...
(`packages/memory/src/frontmatter.ts:204`–`:213`). Real example, from the conformance fixture
(`packages/memory/src/testing.ts:618`):
...
`<!-- reindex:begin -->` … `<!-- reindex:end -->` (`packages/memory/src/reindex.ts:48`–`:50`), under a
`## Contents` heading (`packages/memory/src/reindex.ts:52`). An empty block renders
`_(no entries yet)_` (`packages/memory/src/reindex.ts:51`). Each entry is
`- [<childBasename>](<rel>) — <description>`, where `<rel>` is the child's path relative to the
index file's own directory (`packages/memory/src/reindex.ts:252`), with the ` — <description>` suffix omitted
when the target's description is blank (`packages/memory/src/reindex.ts:257`). The scaffolded body
for a newly created index file is `# <Title>` + intro + `## Contents` + links
(`packages/memory/src/reindex.ts:266`). Intro text is fixed per level: at the root, "Durable
workspace knowledge, organized by topic. Follow the links below to drill down."; elsewhere "Durable
knowledge about `<dir>`." (`packages/memory/src/reindex.ts:120`). The root profile's description is
the fixed constant `"Index of durable operational knowledge for this workspace"`
(`packages/memory/src/tree.ts:13`), and generated descriptions are capped at 120 characters
(`packages/memory/src/tree.ts:16`, `packages/memory/src/reindex.ts:67`).

### 3.3 On-disk layout — the content/machinery split

`createFileStoreLayout` derives two roots and one lock directory
(`packages/memory/src/file-store/layout.ts:14`–`:49`):

| Path | Holds | Source |
| --- | --- | --- |
| `<root>/**/*.md` | the wiki, and nothing else | `packages/memory/src/file-store.ts:48` |
| `<machineryRoot>/.history/<doc path>/<rev>.md` | a revision's pre-image body | `packages/memory/src/file-store/revisions.ts:43`, `:100` |
| `<machineryRoot>/.history/<doc path>/<rev>.json` | that revision's metadata | `packages/memory/src/file-store/revisions.ts:101` |
| `<machineryRoot>/.journal/<batch id>/prepare.json` | the batch's declared intent | `packages/memory/src/file-store/journal.ts:15`, `:120` |
| `<machineryRoot>/.journal/<batch id>/applied` | zero-byte "all writes landed" marker | `packages/memory/src/file-store/journal.ts:16`, `:122` |
| `<machineryRoot>/.journal/<batch id>/commit` | zero-byte "batch finished" marker | `packages/memory/src/file-store/journal.ts:17`, `:123` |
| `<machineryRoot>/.state/indexed/<encoded run id>` | run-dedup ledger; content is the epoch ms as a string | `packages/memory/src/file-store/jobs.ts:101`, `:414` |
| `<machineryRoot>/.state/jobs/<encoded run id>.json` | the durable index queue (**sibling document**) | `packages/memory/src/file-store/jobs.ts:100` |
| `<machineryRoot>/.lock/` + `.lock/holder` | the mkdir-based tree lock and its `<pid>.<random>` token | `packages/memory/src/file-store/layout.ts:49`, `packages/memory/src/file-store/lock.ts:54`, `:95` |
...
(`packages/memory/src/file-store.ts:39`–`:42`, `packages/memory/src/file-store/revisions.ts:43`).
...
(`packages/memory/src/file-store/layout.ts:32`, `packages/memory/tests/integration/file-store.test.ts:38`).
...
```
(`packages/memory/src/journal.ts:18`, `:21`, `:49`.) Each op is
`{ op: "write"|"delete", path, expected_digest: string|null, next_digest: string|null,
revision_id: string|null, previous_body?: string }` (`packages/memory/src/journal.ts:27`).
...
(`packages/memory/src/journal.ts:37`). A concrete record, as a test plants one:
`{version:1, batch_id:"b1", at:1_700_000_000_000, source:{kind:"indexer",run_id:"run-1"}, ops:[], commit:{}}`
(`packages/memory/tests/integration/journal-recovery.test.ts:24`).
...
```
(`packages/memory/src/revisions.ts:21`–`:57`.) The module header states the storage rule: a revision
...
document, "which is why there is no separate trash area" (`packages/memory/src/revisions.ts:1`).

### 3.6 Identifiers

| Identifier | Generation | Source |
| --- | --- | --- |
| Batch id | `${clock().toString(36)}-${randomBytes(6).toString("hex")}` | `packages/memory/src/file-store.ts:197` |
| Revision id | base-36 timestamp zero-padded to 9 chars + `-` + 4 random bytes hex, so lexicographic order matches chronological | `packages/memory/src/revisions.ts:78` |
| Ledger / job filename | `encodeRunId`: run id with non-`[A-Za-z0-9_-]` replaced by `_`, truncated to 48 chars, plus `-` and the first 12 hex of its sha256 | `packages/memory/src/file-store/jobs.ts:64` |
| Lock holder token | `${process.pid}.${Math.random().toString(36).slice(2)}` | `packages/memory/src/file-store/lock.ts:95` |
| `version()` token | sha256 over, per document in path order, `rel \0 size \0 bytes \0` | `packages/memory/src/file-store/documents.ts:238`–`:248` |

`encodeRunId` is pinned to distinguish `a/b` from `a\b`
(`packages/memory/tests/integration/file-store-internals.test.ts:52`).

### 3.7 Storage limits (`MEMORY_STORAGE_LIMITS`)

| Key | Value | Source |
| --- | --- | --- |
| `documentBytes` | 2 MiB | `packages/memory/src/storage-limits.ts:9` |
| `revisionBodyBytes` | 2 MiB | `packages/memory/src/storage-limits.ts:10` |
| `metadataBytes` | 1 MiB | `packages/memory/src/storage-limits.ts:12` |
| `prefixBytes` | 64 KiB | `packages/memory/src/storage-limits.ts:14` |
| `scanEntries` | 10 000 | `packages/memory/src/storage-limits.ts:16` |
| `batchOperations` | 256 | `packages/memory/src/storage-limits.ts:18` |
| `corpusBytes` | 32 MiB | `packages/memory/src/storage-limits.ts:20` |

`MemoryStorageKind` is one of `document | revision body | metadata | corpus | entries | batch operations`
(`packages/memory/src/storage-limits.ts:23`).

### 3.8 History retention

`MEMORY_DEFAULTS.history` = `{ keep_revisions: 20, keep_days: 90, min_revisions: 3 }`
(`packages/memory/src/config.ts:46`–`:53`). The comment states `min_revisions` is a floor that
outranks both other bounds (`packages/memory/src/config.ts:42`), and the prune predicate is
`index >= min_revisions && (index >= keep_revisions || revision.at < cutoff)`
(`packages/memory/src/file-store/revisions.ts:111`–`:115`).
...
(`packages/memory/src/query.ts:114`). BM25F field parameters:
...
(`packages/memory/src/text/bm25.ts:50`–`:56`; saturation `K1 = 1.2` at `:59`.)

Tokenizer caps: `DEFAULT_MIN_LENGTH = 2`, `QUERY_MAX_CHARS = 512`, `QUERY_MAX_TOKENS = 64`,
`DOC_MAX_CHARS = 200_000`, `DOC_MAX_TOKENS = 20_000`
(`packages/memory/src/text/tokenize.ts:34`–`:46`). Grep caps: `GREP_LINE_MAX = 200`,
`GREP_WINDOW_OVERLAP = 64`, `GREP_QUERY_MAX_CHARS = 200`, `GREP_SCAN_MAX_LINES = 200_000`,
`GREP_SCAN_MAX_MS = 2_000`, `GREP_AMBIGUITY_MAX = 3`
(`packages/memory/src/text/grep.ts:32`, `:50`, `:64`, `:77`, `:89`, `:103`).

### 3.10 Run digest

`RunDigest` = `{ commands, files_touched, errors, retries, steering, stats }`
(`packages/memory/src/digest.ts:45`). Fixed vocabularies: bash-family tool names are
`shell, bash, exec, execute, run_command, execute_command`
(`packages/memory/src/digest.ts:65`), and file-path argument keys are
`path, file_path, file, target, source, destination` (`packages/memory/src/digest.ts:66`). Field
truncations: command 300 chars, error 400, args excerpt 200 (500 for the retry signature), steering
400 (`packages/memory/src/digest.ts:122`, `:142`, `:92`, `:133`, `:159`). `renderDigest` emits
...
(`packages/memory/src/digest.ts:179`–`:219`).
...
(`packages/memory/src/workspace-state.ts:8`, `:42`–`:47`). Absence is a valid outcome, never an
error (`packages/memory/src/workspace-state.ts:31`–`:33`).
...
`normalizeMemoryPath` (`packages/memory/src/paths.ts:36`) runs, in order:

1. replace every `\` with `/` (`:37`);
2. reject a leading `/` — "path must be relative" (`:38`);
3. `path.posix.normalize`, then reject `..`, `../…` and any absolute result — "path escapes the
   memory root" (`:41`–`:44`);
4. reject anything not ending in `.md` — "only .md documents are allowed" (`:45`).
...
(`packages/memory/src/paths.ts:57`–`:63`).
...
(`packages/memory/src/file-store.ts:126`–`:219`). Lazy `init()` is memoized per store instance
(`packages/memory/src/file-store/layout.ts:51`), pinned by a test that calls `init()` three times
concurrently (`packages/memory/tests/integration/file-store-internals.test.ts:157`).
...
1. `wikiVanished = split && !existsAsDirectory(root)` — evaluated first, so it observes the state the
   user left (`packages/memory/src/file-store/layout.ts:52`);
2. create `root` (via `ensureWorkspaceSubdir` when `workspaceRoot` is given, else `mkdir 0700`) (`:53`–`:57`);
3. create `machineryRoot` when split (`:58`);
4. if `wikiVanished`, best-effort `rm -rf` of `<machineryRoot>/.state/indexed` and
   `<machineryRoot>/.journal` (`:59`, `:35`–`:44`).

`.history` is deliberately *not* in that discard list, and a test asserts it survives
(`packages/memory/tests/integration/file-store-internals.test.ts:161`).
...
(`packages/memory/src/file-store/jobs.ts:404`–`:415`). Marking the same run id twice is safe — a
...
(`packages/memory/src/testing.ts:750`–`:759`).
...
```
(`packages/memory/src/file-store.ts:211`–`:217`.)

Acquisition (`packages/memory/src/file-store/lock.ts:93`):

| Step | Effect |
| --- | --- |
| `mkdir(lockDir, {mode:0o700})` | exclusive create; `EEXIST` means contention (`:102`, `:115`) |
| write `holder` = `<pid>.<rand>` | on failure, best-effort remove the lock dir and rethrow (`:104`–`:112`) |
| on `EEXIST`: `stealable()` | true only when the dir's mtime is older than `staleMs`, the recorded pid is not alive, **and** a second `stat` still shows it stale (`:81`–`:91`) |
| steal | best-effort `rm -rf` (`:119`–`:124`) |
| deadline check | past `timeoutMs`, throw `memory: timed out waiting for tree lock <dir>` (`:125`–`:129`) |
| back-off | `setTimeout(…, 25)` between attempts (`:130`) |
| heartbeat | `setInterval(utimes(lockDir))` every `heartbeatMs`, `unref`'d (`:139`–`:147`) |
| release | `clearInterval`, then remove the dir **only if `holder` still holds this token** (`:148`–`:157`) |

`holderAlive` treats `process.kill(pid, 0)` throwing `EPERM` as alive
(`packages/memory/src/file-store/lock.ts:77`). Nesting is tracked with an `AsyncLocalStorage`
(`:55`, `:161`); a nested hold takes `NO_RELEASE`, whose comment states why: `mkdir` would fail
`EEXIST` against the caller's own hold and the wait would run to its timeout
(`packages/memory/src/file-store/lock.ts:36`–`:46`).

Every hold whose duration exceeds `warnMs` logs `memory.lock.held_long` with `nested`, `held_ms`,
`threshold_ms` and `lock_dir` (`packages/memory/src/file-store/lock.ts:170`–`:181`). The constant's
...
whole inference (`packages/memory/src/file-store/lock.ts:26`–`:33`).

### 4.4 `runBatch` — the ordered mutation sequence

The order is called out as load-bearing in the function's own doc comment
(`packages/memory/src/batch.ts:84`–`:99`), and the implementation runs:

| # | Step | Code |
| --- | --- | --- |
| 0 | `recovery.assertWritable()` — refuse if a batch is blocked | `packages/memory/src/file-store.ts:206` |
| 1 | run `fn(handle)`, staging only; the tree is untouched | `packages/memory/src/batch.ts:196` |
| 1a | if nothing staged: run `commit` and return — no journal | `packages/memory/src/batch.ts:197`–`:200` |
| 2 | for each staged op in path order: read the current body, compute `expected`/`next` digests, drop no-op writes and deletes of absent documents, mint a revision when eligible | `packages/memory/src/batch.ts:207`–`:253` |
| 2a | if every op collapsed: run `commit` and return | `packages/memory/src/batch.ts:255`–`:258` |
| 3 | validate the serialized record against `metadataBytes` **before** the first revision byte is persisted | `packages/memory/src/batch.ts:268`–`:275` |
| 4 | `putRevision` each revision (body first, metadata second) | `packages/memory/src/batch.ts:276`, `packages/memory/src/file-store/revisions.ts:100`–`:101` |
| 5 | `writeJournal(record)` — intent is now durable | `packages/memory/src/batch.ts:277` |
| 6 | apply every op to the tree | `packages/memory/src/batch.ts:279`–`:282` |
| 7 | `markApplied` — past this point recovery rolls *forward* | `packages/memory/src/batch.ts:283` |
| 8 | `runCommit(record.commit)` | `packages/memory/src/batch.ts:285` |
| 9 | `markCommitted`, then `sweepJournal` | `packages/memory/src/batch.ts:286`–`:287` |
| 10 | `pruneHistory` for the touched paths | `packages/memory/src/batch.ts:288` |
...
(`packages/memory/src/batch.ts:226`). `external_edit: true` is set exactly when
`lastInstalledDigest(path)` is defined and differs from the current body's digest
(`packages/memory/src/batch.ts:238`). When no revision is recorded but a prior body existed, that
body rides inline as `previous_body` on the journal op (`packages/memory/src/batch.ts:251`).

The staged handle merges its own state into reads and listings: `read` returns the staged content
(or `null` for a staged delete) before touching the tree (`packages/memory/src/batch.ts:121`–`:125`),
and `list` overlays staged creations and removes staged deletions
(`packages/memory/src/batch.ts:127`–`:145`). The doc comment states why: the deterministic reindex
runs inside the batch and lists the tree to decide what to link, so a leaf staged moments earlier
would otherwise never be wired into navigation (`packages/memory/src/batch.ts:101`–`:105`).

`reindexView(bx)` wraps a batch so every write it makes is `{ derived: true }`
(`packages/memory/src/batch.ts:303`–`:308`); `tools.ts` is its only production caller
(`packages/memory/src/tools.ts:331`, `:368`, `:396` — sibling document).

### 4.5 Recovery — the state machine

`decideRecovery` (`packages/memory/src/journal.ts:123`) is pure. Read as (marker state, tree state) →
(outcome, effect):

| Condition (checked in this order) | Outcome | Effect at `packages/memory/src/file-store/recovery.ts` |
| --- | --- | --- |
| `record.version > JOURNAL_VERSION` | `required` — "journal written by a newer version (N)" | freeze; entry retained (`:136`) |
| `markers.committed` | `swept` | journal dir removed (`:189`) |
| `markers.applied` | `rolled_forward` | `runCommit(record.commit)` then sweep (`:146`, `:189`) |
| any op's current digest ∉ {`expected_digest`, `next_digest`} | `required` — "`<path>` was modified outside the store while a batch was interrupted" | freeze; the human's bytes untouched (`:136`) |
| an op off its expected digest with `expected_digest !== null`, `revision_id === null` and no `previous_body` | `required` — "`<path>` has no stored pre-image to restore" | freeze; the generic required handler at `packages/memory/src/file-store/recovery.ts:136`, reason string produced at `packages/memory/src/journal.ts:151` |
| otherwise | `rolled_back` | restore each off-expected op (delete when `expected_digest === null`, else write the pre-image) then sweep (`:147`–`:189`) |
...
no-op precisely in the mid-apply case recovery exists for" (`packages/memory/src/journal.ts:115`).
That case is pinned by name — "rolls back mid-apply, which is the case it exists for"
(`packages/memory/tests/unit/journal-recovery.test.ts:71`).
...
(`packages/memory/src/file-store/recovery.ts:148`–`:187`).
...
- a journal scan over the entry budget → one synthetic entry `batch_id: "<journal-scan>"`,
  `required: true` (`packages/memory/src/file-store/recovery.ts:83`–`:92`);
- an over-limit journal record → `required`, record retained as evidence (`:97`–`:103`), pinned at
  `packages/memory/tests/integration/storage-limits.test.ts:294`;
- an unreadable (oversized) tree document → `required` (`:117`–`:127`).

An **unparsable or absent** `prepare.json` reads as `null` and the batch is swept
(`packages/memory/src/file-store/recovery.ts:104`–`:108`), pinned by "a journal that never landed
leaves nothing to undo" (`packages/memory/tests/integration/journal-recovery.test.ts:181`).

`recoverOnce` sets its `recovered` flag *before* awaiting, so two concurrent callers produce one pass
(`packages/memory/src/file-store/recovery.ts:197`–`:201`), pinned at
`packages/memory/tests/integration/file-store-internals.test.ts:193`. `blocked` is sticky for the
lifetime of the store instance, and `assertWritable` throws `MemoryRecoveryRequiredError` from it
(`:202`–`:204`).

### 4.6 `planReindex` / `reindex`

`planReindex` (`packages/memory/src/reindex.ts:203`):
...
(`packages/memory/src/reindex.ts:227`–`:236`, called at `:262` and `:281`), and the emitted change
list is itself charged against `batchOperations` via `assertMemoryStorageCount("batch operations",
"reindex", changes.length + 1, …)` (`:282`–`:287`) — a pass needing more than 256 index-file
rewrites throws.

`ensureDescription`'s own doc comment states it also "repairs the blank description emitted by the
first wiki implementation" (`packages/memory/src/reindex.ts:157`–`:158`), and `ensureIntro` likewise
"upgrade[s] the old empty scaffold (`# title` immediately followed by `## Contents`) without
disturbing a human-written introduction" (`:165`–`:166`) — both are a permanent migration path for
...
end (`packages/memory/src/reindex.ts:137`–`:155`). Prose outside the managed region is never touched
— pinned by "preserves prose outside the managed block and refreshes links on a new leaf"
(`packages/memory/tests/integration/reindex.test.ts:48`).

`derivedDescription` (`packages/memory/src/reindex.ts:86`) only descends into a directory whose
current description is a *generated placeholder* — blank, the bare directory name, or exactly the
synthesized default (`packages/memory/src/tree.ts:80`–`:87`). A hand-written description wins and
stops the recursion. One child → inherit its description; several → `"<Title>: a; b; c"`, capped
(`:109`–`:117`). The comparison is by exact (case-insensitive, trimmed) string match, so a human who
...
(`packages/memory/src/tree.ts:80`–`:87`).

`reindex` is `planReindex` followed by a write of each change
(`packages/memory/src/reindex.ts:311`–`:315`). It is stateless — no checksum, no cache — which is
what lets hand edits survive as long as the frontmatter `description:` is present
(`packages/memory/src/reindex.ts:1`–`:9`), pinned as idempotent at
`packages/memory/tests/integration/reindex.test.ts:42`.

`health` reuses `planReindex` to detect stale navigation
(`packages/memory/src/health.ts:254`), which the reindex module's own header names as the reason for
the plan/apply split (`packages/memory/src/reindex.ts:19`–`:23`).
...
(`packages/memory/src/file-store/documents.ts:55`–`:85`). The comment explains the zero-remainder
...
incomplete (`:61`–`:63`). Both halves are pinned — "an exact document scan boundary drains an empty
pending subtree" (`packages/memory/tests/integration/storage-limits.test.ts:145`) and "list and grep
reject a non-empty subtree beyond the exact scan boundary" (`:166`).
...
- `write` → refuse over `documentBytes`, then `writeFileAtomic` (temp sibling + rename)
  (`:154`–`:158`, `packages/paths/src/atomic.ts:458`);
- `delete` → returns a **boolean**, not `void`: `true` when the document existed and was removed,
  `false` on `ENOENT`; any other error rethrows (`packages/memory/src/file-store/documents.ts:159`–
  `:167`). `MemoryBatch.delete` on the staged handle mirrors this, computing `existed` from the
  merged staged+tree read before staging the delete (`packages/memory/src/batch.ts:179`–`:193`);
...
- `version` → sha256 over `rel \0 size \0 content \0` per document, with a post-read re-`stat` that
  throws "memory: file changed while it was being hashed" on a size or byte-count mismatch
  (`:92`–`:136`, `:238`–`:249`). An `ENOENT` mid-hash contributes zero bytes rather than failing
  (`:131`).

The `.md`-only, non-dot, non-tmp filter lives in one place (`:69`–`:75`) and is pinned by "list
ignores the lock dir, the ledger, dot-files and non-markdown files"
(`packages/memory/tests/integration/file-store.test.ts:27`).

### 4.8 Bounded I/O

`readUtf8FileBounded` (`packages/memory/src/bounded-io.ts:40`) opens a descriptor, stats it, refuses
...
changed while it was being read"; otherwise the bounded text with a `truncated` flag
(`:59`–`:99`). The one-byte probe is pinned by "descriptor-backed reads detect growth after stat
without following it" (`packages/memory/tests/integration/storage-limits.test.ts:109`), which uses
the `afterStat` injection seam (`packages/memory/src/bounded-io.ts:32`).

`scanDirectoryBounded` (`:134`) returns `{inspected, truncated}` where `inspected` counts one past
the budget when truncation occurred, treats a missing directory as an empty scan, and tolerates Bun
deferring `ENOENT` from `opendir` to the first iterator read (`:144`, `:157`–`:160`). A visitor
returning `false` stops the scan (`:155`). The off-by-one look-ahead is pinned exactly:
`scanDirectoryBounded(root, 3, …)` over five entries yields `{inspected: 4, truncated: true}` with
three visits (`packages/memory/tests/integration/storage-limits.test.ts:130`).

### 4.9 Ranked query

`queryMemory` (`packages/memory/src/query.ts:270`):

1. clamp every configuration knob against `MEMORY_STORAGE_LIMITS` (`:261`–`:296`);
2. tokenize the query under the query caps (`:298`);
3. `list()`, filter by `prefix` and `kinds`, slice to `maxDocuments`, recording a `documents`
   truncation reason if anything was dropped (`:302`–`:309`);
4. if no terms survive, return **no hits** rather than everything — the doc comment: "'the of and' is
   not a request for the whole wiki" (`:246`, `:311`–`:319`);
5. per document, read through `readBounded` when the adapter offers it, else `read` + `capUtf8`;
   accumulate the corpus budget and `break` when it would be exceeded (`:324`–`:339`);
6. build per-field term counts for `title` (from `extractTitle` over the parsed body, scanning only
   ...
   humanized directory name — `packages/memory/src/tree.ts:169`, `:183`–`:189`), `description`,
   `tags`, `path` (with `/` and `.` replaced by spaces) and `body` (`:341`–`:360`);
7. `buildStats` then `scoreDoc`, multiply by `metadataBoost`, drop zero scores (`:377`–`:383`).
   A document's authority defaults to `"observed"` when its frontmatter carries none
   (`authority: parsed.frontmatter.authority ?? "observed"`, `packages/memory/src/query.ts:388`),
...
8. sort by score rounded to 4 decimals, then pinned, then authority rank, then `updated_at`
   descending, then kind rank (detail leaf over compilation), then path (`:385`–`:397`);
9. take `limit`, attach the best snippet unless `include_snippets === false` (`:399`–`:411`).

`scoreDoc` (`packages/memory/src/text/bm25.ts:89`) pools a term's frequency **across every field
...
BM25 saturation `pooled / (K1 + pooled)` once (`:99`–`:108`). The doc comment states why: summing
per-field scores would let a term repeated many times in a short field (e.g. `tags`) out-score a
document matching strongly across title, description and body together (`:84`–`:87`). `idf` uses a
...
the workspace's own name appears on most pages (`:67`–`:75`).

`metadataBoost` is multiplicative and clamped to `[0.8, 1.25]` (`packages/memory/src/query.ts:125`,
`:142`–`:148`). Its doc comment names two consequences and both are tested: a non-matching document
scores zero and zero times any multiplier is still zero
(`packages/memory/tests/component/query.test.ts:207`), and a document wins on metadata alone only
when its lexical score is at least `BOOST_MIN / BOOST_MAX` = 64 % of its rival's
(`packages/memory/tests/component/query.test.ts:216`).

`bestSnippet` picks the window of up to `snippetLines` consecutive lines containing the most distinct
query terms, prefixing `…` when the window does not start at line 1
(`packages/memory/src/query.ts:218`–`:253`), pinned by "excerpts the part of the body that matched,
not the opening" (`packages/memory/tests/component/query.test.ts:94`).
...
NFC (`packages/memory/src/text/tokenize.ts:108`–`:116`). The mark-stripping regex uses a lookbehind
...
(`packages/memory/src/text/tokenize.ts:26`–`:31`). `\p{M}` is *in* the token class so a decomposed
dotted `i` stays one token (`:14`–`:22`), pinned at
`packages/memory/tests/unit/tokenize.test.ts:48`. Stopwords are stored already folded (`nao`, `sao`,
`ja`) because the pipeline folds before lookup (`packages/memory/src/text/tokenize.ts:71`–`:80`),
pinned at `packages/memory/tests/unit/tokenize.test.ts:361`.

`createGrepScanner` (`packages/memory/src/text/grep.ts:246`) compiles a regex only when all three of
`opts.regex === true`, `query.length <= GREP_QUERY_MAX_CHARS` and `isBoundedPattern(query)` hold, and
falls back to the keyword path on any compile failure (`:251`–`:258`). The keyword path matches when
the line shares at least one significant token with the query; a query that tokenizes to nothing
never matches (`:268`). `ok()` is false once `GREP_SCAN_MAX_LINES` lines have been tested or
`GREP_SCAN_MAX_MS` of the injected clock has elapsed (`:270`–`:272`).

`isBoundedPattern` (`packages/memory/src/text/grep.ts:144`) is a single left-to-right syntactic pass
refusing four families:

| Family | Example | Rejection site |
| --- | --- | --- |
| quantifier applied to a group | `(a+)+$`, `(a\|a)*$`, `(?:a*)*b` | `:187` (`prev === "close"`) |
| backreference | `\1`…`\9`, `\k<name>` | `:152` |
| lookaround (four forms) | `(?=`, `(?!`, `(?<=`, `(?<!` | `:166`–`:175` |
| more than `GREP_AMBIGUITY_MAX` markers | a 4th of `* + ? { \|` | `:189` |

`(?<name>` is read as a named group and admitted (`:172`, pinned at
`packages/memory/tests/unit/tokenize.test.ts:316`); `?` immediately after `(` is a group modifier and
is not counted (`:183`); metacharacters inside `[…]` or behind a backslash are literals (`:150`,
`:157`).

`matchWindowed` (`:309`) tests a line up to one window whole; longer lines are walked in
...
anchored pattern can over-report at a boundary the line does not have (`:302`–`:307`).

### 4.11 The `Memory` facade over the store

`createMemory` (`packages/memory/src/memory.ts:18`) composes the store into the host-facing facade.
The store-relevant members:

| Member | Behaviour | Source |
| --- | --- | --- |
| `reindex(tx?)` | uses the supplied transaction directly, or wraps `reindexTree` in `store.exclusive` when omitted | `packages/memory/src/memory-contract.ts:56`, `packages/memory/src/memory.ts:24-29` |
| `review()` | one `store.list()` into `reviewDigest` (totals by kind, 10 most recent, undescribed paths) | `packages/memory/src/memory.ts:29`, `packages/memory/src/review.ts:50`, `:13` |
| `query(input)` | `queryMemory({tx: store, input})` — no config override | `packages/memory/src/memory.ts:43` |
| `store` | the injected port, re-exposed verbatim | `packages/memory/src/memory.ts:87` |
...
lease timing (`packages/memory/src/memory.ts:123`–`:146`), pinned by "uses one injected clock for
durable queue timestamps and live lease timing"
(`packages/memory/tests/integration/memory.test.ts:40`).
...
(`packages/memory/src/memory.ts:31`–`:40`). The rate limiter itself is consumed by `drainIndexJobs`,
...
(`packages/memory/src/testing.ts:1`–`:9`, `:639`). A case needing a capability the harness lacks
(`poke`) returns early rather than failing (`:637`). The harness contract is
`{ store, poke?, cleanup }` (`packages/memory/src/testing.ts:596`). A 32nd case asserted rollback on
...
`packages/memory/src/file-store.ts:115` states "exclusion only, not rollback".

`packages/memory/tests/contract/store.test.ts:38` drives every case against both shipped backends:
the file store over a fresh temp root (with `poke` writing straight to disk) and the in-memory store
(with `poke` writing through the store itself) (`:14`, `:23`, `:33`).

Case groups, by first line:

| Group | Range | Owner |
| --- | --- | --- |
| documents, keys, listing, grep | `packages/memory/src/testing.ts:642`–`:726` | this document |
| exclusive-section serialization and visibility | `:727`–`:750` | this document |
| the indexed ledger | `:751`–`:772` | this document |
| the durable job queue | `:773`–`:1011` | **memory-indexer-and-jobs** |
| `version()` | `:1013`–`:1026` | this document |
| out-of-band edits, batches, revisions, recovery | `:1028`–`:1065` | this document |
...
`memory_path_invalid`.
Production: `packages/memory/src/paths.ts:36`–`:47`.
Test: `packages/memory/src/testing.ts:689`, driven at `packages/memory/tests/contract/store.test.ts:41`
...
Production: `packages/memory/src/paths.ts:37`–`:41`.
Test: `packages/memory/src/testing.ts:679`.
...
Production: `packages/memory/src/file-store.ts:211` over `packages/memory/src/file-store/lock.ts:162`;
in-memory equivalent at `packages/memory/src/testing.ts:583`.
Test: `packages/memory/src/testing.ts:727`, `:742`.
...
deleted.
Production: `packages/memory/src/file-store/documents.ts:238`.
Test: `packages/memory/src/testing.ts:1013`.
...
commits.
Production: `packages/memory/src/batch.ts:196` (nothing is applied until step 6 at `:279`), staging
reads at `:121`.
Test: `packages/memory/src/testing.ts:1038`, `:1057`.
...
Production: `packages/memory/src/batch.ts:226`–`:243`.
Test: `packages/memory/src/testing.ts:1070`, `:1096`.
...
Production: `packages/memory/src/revisions.ts:90`, `packages/memory/src/batch.ts:232` (`digest`
omitted when `next === null`), `packages/memory/src/file-store/revisions.ts:68`.
Test: `packages/memory/src/testing.ts:1105`, `:1121`.
...
Production: `packages/memory/src/batch.ts:238`.
Test: `packages/memory/src/testing.ts:1136` (requires the harness's `poke`).

**MS-09 (INV-137).** A batch's declared `commit` work runs even when the batch body stages nothing.
Production: `packages/memory/src/batch.ts:197`–`:199` and the second early return at `:255`.
Test: `packages/memory/src/testing.ts:1155`.
...
Production: `packages/memory/src/file-store/recovery.ts:77` (an empty `listBatchIds` yields no
entries) and the sweep at `:189`.
Test: `packages/memory/src/testing.ts:1169`; the file-backed variant at
`packages/memory/tests/integration/journal-recovery.test.ts:169`.
...
Production: `packages/memory/src/file-store/layout.ts:19`–`:49` (every machinery path is derived from
`machineryRoot`).
...
Test: `packages/memory/tests/integration/machinery-split.test.ts:126`.
...
Production: `packages/memory/src/file-store/layout.ts:52` (the existence probe runs before the
directory is recreated) and `:59`.
Test: `packages/memory/tests/integration/machinery-split.test.ts:147`, `:163`.
...
Production: `packages/memory/src/file-store/layout.ts:35`–`:43` (the discard covers `.journal` as well
as `.state/indexed`).
Test: `packages/memory/tests/integration/machinery-split.test.ts:174`.
...
Production: `packages/memory/src/file-store/layout.ts:20` (`split` is false, so the discard never
runs).
Test: `packages/memory/tests/integration/machinery-split.test.ts:190`.
...
Production: `packages/memory/src/index.ts:44`.
Test: `packages/memory/tests/architecture/file-store-exports.test.ts:7`.

**MS-18.** The run-dedup ledger never appears in the document listing.
Production: `packages/memory/src/file-store/jobs.ts:101` (it lives under `.state/`) and
`packages/memory/src/file-store/documents.ts:69` (dot-prefixed entries are skipped).
Test: `packages/memory/src/testing.ts:762`; the file-specific form at
`packages/memory/tests/integration/file-store.test.ts:27`.
...
Production: `packages/memory/src/file-store/documents.ts:69`.
Test: `packages/memory/tests/integration/journal-recovery.test.ts:238`.
...
Production: `packages/memory/src/file-store/documents.ts:157` →
`packages/paths/src/atomic.ts:458`; directory mode at `packages/memory/src/file-store/layout.ts:32`.
Test: `packages/memory/tests/integration/file-store.test.ts:38`, `:46`.
...
Production: `packages/memory/src/file-store/journal.ts:63`, `:120`;
`packages/memory/src/file-store/revisions.ts:100`–`:101`;
`packages/memory/src/file-store/jobs.ts:414`. The durability distinction is stated at
`packages/paths/src/atomic.ts:488`.
...
Production: `packages/memory/src/file-store/revisions.ts:52` (only `.json` entries are collected) and
`:74` (a `read` first requires the id to appear in `list`).
Test: `packages/memory/tests/integration/journal-recovery.test.ts:227`;
`packages/memory/tests/integration/file-store-internals.test.ts:79` adds the mirror case — a `.json`
...
Production: `packages/memory/src/file-store/lock.ts:81`–`:91`, `:125`–`:130`.
Test: `packages/memory/tests/integration/file-store.test.ts:53` (steal), `:71` (live holder times
out), `:88` (each attempt sleeps 25 ms, so reaching a 200 ms deadline cannot be instant).

**MS-24.** A lock is released only by the holder that owns the current token.
Production: `packages/memory/src/file-store/lock.ts:150`.
Test: unpinned.
...
Production: `packages/memory/src/file-store/lock.ts:34`, `:170`.
Test: `packages/memory/tests/integration/file-store-observability.test.ts:31`, `:48`, `:58`.
...
Production: `packages/memory/src/file-store/recovery.ts:51`–`:75`, called only from `recoverOnce`
(`:200`).
Test: `packages/memory/tests/integration/file-store-observability.test.ts:150`, `:176`, `:188`,
`:234`.
...
Production: `packages/memory/src/journal.ts:157`–`:174`,
`packages/memory/src/file-store/recovery.ts:202`, gate at `packages/memory/src/file-store.ts:206`.
Test: `packages/memory/tests/integration/journal-recovery.test.ts:191` (reads and `list` still work;
...
Production: `packages/memory/src/journal.ts:139`–`:146`.
Test: `packages/memory/tests/unit/journal-recovery.test.ts:105`;
`packages/memory/tests/integration/journal-recovery.test.ts:191` asserts the bytes survive.

**MS-29.** A journal record whose `version` exceeds `JOURNAL_VERSION` is never acted on.
Production: `packages/memory/src/journal.ts:130`.
Test: `packages/memory/tests/unit/journal-recovery.test.ts:115`;
`packages/memory/tests/integration/file-store-internals.test.ts:196`.
...
Production: `packages/memory/src/journal.ts:137`, `packages/memory/src/file-store/recovery.ts:146`.
Test: `packages/memory/tests/unit/journal-recovery.test.ts:49`;
`packages/memory/tests/integration/journal-recovery.test.ts:132`.
...
Production: `packages/memory/src/journal.ts:147`–`:153`.
Test: `packages/memory/tests/unit/journal-recovery.test.ts:125`, `:140`.

**MS-32.** An unreadable or absent `prepare.json` is swept, not treated as a fatal condition.
Production: `packages/memory/src/file-store/journal.ts:82`–`:94` (JSON parse failure and shape
mismatch both return `null`), `packages/memory/src/file-store/recovery.ts:104`.
Test: `packages/memory/tests/integration/journal-recovery.test.ts:181`.
...
swept as corrupt.
Production: `packages/memory/src/file-store/journal.ts:95`–`:103` (the comment states the intent
verbatim), `packages/memory/src/file-store/recovery.ts:97`.
Test: `packages/memory/tests/integration/storage-limits.test.ts:294`, `:310`.
...
Production: `packages/memory/src/batch.ts:181` (`delete` reads first) and `:208`.
Test: `packages/memory/tests/integration/storage-limits.test.ts:90`.

**MS-36.** An oversized document write is refused before the file is created or replaced.
Production: `packages/memory/src/file-store/documents.ts:156` (the assertion precedes
`writeFileAtomic`).
Test: `packages/memory/tests/integration/storage-limits.test.ts:77`.
...
the 257th path.
Production: `packages/memory/src/batch.ts:154`–`:168` (checked in `handle.write`, before step 4), and
`:182` (`handle.delete`'s identical guard).
Test: `packages/memory/tests/integration/storage-limits.test.ts:334`, `:353` — both exercise 257
...
Production: `packages/memory/src/bounded-io.ts:150`–`:154`,
`packages/memory/src/file-store/documents.ts:64`–`:79`.
Test: `packages/memory/tests/integration/storage-limits.test.ts:130`, `:145`, `:166`.
...
Production: `packages/memory/src/file-store/documents.ts:87`, `:181`, `:214`, `:110`.
Test: `packages/memory/tests/integration/storage-limits.test.ts:166`, `:195`.
...
Production: `packages/memory/src/frontmatter.ts:115`, `:125`–`:127`.
Test: `packages/memory/tests/unit/frontmatter.test.ts:14`, `:21`, `:27`.
...
Production: `packages/memory/src/frontmatter.ts:161` (unknown line pushed to `extra`), `:211`
(re-emitted in place at a fixed position).
Test: `packages/memory/tests/unit/frontmatter.test.ts:46`, `:59`, `:68`; the practical consequence —
a user's `owner:` key surviving a placeholder-description repair — at
`packages/memory/tests/integration/reindex.test.ts:160`.
...
Production: `packages/memory/src/frontmatter.ts:157`–`:163`, `:209`–`:210`.
Test: `packages/memory/tests/unit/frontmatter.test.ts:89`, `:97`, `:104`.

**MS-43.** A `description` containing newlines is flattened to one line on write, so the reindex can
always find it.
Production: `packages/memory/src/frontmatter.ts:205`.
Test: `packages/memory/tests/unit/frontmatter.test.ts:38`.
...
Production: `packages/memory/src/reindex.ts:280`.
Test: `packages/memory/tests/integration/reindex.test.ts:42`.
...
Production: `packages/memory/src/reindex.ts:270`–`:273`.
Test: `packages/memory/tests/integration/reindex.test.ts:174`.

**MS-46.** Prose outside the managed Contents block is never touched, and a hand-written description
outranks a derived one.
Production: `packages/memory/src/reindex.ts:137`–`:155`, `:96`–`:99`.
Test: `packages/memory/tests/integration/reindex.test.ts:48`, `:101`.

**MS-47.** A child with no description is linked as a bare link, with no ` — ` separator.
Production: `packages/memory/src/reindex.ts:257`.
Test: `packages/memory/tests/integration/reindex.test.ts:152`.
...
Production: `packages/memory/src/reindex.ts:238`–`:268`.
Test: `packages/memory/tests/integration/reindex.test.ts:19`, `:143`.

**MS-49.** A query that reduces to no significant terms returns no hits, not the whole wiki.
Production: `packages/memory/src/query.ts:326`.
Test: `packages/memory/tests/component/query.test.ts:71`, `:78`.

**MS-50.** Ranking is a total order and independent of write order: the same corpus and query rank
identically every time.
Production: `packages/memory/src/query.ts:401`–`:413` (final tiebreak on path).
Test: `packages/memory/tests/component/query.test.ts:135`, `:144`.
...
Production: `packages/memory/src/query.ts:125`–`:162`.
Test: `packages/memory/tests/component/query.test.ts:207`, `:216`, `:231`.
...
Production: `packages/memory/src/text/grep.ts:251` (all three gates share one fallback), `:144`.
Test: `packages/memory/tests/unit/tokenize.test.ts:267` (behavioural: `(build+)+$` matches
`the build is broken` as a keyword search, which the compiled regex would not have), `:328`, `:334`,
`:339`, `:345`.
...
Production: `packages/memory/src/text/grep.ts:309`–`:317`, `:329`.
Test: `packages/memory/tests/unit/tokenize.test.ts:171`, `:279`; the runtime bound at
`packages/memory/tests/integration/grep-runtime-bound.test.ts:5`.

**MS-54.** The window step is strictly less than the window width, so a boundary cannot hide a match
shorter than the overlap.
Production: `packages/memory/src/text/grep.ts:311`.
Test: `packages/memory/tests/unit/tokenize.test.ts:187`, `:237`.
...
Production: `packages/memory/src/text/grep.ts:270`, `:314`.
Test: `packages/memory/tests/unit/tokenize.test.ts:176`, `:260`, `:288`.
...
Production: `packages/memory/src/text/tokenize.ts:108`–`:116`; snippet from `parsed.body` at
`packages/memory/src/query.ts:379`, `:418`.
Test: `packages/memory/tests/unit/tokenize.test.ts:37`, `:43`;
`packages/memory/tests/component/query.test.ts:59`, `:65`.
...
alive on its own.
Production: `packages/memory/src/clock.ts:37`.
Test: unpinned.
...
Production: `packages/memory/src/file-store/revisions.ts:111`–`:115`, constants at
`packages/memory/src/config.ts:46`.
Test: `packages/memory/tests/integration/file-store-internals.test.ts:124` calls `prune` directly
...
Production: `packages/memory/src/file-store/documents.ts:159`–`:167`,
`packages/memory/src/batch.ts:179`–`:193`.
Test: `packages/memory/src/testing.ts:649`–`:657` ("reports an absent document as null and delete as
false"), driven at `packages/memory/tests/contract/store.test.ts:41` against both backends.
...
Production: `packages/memory/src/file-store/jobs.ts:404`–`:415`.
Test: `packages/memory/src/testing.ts:750`–`:759` ("records a run in the ledger at most once").
...
Production: `packages/memory/src/text.ts:9`–`:12`.
Test: `packages/memory/tests/unit/text.test.ts:11`–`:14` ("never exceeds the cap, including at zero" —
...
Production: `packages/memory/src/memory.ts:24`–`:30`.
...
| --- | --- | --- |
| Bad document key | throws `MemoryPathError` synchronously inside an async method (so it surfaces as a rejected promise) | `packages/memory/src/paths.ts:36` |
| On-disk document over `documentBytes` | strict `read` throws `MemoryStorageLimitError`; `grep` skips it outright; `list` instead truncates it to a `prefixBytes` read (`truncate: true`), same as `readBounded`, and never observes the oversize; `readBounded` itself returns a truncated prefix | `packages/memory/src/bounded-io.ts:59`, `packages/memory/src/file-store/documents.ts:168`–`:197` (list), `:211` (grep) |
| File grew between stat and read | `MemoryStorageLimitError` when it crossed the limit, otherwise `Error("memory: file changed while it was being read")` | `packages/memory/src/bounded-io.ts:82`–`:92` |
| File changed during `version()` hashing | `Error("memory: file changed while it was being hashed")` | `packages/memory/src/file-store/documents.ts:126` |
| File vanished mid-`version()` | contributes 0 bytes, no error | `packages/memory/src/file-store/documents.ts:131` |
| File vanished mid-`grep` | swallowed if `ENOENT`, rethrown otherwise | `packages/memory/src/file-store/documents.ts:231` |
| `stat` fails while listing | `updated_at` falls back to the injected clock | `packages/memory/src/file-store/documents.ts:183`–`:186` |
| Missing directory in a scan | reported as an empty scan | `packages/memory/src/bounded-io.ts:144` |
| Lock contention | poll every 25 ms until `timeoutMs`, then `Error("memory: timed out waiting for tree lock <dir>")`; a debug `memory.lock.wait` records `waited_ms` and `stolen` | `packages/memory/src/file-store/lock.ts:125`, `:133` |
| Lock cleanup / steal / release / heartbeat failure | best-effort, routed through one observation seam rather than `process.emitWarning` | `packages/memory/src/file-store/tasks.ts:1`–`:35` |
| Interrupted batch, `committed` | swept | `packages/memory/src/journal.ts:136` |
| Interrupted batch, `applied` | commit work replayed, then swept | `packages/memory/src/file-store/recovery.ts:146` |
| Interrupted batch, neither marker, tree consistent | rolled back to the pre-batch bytes | `packages/memory/src/file-store/recovery.ts:147`–`:188` |
| Interrupted batch, third-party edit / newer version / missing pre-image / over-limit record / over-limit document / over-limit journal scan | `required`; the store keeps serving reads and refuses every `batch` with `MemoryRecoveryRequiredError` | `packages/memory/src/journal.ts:130`–`:153`, `packages/memory/src/file-store/recovery.ts:83`–`:127`, `:202` |
| Corrupt `prepare.json` | read as `null`, batch swept | `packages/memory/src/file-store/journal.ts:89`–`:94` |
| Corrupt revision `.json` | skipped silently by `list` (the `try` around `JSON.parse` has an empty catch) | `packages/memory/src/file-store/revisions.ts:61`–`:66` |
| Revision `.json` truncated or over `metadataBytes` | skipped; the read is `truncate: true` and a truncated result returns early | `packages/memory/src/file-store/revisions.ts:53`–`:58` |
| Revision list past `corpusBytes` | the visitor returns `false`, stopping the scan, without an error | `packages/memory/src/file-store/revisions.ts:59` |
| Revision pruning failure | best-effort per file, never rejects | `packages/memory/src/file-store/revisions.ts:117`–`:123` |
| Revision pruning interrupted between its two deletes | `prune()` removes a revision's `.json` metadata first, then its `.md` body, in two separate best-effort calls; since `list()` (and `read()`, per MS-22) only enumerates `.json` files, a crash between the two permanently orphans the `.md` body — it no longer appears in any future `list()`, so a later `prune()` pass can never target it. Untested: no test in `packages/memory/tests` exercises an interrupted `prune()` | `packages/memory/src/file-store/revisions.ts:116`–`:123` |
| Unclosed frontmatter in an index file | that directory is skipped for the whole pass; `memory.document.skipped` at debug | `packages/memory/src/reindex.ts:270` |
| Child document with no description | linked without a description, `memory.document.skipped { reason: "no_description" }` at debug | `packages/memory/src/reindex.ts:256` |
| Regex refused / over-long / uncompilable | degrades to keyword scan, silently | `packages/memory/src/text/grep.ts:251` |
| Grep budget exhausted | returns whatever was found; the module states this is silent by design | `packages/memory/src/text/grep.ts:74`, `:270` |
| Query over its caps | still answers, flagging `truncated` plus a `truncation_reasons` list of `query` / `documents` / `document_bytes` / `corpus_bytes` | `packages/memory/src/query.ts:315`, `:324`, `:347`, `:352` |
| `JSON.stringify` failure in the digest | the args excerpt degrades to `"{}"` | `packages/memory/src/digest.ts:95` |
| git unavailable / not a repo / probe timeout | `WorkspaceState` is `undefined`, debug log only | `packages/memory/src/workspace-state.ts:42`–`:58` |

There are **no retries and no timeouts** in this layer other than the tree-lock wait
(`packages/memory/src/file-store/lock.ts:97`), the git probe
(`packages/memory/src/workspace-state.ts:8`) and the grep wall-clock budget
(`packages/memory/src/text/grep.ts:89`). Retry policy for index jobs is the sibling document's.
...
- `file-store.ts` is the only composer: it imports all seven `file-store/*` modules plus `batch.ts`
  (`packages/memory/src/file-store.ts:16`–`:23`). Nothing else does.
- `batch.ts` depends on `frontmatter`, `paths`, `revisions`, `storage-limits` and `journal`
  (`packages/memory/src/batch.ts:11`–`:24`) but on **no** backend — the `BatchPrimitives` port
  (`:52`) is what keeps the sequence backend-agnostic, and both adapters implement it
  (`packages/memory/src/file-store.ts:174`, `packages/memory/src/testing.ts:504`). The in-memory
  ...
  batch body returns (`packages/memory/src/testing.ts:498`–`:542`).
- `journal.ts` holds the record shape and the **pure** decision; the I/O lives in
  ...
  the rule is testable without a filesystem (`packages/memory/src/journal.ts:9`).
- `tree.ts` was extracted from the reindex pass so health diagnostics and ranked query ask the same
  code rather than reimplementing the pyramid (`packages/memory/src/tree.ts:5`); its consumers are
  `packages/memory/src/reindex.ts:28` and `packages/memory/src/query.ts:27`.
- `text/grep.ts` is shared by both adapters — `packages/memory/src/file-store/documents.ts:17` and `packages/memory/src/testing.ts:33` — so
  "what counts as a match" has one definition (`packages/memory/src/text/grep.ts:4`).
- `similar.ts` imports `paths` and `text/tokenize` but is imported only by
  `packages/memory/tests/component/query.test.ts:4`, and is not in the barrel
  (`packages/memory/src/index.ts`). Its own header states it "has no production caller" and is kept as
  the baseline that keeps the ranker comparison honest (`packages/memory/src/similar.ts:6`–`:15`). The
  ...
  (`packages/memory/src/similar.ts:5`–`:9`). The indexer now shares `query.ts`'s BM25 ranker instead.
- `file-store/tasks.ts` is the single shared failure-observation seam for every best-effort file-store
  ...
  (`packages/memory/src/file-store/tasks.ts:9`–`:16`). Five call sites route through it: lock cleanup,
  the stale-lock steal, lock release and the heartbeat (`packages/memory/src/file-store/lock.ts:106`, `:119`, `:141`,
  `:151`), job pruning (`packages/memory/src/file-store/jobs.ts:392`), revision metadata and body pruning
  (`packages/memory/src/file-store/revisions.ts:117`, `:121`), the orphan-bookkeeping cleanup on a vanished split wiki
  (`packages/memory/src/file-store/layout.ts:40`), and journal sweep (`packages/memory/src/file-store/journal.ts:132`).
...
carries `node:assert/strict` and nothing else (`packages/memory/src/testing.ts:10`).
...
2. **Durability of the machinery writes is unpinned.** MS-21 states the code calls `writeFileDurable`,
   and `packages/paths/src/atomic.ts:488` states what that adds, but no test in
...
4. **Rationale is largely absent from the code and deliberately not inferred here.** Where a doc
   ...
   (`packages/memory/src/paths.ts:57`), the `applied` marker splitting the recovery policy
   (`packages/memory/src/journal.ts:110`), the multiplicative boost clamp
   (`packages/memory/src/query.ts:143`), the window-versus-truncate trade
   (`packages/memory/src/text/grep.ts:34`), the "never index from inside `exclusive`" rule
   (`packages/memory/src/file-store/lock.ts:26`) — it is quoted above. Everywhere else the mechanism
