...
it through `runLeader` (`packages/workflows/src/run-leader.ts`), meters its output tokens against
a workflow-child ledger (`packages/workflows/src/ledger.ts`), and bounds how many run at once with a
FIFO semaphore (`packages/workflows/src/concurrency.ts`). A separate `WorkflowLeaderCount`
...
ad-hoc leader (`packages/workflows/src/tool.ts`); `run_work_items` takes a decomposition and lets
the runtime derive the dependency order and the write-conflict separation
(`packages/workflows/src/work-items.ts`, `packages/workflows/src/schedule.ts`); `run_round`
...
| `./schemas` | `packages/workflows/src/schemas.ts` | result schemas + `WORKFLOW_LIMITS` re-export (`packages/workflows/src/schemas.ts`) |
...
| `runLeader` | `(spec, ctx, runId?, heldReservation?) => Promise<LeaderResult>` | `packages/workflows/src/run-leader.ts` |
| `createWorkflowLedger` | `(total: number \| null) => WorkflowLedger` | `packages/workflows/src/ledger.ts` |
...
| `createWorkflowSemaphore` | `= createSemaphore` from `@clarvis/capability` | `packages/workflows/src/concurrency.ts` |
| `buildRunLeaderTool` / `RUN_LEADER_TOOL_NAME` | `(profiles?) => NamespacedTool` / `"run_leader"` | `packages/workflows/src/tool.ts`, `packages/workflows/src/tool.ts` |
| `WORKFLOW_LIMITS` | frozen numeric ceilings | `packages/workflows/src/limits.ts` |
...
| `recordWorkflowTrace`, `WORKFLOW_TRACE_KINDS`, `WORKFLOW_PERSISTED_TRACE_PROJECTORS`, the three `is*` guards | trace vocabulary | `packages/workflows/src/trace-events.ts` |
...
`packages/workflows/src/run-round.ts`; `scheduleWorkItems` (`packages/workflows/src/schedule.ts`);
`toWorkItem` / `workItemBrief` in `packages/workflows/src/work-items.ts`; `interpolate` /
`placeholders` (`packages/workflows/src/interpolate.ts`), and the pure round vocabulary in
...
| Property | Type | Bound | File |
| --- | --- | --- | --- |
| `title` | string | `minLength 1`, `maxLength LEADER_TITLE_MAX` (= `TASK_TITLE_MAX`, 60) | `packages/workflows/src/tool.ts`, `packages/workflows/src/tool.ts` |
| `prompt` | string | `minLength 1`, `maxLength WORKFLOW_LIMITS.textChars` | `packages/workflows/src/tool.ts` |
| `profile` | string enum | present **only** when `profiles.length > 0`; enum = profile names | `packages/workflows/src/tool.ts` |
| `expect_schema` | object | free-form JSON Schema | `packages/workflows/src/tool.ts` |
...
### 3.1 `WORKFLOW_LIMITS` (`packages/workflows/src/limits.ts`)
...
| `artifactBytes` / `briefBytes` / `catalog*` | see `packages/workflows/src/limits.ts` | document loading (delegated) |

`isBoundedWorkflowString(value, max)` (`packages/workflows/src/limits.ts`) is the single string-admission predicate used
...
| agent handle id | `ag_` + 8 hex, minted by the supervision registry | asserted `packages/workflows/tests/component/run-leader.test.ts` |
...
### 3.3 Selector and accept grammars (`packages/workflows/src/rounds.ts`)
...
| Written | Parsed | File |
| --- | --- | --- |
| `once` | `{ kind: "once" }` | `packages/workflows/src/rounds.ts` |
| `each(discover.work_items)` | `{ kind:"each", source }` | `packages/workflows/src/rounds.ts` |
| `each(review.findings where needs_verification)` | `… where:{field}` | `packages/workflows/src/rounds.ts` |
| `each(r.items where flag = true)` | `equals` coerced by `literal()` — `true`/`false`, numeric, else string, quotes stripped | `packages/workflows/src/rounds.ts` |
| `all(review.coverage_gaps)` | `{ kind:"all", source }`; a `where` on `all` is **refused** | `packages/workflows/src/rounds.ts` |
| `all(f, v)` / `any(f, v)` / `majority(f, v)` | rule without `count`; a count supplied is refused | `packages/workflows/src/rounds.ts` |
| `threshold(f, v, n)` | requires `count`, else `null` | `packages/workflows/src/rounds.ts` |

Pinned exhaustively in `packages/workflows/tests/unit/rounds.test.ts`.

### 3.4 Brief interpolation (`interpolate.ts`)

Placeholder regex `\{\{\s*([A-Za-z_][\w]*(?:\.[\w-]+)*)\s*\}\}` (`packages/workflows/src/interpolate.ts`). Only three
roots resolve — `item`, `args`, `state` (`packages/workflows/src/interpolate.ts`). A bare `{{item}}` renders the whole
item (`packages/workflows/src/interpolate.ts`); a non-string value is `JSON.stringify`d (`packages/workflows/src/interpolate.ts`). An
unresolvable placeholder is an **error**, and the first failure wins
(`packages/workflows/src/interpolate.ts`; `packages/workflows/tests/unit/interpolate.test.ts`).

Field access after the root is the same code as §3.3's selector `where` clause and `foldRound`'s
`accept`/dedupe field reads: `packages/workflows/src/interpolate.ts` calls `readPath` imported from `rounds.ts`
(`packages/workflows/src/interpolate.ts`), and `rounds.ts`'s own doc comment states the sharing is deliberate — "two
copies of what a dotted path means is one too many" (`packages/workflows/src/rounds.ts`).

### 3.5 Trace records (`trace-events.ts`)

Three kinds: `workflow_run_started`, `workflow_run_completed`, `workflow_run_failed`
(`packages/workflows/src/trace-events.ts`). Start detail (`packages/workflows/src/trace-events.ts`): `run_id`, `parent_run_id`, `title`,
`task`, optional `profile`, `round_id`, `pass`, `item_index`, `replica`, `replica_count`. Terminal
detail (`packages/workflows/src/trace-events.ts`): `run_id`, `parent_run_id`, `status`, optional
`error:{code,message}`.
...
(`packages/workflows/src/trace-events.ts`); the overloads are what tie a kind to its detail type at compile time
(`packages/workflows/src/trace-events.ts`).
...
`{discovery, findings, verdict}` (`packages/workflows/src/schemas.ts`). Pinned by the
...
### 3.8 Leader result text (`packages/workflows/src/result-text.ts`)
...
falling back to the literal `"[unserializable result]"` if that throws (`packages/workflows/src/result-text.ts`).
...
`runLeader(spec, ctx, runId, heldReservation?)` (`packages/workflows/src/run-leader.ts`):
...
### 4.7 The scheduler (`packages/workflows/src/schedule.ts`)

`scheduleWorkItems` runs five ordered checks before deriving anything:

| # | Check | Code | File |
| --- | --- | --- | --- |
| 1 | batch size ≤ `workItems` | `limits_exceeded` | `packages/workflows/src/schedule.ts` |
| 2 | per-item string/array bounds | `limits_exceeded` (ids truncated to `identifierChars` in the message) | `packages/workflows/src/schedule.ts` |
| 3 | ids unique | `duplicate_id`, `ids` = repeated ids in first-seen order | `packages/workflows/src/schedule.ts` |
| 4 | every dependency names a batch member | `unknown_dependency`, `ids` = the dependents | `packages/workflows/src/schedule.ts` |
| 5 | no cycle | `dependency_cycle`, `ids` = **only** the unplaced members | `packages/workflows/src/schedule.ts` |

Then per topological layer (`topologicalLayers`, `packages/workflows/src/schedule.ts`) it normalizes paths and packs the
layer into conflict-free waves (`packages/workflows/src/schedule.ts`).

- **Normalization** (`packages/workflows/src/schedule.ts`): trim, `\` → `/`, strip leading `./` repeatedly, collapse
  `//+`, drop trailing `/`, **lowercase**. An entry that normalizes to empty is dropped
  (`packages/workflows/src/schedule.ts`), which leaves a mutator unscoped (`packages/workflows/tests/unit/schedule.test.ts`).
- **Overlap** (`packages/workflows/src/schedule.ts`): equality or per-segment containment — `packages/loop` covers
  `<workspace>/packages/loop/src/file.ts`, `packages/loo` covers neither.
- **Conflict** (`packages/workflows/src/schedule.ts`) is reader-writer, not writer-writer: two non-mutators never
...
- **Packing** (`packages/workflows/src/schedule.ts`) is first-fit in **input order**, so the same `work_items[]`
  always yields the same waves (`packages/workflows/tests/unit/schedule.test.ts` asserts both determinism and
  the exact `[["a","b"],["d"],["c"]]` shape).

Complexity is documented as O(n²·f²) with an explicit instruction not to optimize it
(`packages/workflows/src/schedule.ts`).
...
2. `selectItems(over, state)` — `once` yields `[undefined]`, `each` yields the
   ...
   (`packages/workflows/src/rounds.ts`).
...
**`applyAccept`** (`packages/workflows/src/rounds.ts`): a replica whose `field` is not a string is tallied under
`UNAVAILABLE` = `"(unavailable)"` and stays in the denominator (`packages/workflows/src/rounds.ts`).
`all` requires `total > 0 && hits === total`; `any` is `hits > 0`; `majority` is `hits*2 > total`
(so a tie is not a majority); `threshold` is `hits >= count` (`packages/workflows/src/rounds.ts`). Pinned:
`packages/workflows/tests/unit/rounds.test.ts`.

**`nextRepeat`** (`packages/workflows/src/rounds.ts`) precedence: budget → `max_rounds` → (`until === "no_new"` only)
`dryRounds >= dry_rounds ?? 2` (`packages/workflows/src/rounds.ts`, default). Under `until: "budget"` dry
passes are ignored entirely (`packages/workflows/tests/unit/rounds.test.ts`).

**`admitNew` / `dedupeKey`** (`packages/workflows/src/rounds.ts`): dedupe is against everything seen so far,
never against survivors; a key is the declared fields joined by a `\0` **escape**
(`packages/workflows/src/rounds.ts`, with the escape called out); values normalize case-insensitively,
arrays sort before joining (`packages/workflows/src/rounds.ts`).
...
| --- | --- | --- |
| `run_leader` | `parseLeaderSpec` | non-object → error; `prompt` required non-empty ≤ `textChars`; `title` ≤ `TASK_TITLE_MAX*2` then re-validated by `parseTaskTitle` — **no fallback to the prompt** (`packages/workflows/src/tool.ts`); `profile` non-empty ≤ `identifierChars`; `expect_schema` accepted as any non-null object |
...
- `reserveOutput(n)` (the `OutputTokenBudget` port) grants `min(n, remaining())`, and `settle`
  converts `min(amount)` into spend exactly once (`directReservation` inside
...
`workflowLogger` (`packages/workflows/src/log.ts`); nothing constructs a logger (`packages/workflows/src/log.ts`).
...
| `workflow.leader_started` | debug (guarded by `levelEnabled`) | `packages/workflows/src/run-leader.ts` |
...
| `workflow.schedule_derived` / `_refused` | debug / warn | `packages/workflows/src/schedule-log.ts` |
...
(`packages/workflows/src/log.ts`); `packages/workflows/tests/component/observability.test.ts` asserts the started-leader record does
not contain the brief. `joinIds` (`packages/workflows/src/log.ts`) exists because array field values are dropped by the
sink, and truncates past 32 entries (`packages/workflows/src/log.ts`).
...
`@clarvis/loop`. Production: `packages/workflows/src/types.ts`,
`packages/workflows/src/elicit-mux.ts`. Test:
`packages/workflows/tests/architecture/dependency-direction.test.ts` (an exact-array equality,
so a third site fails it). `packages/workflows/src/log.ts` records that `withBoundLogger` is typed *structurally*
...
`total`, whatever the batch size. Production: `packages/workflows/src/ledger.ts`.
Test: `packages/workflows/tests/unit/ledger.test.ts`;
`packages/workflows/tests/component/run-leader.test.ts`.
...
order. Production: `packages/workflows/src/schedule.ts`. Test:
`packages/workflows/tests/unit/schedule.test.ts`.

**INV-W21.** Path comparison is case-insensitive and separator-normalized, and containment is tested
per path segment. Production: `packages/workflows/src/schedule.ts`. Test:
`packages/workflows/tests/unit/schedule.test.ts`.

**INV-W22.** The schedule is deterministic and follows the emitted item order rather than a sort.
Production: `packages/workflows/src/schedule.ts` (first-fit in input order). Test:
`packages/workflows/tests/unit/schedule.test.ts`.
...
`packages/workflows/src/interpolate.ts`. Test:
`packages/workflows/tests/unit/interpolate.test.ts`.

**INV-W28.** A replica that failed or answered off-schema stays in the accept denominator, tallied as
`(unavailable)`. Production: `packages/workflows/src/rounds.ts`. Test:
`packages/workflows/tests/unit/rounds.test.ts`.
...
verification. Production: `packages/workflows/src/rounds.ts` (`next` seeded from `seen`) and
...
`packages/workflows/src/rounds.ts`. Test:
`packages/workflows/tests/unit/rounds.test.ts`.
...
emitting a partial event. Production: the projectors in
`packages/workflows/src/trace-events.ts`. Test:
`packages/workflows/tests/unit/trace-events.test.ts`.
...
| leader `executeRun` throws | caught in `runLeader`; `status:"error"`, `code:"leader_run_failed"`; the stack exists only in the log | `packages/workflows/src/run-leader.ts` |
...
| a replica died | counted in the accept denominator as `(unavailable)` | `packages/workflows/src/rounds.ts` |
...
| host wired no logger | `NOOP_LOGGER`, and `withBoundLogger` returns the deps object **by identity** so `executeRun` sees exactly what the host supplied | `packages/workflows/src/log.ts`; test `packages/workflows/tests/component/observability.test.ts` |
| a `cause` that is neither `Error` nor string | dropped rather than coerced to `[object Object]` | `packages/workflows/src/log.ts` |

Silently tolerated: a `file` entry that normalizes to the empty string is dropped, which silently
widens a mutator to unscoped (`packages/workflows/src/schedule.ts`) — visible only through
...
| --- | --- | --- |
| `@clarvis/capability` | runtime, static | `bind`, `parseTaskTitle`, `TASK_TITLE_MAX` (`packages/workflows/src/capability.ts`); `createSemaphore` (`packages/workflows/src/concurrency.ts`); `levelEnabled`, `NOOP_LOGGER`, `sanitizeErrorMessage` (`packages/workflows/src/run-round.ts`, `packages/workflows/src/log.ts`); `elicitWithClockPause` (`packages/workflows/src/run-workflow.ts`) |
| `@clarvis/supervision` | runtime, static | `AGENT_REGISTRY_PORT` + `registerBackgroundChild` (`packages/workflows/src/capability.ts`, `packages/workflows/src/dispatch.ts`); `AGENTS_MAX_LIVE_CHILDREN` for the floor (`packages/workflows/src/settings.ts`) |
| `@clarvis/loop` | two sites: type-only from the root, runtime from the workflow adapter | `packages/workflows/src/types.ts` type-imports `ExecuteRunArgs`, `ExecuteRunDeps`, `ExecuteRunOutcome`; `packages/workflows/src/elicit-mux.ts` value-imports `createElicitSerializer` from `@clarvis/loop/workflows` — pinned by `packages/workflows/tests/architecture/dependency-direction.test.ts` |
| `zod` | runtime | the settings schema (`packages/workflows/src/settings.ts`) |
...
(`packages/workflows/src/types.ts`): only `generateExecutionId` and `executeRun`. A host supplies the real
...
| Consumer | Edge | Cite |
| --- | --- | --- |
| `@clarvis/kernel` — capability registry | registers `workflowsSettingsSpec` at module load | `packages/kernel/src/config/capability-registry.ts` |
...
| `@clarvis/kernel` — event mapping | consumes the trace-event guards/types | `packages/kernel/src/runs/map-events.ts` |
| `@clarvis/kernel` — workflow policy | `LeaderProfileInfo` (type-only) | `packages/kernel/src/application/workflow-policy.ts` |

Nothing in `@clarvis/loop`, `@clarvis/supervision` or `@clarvis/capability` imports this package;
their only occurrences of the name are TSDoc prose (`packages/loop/src/workflows.ts`,
`packages/supervision/src/index.ts`, `packages/supervision/src/spawn-child.ts`,
`packages/capability/src/agents-port.ts`). `@clarvis/loop`'s
`packages/loop/tests/architecture/no-feature-names.test.ts` explicitly exempts `@clarvis/workflows` from its
...
kernel comments state that at `packages/kernel/src/kernel.ts`, and the grant is stripped from
