...
(`packages/capability/src/elicit.ts:85`) — carries every such question: the model's own `ask_user`
...
same per-run FIFO serializer (`packages/loop/src/runtime/elicit-relay.ts:14`) so at most one prompt is
ever live for a given run at a time, and — one level up — a workflow's concurrently running leaders
share a second, tree-wide FIFO (`packages/workflows/src/elicit-mux.ts:56`) so at most one prompt is
...
(`packages/kernel/src/runs/elicit-bridge.ts:33`), and each of `code` (a terminal) and `server` (an
...
(`packages/server/src/mcp/elicitation.ts:70`). A run that has no human attached at all (the server's
...
|---|---|---|---|
| `ElicitationAction` | type | `packages/capability/src/elicit.ts:15` | `"accept" \| "decline" \| "cancel"` |
| `ElicitationOutcome` | interface | `packages/capability/src/elicit.ts:22` | `{ action, answer?, noResponse? }` |
| `ElicitRequestedSchema` | interface | `packages/capability/src/elicit.ts:33` | `{ type: "object", properties: Record<string,{type:"string",enum?,description?}>, required: string[] }` |
| `ElicitParams` | interface | `packages/capability/src/elicit.ts:44` | `{ message, requestedSchema, kind?: "ask_user"\|"guard_confirm"\|"plan_review"\|"workflow_review"\|(string&{}) }` |
| `ElicitRawResult` | interface | `packages/capability/src/elicit.ts:75` | `{ action, content?: Record<string,unknown> }` |
| `Elicit` | type | `packages/capability/src/elicit.ts:85` | `(params, {signal?, timeoutMs?}) => Promise<ElicitRawResult>` |
| `ElicitTimeoutError` | class | `packages/capability/src/elicit.ts:97` | thrown when the wait bound elapses |
| `elicitWithClockPause` | function | `packages/capability/src/elicit.ts:126` | runs an `Elicit` call with the compute clock paused; swallows `ElicitTimeoutError` into `onNoResponse` |

### 2.2 The `ask_user` tool (`@clarvis/loop`)

| Item | Value |
|---|---|
| Wire name | `ASK_USER_TOOL_NAME` = `"ask_user"` (`packages/loop/src/runtime/tools/wire-names.ts`, re-exported `packages/loop/src/runtime/tools/ask-user-tool.ts:24-26`) |
| Descriptor | `askUserTool: NamespacedTool` (`packages/loop/src/runtime/tools/ask-user-tool.ts:40`) |
| Input schema | `{ question: string (minLength 1, required), options?: string[] }` (`:49-65`) |
| Capability name | `ASK_USER_CAPABILITY_NAME` = `"ask-user"` (`packages/loop/src/runtime/capabilities/ask-user.ts:22`) |
| Grant that gates it | run request's entry profile must include `"ask_user"` (`packages/loop/src/runtime/capabilities/ask-user.ts:39`) |
| Activation | only the run's **entry agent** (`scope.entry`); a spawned sub-agent never gets it (`:44`) |
| Advertisement | `advertised: false` — prompt-driven, not registry-listed (`packages/loop/src/runtime/capabilities/ask-user.ts:68`) |
| Reservation | `ASK_USER_TOOL_NAME` is one of `BUILTIN_WIRE_NAMES` (`packages/loop/src/runtime/tools/wire-names.ts:42-45`), which spreads into `RESERVED_WIRE_NAMES` (`:119-122`); `buildRegistry` seeds that set into the registry's `used` names, so no MCP server tool can take it (`packages/loop/src/runtime/tools/mcp-registry.ts:37`) |
| Bypass entrypoint | `askUserAgentCapability(askUser)` (`packages/loop/src/runtime/capabilities/ask-user.ts:62-72`) — the per-agent attachment alone, for a caller that already holds an `AskUser`; contributes the tool/handler without the run-level grant/entry gate `createAskUserCapability` enforces. Used directly by `packages/loop/tests/unit/run-agent.test.ts` and `packages/loop/tests/unit/ask-user-call.test.ts` to attach the tool for testing without those checks |

### 2.3 Guard elicitation (`@clarvis/kernel`)

| Symbol | Location | Shape |
|---|---|---|
| `GuardElicitParams` | `packages/kernel/src/guard/guard-elicit.ts:36` | `ElicitParams & { detail?: ElicitationCommandDetail }` |
| `createGuardElicit` | `packages/kernel/src/guard/guard-elicit.ts:118` | adapts an `Elicit` into a boolean `GuardElicit` (`req => Promise<boolean>`) |
| `createGuardSessionAllowlist` | `packages/kernel/src/guard/guard-elicit.ts:73` | in-memory, session-lifetime `GuardSessionAllowlist` |
| Requested schema | `decision: enum["deny","allow"]` or, when a session allowlist and a decidable non-empty command are both present, `enum["deny","allow","allow_session"]` (`:131-153`) |
| `kind` sent | `"guard_confirm"` (`:140`) |
| Wait bound | `ELICIT_NO_TIMEOUT_MS` = `2_147_483_647` ms — effectively unbounded unless the signal aborts (`:11-27,165-168`) |
...
(`packages/tools/src/guard/types.ts:41-48,152-159`). `createGuardElicit` itself still returns a
boolean; the kernel resolver enriches human answers and preserves judge/session attribution before
returning its run-bound guard callback (`packages/kernel/src/guard/resolver.ts:178-190,287-315`). The
...
| Item | Location | Shape |
|---|---|---|
| `clarvis_run` input fields (elicitation-relevant) | `packages/server/src/mcp/tools.ts:70-71` | `elicitations: enum["auto_decline","await"].default("auto_decline")`, `elicitation_wait_ms?: number.int().min(1000).max(600000)` |
| `clarvis_run` output field | `packages/server/src/mcp/tools.ts:75-80` | `posture: { elicitation, guard_confirmations, plans_effective?, downgrades: string[], auto_answered: number }` |
| `clarvis_respond` input | `packages/server/src/mcp/tools.ts:118-123` | `{ execution_id, id, action, content? }` |
| `clarvis_respond` description | `packages/server/src/mcp/tools.ts:153-155` | `"Only meaningful when the run was started with elicitations: \"await\"; otherwise questions are declined automatically."` |
| `ElicitationPosture` | `packages/server/src/mcp/elicitation.ts:13` | `"relay" \| "tool" \| "auto_decline"` |
| `AppliedPosture` | `packages/server/src/mcp/elicitation.ts:16` | `{ elicitation, guard_confirmations, plans_effective?, prompt_cache_ttl?, downgrades: string[], auto_answered: number }` |
| `resolvePosture(input)` | `packages/server/src/mcp/elicitation.ts:70` | decides the posture before the run starts |
| `createElicitationController(opts)` | `packages/server/src/mcp/elicitation.ts:188` | owns one run's `onElicit` handler + pending map |
| MCP wire request the server sends its own client | `packages/server/src/mcp/server.ts:148-150` | `{ method: "elicitation/create", params: request }` via `extra.sendRequest`, answer validated against `z.object({ action: enum, content?: record })` |
| Env defaults | `packages/server/src/config/env.ts:55-57` | `CLARVIS_SERVER_ELICIT_TOOL_WAIT_MS` = 120,000; `CLARVIS_SERVER_ELICIT_RELAY_MS` = 600,000; `CLARVIS_SERVER_ELICIT_BACKSTOP_MS` = 60,000 |
| Backstop wiring | `packages/server/src/bin.ts:323` | `CLARVIS_DEFAULT_ELICIT_WAIT_MS := CLARVIS_SERVER_ELICIT_BACKSTOP_MS` (60s), overriding the engine's own 30-minute default for every server-hosted run |
...
| Capability advertisement | `packages/mcp-client/src/client.ts:193-202` | `capabilities: relay ? { elicitation: {} } : {}` — presence of a `relay` is what tells the external server Clarvis can answer |
| Request handler | `packages/mcp-client/src/client.ts:198-202` | `client.setRequestHandler(ElicitRequestSchema, ...)` forwards `request.params` to `relay.handle` |
| Pooled connections | `packages/mcp-client/src/client.ts:181-183` (doc) | opened **without** a relay — no single human to route to for a shared subprocess |
...
| Symbol | Location | Shape |
|---|---|---|
| `ElicitMux` | `packages/workflows/src/elicit-mux.ts:22` | `{ manager: Elicit; forLeader(runId: string): Elicit }` |
| `ElicitMuxOptions` | `packages/workflows/src/elicit-mux.ts:28` | `{ logger?: Logger }` |
| `createElicitMux(user, options?)` | `packages/workflows/src/elicit-mux.ts:56` | wraps a host `Elicit` in one tree-wide FIFO |
...
| `ElicitField`/`ElicitFieldKind` | `packages/code/src/adapters/elicitation.ts:12-23` | one input field of a parsed form — `{name, title, description?, required, kind: "select"\|"text"\|"number"\|"boolean", options, default?}` |
| `initialValues`/`ChoiceInitialSelection` | `packages/code/src/adapters/elicitation.ts:150-175` | seeds each field's starting string value, per the `"first"`/`"none"` policy (§3.6) |
...
`buildElicitParams` (`packages/loop/src/runtime/tools/ask-user-tool.ts:132`) turns an `ask_user` call's
...
`ELICIT_RESPONSE_FIELD = "response"` (`packages/loop/src/runtime/tools/ask-user-tool.ts:33`) is the single property name every
`ask_user` answer is keyed by; `extractAnswer` (`:155-161`) reads it back: the field's own string value
...
`decision` instead (`packages/kernel/src/guard/guard-elicit.ts:144`), and carry the additional
`GuardElicitParams.detail: ElicitationCommandDetail` — `{ command, cwd, reason, warning? }` — built at
`:156-163` only when a `command` string argument and a `workspaceRoot` are both known; `cwd` resolves a
relative `cwd` argument against `workspaceRoot` via `node:path`'s `resolve`.

### 3.2 Protocol `ElicitationRequest` id format

`createElicitBridge` mints ids as `` `${executionId}:elicit:${seq++}` `` (`packages/kernel/src/runs/elicit-bridge.ts:51`), a
...
`packages/loop/src/runtime/tools/ask-user-tool.ts:171`:
...
|---|---|---|
| `elicitation_requested` | `ElicitationRequestedDetail` (`packages/capability/src/trace-kinds.ts:567`): `{ agent?, subagent_instance_id?, iteration_ref?, source: "ask_user"\|"tool_relay", question, options? }` | `handleAskUserCall` (`packages/loop/src/runtime/tools/ask-user-call.ts:80`, `source: "ask_user"`) and `buildElicitRelay`'s relay handler (`packages/loop/src/runtime/elicit-relay.ts:88`, `source: "tool_relay"`) |
| `user_question` | `UserQuestionDetail` (`packages/capability/src/trace-kinds.ts:340`): `{ agent, subagent_instance_id?, iteration_ref, question, outcome, answer?, options? }` | `handleAskUserCall` (`packages/loop/src/runtime/tools/ask-user-call.ts:97`) |
...
(`packages/loop/src/runtime/elicit-relay.ts:88-91`) has no call analogous to `handleAskUserCall`'s
...
`sessionKey(segment)` (`packages/kernel/src/guard/guard-elicit.ts:63`) = the segment's
...
(`:73-83`) both return/no-op immediately for an `undecidable` or zero-segment command — an
...
`fieldFromSchema`/`optionsFromSchema` (`packages/code/src/adapters/elicitation.ts:44-84`) turn one
...
`acceptResult`/`DECLINE_RESULT`/`CANCEL_RESULT` (`:220-227`) are the three `ElicitResult` constants a
view resolves an elicitation with.

### 3.7 Guard elicitation message construction

`createGuardElicit` (`packages/kernel/src/guard/guard-elicit.ts:118-164`) builds the human-facing
...
- **Reason.** `req.reason` is used when present; otherwise a default reason is synthesized as
  `` `Tool "${req.tool}" requires confirmation.` `` (`:120`).
- **Command line.** When `req.args?.command` is a non-blank string, the message gets a literal
  ...
  `` `Command segments: ${req.shell.segments.map(s => s.normalized).join(", ")}` `` (`:122-129`).
- **Undecidable warning.** When `req.shell?.undecidable`, the fixed line
  `"Warning: this command contains undecidable expansions."` (`UNDECIDABLE_WARNING`, `:29`) is appended
  (`:130`).
- **`allow_session` offer.** The `decision` schema's `enum` includes `"allow_session"` only when an
  `opts.allowlist` was supplied *and* `req.shell` is present, decidable, and non-empty (`:131-153`);
...
- **Structured `detail`.** `ElicitationCommandDetail` is attached only when both a `command` string arg
  and `opts.workspaceRoot` are known (`:156-163`); its `cwd` resolves a relative `req.args?.cwd` against
...
2. `buildAskUserHandler` (`packages/loop/src/runtime/capabilities/ask-user.ts:81`) matches it and
   calls `handleAskUserCall` (`packages/loop/src/runtime/tools/ask-user-call.ts:46`).
3. `handleAskUserCall` opens a call envelope (`openCallEnvelope`), validates arguments against
   ...
   `askUser` (`packages/loop/src/runtime/tools/ask-user-call.ts:67-69`).
4. On valid arguments: `envelope.start()`, records `elicitation_requested` (`:73-80`), then awaits
   `askUser(askArgs)` — the `AskUser` port built by `buildAskUser` (`packages/loop/src/runtime/tools/ask-user-tool.ts:197`), which in
   turn calls `elicitWithClockPause` (`packages/capability/src/elicit.ts:126`) around the underlying `Elicit`.
5. `elicitWithClockPause` pauses the run's compute clock (`clock.pause()`), awaits the elicit, and in
   `finally` resumes it (`packages/capability/src/elicit.ts:138-154`) — so wall-clock time spent waiting on the human is never
...
7. On an `ElicitTimeoutError` specifically: `onNoResponse` → `{action:"decline", noResponse:true}`,
   logged as `capability.elicit_no_response` (`packages/capability/src/elicit.ts:144-148`). Any other rejection propagates —
   **except** that an `ElicitTimeoutError` racing a signal abort is itself preempted: `elicitWithClockPause`
   checks `signal?.aborted` before checking the error's type (`packages/capability/src/elicit.ts:143`), so a timeout that fires
...
8. Back in `handleAskUserCall`: if the promise rejects and the call's `signal` is aborted, the outcome
   is `{kind:"cancelled"}` (no trace, no model-facing text — the run is unwinding) (`packages/loop/src/runtime/tools/ask-user-call.ts:85-88`);
   otherwise a non-abort rejection becomes an `error` result reading
   `` `could not reach the user (${reason}).` `` (`:89-94`).
9. On success (including timeout-as-decline): records `user_question` (`:96-105`) and returns
...
10. `buildAskUserHandler` maps `{kind:"cancelled"}` to a cancelled `HandlerVerdict`, else a `result`
    whose `progress` is `true` unless `oc.error === true` (`packages/loop/src/runtime/capabilities/ask-user.ts:98-99`).
...
|---|---|---|
| Configured run wait | `request.elicit_wait_ms ?? CLARVIS_DEFAULT_ELICIT_WAIT_MS` (default 1,800,000 ms) | supplies ordinary `Elicit` call sites and the outer guard-confirmation wrapper (`packages/loop/src/runtime/orchestrator.ts:437`, `packages/loop/src/runtime/capabilities/tools.ts:174,208-211`) |
| Enforcement | `withElicitWaitBound(elicit, graceMs=1000)` wraps the run's `elicit` once, at orchestrator construction, adding `ELICIT_WAIT_GRACE_MS` (1000 ms) grace so the *outer* bound fires slightly after the transport's own deadline (`packages/loop/src/runtime/tools/ask-user-tool.ts:106-122`, `packages/loop/src/runtime/orchestrator.ts:438-439`) | `boundPromise` (`packages/loop/src/runtime/support/bounded.ts:38`) |
...
(`packages/loop/src/runtime/support/bounded.ts:5`); an already-aborted signal settles via `onAbort` before the inner promise can
...
`elicit` (`packages/kernel/src/runs/elicit-bridge.ts:49-70`) never reads `opts.timeoutMs` and starts no timer of its own — every
...
`buildElicitRelay` (`packages/loop/src/runtime/elicit-relay.ts:62`) builds a single `createElicitSerializer()` FIFO
(`:14-24`, a promise chain that runs each queued `job` after the previous settles, success or failure)
...
- `relay: ElicitationRelay`: what an MCP server's own elicitation request is routed through
  (`:81-105`) — it additionally pauses the run's compute clock for the duration
...
| State | Event | Effect |
|---|---|---|
| no pending entry for id | `bridge.elicit(params, opts)` called | mints `id`, builds `ElicitationRequest` (`kind: params.kind ?? "ask_user"` — a caller that omits `kind` is protocol-labeled as an ordinary `ask_user` question, `packages/kernel/src/runs/elicit-bridge.ts:56`), stores `{request, resolve}` in `pending`, delivers to every already-registered handler (`packages/kernel/src/runs/elicit-bridge.ts:49-62`) |
| entry pending, no handler yet | `bridge.onElicit(handler)` registers | handler pushed, then **replayed every still-pending request** (`:74-77`) |
| entry pending | `bridge.respond({id, action, content?})` | entry removed from `pending`, its `resolve` settles with `{action, content?}` (`:78-85`) |
| entry pending | the elicit call's `opts.signal` aborts | entry removed from `pending`, resolves as `{action:"cancel"}` (`:63-69`) |
| entry absent (unknown/already-settled id) | `respond` called | no-op — `item === undefined` returns silently (`:79-80`) |
| — | a registered handler throws | swallowed by `deliver`'s `try/catch`; "cannot break or settle the engine's pending question" (`:41-47`) |
...
`packages/kernel/tests/unit/elicit-bridge.test.ts:6-21` ("an elicitation raised before onElicit
...
the entry is already stored in `pending` and delivered to every handler (`:61-69`), and — unlike
...
**before** awaiting `transport.request(methods.start, ...)` (`packages/kernel/src/transport/client.ts:454-462`), so a
...
and delivers directly once a handler exists (`packages/kernel/src/transport/client.ts:305-309`); `RunHandle.onElicit` on the client
side then splices and replays the buffer the moment a handler attaches (`packages/kernel/src/transport/client.ts:497-501`). The
server side of the same transport wires `handle.onElicit` straight into
`notifications.notify(N.runElicitation, {request})` (`packages/kernel/src/transport/server.ts:361-365`).

### 4.5 Server posture resolution (`resolvePosture`)

`resolvePosture` (`packages/server/src/mcp/elicitation.ts:70`) is a pure decision made **before** the run starts:
...
- `requested === "await"` but the client declares elicitation anyway →
  `"elicitations:await→relay (client declares the elicitation capability)"` (`:78-80`) — relay always
...
- `elicitation === "auto_decline"` and `requestedPlans === "review"` → downgraded to `plans_effective:
  "on"`, `"plans:review→on (an unanswered review gate cancels the run)"` (`:96-99`) — because an
...
gates was missing (`:82-93`).

Under `auto_decline`, `prompt_cache_ttl` is pinned to `"5m"` with the note "no elicitation can pause
this run" (`:101-103`, `:109`) — left `undefined` otherwise so the kernel derives it as usual.

### 4.6 `ElicitationController` — answering everything a run asks

`createElicitationController` (`packages/server/src/mcp/elicitation.ts:188`) is `attach`ed once per run, before the run's first
iteration (`attach(runHandle)` installs `runHandle.onElicit(...)`, `:214-263`). Per incoming
`ElicitationRequest`:

1. If `disposed`, auto-decline immediately (`:217-219`).
2. Always `opts.publish(request)` first — the question reaches the run's own event stream regardless
   of posture (`:221`).
3. `kind === "guard_confirm"` and `posture.guard_confirmations === "denied"` → auto-decline (`:223-226`).
4. `posture.elicitation === "auto_decline"` → auto-decline (`:228-231`).
...
6. Otherwise (`tool` posture) — schedules a timeout (`opts.scheduleTimeout ?? scheduleSystemTimeout`,
   ...
   `clarvis_respond` can answer it first (`:258-262`).

`respond(response)` (`:266-282`) only accepts an answer in `tool` posture; otherwise returns
...
`dispose()` (`:284-287`) sets `disposed = true` and force-auto-declines every still-pending question —
called on session/connection teardown so no question is left hanging past the connection's life.

`reportAnswered(logger, posture, action, auto)` (`packages/server/src/mcp/elicitation.ts:160-170`) logs one `elicit.answered`
event (fields: `posture`, `action`, `auto`) for every question the controller settles, at three call
sites: inside `autoDecline` (`:207-211`, `auto: true`), after a successful relay answer (`:245`,
`auto: false`), and inside `respond()` for an accepted `tool`-posture answer (`:279`, `auto: false`).
...
+= 1`, `:208`) as a side effect of each auto-decline, so it grows across the run's lifetime rather than
being computed once.

### 4.7 The workflow elicit mux — state machine

`createElicitMux(user, options?)` (`packages/workflows/src/elicit-mux.ts:56`) builds one internal `createElicitSerializer()`
...
| State | Event | Effect |
|---|---|---|
| signal already aborted at call time | channel invoked | logs `workflow.elicit_skipped`, resolves `{action:"cancel"}` immediately — **`user` is never called**, `depth` never incremented (`packages/workflows/src/elicit-mux.ts:73-76`) |
| not yet aborted | channel invoked | `depth += 1`, logs `workflow.elicit_queued`, enqueues `serialize(() => ...)` (`:77-93`) |
| queued, reaches front of FIFO | signal aborted **by the time it is dequeued** | logs `workflow.elicit_skipped` (with the actual waited time), resolves `{action:"cancel"}` — `user` is skipped, never called for this request (`:86-89`) |
| queued, reaches front, not aborted | — | calls `user(params, opts)`, tagging the message with `` `[leader ${runId.slice(0,8)}] ` `` prefix for a leader channel, untagged for the manager channel (`:90-93`, `:116`) |
| a `signal` was supplied and the queued promise is still pending | signal aborts | listener resolves the **outer** promise immediately as `{action:"cancel"}`, independent of whether the queued job has reached the transport yet (`:96-112`) |
| the underlying `user` call rejects | signal is aborted | resolves `{action:"cancel"}` rather than rejecting (`:108`) |
| the underlying `user` call rejects | signal is **not** aborted | rejects with the real error, wrapped in `Error` if not already one (`:109`) |

### 4.8 `code`'s elicit slot

`createElicitSlot()` (`packages/code/src/adapters/elicit-slot.ts:23`) holds at most one live `ElicitRequestParams` at a time:
calling `ask(params)` while a previous call is still `pending` first resolves the earlier one as
`CANCEL_RESULT`, **then** installs the new request (`:36-41`) — so the UI never shows two questions
...
`onElicit` callback throws, `reportElicitFailure` (`:282-294`) logs `elicit.handler.failed` and still
...
boolean a guard actually branches on (`packages/kernel/src/guard/guard-elicit.ts:169-176`): any `result.action !== "accept"`
...
`deriveRunShape` (`packages/loop/src/validation/request/run-shape.ts:12-30`) computes, once per run
before it starts, whether *any* mechanism in this document might need to reach a human:
`userInputEnabled = askUserGranted || capabilityNeedsHuman || softMode` (`:26`) — `askUserGranted` is
...
`request.elicit_wait_ms !== 0` (`:28`); its only consumer is outside this document's scope.

`packages/loop/src/runtime/execute-run.ts:306-313` checks `shape.userInputEnabled` immediately after computing the shape: if it is
`true` but the host supplied no `elicit` callback at all, the run never starts — it throws
`ValidationError("elicitation_not_supported", ...)` (`:304-309`) rather than admitting a run that would
...
Production: `packages/workflows/src/elicit-mux.ts:67-93,114-117`.
Test: `packages/workflows/tests/contract/elicit-mux.test.ts:14` (concurrency + tagging),
`packages/workflows/tests/contract/elicit-mux.test.ts:43` (manager untagged).
...
Production: `packages/workflows/src/elicit-mux.ts:86-89,96-112`.
Test: `packages/workflows/tests/contract/elicit-mux.test.ts:53` (settles at once, `<50ms`),
`packages/workflows/tests/contract/elicit-mux.test.ts:79` (never seen by `user` even when its turn
arrives).
...
cancellation).
Production: `packages/workflows/src/elicit-mux.ts:106-109`.
Test: `packages/workflows/tests/contract/elicit-mux.test.ts:106`.
...
Production: `packages/workflows/src/elicit-mux.ts:73-76`.
Test: `packages/workflows/tests/contract/elicit-mux.test.ts:119`.
...
Production: `packages/loop/src/runtime/capabilities/ask-user.ts:39,44`.
Test: `packages/loop/tests/integration/ask-user-grant-gating.test.ts:18-63` confirms only the
...
Production: `packages/capability/src/elicit.ts:139-154`.
Test: `packages/capability/tests/unit/elicit.test.ts:61-101` (pause/resume around a successful result,
...
Production: `packages/loop/src/runtime/tools/ask-user-tool.ts:106-122`.
...
Production: `packages/kernel/src/runs/elicit-bridge.ts:74-76`.
Test: `packages/kernel/tests/unit/elicit-bridge.test.ts:6-21`.
...
Production: `packages/kernel/src/transport/client.ts:293-298,452-457`.
...
Production: `packages/server/src/mcp/elicitation.ts:96-99`.
Test: `packages/server/tests/unit/elicitation.test.ts:71` ("downgrades plans:review only when no
...
Production: `packages/server/src/mcp/elicitation.ts:82-93`.
...
|---|---|---|
| `ask_user` called with invalid arguments (fails `askUserTool.inputSchema`) | `openCallEnvelope`'s `envelope.invalid` (`packages/loop/src/runtime/tools/ask-user-call.ts:67-69`) | error tool result; `askUser` is **never invoked** |
| The wait bound elapses with no answer | `ElicitTimeoutError` swallowed by `elicitWithClockPause` (`packages/capability/src/elicit.ts:144-149`) | `{action:"decline", noResponse:true}` — the model reads this as an ordinary decline, never as an exception |
| The run's own signal aborts while `askUser` is pending | `handleAskUserCall`'s catch checks `signal?.aborted` (`packages/loop/src/runtime/tools/ask-user-call.ts:85-88`) | `{kind:"cancelled"}` — no trace record, no tool-result text; the run is unwinding |
| `askUser` rejects for any other reason (transport failure, non-abort, non-timeout) | same catch, non-abort branch (`packages/loop/src/runtime/tools/ask-user-call.ts:89-94`) | error tool result `` `could not reach the user (${reason}).` ``, `error: true` |
| A relayed MCP-server elicitation (`buildElicitRelay`'s `relay`) times out | caught specifically for `ElicitTimeoutError` (`packages/loop/src/runtime/elicit-relay.ts:97-100`) | `{action:"decline"}` returned to the MCP server — never propagated as a throw |
| A relayed elicitation fails for a non-timeout reason | same catch, `else` branch (`packages/loop/src/runtime/elicit-relay.ts:101`) | rethrown — the MCP dispatch layer sees a real failure |
| A registered kernel-bridge `onElicit` handler throws | `deliver`'s try/catch (`packages/kernel/src/runs/elicit-bridge.ts:41-47`) | swallowed; "cannot break or settle the engine's pending question" — every other handler and the pending state are unaffected |
...
| A run needs a human (`shape.userInputEnabled === true`) but no `elicit` callback was supplied at all | `packages/loop/src/runtime/execute-run.ts:301-312`, checked immediately after `deriveRunShape` | the run never starts: throws `ValidationError("elicitation_not_supported", ...)` — the only elicitation failure resolved at request validation rather than per-question (§4.10) |
...
| A pooled (stdio + `shared`) MCP connection is acquired with a `relay` | `ConnectionManager`'s `openFresh(o, signal, pooled=true)` (`packages/mcp-client/src/connection-manager.ts:669-740`) | the `relay` is dropped — opened `...(o.relay && !pooled ? { relay: o.relay } : {})` — so the connection advertises no `elicitation` capability at all; `warnRelayDropped` (`:649-657`) logs `mcp.pool.relay_dropped` once per server name, not once per acquire |
...
- `@clarvis/capability`'s `elicit.ts` is a pure-type-plus-one-function leaf; it depends only on
  ...
  (`packages/capability/src/elicit.ts:1-3`). Nothing here can reach the engine, by construction — this
  ...
  helper" (`packages/capability/src/elicit.ts:122-124`).
- `@clarvis/loop`'s `ask-user-tool.ts`/`ask-user-call.ts`/`elicit-relay.ts`/`capabilities/ask-user.ts`
  ...
  (`packages/loop/src/runtime/elicit-relay.ts:1`), which is the type-level coupling that lets the loop hand an MCP relay to the
...
- `@clarvis/workflows`'s `elicit-mux.ts` imports `createElicitSerializer` from `@clarvis/loop/workflows`
  (`packages/workflows/src/elicit-mux.ts:15`) — this is one of the package's only two permitted edges into the engine
  (INV-173, owned by `specs/capabilities/workflows-scheduling.md`), enforced by
  `packages/workflows/tests/architecture/dependency-direction.test.ts:15`.
- `@clarvis/kernel`'s `elicit-bridge.ts` imports `Elicit`/`ElicitRawResult` from `@clarvis/loop` and
  ...
  (`packages/kernel/src/runs/elicit-bridge.ts:1-2`) — it is the type-level bridge between the two, and `guard-elicit.ts` imports
  its `GuardElicitParams` back (`packages/kernel/src/guard/guard-elicit.ts:3`) to attach structured `detail`.
- `@clarvis/kernel`'s `managed-run.ts` constructs one `ElicitBridge` per run
  (`packages/kernel/src/runs/managed-run.ts:177`) and wires `bridge.elicit` into the `ManagedRunContext.elicit` the engine's
  `execute` closure receives (`:270`); `RunHandle.onElicit`/`respond` on the returned handle forward
  straight to `bridge.onElicit`/`bridge.respond` (`:327-331`).
...
- `@clarvis/mcp-client`'s `client.ts` is the only place `ElicitRequestSchema`/`ElicitResult` from the
  ...
  `ElicitationRelay` through to it per connection (`packages/loop/src/runtime/open-tool-pool.ts:1,58-66,69-77`), never constructing the
  SDK types itself.
- `code`'s `adapters/elicit-types.ts` is a deliberately independent local mirror of the protocol shapes
  — "so code carries no `@modelcontextprotocol/sdk` dependency" (`packages/code/src/adapters/elicit-types.ts:1-7`) — with
...
- The soft-budget escalation ask (`buildSoftLimitAsk`, wired at
  `packages/loop/src/runtime/entry-inputs.ts:145-146`) shares the same serialized `elicit` and the same
...
- `packages/kernel/src/guard/resolver.ts:226-240` constructs a `createGuardSessionAllowlist()` and a
...
- **ELI-01's sub-agent-exclusion half is production-only.** `packages/loop/tests/integration/ask-user-grant-gating.test.ts`
  ...
  `packages/loop/src/runtime/capabilities/ask-user.ts:44` (`if (!scope.entry) return null`) — remains production-verified only, with no located
...
- ~~**Why `guard_confirm`'s wait bound is a 32-bit-max sentinel rather than `undefined`.**~~
  **Resolved, in two parts: the numeric choice is fully explained; the sentinel-vs-omission style
  ...
  defined once in `packages/loop/src/runtime/support/bounded.ts:5`, whose own comment states the
  ...
  supplied `timeoutMs` to it (`packages/loop/src/runtime/support/bounded.ts:59-64`,
  ...
  `ELICIT_NO_TIMEOUT_MS` in `packages/kernel/src/guard/guard-elicit.ts:27` is the identical constant,
  ...
  (`packages/kernel/src/runs/elicit-bridge.ts:49-66`), **never reads `opts.timeoutMs` at all** — it
  ...
  ... })` (`packages/kernel/src/guard/guard-elicit.ts:165-168`) has the same observable effect as
  omitting `timeoutMs`
  **unconditionally**, not just "absent an abort" — the field is dead for every call this bridge ever
  ...
  (`packages/loop/src/runtime/capabilities/tools.ts:174,208-211`), racing it against
  ...
  (`packages/loop/src/runtime/capabilities/tools.ts:97-101`) — confirming this outer wrap, not the
  ...
  2026-08-22 as a recorded derivation; no behaviour changed.** The reason is now at the constant
  (`packages/kernel/src/guard/guard-elicit.ts:11`-`:26`), and it is the opposite of the "defensive
  ...
  (`packages/capability/src/elicit.ts:80`-`:88`), so the sentinel is this function's declaration,
  ...
  (`packages/kernel/tests/unit/guard.test.ts:252`-`:265`), which captures what the backend is handed
