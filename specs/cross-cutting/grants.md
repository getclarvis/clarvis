...
capability's behaviour (`packages/capability/src/api.ts`). The engine owns
four such strings itself (`BuiltinGrant`, `packages/capability/src/api.ts`);
...
| --- | --- | --- |
| `BuiltinGrant` | `"ask_user" \| "read_workspace" \| "edit_workspace" \| "run_commands"` | `packages/capability/src/api.ts` |
| `Grant` | `BuiltinGrant \| (string & {})` — deliberately open | `packages/capability/src/api.ts` |
| `AgentProfile.grants` | `Grant[] \| undefined` | `packages/capability/src/api.ts` |
| `AgentProfile.tools` | `string[]` (MCP `server.tool` names; unrelated to grants) | `packages/capability/src/api.ts` |
| `AgentProfile.can_spawn` | `string[] \| undefined` — profile names this agent may spawn | `packages/capability/src/api.ts` |
| `AgentProfile.default_spawn` | `string \| undefined` | `packages/capability/src/api.ts` |
| `BUILTIN_GRANT_NAMES` | `["ask_user","read_workspace","edit_workspace","run_commands"]`, static discovery aid on `grantSchema.options` | `packages/loop/src/validation/request/grant-registry.ts`, `packages/loop/src/validation/request/profile-schemas.ts` |

### 2.2 Capability grant contribution

| Symbol | Signature | File |
| --- | --- | --- |
| `CapabilityGrantDeclaration` | `{ readonly name: string; readonly entryCanSpawn?: boolean }` | `packages/capability/src/contract.ts` |
| `Capability.grants` | `readonly CapabilityGrantDeclaration[] \| undefined` | `packages/capability/src/contract.ts` |
| `CapabilityRegistry.registerGrant(declaration)` | throws on empty name; second registration of the same name is a silent no-op if identical, else throws | `packages/capability/src/registry.ts` |
| `CapabilityRegistry.grants()` | `readonly CapabilityGrantDeclaration[]`, registration order | `packages/capability/src/registry.ts` |
| `composeCapabilityRegistry(base, declarations)` | copies a registry, appending per-run grant declarations without mutating the host's long-lived one | `packages/capability/src/registry.ts` |
| `requireKnownGrants(data, registry?)` | throws `ValidationError("invalid_profile", …)` on the first grant not in `BUILTIN_GRANT_NAMES ∪ registry.grants()` | `packages/loop/src/validation/request/grant-registry.ts` |
| `RunCapabilityContext.entryGrants` | `readonly string[]` — the run's entry profile's own grants, sourced as `shape.entryProfile.grants ?? []` | `packages/capability/src/contract.ts`; sourced at `packages/loop/src/runtime/orchestrator.ts` |
| `SubagentCapabilitiesFactory` | `(grants: readonly string[] \| undefined) => AgentActivation` — the grant-gated per-subagent factory threaded into the spawn path | `packages/capability/src/contract.ts` |

### 2.3 Every grant name that actually exists in this repository

| Grant | Owner | Declares `entryCanSpawn` | File |
| --- | --- | --- | --- |
| `ask_user` | engine (`BuiltinGrant`) | no | `packages/capability/src/api.ts` |
| `read_workspace` | engine (`BuiltinGrant`) | no | `packages/capability/src/api.ts` |
| `edit_workspace` | engine (`BuiltinGrant`) | no | `packages/capability/src/api.ts` |
...
| `AGENT_TOOL_NAMES` / `READ_ONLY_TOOL_NAMES` / `EDIT_TOOL_NAMES` / `EXEC_TOOL_NAMES` / `FILE_MUTATING_TOOL_NAMES` | derived tool-name sets | `packages/loop/src/runtime/tools/builtin/names.ts` |
| `createAgentToolset(opts)` | builds the per-agent `{ defs, names, dispatch }` gated by `canMutate`/`canExec` | `packages/loop/src/runtime/tools/builtin/toolset.ts` |
| `createAgentToolsCapability(opts?)` | the `tools` capability (`AGENT_TOOLS_CAPABILITY_NAME = "tools"`) | `packages/loop/src/runtime/capabilities/tools.ts` |
...
```ts
// packages/loop/src/validation/request/profile-schemas.ts
...
(`packages/loop/src/validation/request/grant-registry.ts`) — it lists only
...
`packages/loop/src/validation/request/profile-schemas.ts` describes, in
...
it is absent (`packages/loop/src/validation/profile-readiness.ts`) — a caller that supplies the built-ins plus its registry's
...
| Set | Derivation | Content (9 total tools) | File |
| --- | --- | --- | --- |
| `AGENT_TOOL_NAMES` | `tools.map(t => t.name)` | all 9 | `packages/loop/src/runtime/tools/builtin/names.ts` |
| `READ_ONLY_TOOL_NAMES` | `readOnlyTools.map(t => t.name)` | `read_file, read_image, list_dir` (3) | `packages/loop/src/runtime/tools/builtin/names.ts`; source flags at `packages/tools/src/tools/registry.ts` |
| `EDIT_TOOL_NAMES` | `AGENT_TOOL_NAMES` minus `READ_ONLY_TOOL_NAMES` | `write_file, edit_file, apply_patch, shell, shell_session, remove` (6) | `packages/loop/src/runtime/tools/builtin/names.ts`; source flags `packages/tools/src/tools/registry.ts` |
| `EXEC_TOOL_NAMES` | hardcoded literal list | `shell, shell_session` (2) | `packages/loop/src/runtime/tools/builtin/names.ts` |
| `FILE_MUTATING_TOOL_NAMES` | `EDIT_TOOL_NAMES` minus command runners | `write_file, edit_file, apply_patch, remove` (4) | `packages/loop/src/runtime/tools/builtin/names.ts` |

The tool definitions and their names are owned by
...
1. `parseRunRequest` parses `profiles[].grants` against `grantSchema` — any
   non-empty string, syntactically (`packages/loop/src/validation/request/profile-schemas.ts`).
2. The `registry` that validation checks against is **not** a static, boot-time
   ...
   (`packages/loop/src/runtime/execute-run.ts`). The identical
...
3. `validateBody` calls `requireKnownGrants(data, registry)` **before**
   ...
   (`packages/loop/src/validation/request-schema.ts`) — every grant on
...
1. Computes `caps = agentToolCaps(scope.grants, ctx.env.CLARVIS_AGENT_TOOLS_MAX_GRANT)`.
   `agentToolCaps` (`packages/loop/src/runtime/tools/builtin/grants.ts`):
...
3. Otherwise `createAgentToolset({ workspaceRoot, canMutate: caps.canMutate,
   ...
   - `resolveConfig({ readOnly: !canMutate, … })` — `@clarvis/tools` itself
     ...
     (`packages/loop/src/runtime/tools/builtin/toolset.ts`,
     `packages/tools/src/core.ts`, `packages/tools/src/tools/registry.ts`).
...
4. `dispatch(name, …)` on the built toolset rejects any call whose `name` is
   ...
   (`packages/loop/src/runtime/tools/builtin/toolset.ts`) — a
...
(`packages/loop/src/runtime/spawn-shape.ts`):
...
1. The spawnable-profile registry is built **only** from the entry profile's
   ...
   (`packages/loop/src/runtime/run-shape.ts`). Since no shipped
...
3. The spawned child's capability activation uses **its own** profile's
   ...
   (`packages/loop/src/runtime/subagents/run-subagent.ts` sets
...
`shape.entryProfile.grants ?? []` — `packages/loop/src/runtime/orchestrator.ts`)
...
- `ask_user`: `forRun` returns `null` unless `ctx.entryGrants.includes("ask_user")`;
  ...
  (`packages/loop/src/runtime/capabilities/ask-user.ts`). The profile
  schema calls this grant "inert" on a sub-agent profile
  (`packages/loop/src/validation/request/profile-schemas.ts`), and the
  ...
  (`packages/loop/src/runtime/usage-accounting.ts`) — see §6. At the
  ...
  (`packages/loop/tests/integration/ask-user-grant-gating.test.ts`).
...
(`packages/kernel/src/workflows/workflows-service.ts`, the function itself, doc-commented "so a leader can never become a manager
...
1. **A profile's grant is accepted only if it is engine-built-in or
   ...
   Production: `packages/loop/src/validation/request/grant-registry.ts`.
   Test: `packages/loop/tests/unit/request-grant-registry.test.ts` (accepts
...
2. **`agentToolCaps` derives `canRead`/`canMutate`/`canExec` from
   ...
   the ceiling's rank.**
   Production: `packages/loop/src/runtime/tools/builtin/grants.ts`.
   Test: `packages/loop/tests/unit/grants.test.ts` (empty/irrelevant
...
3. **A run's spawn-capability (whether it gets a supervision registry) is
   ...
   Production: `packages/loop/src/runtime/spawn-shape.ts`.
   Test: `packages/loop/tests/unit/spawn-shape.test.ts` (non-lead with an
...
4. **The engine's readiness pre-check (`unknown_grant`) and the hard
   ...
   Production: `packages/loop/src/validation/profile-readiness.ts`
   (mirrors `requireKnownGrants`, skipped when `knownGrants` is omitted).
   Test: `packages/loop/tests/unit/profile-readiness.test.ts` — a
   ...
   and the test `covers every rule in the table exactly once` pins
...
5. **`ask_user` activates only for the run's entry agent, never for a spawned
   ...
   `packages/loop/tests/integration/subagent-config-warnings.test.ts`
...
7. **A capability's own grant declaration is idempotent under re-registration
   ...
   Production: `packages/capability/src/registry.ts`
   ...
   Test: `packages/capability/tests/unit/registry.test.ts` (an identical
   repeat is a silent no-op) (a disagreeing repeat throws
...
8. **`composeCapabilityRegistry` never mutates the host's long-lived
   ...
   afterward.
   Production: `packages/capability/src/registry.ts`.
   Test: `packages/capability/tests/unit/registry.test.ts` (`it("composes
...
| --- | --- | --- |
| Profile grant not in built-ins or the run's registry | Whole request rejected at validation: `ValidationError("invalid_profile", "profile '<name>'.grants contains undeclared grant '<grant>'.")` | `packages/loop/src/validation/request/grant-registry.ts` |
| Agent's grants clear no `read` ceiling (`!caps.canRead`) | `tools` capability's `forAgent` returns `null` — the agent gets no coding toolset, not a degraded one | `packages/loop/src/runtime/capabilities/tools.ts` |
...
- **`@clarvis/loop`'s `grant-registry.ts` is the only place `BUILTIN_GRANT_NAMES`
  ...
  (`packages/capability/src/registry.ts`: "Registration must happen
  ...
  `composeCapabilityRegistry`, `packages/capability/src/registry.ts`).
- **`packages/loop/src/runtime/tools/builtin/grants.ts` has no import of
  ...
  `@clarvis/capability`, `packages/loop/src/runtime/tools/builtin/grants.ts`)
  — this is deliberate per its own doc comment ("dep-free grant logic, no
  @clarvis/tools import", `packages/loop/src/runtime/tools/builtin/grants.ts`),
...
- **`spawn-shape.ts`'s `canSpawnChildren` depends only on `RunShape` and
  `CapabilityGrantDeclaration[]`** (`packages/loop/src/runtime/spawn-shape.ts`)
...
- **What depends on this document's subsystem, from outside**: `code`'s Run
  ...
  (`packages/loop/src/validation/request/profile-schemas.ts`) rather
...
- **`@clarvis/kernel`'s own `ConfigService.knownGrants()` independently
  ...
  (`packages/kernel/src/kernel.ts`). The trailing hardcoded
