...
`packages/loop/src/runtime/tools/builtin/execute-agent-tool-call.ts`, and
`mapEntry` in `packages/trace/src/trace-mapper.ts`, and
...
through one narrow port, `Logger` (`packages/capability/src/ports.ts`), and nowhere else. The
port is declared in `@clarvis/capability` so a capability can log without depending on a logging
library; the one concrete backend (`pino`) lives in `packages/loop/src/logger.ts`, kept out of the
contract package on purpose (`packages/loop/src/logger.ts`: "Returns pino's own logger, not the
...
- **A structural contract a package can log against without adopting a backend.** `Logger` has four
  ...
  (`packages/capability/src/ports.ts`), and pino satisfies it structurally with no adapter
  (`packages/loop/src/logger.ts`).
- **Per-component verbosity without a per-run knob.** `CLARVIS_LOG_LEVEL` sets a process-wide floor;
  `CLARVIS_LOG` overrides it per named subsystem (`packages/capability/src/log.ts`). Both
...
is not the terminal" (`packages/tools/tests/architecture/logging-channel.test.ts`). No package may
...
| --- | --- | --- | --- |
| `LogFn` | interface | `packages/capability/src/ports.ts` | `(obj: unknown, msg?: string, ...args) => void` overloaded with `(msg: string, ...args) => void` — matches pino's own `LogFn` structurally |
| `Logger` | interface | `packages/capability/src/ports.ts` | `{ debug, info, warn, error: LogFn; child?(bindings): Logger; level?: string }` |
| `LOG_LEVELS` | const tuple | `packages/capability/src/log.ts` | `["debug","info","warn","error","silent"]` — deliberately excludes pino's `trace`/`fatal` |
| `LogLevel` | type | `packages/capability/src/log.ts` | `(typeof LOG_LEVELS)[number]` |
| `DEFAULT_LOG_LEVEL` | const | `packages/capability/src/log.ts` | `"info"` |
| `isLogLevel` | fn | `packages/capability/src/log.ts` | type guard: `value is LogLevel` |
| `NOOP_LOGGER` | const `Logger` | `packages/capability/src/log.ts` | discards every call; `child` returns itself; `level: "silent"` |
| `levelEnabled` | fn | `packages/capability/src/log.ts` | `(logger, level) => boolean` — guards payload *construction*, not just emission |
| `bind` | fn | `packages/capability/src/log.ts` | `(logger, bindings) => Logger` — derives via `child`, else returns `logger` unchanged |
| `parseLogScopes` | fn | `packages/capability/src/log.ts` | parses `CLARVIS_LOG`'s `component=level,...` spec into a `Map` |
| `activeLevelOf` | fn | `packages/capability/src/log.ts` | reads a logger's own reported level, or `undefined` if unrecognized |
| `levelFor` | fn | `packages/capability/src/log.ts` | resolves the effective level for one component, longest dot-prefix wins |
| `componentLogger` | fn | `packages/capability/src/log.ts` | derives a component-stamped, level-pinned child |
| `bindLevelled` | fn | `packages/capability/src/log.ts` | general form: binds arbitrary fields plus an optional level |
| `Sampler` | type | `packages/capability/src/log.ts` | `(key: string) => boolean` |
| `evictOldest` | fn (private) | `packages/capability/src/log.ts` | bounded-key eviction helper shared by `createSampler`/`createRateLimiter`: once `maxKeys` distinct keys are tracked, evicts the single oldest-inserted key before a new one is recorded |
| `DEFAULT_MAX_KEYS` | const | `packages/capability/src/log.ts` | `1_024` — the default `maxKeys` for both `createSampler` and `createRateLimiter` |
| `createSampler` | fn | `packages/capability/src/log.ts` | first-8-then-powers-of-2 admission policy, `maxKeys` defaults to `DEFAULT_MAX_KEYS` |
| `RateLimiterOptions` | interface | `packages/capability/src/log.ts` | `{ windowMs?, maxKeys?, clock? }` |
| `DEFAULT_RATE_LIMIT_MS` | const | `packages/capability/src/log.ts` | `60_000` — the default `windowMs` when `createRateLimiter` is called with none |
| `createRateLimiter` | fn | `packages/capability/src/log.ts` | admits one occurrence of a key per `windowMs` (default 60,000 ms), `maxKeys` defaults to `DEFAULT_MAX_KEYS` |

Every one of the above that its module exports is re-exported through `@clarvis/capability`'s root
`index.ts` (`packages/capability/src/index.ts`). The three exceptions are module-private and
...
(`packages/capability/src/log.ts`).

### 2.2 The backend (`@clarvis/loop`)

| Symbol | Location | Role |
| --- | --- | --- |
| `createLogger(level?, opts?)` | `packages/loop/src/logger.ts` | builds the one pino logger a host distributes; `opts.destination` is `1` (stdout) or `2` (stderr, default); `opts.service` stamps `service` on every record, defaulting to `"@clarvis/loop"` |
| `CreateLoggerOptions` | `packages/loop/src/logger.ts` | `{ destination?: 1 \| 2; service?: string }` |
| `type Logger` (re-export) | `packages/loop/src/logger.ts` | re-exports `@clarvis/capability`'s port so an in-engine consumer imports it from the same path it always did |

### 2.3 Kernel component-logger factory (`@clarvis/kernel`)

| Symbol | Location | Role |
| --- | --- | --- |
| `ComponentLoggers` | `packages/kernel/src/component-loggers.ts` | `(component: string) => Logger` |
| `componentFloor(root, configured)` | `packages/kernel/src/component-loggers.ts` | host's own reported level, else the configured `CLARVIS_LOG_LEVEL` |
| `createComponentLoggers(root, spec, fallback)` | `packages/kernel/src/component-loggers.ts` | builds a memoizing per-component factory; all-silent when `root` is `undefined` or already `"silent"` |
| `createAuditLogger(root, enabled)` | `packages/kernel/src/component-loggers.ts` | builds the audit channel: `{component:"audit", audit:true}` pinned at `"info"`; `NOOP_LOGGER` when `root` is absent, disabled, or itself `"silent"` |
| exported from | `packages/kernel/src/index.ts` | `export { createAuditLogger, createComponentLoggers }` |

### 2.4 Per-package log modules (structural sub-ports)

| Package | File | Port name | Discards-everything const | Process-wide sink |
| --- | --- | --- | --- | --- |
| `@clarvis/tools` | `packages/tools/src/lib/log.ts` | `ToolsLogger` | `NOOP_TOOLS_LOGGER` | `WarnSink` / `warn()` / `setWarnSink()` |
| `@clarvis/tools` (structured warning shape) | `packages/tools/src/lib/log.ts` | `ToolsWarning` | n/a | n/a — carried as `WarnSink`'s second parameter |
| `@clarvis/skills` | `packages/skills/src/lib/log.ts` | `SkillDiagnostics` (`{warningSink, logger}`) | `DEFAULT_DIAGNOSTICS` | `WarnSink` / `warn()` / `defaultWarnSink` |
| `@clarvis/paths`* | `packages/paths/src/diag.ts` | `PathsLogger` | `NOOP_PATHS_LOGGER` | `pathsLogger` / `setPathsLogger` (referenced, not in this document's scope) |
...
`ToolsWarning` (`packages/tools/src/lib/log.ts`) is the structured half of a `WarnSink` call:
`{ event: string; level?: "debug"|"warn"|"error"; fields?: Record<string, unknown> }`. `WarnSink`'s
type is `(message: string, warning?: ToolsWarning) => void`, so a host bridging `warn()` into
...
(`packages/workflows/src/log.ts`), since the package already depends on `@clarvis/capability`.
`@clarvis/plan`'s `log.ts` goes further: it declares no logging port or `Logger` reference at all —
its sole import from `@clarvis/capability` is `sanitizeErrorMessage` (`packages/plan/src/log.ts`),
...
| --- | --- | --- | --- |
| `@clarvis/workflows` | `packages/workflows/src/log.ts` | `workflowLogger(ctx)` | `ctx.deps.logger ?? NOOP_LOGGER`, the one permitted normalization site |
| `@clarvis/workflows` | `packages/workflows/src/log.ts` | `withBoundLogger(deps, bindings)` | derives engine deps whose logger carries `bindings`, preserving `deps` identity when there is no logger |
| `@clarvis/workflows` | `packages/workflows/src/log.ts` | `faultFields(err)` | projects a thrown value into `{err, stack?, cause?}`, sanitized |
| `@clarvis/workflows` | `packages/workflows/src/log.ts` | `joinIds(ids)` | joins a bounded id list into one scalar field, so the record stays flat and filterable |
...
| --- | --- | --- |
| `CLARVIS_LOG_LEVEL` | `z.enum(LOG_LEVELS).default(DEFAULT_LOG_LEVEL)` | the process-wide floor |
| `CLARVIS_LOG` | `z.string().optional()` | per-component override, `component=level,...`, parsed by `parseLogScopes` |
| `CLARVIS_LOG_AUDIT` | `boolFromEnv(true)` | whether the audit channel is built at all |
...
it did" (`packages/capability/src/env.ts`). `CLARVIS_LOG`'s own comment
(`packages/capability/src/env.ts`) argues something different — that the component vocabulary
is deliberately open, "the way `TraceKind` is" — and never mentions silencing or unmasking.
`CLARVIS_LOG_LEVEL`'s comment is about neither: it explains why the level enum excludes
`trace`/`fatal`.
...
what those checks mean (`packages/loop/src/runtime/execute-run.ts`).
...
(`packages/capability/src/log.ts`, and `packages/kernel/src/component-loggers.ts`). Parsing
rules, all pinned by test (`packages/capability/tests/unit/log.test.ts`):
...
Order of operations in `createFileKernel` (`packages/kernel/src/file-kernel.ts`):
...
4. Build the audit channel: `createAuditLogger(logger, env.CLARVIS_LOG_AUDIT)`.
...
execution id is minted — `packages/loop/src/runtime/execute-run.ts`). Hand a lower layer the
...
across the plan-approval gate" (`packages/kernel/src/component-loggers.ts`).
...
preference — it is a host saying it has no channel"; pinning audit at `"info"` against a
...
1. `refuseLoggerOnWire(opts)` runs **before** `createFileKernel` is even called.
2. `loggerDescriptor(opts.logger)` walks the logger's own prototype chain (not just its own symbols,
   ...
   for a symbol whose `.toString()` is `"Symbol(pino.stream)"`.
...
Observed concretely in `reportIterationCache` (`packages/loop/src/runtime/loop/iteration-metrics.ts`):
...
this runs on every iteration of every agent".
...
- Test: `packages/tools/tests/architecture/logging-channel.test.ts` — scans every `.ts` file
...
- The same test also asserts the scan actually covers something (`files.length > 40`,
  `packages/tools/tests/architecture/logging-channel.test.ts`), which rules out a silently-empty scan passing vacuously.
...
- Production: `packages/tools/src/lib/log.ts` (`defaultSink`).
- Test: `packages/tools/tests/architecture/logging-channel.test.ts` — strips block comments from
...
- Production: `packages/kernel/src/serve.ts` (`refuseLoggerOnWire`, called before
  `createFileKernel`).
- Test: `packages/kernel/tests/integration/serve.test.ts` — seven cases: a direct
  `destination: 1` logger is rejected; a `.child()` of one is still rejected, walking the
  prototype chain; a grandchild is still rejected; a `destination: 2` (`silent`
  level) logger and its child are accepted; a logger sharing an *explicit* `opts.output`'s
  descriptor is rejected — the explicit output in this test is given `fd: 1`, the same descriptor as
  the default wire, not a descriptor that is not `1`; a `silent`-level, `destination: 1`
  ...
  — this is the one acceptance case among the seven, not a rejection; a logger exposing no
...
- Production: `packages/capability/src/log.ts`.
- Test: `packages/capability/tests/unit/log.test.ts`.
...
- Production: `packages/capability/src/log.ts` (`bind`) (`bindLevelled`).
- Test: `packages/capability/tests/unit/log.test.ts` (`bind`) (`bindLevelled`) (`componentLogger`, "degrades to the logger itself rather than refusing to log").
...
- Production: `packages/capability/src/log.ts`.
- Test: `packages/capability/tests/unit/log.test.ts` — asserts the admitted sequence for 40
...
`DEFAULT_MAX_KEYS = 1_024`. Once a sampler or limiter is tracking `maxKeys` distinct keys,
...
`counts.delete(key); counts.set(key, next)`, `createRateLimiter` via
`emitted.delete(key); emitted.set(key, now)` — which is what keeps the map ordered as an
LRU-by-insertion-order structure rather than merely a size-capped one.
- Production: `packages/capability/src/log.ts`.
- Test: `packages/capability/tests/unit/log.test.ts` (`createSampler`, "evicts the oldest key
  past its ceiling rather than growing" — with `maxKeys: 2`, evicting `"a"` after `"b"` and `"c"`
  arrive still leaves `"a"` readmitted as if new) (`createRateLimiter`, the same shape
  with `maxKeys: 2`).
...
| --- | --- | --- |
| Host supplies no `Logger` at all | Every derivation degrades to `NOOP_LOGGER`; nothing throws | `packages/capability/src/log.ts`, `packages/kernel/src/component-loggers.ts` (`root === undefined`) |
| Host's logger reports `level: "silent"` | Component and audit factories both short-circuit to `NOOP_LOGGER`, ignoring any `CLARVIS_LOG` scope that would otherwise apply | `packages/kernel/src/component-loggers.ts` |
| `CLARVIS_LOG` names an unrecognized level (e.g. `trace`, `fatal`) for a component | That single entry is skipped by `parseLogScopes`; the component falls back to the configured floor — a silent no-op, not an error | `packages/capability/src/log.ts`; test `packages/capability/tests/unit/log.test.ts` |
| `CLARVIS_LOG` names a component that does not exist | Silent no-op — the vocabulary is deliberately open, "like `TraceKind`" | `packages/capability/src/log.ts` |
| Backend implements no `child` | `bind`/`componentLogger`/`bindLevelled` all return the original logger unchanged rather than throwing or silently dropping bindings | `packages/capability/src/log.ts` |
| Logger's `level` is present but unrecognized by this port | `levelEnabled` and `activeLevelOf` both treat this as "emitting" (`levelEnabled` returns `true`; `activeLevelOf` returns `undefined`) — "a diagnostic lost to an unparsable level is worse than one written needlessly" | `packages/capability/src/log.ts` |
| `serveFileKernelOverStdio` is given a logger bound to the wire's own descriptor | Throws synchronously before constructing the kernel — the only point this can be caught, since "once the pump starts, the corruption looks like a peer that sends malformed JSON" | `packages/kernel/src/serve.ts` |
| A `@clarvis/tools` `.gitignore`-loader or `serializeError` site has no `RuntimeConfig` in scope | Falls back to the process-wide `WarnSink`, defaulting to raw `process.stderr.write` unless a host has called `setWarnSink` | `packages/tools/src/lib/log.ts` |
| Two hosts in one process both call `setWarnSink` | Last writer wins; the earlier host's warnings are silently attributed to the wrong (or no) destination — an acknowledged, un-mitigated hazard | `packages/tools/src/lib/log.ts` ("wrong the day a second kernel shares the process") |
...
- **`componentLogger`/`bindLevelled` live in `@clarvis/capability` because two layers need them and
  ...
  still rejecting the two-argument call".
- **`@clarvis/loop` is the sole owner of the pino backend**, and this is enforced by the package's own
  ...
  `packages/loop/src/logger.ts` within this document's scope, and every other consumer receives an
...
- **`@clarvis/tools`, `@clarvis/skills` (and, outside this document's direct scope,
  ...
  `packages/hooks/src/types.ts`) rather than depending on
  `@clarvis/capability`. `packages/tools/src/lib/log.ts` states the reason directly: "this
...
- **`@clarvis/workflows` and `@clarvis/plan` both depend on `@clarvis/capability` directly** (both
  ...
  (`packages/workflows/src/log.ts`) and consumes the `Logger` port by structural typing through
...
- **`levelEnabled` is consumed widely across the engine** (at least
  ...
  `packages/loop/src/runtime/loop/iteration-metrics.ts` is in scope for this document.
