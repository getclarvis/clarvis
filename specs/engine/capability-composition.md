...
`packages/loop/src/runtime/build-run-deps.ts:362`), fold what activates for one run
...
| `./host` | `src/host.ts` | Host-facing re-exports across the groups in §2.1a; the surface `@clarvis/kernel` builds config on top of (`packages/loop/src/host.ts:1-104`). |
...
### 2.1a `./host`'s re-export groups (`packages/loop/src/host.ts:1-104`)
...
| Marketplace manifest | `marketplaceSchema`, `pluginGitSelectorIssue`, `pluginGitUrlIssue`, `pluginNpmSourceIssue`, `Marketplace`, `MarketplaceEntry` | `settings/marketplace-schema.js` (`packages/loop/src/host.ts:15-22`) |
| Plugin-agent files | `parsePluginManifest`, `readPluginAgentFiles`, `PluginAgentFile`, `PluginAgentFilesResult` | `settings/plugin-agents.js` (`packages/loop/src/host.ts:23-28`) |
| Plugin resource limits | `PLUGIN_RESOURCE_LIMITS`, `readBoundedPluginText`, `BoundedPluginTextResult` | `settings/plugin-resources.js` (`packages/loop/src/host.ts:29-33`) |
| Plugin-manifest schema/typos | `pluginSettingsFragment`, `suspectedManifestTypos`, `unknownManifestKeys`, `PluginManifest`, `SuspectedManifestTypo` | `settings/plugin-schema.js` (`packages/loop/src/host.ts:34-40`) |
| Settings merge | `mergeProviders`, `mergeSettings`, `SettingsScope` | `settings/settings-merge.js` (`packages/loop/src/host.ts:41`) |
| Capability settings | `readCapabilitySettings`, `settingsSchemaFor` | `settings/capability-settings.js` (`packages/loop/src/host.ts:42`) |
| Settings schema | `mcpServerSettingsSchema`, `mcpServerPluginSchema`, `pluginNameField`, `pluginRefField`, `settingsSchema`, `McpServerSettings`, `SettingsFile`, plus `settingsServerToEngine` | `settings/settings-schema.js`, `settings/engine-server.js` (`packages/loop/src/host.ts:43-52`) |
| Guard/sandbox config | `defaultGuardMode`, `GuardConfig`, `ResolvedSandboxSettings`, `SandboxSettings` | `runtime/capabilities/tools-settings.js` (`packages/loop/src/host.ts:53-57`) |
| Request-schema helpers | `parseModelRef`, `resolveProvider`, `providerConfigSchema`, `grantSchema`, `BUILTIN_GRANT_NAMES`, `profileReadinessIssues`, `ReadinessIssue`, `ReadinessProfile` | `@clarvis/capability`, `validation/request-schema.js`, `validation/request/grant-registry.js`, `validation/profile-readiness.js` (`packages/loop/src/host.ts:58-66`) |
| Wire names / misc helpers | `deriveEventSpan`, `EventSpan`, `CONTROL_PLANE_TOOL_NAMES`, `SUBMIT_RESULT_TOOL_NAME`, `boundPromise`, `contentToText`, `errorText`, `isWellFormedHttpUrl`, `readJsonFile`, `ownerFromWorkspace`, `loadEnv` | various (`packages/loop/src/host.ts:67-79`) |
| Extension admission / run-deps construction | `createExtensionAdmissionController`, `ExtensionCallUnavailableError`, `ExtensionAdmissionController`, `ExtensionAdmissionOptions`, `ExtensionAdmissionSnapshot`, `MCPStatus`, `NamespacedTool`, `ToolTransport`, `buildExecuteRunDeps`, `createHostExtensionAdmission`, `createHostModelCallAdmission`, `hooksEffective`, `BuildRunDepsOptions`, `BuiltRunDeps`, `HostExtensionAdmission`, `HostModelCallAdmission`, `SkillRootInput`, `PluginBootstrapSkill` | `@clarvis/capability`, `./runtime/build-run-deps.ts`, `./runtime/capabilities/skills-settings.ts` (`packages/loop/src/host.ts:80-102`) |
| Host logging / product version | `createLogger`, `CreateLoggerOptions`, `Logger`, `VERSION` | `./logger.ts`, `./version.ts` (`packages/loop/src/host.ts:103-104`) |
...
(`packages/loop/src/host.ts:78`), which is a four-line module whose single statement re-exports it
...
consumers import it straight from `@clarvis/paths` instead — `packages/kernel/src/file-kernel.ts:84`,
`packages/kernel/src/kernel.ts:12`, `packages/code/src/adapters/workspace-client-manager.ts:5`, and
...
### 2.2 `buildExecuteRunDeps` (`packages/loop/src/runtime/build-run-deps.ts:362`)

```ts
buildExecuteRunDeps(options: BuildRunDepsOptions): Promise<BuiltRunDeps>
```

`BuildRunDepsOptions` (`packages/loop/src/runtime/build-run-deps.ts:114-173`):
...
| `builtins?` | `BuiltinCapabilityToggles` | `{ tools?, skills?, hooks? }`, each defaults **on** (`packages/loop/src/runtime/build-run-deps.ts:149-151`, `:401-403`) |
...
| `mcpAuthorization?` | `MCPAuthorizationOptions` | persistent browser OAuth for remote MCP transports; absent means no auth provider is constructed (`packages/loop/src/runtime/build-run-deps.ts:158-159`, `:410-421`) |
| `modelCallAdmission?`, `extensionAdmission?` | host-shared physical gates | if supplied, `dispose()` does not close them (`packages/loop/src/runtime/build-run-deps.ts:627-638`) |

Return shape `BuiltRunDeps` (`packages/loop/src/runtime/build-run-deps.ts:263-275`): `deps: ExecuteRunDeps`, `resolved:
...
the caller supplies none (`packages/loop/src/runtime/build-run-deps.ts:461-464`) — the point being that a host owning several
...
Computed once per call (`packages/loop/src/runtime/build-run-deps.ts:401-403`):
...
| `useSkills` | `builtins.skills`; **additionally** re-checked against `env.CLARVIS_SKILLS_ENABLED` at the dynamic-import call site (`packages/loop/src/runtime/build-run-deps.ts:402`, `:477-485`) | `CLARVIS_SKILLS_ENABLED` defaults `true` (`packages/capability/src/env.ts:114`) |
...
`CLARVIS_SKILLS_ENABLED=false` still gets no package load (`packages/loop/src/runtime/build-run-deps.ts:477-485`).
`CLARVIS_AGENT_TOOLS_ENABLED` is a **different**, per-run gate consulted inside the tools
capability's own `forRun` (`packages/loop/src/runtime/capabilities/tools.ts:128`), not by
...
### 2.4 `importOptional` — the one dynamic-import chokepoint (`packages/loop/src/runtime/build-run-deps.ts:280-307`)
...
(`packages/loop/src/runtime/build-run-deps.ts:294-305`) — naming both the package to install and the
...
the separate `reportBuiltinDisabled(logger, pkg, feature)` (`packages/loop/src/runtime/build-run-deps.ts:317-322`) whenever
`useSkills`/`useHooks`/`useTools` is false — one call site per built-in
(`packages/loop/src/runtime/build-run-deps.ts:477-478,524-525`). So the full outcome enum a reader must know to interpret this
...
| Call | Line | Guard |
|---|---|---|
| `import("@clarvis/skills")` | `packages/loop/src/runtime/build-run-deps.ts:485` | `if (useSkills && env.CLARVIS_SKILLS_ENABLED)` |
| `import("@clarvis/hooks/capability")` | `packages/loop/src/runtime/build-run-deps.ts:531` | `if (useHooks)` |
| `import("@clarvis/tools")` | `packages/loop/src/runtime/build-run-deps.ts:547` | `if (useTools)` |
| `import("./capabilities/tools.ts")` | `packages/loop/src/runtime/build-run-deps.ts:569` | `if (useTools)` (local dynamic import, not a `@clarvis/*` specifier) |
| `import("@clarvis/skills/capability")` | `packages/loop/src/runtime/build-run-deps.ts:598` | `if (useSkills)` |
...
| `BUILTIN_CAPABILITY_NAMES` | `packages/loop/src/runtime/orchestrator.ts:140-144` | `{ tools: "tools", skills: "skills", hooks: HOOKS_CAPABILITY_NAME }` — a hand-duplicated literal (see §5, INV-068) |
...
Every one of `settings-schema.ts` (`packages/loop/src/settings/settings-schema.ts:3,421`), `settings-merge.ts`
(`packages/loop/src/settings/settings-merge.ts:7,143-150`), `plugin-schema.ts` (`packages/loop/src/settings/plugin-schema.ts:4-5,103,181`),
...
### 3.5 `run.composed` log line (`packages/loop/src/runtime/orchestrator.ts:519-548`)
...
(`packages/loop/src/runtime/orchestrator.ts:515-519`).

## 4. Behavior

### 4.1 `buildExecuteRunDeps` assembly order (`packages/loop/src/runtime/build-run-deps.ts:362-640`)

1. Reject a blank `workspaceRoot` and mutually-exclusive skill root inputs (`:384-390`).
2. Compute per-component sub-loggers and set the paths-package logger (`:392-400`).
3. Compute `useTools`/`useSkills`/`useHooks` (`:401-403`).
4. Resolve the trace store (`:405-408`).
5. If `mcpAuthorization` is present, build one OAuth coordinator; inject it into the MCP client
   factory, then build the connection manager (`:410-451`). This stays independent of the optional
...
10. **Tools** (only if `useTools`): dynamically import `@clarvis/tools` to install its warning
    ...
    passes them to each run's toolsets (`packages/loop/src/runtime/build-run-deps.ts:541-590`).
11. Push `createAskUserCapability()` **unconditionally, third** (`:592`) — ask-user is the one
...
12. **Skills capability** (only if `useSkills`): dynamically import `@clarvis/skills/capability`
    and push `createSkillsCapability(skills, {...bootstraps})` **fourth** (`:593-606`).
13. Push every embedder-supplied `capabilities` entry **last, in caller order** (`:607`).
14. Assemble `ExecuteRunDeps` and return `{ deps, resolved, skills, modelCallAdmission,
    ...
    coordinator before closing only the admission gates this call created (`:609-638`).
...
### 4.2 Per-run activation (`packages/loop/src/runtime/orchestrator.ts:188-344`)

1. `allCapabilities = deps.capabilities ?? []` — the **full registered list**
   (`packages/loop/src/runtime/orchestrator.ts:196`).
2. `capabilityToolMetadata = collectCapabilityToolMetadata(allCapabilities)` — reserved names/tool
   effects computed over **registration**, not activation (`:197`).
3. Concurrently, for each registered capability, call `capability.forRun(ctx)` under a bounded wall
   ...
   `null` for this run, logged, not thrown (`packages/loop/src/runtime/orchestrator.ts:239-294`).
4. Every non-null activation is wrapped by `admittedRunCapability` (`packages/loop/src/runtime/extension-admission.ts:138-196`),
   ...
   `packages/loop/src/runtime/extension-admission.ts:132-137`). Every wrapped call is admitted on the `"normal"` lane **except**
   `onRunEnd` (both the lifecycle-hook form, `packages/loop/src/runtime/extension-admission.ts:69`, and the `RunCapability`
   form, `:182`) and `finalizeRun` (`:190`), which are admitted on the distinct `"run_end"` lane —
...
5. `runCapabilities = orderCapabilities(<filtered, admitted, non-null activations>)`
   (`packages/loop/src/runtime/orchestrator.ts:241-311`); `hooks = runCapabilities.flatMap(c => c.lifecycle ?? [])`
   (`:312`).
6. `seedBlocks` are computed by calling each **activated** capability's `seedBlock()` under the same
   setup-timeout budget, a timeout dropping the block with a warning (`packages/loop/src/runtime/orchestrator.ts:313-341`).
7. `seedMarkers = allCapabilities.map(c => c.seedMarker).filter(...)` — over the **registered**
   set, not the activated one (`packages/loop/src/runtime/orchestrator.ts:342-344`) — matching the contract's own rationale
   (§3.3).
...
### 4.3 Per-agent fold (`packages/loop/src/runtime/loop/run-agent.ts:297-419`)
...
- `forceToolOnNudge` is resolved as `entryProfile.orchestration?.force_tool_on_nudge ??
  ...
  (`packages/capability/src/api.ts:369-378`, whose own doc comment states the same loop-not-capability
...
**INV-068.** `BUILTIN_CAPABILITY_NAMES` (`packages/loop/src/runtime/orchestrator.ts:140-144`) is a
...
`@clarvis/skills` itself at `packages/loop/src/runtime/build-run-deps.ts:2` and
`@clarvis/skills/capability` at `:35`. The sibling `runtime/capabilities/skills-settings.ts:50` stays
...
[prompt-cache-and-prefix-stability](../cross-cutting/prompt-cache.md), `specs/cross-cutting/prompt-cache.md:376`, for an unrelated
...
[prompt-cache-and-prefix-stability](../cross-cutting/prompt-cache.md), `specs/cross-cutting/prompt-cache.md:387`.) The `settings-specs.ts` walk that proves INV-076 is not vacuous: it actually reaches
...
- **A capability's `seedMarker`/`reservedWireNames`/`toolEffects` are computed over every
  *registered* capability, never only the activated subset**, so a gated-off capability's tool
  ...
  `packages/loop/src/runtime/orchestrator.ts:196-197,342-344`. Test:
...
- **`importOptional` is the only load seam for an enabled built-in's failed optional-package load**;
  ...
  Production: `packages/loop/src/runtime/build-run-deps.ts:280-307`. Unpinned by a dedicated
...
|---|---|---|
| Enabled built-in's optional package fails to `import()` | `buildExecuteRunDeps` **throws** an actionable `Error` naming the package and the opt-out | `packages/loop/src/runtime/build-run-deps.ts:280-307` |
| `workspaceRoot` blank | `buildExecuteRunDeps` throws before touching any built-in | `packages/loop/src/runtime/build-run-deps.ts:384-386`; test `packages/loop/tests/integration/execute-run-entrypoints.test.ts:361-369` |
| Both `skillRoots` and `extraSkillRoots` supplied | `buildExecuteRunDeps` throws before optional package construction | `packages/loop/src/runtime/build-run-deps.ts:387-390` |
...
| Skill discovery throws (initial, array roots) | Skills disabled for these deps; warned, not thrown | `packages/loop/src/runtime/build-run-deps.ts:507-518` |
| Skill discovery throws (function roots, rescan) | Falls back to last good scan, or an empty provider on first failure | `dynamicSkills`, `packages/loop/src/runtime/build-run-deps.ts:203-255`; test `packages/loop/tests/integration/execute-run-entrypoints.test.ts:371-394` |
| Root provider function throws | Treated as "no roots" for that rescan, logged at `debug` | `packages/loop/src/runtime/build-run-deps.ts:212-226` |
| Workspace path does not resolve at all | Skills come back `undefined` rather than throwing | test `packages/loop/tests/integration/execute-run-entrypoints.test.ts:348-359` |
| A capability's `forRun` exceeds `CLARVIS_CAPABILITY_SETUP_TIMEOUT_MS` | Activation resolves `null` for this run; warned | `packages/loop/src/runtime/orchestrator.ts:283-293` |
| A capability's `forRun` throws `ExtensionCallUnavailableError` (host gate saturated) | Activation resolves `null`; warned, not thrown | `packages/loop/src/runtime/orchestrator.ts:270-281` |
| A capability's `seedBlock()` exceeds its setup budget | Block omitted from the seed; warned | `packages/loop/src/runtime/orchestrator.ts:313-341` |
| A capability's extension-admitted `seedBlock` hits a saturated host gate | The block degrades to `undefined`; logged `capability.extension_saturated` | `packages/loop/src/runtime/extension-admission.ts:153-171`; test `packages/loop/tests/unit/extension-admission.test.ts:132-185` |
| A capability's extension-admitted lifecycle method, `onRunEnd`, or `finalizeRun` hits a saturated host gate | The `ExtensionCallUnavailableError` propagates to the owning caller; this wrapper supplies no fallback | `packages/loop/src/runtime/extension-admission.ts:24-127,178-193` |
| A capability's `finalizeRun` throws or exceeds `CLARVIS_CAPABILITY_RUN_END_TIMEOUT_MS` | Its state slot is omitted from the run record; the run itself is unaffected | `packages/loop/src/runtime/execute-run.ts:196-247` |
...
- **No test in this document's scope exercises `importOptional`'s thrown-error path directly**
  ...
  reading of `packages/loop/src/runtime/build-run-deps.ts:280-307` rather than by a dedicated negative test.
