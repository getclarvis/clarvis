...
| `validateDelegateTaskArgs` | fn | `packages/loop/src/runtime/subagents/delegate-task.ts:92-95` | `(raw, options) => DelegateTaskArgsResult` |
| `prepareSpawn` | fn | `packages/loop/src/runtime/subagents/delegate-task.ts:321-324` | `(rawArgs, ctx) => Promise<PrepareSpawnResult>` |
| `buildRunSubagentInput` | fn | `packages/loop/src/runtime/subagents/delegate-task.ts:495-498` | `(profile, base) => RunSubagentInput` |
| `runPreparedSubagent` | fn | `packages/loop/src/runtime/subagents/delegate-task.ts:570-573` | `(prepared, ctx) => Promise<SpawnResult>` |
...
| Id | Shape | Minted at |
| --- | --- | --- |
| `subagentInstanceId` (a.k.a. `delegation_id`) | `randomUUID()` | `packages/loop/src/runtime/subagents/delegate-task.ts:399` |
...
| --- | --- | --- |
| `delegation_created` | `delegation_id`, `task_id?`, `title`, `task`, `profile`, `tools` | `packages/loop/src/runtime/subagents/delegate-task.ts:408-421` |
| `delegation_started` | `delegation_id`, `task_id?`, `model` | `packages/loop/src/runtime/subagents/delegate-task.ts:602-612` |
| `delegation_failed` | `delegation_id`, `task_id?`, `status` (`"cancelled"` \| `"error"`) | `packages/loop/src/runtime/subagents/delegate-task.ts:667-677` (throw path) |
| `delegation_failed` | `delegation_id`, `task_id`, `status` | `packages/loop/src/runtime/subagents/delegate-task.ts:714-724` (tracked failure path) |
| `delegation_completed` / `delegation_failed` | `delegation_id`, `task_id?`, `status` | `packages/loop/src/runtime/subagents/delegate-task.ts:735-746` |
...
| --- | --- | --- |
| `{ subagentInstanceId, status, result }` | `packages/loop/src/runtime/subagents/delegate-task.ts:655-660` (throw path) | `"cancelled"` when `ctx.signal?.aborted`, else `"error"` |
| `{ subagentInstanceId, status, result }` | `packages/loop/src/runtime/subagents/delegate-task.ts:695-700` (normal path) | the outcome's own `status` (`completed` / `budget_exhausted` / `cancelled` / `error`) |

The hook compiler and `subagent_start` payload are pinned at
`packages/hooks/tests/component/capability.test.ts:394-408`; no `@clarvis/loop` test independently
...
4. **Throw path** (`:647-684`): accumulate usage from the pre-seeded sink; if `ctx.signal.aborted`
...
5. **Normal path**: accumulate usage (`:686`), render text via `mapOutcomeToText` (`:688`), record
   `delegation_completed`/`delegation_failed` on the trace (`:689-694`), fire `onSubagentComplete`
   (`:695-700`).
