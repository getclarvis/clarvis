...
(`packages/loop/src/validation/request-schema.ts:33`) and `settingsSchemaFor`
(`packages/loop/src/settings/capability-settings.ts:42`) are the two answers to that problem, built on
...
| --- | --- | --- |
| `validateBody` | `(raw: unknown, env: EnvConfig, registry?: CapabilityRegistry) => ValidatedRunRequest` | `packages/loop/src/validation/request-schema.ts:33` |
| `ValidatedRunRequest` | `{ request: RunRequest; shape: RunShape }` | `packages/loop/src/validation/request-schema.ts:27` |
| `runRequestSchema` | Zod object, `.strict()` | `packages/loop/src/validation/request/request-schema.ts:160` |
| `ParsedRunRequest` | `z.infer<typeof runRequestSchema>` | `packages/loop/src/validation/request/request-schema.ts:229` |
| `budgetSchema` | Zod object, `.strict()` | `packages/loop/src/validation/request/request-schema.ts:96` |
| `agentProfileSchema` | Zod object, `.strict()` | `packages/loop/src/validation/request/profile-schemas.ts:134` |
...
| `BUILTIN_GRANT_NAMES` | `readonly ["ask_user","read_workspace","edit_workspace","run_commands"]` | `packages/loop/src/validation/request/grant-registry.ts:17` |
| `requireKnownGrants` | `(data, registry?) => void`, throws `invalid_profile` | `packages/loop/src/validation/request/grant-registry.ts:22` |
| `profileReadinessIssues` | `(ctx: ReadinessContext) => ReadinessIssue[]` | `packages/loop/src/validation/profile-readiness.ts:211` |
| `PROFILE_READINESS_RULES` | `readonly ReadinessRule[]`, 8 rules | `packages/loop/src/validation/profile-readiness.ts:94` |
| `createAjv` / `createStrictAjv` | `() => AjvInstance` | `packages/loop/src/validation/ajv.ts:70`, `:81` |
| `INPUT_LIMITS` | `Record<string, number>` ceiling table | `packages/loop/src/validation/input-limits.ts:2` |
| `boundedRecord` | `<K,V>(value: Record<K,V>, maxEntries: number) => boolean` | `packages/loop/src/validation/input-limits.ts:27` |
| `positiveIntField` / `nonnegativeIntField` | `(label: string) => ZodNumber` — the shared "must be a positive/non-negative integer" field, used by `budgetSchema`, `agentProfileSchema` (`retry`, `compaction`, `stagnation_threshold`, `call_timeout_ms`) and `providerConfigSchema`'s `models[].context_window_tokens`/`max_output_tokens` | `packages/loop/src/validation/request/numeric-schemas.ts:3`, `:9` |

Barrel: `packages/loop/src/validation/index.ts:5-7` re-exports `ajv`, `profile-readiness` and
...
| `agentPromptOf` | `(basePrompt?: string, body: string) => string \| undefined` | `packages/loop/src/settings/agent-frontmatter.ts:141` |
| `editDistance` | `(a: string, b: string, limit: number) => number` | `packages/loop/src/settings/typo-suggestion.ts:21` |
| `typoBudget` | `(candidate: string) => number` | `packages/loop/src/settings/typo-suggestion.ts:49` |
...
`runRequestSchema` (`packages/loop/src/validation/request/request-schema.ts:160`) is a strict object
...
`...capabilityRequestParamFields` (`packages/loop/src/validation/request/request-schema.ts:224`) — a
...
whitespace — and `EXECUTION_ID_MIN`/`EXECUTION_ID_MAX` (`:8`, `:11`) are `1`/`128`. `request-schema.ts`
...
(`packages/loop/src/validation/request/request-schema.ts:22-24`) and again for `continue_from`
(`:77-79`), each with its own field-named error message. A caller that supplies neither field gets a
...
Example minimal valid body (from the test fixture, `packages/loop/tests/helpers/request.ts:7`):
...
`messagesField`/`messageSchema` (`packages/loop/src/validation/request/message-schemas.ts:3-115`)
...
the whole `messages` array) — all defined at `packages/loop/src/validation/request/message-schemas.ts:3-7`. `contentPartSchema` is a
`discriminatedUnion("type", [textPartSchema, imagePartSchema])` (`:21-42`); array (multimodal)
content is legal only on a `user` message, enforced by `messageSchema`'s own `superRefine`
(`:75-82`), which also enforces the per-message aggregate cap (`:83-89`); `messagesField`'s own
`superRefine` (`:100-115`) walks the array and enforces the whole-history aggregate cap. The doc
comment on `messagesField` states why a per-part cap alone does not bound the sum: "100 legal 10 MB
images in one message (and 10,000 such messages) passed validation" before this refinement existed
(`:97-99`). Tests: `packages/loop/tests/unit/request-message-schemas.test.ts:24` (multimodal
rejected on a `system` message), `:50-61` (per-message aggregate cap defeats individually-legal
parts), `:63-67` (whole-history aggregate cap defeats individually-legal messages).

One agent profile (`agentProfileSchema`,
`packages/loop/src/validation/request/profile-schemas.ts:134`): `name`, `description?`, `model`,
...
`packages/loop/src/validation/request/profile-schemas.ts:106-110`, proven open by
`packages/loop/tests/unit/request-profile-validation.test.ts:34` accepting an extra
`capability_owned` key). `compactionSchema` carries a `superRefine`
(`packages/loop/src/validation/request/profile-schemas.ts:65-86`) enforcing two cross-field rules
...
`packages/loop/src/validation/request/provider-schemas.ts:26`): `name` (regex `^[a-z0-9_-]+$`),
...
`splitAgentFrontmatter` (`packages/loop/src/settings/agent-frontmatter.ts:39`) splits a markdown file
...
`packages/loop/tests/unit/agent-frontmatter.test.ts:18-19`).

`agentFrontmatterSchema` (`packages/loop/src/settings/agent-frontmatter.ts:97`) is
...
(`packages/loop/src/settings/agent-frontmatter.ts:79-95`).

The frontmatter's own `budget` field (`packages/loop/src/settings/agent-frontmatter.ts:113-117`)
...
`packages/loop/src/settings/settings-schema.ts:18-25`) that §4.4 documents for the top-level
...
`ValidationError` (`packages/capability/src/errors.ts:30`) carries an `ErrorCode`
(`packages/capability/src/run.ts:215` — open union, `BuiltinErrorCode | (string & {})`). The 42
built-in codes are enumerated at `packages/capability/src/run.ts:158-201`; the ones this subsystem
...
`INPUT_LIMITS` (`packages/loop/src/validation/input-limits.ts:2-25`) is the one table both the
...
`validateBody` (`packages/loop/src/validation/request-schema.ts:33-49`) runs, in this exact order:
...
(`packages/loop/src/validation/request-schema.ts:32`), and
`packages/loop/tests/component/request-schema-facade.test.ts:19-30` pins one instance of it
...
`parseRunRequest` (`packages/loop/src/validation/request/parsing.ts:113`) calls
`runRequestSchemaFor(registry).safeParse(raw)`; on failure it takes only the **first** issue
(`pickFirstIssue`, `packages/loop/src/validation/request/parsing.ts:66`) and maps it to an `ErrorCode`
via `classifyIssue` (`packages/loop/src/validation/request/parsing.ts:8-64`), which switches on the
...
(`packages/loop/src/validation/request/parsing.ts:20-22`); `messages` → `messages_empty` for an
...
`packages/loop/tests/unit/request-parsing.test.ts:8-50` pins this table entry-by-entry against 19
distinct malformed inputs.

`runRequestSchemaFor(registry)` (`packages/loop/src/validation/request/parsing.ts:95-111`) extends
...
(`packages/loop/src/validation/request/parsing.ts:100-106`, pinned at
`packages/loop/tests/unit/request-parsing.test.ts:69-80`).
...
(`packages/loop/src/validation/request/identity-rules.ts:5-83`)
...
`enforceBudgetMode` (`packages/loop/src/validation/request/budget-rules.ts:6-43`):
...
`enforceEnvCeilings` (`packages/loop/src/validation/request/budget-rules.ts:54-97`) rejects, against
...
`settingsBudgetSchema` (`packages/loop/src/settings/settings-schema.ts:18-25`) is `budgetSchema` with
...
`packages/loop/tests/unit/settings-schema.test.ts:264-271`) while the same body would fail
`budgetSchema` at the request boundary.

### 4.5 Provider rules (`provider-rules.ts`)

`rejectProviderConfigIssues` (`packages/loop/src/validation/request/provider-rules.ts:83-136`):
...
(`isWellFormedHttpUrl`, `packages/loop/src/http-url.ts:8`); requires `base_url` when
...
(`packages/loop/src/validation/request/provider-rules.ts:111-129`). Tests first prove structural
acceptance and then semantic rejection for both kinds at
`packages/loop/tests/unit/request-provider-validation.test.ts:11-42`.

`rejectProviderMapIssues` (`packages/loop/src/validation/request/provider-rules.ts:15-50`) applies
...
`referencedProviders` (`packages/loop/src/validation/request/provider-rules.ts:68-73`) — the `used`
...
`profiles`. Proven at `packages/loop/tests/unit/request-provider-validation.test.ts:169-188`: an
...
`requireResolvableModelProviders` (`packages/loop/src/validation/request/provider-rules.ts:148-157`)
...
`enforcePerProfileRules` (`packages/loop/src/validation/request/profile-rules.ts:6-69`) first sums a
...
`packages/loop/tests/unit/request-profile-validation.test.ts:117-127` with 33 profiles each carrying
...
`deriveRunShape` (`packages/loop/src/validation/request/run-shape.ts:12-30`) computes: `entry` (the
...
`requireKnownGrants` (`packages/loop/src/validation/request/grant-registry.ts:22-39`) builds the
...
"edit_workspace", "run_commands"]` (`packages/loop/src/validation/request/grant-registry.ts:9-19`).
...
`PROFILE_READINESS_RULES` (`packages/loop/src/validation/profile-readiness.ts:94-201`) is a data
...
`packages/loop/tests/unit/profile-readiness.test.ts:92-98`, two ghost targets → two issues).

`ReadinessCode` (`packages/loop/src/validation/profile-readiness.ts:18-27`) has a **9th** member,
...
(`packages/code/src/adapters/agent-files.ts:219`), reusing this module's `ReadinessCode` type for a
verdict this module itself never reaches.

### 4.9 Settings composition: `settingsSchemaFor`, `mergeSettings`

`settingsSchemaFor(registry)` (`packages/loop/src/settings/capability-settings.ts:42-69`): returns
the bare `settingsSchema` when the registry is empty; otherwise, for every registered spec, throws a
plain `Error` if `spec.key` already names a built-in block (`packages/loop/src/settings/capability-settings.ts:46-51`)
or if the spec declares any plugin-manifest surface (`pluginContributable`, `pluginDescription`,
`pluginForbiddenReason` — `packages/loop/src/settings/capability-settings.ts:52-63`, because the
...
`splitAgentFrontmatter(raw, mode)` (`packages/loop/src/settings/agent-frontmatter.ts:39-63`): strips
...
the **only** call in `packages/loop/src/runtime/tools/result-contract.ts:129`, vetting a caller-supplied
...
Production: `packages/loop/src/validation/request/message-schemas.ts:67-90` (role enum has no
`"tool"` member). Test: `packages/loop/tests/contract/native-tool-wire-stable.contract.test.ts:32`,
`:36`.

**INV-047.** The wire role enum still excludes `tool` — a caller-sent message with `role: "tool"` is
rejected by `validateBody`.
Production: `packages/loop/src/validation/request/message-schemas.ts:69-71`
(`z.enum(["system","user","assistant"])`). Test:
`packages/loop/tests/contract/native-tool-wire-stable.contract.test.ts:51`.
...
`packages/loop/src/validation/request/request-schema.ts:236-257`. Test:
`packages/loop/tests/contract/schema-type-parity.contract.test.ts:8`, `:11`.
...
(`packages/loop/src/validation/request/request-schema.ts:245-253`): `_agentProfileDriftLock`
...
Production: `packages/loop/src/validation/request/identity-rules.ts:5-83`. Test:
...
Production: `packages/loop/src/validation/request/budget-rules.ts:6-27`. Test:
`packages/loop/tests/unit/request-budget-rules.test.ts:55-61`.
...
Production: `packages/loop/src/validation/request/provider-rules.ts:33-40`, `:68-73`. Test:
`packages/loop/tests/unit/request-provider-validation.test.ts:136-155`.
...
Production: `packages/loop/src/settings/agent-frontmatter.ts:79-97`. Test:
`packages/loop/tests/unit/agent-frontmatter.test.ts:49-61`, `:96-99`.
...
Production: `packages/loop/src/settings/capability-settings.ts:45-63`. Test:
`packages/loop/tests/unit/capability-settings.test.ts:78-119`, `:145-151` (exhaustively, over every
...
Production: `packages/loop/src/validation/request/profile-schemas.ts:65-86`. Test:
`packages/loop/tests/unit/request-profile-validation.test.ts:48-49`.
...
(`packages/loop/src/validation/request/request-schema.ts:236-257`) exist purely so a change to
...
- **Why `capabilityRequestParamFields`/`capabilitySettingsFields` are spread statically from
  ...
  (`packages/loop/src/settings/capability-settings.ts:31-37`, `packages/loop/src/validation/request/request-schema.ts:154-159`),
  and the reason for the split itself is now stated at
  `packages/loop/src/runtime/capabilities/settings-specs.ts:15`–`:29`: the static spread *is* the
...
- **The precise reason `guard_judge`'s provider is folded into `referencedProviders` but not into any
  ...
  is stated as intentional in the doc comment (`packages/loop/src/validation/request/provider-rules.ts:56-66`)
  ...
  `packages/loop/src/validation/request/provider-rules.ts:70-71` touches `guard_judge`).
...
- **The exact value and derivation of `envRefPattern()`** (imported from `@clarvis/capability` and
  used at `packages/loop/src/validation/request/provider-rules.ts:12`) is out of this document's scope
