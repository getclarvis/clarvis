...
| `NOOP_HOOK_LOGGER` | const | `packages/hooks/src/types.ts:178` |
| `HookFailure`, `HookFailureKind`, `HookInvocation`, `HookLogger`, `HookMatch`, `HookOutcome`, `HookResult`, `HookSpec`, `ToolCandidate` | interface/type | `packages/hooks/src/types.ts:112,108,57,164,20,135,143,42,51` |
...
(`packages/hooks/src/event-serialization.ts:1,24,179`).

### 2.2 Entrypoint `./capability` (`src/capability.ts`)

| Symbol | Kind | Defined at |
|---|---|---|
| `HOOKS_SEED_MARKER` (`= "<workspace-hooks>"`) | const | `packages/hooks/src/capability.ts:42` |
| `compileWorkspaceHooks` | fn | `packages/hooks/src/capability.ts:113` |
| `buildSeedBlock` | fn | `packages/hooks/src/capability.ts:202` |
| `runCredentialNames` | fn | `packages/hooks/src/capability.ts:252` |
| `WorkspaceHooksOptions` | interface | `packages/hooks/src/capability.ts:270` |
| `createWorkspaceHooksCapability` | fn | `packages/hooks/src/capability.ts:312` |
| `createHooksCapability` | fn | `packages/hooks/src/capability.ts:362` |
...
| `EXTERNAL_HOOK_EVENT_NAMES` | Clarvis→foreign event-name table (§3.4) | `:90-99` |
| `EXTERNAL_TOOL_NAMES` | foreign-normalized-name→Clarvis-tool-name table (§3.4) | `:125-141` |
| `EXTERNAL_TOOLS_WITHOUT_COUNTERPART` | `Set` of 5 foreign names with no Clarvis tool | `:152-158` |
| `normalizeToolName` | fn: strip non-alnum, lower-case | `:169-171` |
| `HOOK_DEFAULT_TIMEOUT_MS` | per-class default timeout table | `:207-213` |
| `MAX_HOOKS_PER_SOURCE` (`64`), `MAX_HOOKS_PER_RUN` (`128`), `MAX_HOOK_COMMAND_CHARS` (`8192`), `MAX_HOOK_MATCH_PATTERNS` (`64`), `MAX_HOOK_PATTERN_CHARS` (`2048`), `MAX_HOOK_TIMEOUT_MS` (`60000`) | consts | `:216-221` |
| `hookSchema` | zod schema, one validated hook entry | `:232-376` |
| `HookConfig` | `z.infer<typeof hookSchema>` | `:379` |
...
| `HOOKS_PLUGIN_FIELDS.hooks` | same schema, plugin-manifest field | `:41-48` |
| `hooksSettingsSpec` | `CapabilitySettingsSpec` — `key: "hooks"`, merges scopes as **all operator hooks then all plugin hooks**, capped at `MAX_HOOKS_PER_RUN`, `pluginContributable: true` | `:55-65` |
...
(`packages/hooks/src/runner.ts:301-309`):
...
```
Schema at `packages/capability/src/hooks-config.ts:232-376`. Cross-field rules enforced by
`superRefine` (`:323-372`): `match` only valid for `pre_tool_use`/`post_tool_use` and must set
...
and fails simply contributes nothing", `:359-361`); `on_failure: "deny"` is rejected on any other
observer event.
...
`SERIALIZE` (`packages/hooks/src/event-serialization.ts:99-170`) is a **total, statically checked**
record over `HookConfig["event"]` (`satisfies { [Event in HookEvent]: ... }`, `:170`) — a new
...
(`:89-97`). Free-text fields are clamped to `TEXT_CLAMP = 8_000` chars
(`clampText`, `:48,52-55`); structured values are clamped to `VALUE_CLAMP = 32_000` bytes of JSON,
replaced by `{ truncated: true, bytes }` when they exceed it, or `{ truncated: true }` when they
fail to serialize at all (`clampValue`, `:49,58-67`). `post_tool_use`'s `tool_response` drops raw
image bytes and reports `image_count` instead (`:104-113`; test at
`packages/hooks/tests/component/capability.test.ts:427-446`).

Every event's exact field set (`packages/hooks/src/event-serialization.ts:99-169`), each individually pinned by a
`payloadFor("<event>", …)` assertion in `packages/hooks/tests/component/capability.test.ts:300-460`:

| Event | Fields on the stdin payload | Test |
|---|---|---|
| `pre_tool_use` | `tool_name`, `tool_input` (clamped value) | shape: `packages/hooks/tests/component/runner.test.ts:136-153`; clamping: `packages/hooks/tests/component/capability.test.ts:400-407` |
| `post_tool_use` | `tool_name`, `tool_input`, `tool_response: { text, progress, task_id, image_count }` | `packages/hooks/tests/component/capability.test.ts:428-446` |
| `pre_finalize` | `agent`, `subagent_instance_id`, `mode`, `text`, `value` (clamped) | `packages/hooks/tests/component/capability.test.ts:410-425` |
| `pre_delegate_task` | `title`, `task` (clamped text), `profile`, `task_id` | `packages/hooks/tests/component/capability.test.ts:340-346` |
| `run_start` | `mode`, `entry`, `lead_model`, `subagent_model` | `packages/hooks/tests/component/capability.test.ts:347-351` |
| `run_end` | `status`, `error_code`, `iterations_used`, `elapsed_ms` | `packages/hooks/tests/component/capability.test.ts:353-362` |
| `subagent_complete` | `subagent_instance_id`, `status`, `result` (clamped text) | `packages/hooks/tests/component/capability.test.ts:364-368` |
| `pre_compact` | `agent`, `subagent_instance_id`, `estimated_tokens` | `packages/hooks/tests/component/capability.test.ts:370-374` |
| `model_call_error` | `agent`, `subagent_instance_id`, `iteration`, `model`, `message` (clamped) | `packages/hooks/tests/component/capability.test.ts:376-386` |
| `budget_exhausted` | `agent`, `reason`, `tokens_used`, `iterations_used` | `packages/hooks/tests/component/capability.test.ts:388-397` |
| `user_steer` | `agent`, `subagent_instance_id`, `iteration`, `message` (clamped), `id` | `packages/hooks/tests/component/capability.test.ts:327-338` |
| `session_start` | none — projects to `{}` (`packages/hooks/src/event-serialization.ts:168`) | not exercised via `payloadFor` in this document's scope |

(`pre_delegate_task` through `budget_exhausted` above share one test body, "projects every
remaining event onto snake_case fields", `packages/hooks/tests/component/capability.test.ts:340-398`.)

`defaultTimeoutFor` (`packages/hooks/src/event-serialization.ts:39-45`) resolves `HookInvocation.defaultTimeoutMs` by
...
in that order (`packages/hooks/tests/component/capability.test.ts:484-497`).
...
`EXTERNAL_TOOL_NAMES` (`:125-141`, keyed by `normalizeToolName`: letters+digits only, lower-cased,
`:169-171`): `bash`/`shell→shell`, `read`/`readfile→read_file`, `write`/`writefile→write_file`,
`edit`/`editfile→edit_file`, `multiedit→multi_edit`, `applypatch→apply_patch`, `glob→glob`,
`grep→grep`, `ls`/`listdir→list_dir`, `task→delegate_task`. Measured against a public catalog of
...
(`:107-112`). `EXTERNAL_TOOLS_WITHOUT_COUNTERPART` (`:152-158`) lists 5 foreign names with no
...
```
(`packages/hooks/src/types.ts:135-140`, `:108-109`.)
...
`createWorkspaceHooksCapability`, never supplies it (`packages/hooks/src/capability.ts:320-322`
...
1. The host builds a `HookInvocation` for the event (data payload, optional tool candidate,
   ...
   `compileWorkspaceHooks` path this is `hookInvocationFor` (`packages/hooks/src/event-serialization.ts:179-192`).
...
4. `runner.resolve(result, inv)` (`packages/hooks/src/runner.ts:357-366`) folds the result into the `HookOutcome` the
...
against, with four rules each pinned by a dedicated test (`packages/hooks/tests/unit/match.test.ts:81-181`):
...
`matchesCandidate(compiled, candidate)` (`packages/hooks/src/match.ts:145-159`) combines a compiled filter and a
...
`packages/hooks/tests/unit/match.test.ts:126-129`). The doc comment states this path is reachable only for an embedder that
...
`hookSchema`'s own `superRefine` is what performs that rejection (`packages/capability/src/hooks-config.ts:323-372`).

### 4.3 `compileWorkspaceHooks` — folding a list of results into one `LifecycleHook` verdict

For a **gate** event (`packages/hooks/src/capability.ts:126-149`): iterates every selected spec in
order; a `deny` short-circuits the whole loop immediately (test:
`packages/hooks/tests/component/capability.test.ts:227-251`, "short-circuits on the first deny, so
...
later rewrite does not re-run selection" (`packages/hooks/src/capability.ts:120-124`) — but every *subsequent* hook in
the same pass now sees the invocation rebuilt from the replaced arguments (test:
`packages/hooks/tests/component/capability.test.ts:146-164`, "threads the replacement forward so the last writer wins"). At the
...
For an **observer** event (`:151-157`): all selected hooks run concurrently via
...
For a **compaction** event (`pre_compact`, `:159-171`): every selected hook runs in sequence; each
...
`EVENT_METHOD` (`:64-76`) is a **total** record over gate+observer+compaction events mapping each to
...
object (`:173-182`), and the function returns `undefined` when nothing gate/observer/compaction-shaped
is configured (`:184`; test: "returns undefined when nothing gate- or observer-shaped is
configured", `packages/hooks/tests/component/capability.test.ts:55-58`).
...
logs a warning instead of propagating (`packages/hooks/src/capability.ts:202-224`; test:
`packages/hooks/tests/component/capability.test.ts:550-576`, "swallows a throwing context hook
rather than failing the run"). Returns `undefined` when nothing contributed; otherwise
`` `${HOOKS_SEED_MARKER}\n${texts.join("\n\n")}\n${HOOKS_SEED_CLOSE}` `` (`:223-224`).
...
1. `opts.resolveHooks(ctx) ?? []`; if empty, returns `null` — the capability does not activate
   (`packages/hooks/src/capability.ts:317-318`; test: "does not activate for a run with no hooks
   configured", `packages/hooks/tests/component/capability.test.ts:512-518`). Re-read on **every** call (`WorkspaceHooksOptions.resolveHooks`
   remark, `:272-278`; test: "re-reads the configuration on every run", `packages/hooks/tests/component/capability.test.ts:520-529`).
...
3. Logs `hooks.env_filtered` with only the withheld **counts**, never names
   (`packages/hooks/src/capability.ts:324-332`).
4. Builds a `HookRunner` bound to `ctx.workspaceRoot`, the filtered env, the normalized logger
   (`ctx.logger ?? NOOP_LOGGER`, `:323`), and `ctx.executionId` as `sessionId` (`:333-338`).
5. `compileWorkspaceHooks` builds the `LifecycleHook` (or `undefined` if nothing gate/observer/
   compaction-shaped is configured); the returned `RunCapability` carries `lifecycle: [hook]` only
   when defined (`:341-343`), and `seedBlock` always runs `buildSeedBlock` regardless (so a
   context-only config still contributes; a hook-only config contributes `undefined` from
   `seedBlock`, per test `packages/hooks/tests/component/capability.test.ts:594-601`). `forAgent` always returns `null` — no
   per-agent surface (`:345`; test `packages/hooks/tests/component/capability.test.ts:531-539`).
...
(`:315`), so a stale `<workspace-hooks>` block from a prior run is stripped even on a later run
where hooks are configured away entirely (test: "always declares its seed marker, so a stale block
is stripped even when off", `packages/hooks/tests/component/capability.test.ts:504-510`).

### 4.9 `runCredentialNames` — the denylist a run's own credential surface derives

Collects, per run (`packages/hooks/src/capability.ts:252-267`):
...
- every name `${VAR}`-interpolated into a provider's own `headers` and into each of its `models`
  ...
  drop every model-level header in silence", `:248-250`; pinned by the two "T9" tests, one per
...
- plus `extra` (the caller-supplied `credentialNames` list) — because "the kernel narrows a run's
  ...
  inherited environment (`:240-246`; test: "keeps host-managed names even when the run narrowed
...
(`packages/hooks/src/capability.ts:362-377`). It takes `hooks: readonly LifecycleHook[]` that are
already built — not a `HookConfig[]` to resolve from settings — and an overridable `name` that
defaults to `HOOKS_CAPABILITY_NAME` (`:363-364`). Its `forRun` returns `null` only when the supplied
list is empty; otherwise it returns a `RunCapability` whose `lifecycle` is the list **verbatim**
and whose `forAgent` always returns `null` (`:368-374`). Three asymmetries from
`createWorkspaceHooksCapability`:
- **No `seedMarker` at all** — the returned `Capability` object carries only `name` and `forRun`
  (`:366-376`), unlike `createWorkspaceHooksCapability`'s `Capability`, which always declares
  `HOOKS_SEED_MARKER` (`:315`).
...
accepts an override" (`packages/hooks/tests/component/capability.test.ts:604-623`).
...
1. **A hook's runner surface never throws.** `runHookCommand` documents "**Never rejects**"
   ...
   (`packages/hooks/tests/component/capability.test.ts:299-310`).
2. **An observer event can never deny, however the hook resolves.** `runner.resolve`: a successful
   non-gate `deny` downgrades to `pass` (`packages/hooks/src/runner.ts:359`); a non-gate failure resolves to `pass`
   regardless of `on_failure` (`:363`). Pinned by `packages/hooks/tests/component/runner.test.ts:573-593`.
3. **Cancellation never denies, even for a fail-closed hook.** `runner.resolve`: `aborted` resolves
   to `pass` before `on_failure` is even consulted (`packages/hooks/src/runner.ts:362`). Pinned by
   `packages/hooks/tests/component/runner.test.ts:623-637` and by `classify`'s precedence
...
4. **Exit code 2 at a gate is a verdict (`deny`), not an ordinary failure**, and is honoured even
   ...
   `packages/hooks/tests/component/runner.test.ts:342-361` and the real-shell integration path is
   ...
   `packages/hooks/tests/component/runner.test.ts:391-404`).
5. **A command that does not exist is `exit_nonzero` (code 127), never `spawn_failed`** — "what
   gets spawned is the shell, and the shell exists" (`packages/hooks/src/types.ts:104-106`, `packages/hooks/src/runner.ts:113-114`).
...
6. **A rewrite outcome is representable only where `HookInvocation.rewritable` is true**, and
   ...
   allowed. Production: `packages/hooks/src/types.ts:82-88`, enforced in `packages/hooks/src/parse.ts:239-241,159-160`. Pinned by
   ...
   `packages/hooks/tests/component/runner.test.ts:381-389`.
7. **`REWRITABLE_EVENTS` contains only `pre_tool_use`, not `post_tool_use`.** A rewrite fired after
   ...
   (`packages/hooks/src/event-serialization.ts:33-37`). Unpinned directly by a dedicated `post_tool_use`-rewrite test in
...
8. **A rewrite replaces arguments wholesale, never merges them.** Stated at the `HookOutcome` type
   (`packages/hooks/src/types.ts:140`) and at the loop's own `HookVerdict` (`packages/capability/src/api.ts:482-483`,
   "The replacement is total, not a merge"). Within `compileWorkspaceHooks`, later hooks see the
   replaced arguments and "the last writer wins" (`packages/hooks/src/capability.ts:123`); pinned by
   `packages/hooks/tests/component/capability.test.ts:146-164`.
9. **Selection is decided once per fire point, against the model's original arguments, and is not
   ...
   once, before the loop over selected specs (`packages/hooks/src/capability.ts:133`); a later replacement changes
...
10. **A `deny` short-circuits the gate fold; hooks after it never run.** `packages/hooks/src/capability.ts:136`
    ("return" inside the `for` loop). Pinned by
    `packages/hooks/tests/component/capability.test.ts:227-251`.
11. **Configuration order is load-bearing: operator hooks always precede plugin hooks**, and the
    ...
    `hooksSettingsSpec.merge` in `packages/loop/src/runtime/capabilities/hooks.ts:58-61`, outside
    `@clarvis/hooks` itself but consumed by it; the ordering guarantee "an operator always gets
    the first verdict" is stated at `packages/hooks/src/runner.ts:85-86` and at `packages/loop/src/runtime/capabilities/hooks.ts:52-53`, and demonstrated by
...
12. **The `session_start`/`pre_compact` seed/contribution channels never fail or block a run.**
    ...
    (`packages/capability/src/hooks-config.ts:355-362`); `buildSeedBlock` additionally swallows a throwing runner in
    production code, not merely by convention (`packages/hooks/src/capability.ts:192-194,215-221`; pinned by
    `packages/hooks/tests/component/capability.test.ts:550-576`).
...
14. **Withheld environment variables are counted, never named**, anywhere in this package's own
    ...
    (`packages/hooks/src/capability.ts:325-331`). Unpinned by a direct assertion that the log line's JSON never
...
19. **Argument-match patterns are tested only against a clamped prefix** (`ARG_MATCH_MAX_CHARS =
    ...
    `packages/hooks/tests/unit/match.test.ts:174-180`.
20. **A broken `match.args` regex makes the whole filter never fire, rather than being ignored.**
    ...
    `packages/hooks/tests/unit/match.test.ts:126-129` and
...
22. **`hooks[].event` is a closed, exhaustive enum of exactly the four vocabulary groups
    (gate+observer+context+compaction), and `hookSchema` rejects anything else.** Production:
    `HOOK_EVENTS` composition and `z.enum(HOOK_EVENTS, …)` (`packages/capability/src/hooks-config.ts:173-178,234-237`).
...
(`packages/hooks/src/subprocess.ts:15-16`; `packages/hooks/src/types.ts:118`).
...
`packages/hooks/src/capability.ts:21-32` and `packages/hooks/src/event-serialization.ts:2-21`); `@clarvis/tools/shell` — the narrow
...
(`packages/loop/src/runtime/build-run-deps.ts:345-346,466`, with the
...
(`packages/hooks/src/capability.ts:10-15`; `packages/loop/src/runtime/build-run-deps.ts:476-483`).
...
keys would silently disable both fast paths for the whole run (`packages/hooks/src/capability.ts:102-106`). This is
...
- **Why `pre_delegate_task`, `run_start`, `model_call_error` and `budget_exhausted` have no foreign
  ...
  the wrong moment is worse than an honest gap", `packages/capability/src/hooks-config.ts:87-88`); no comment explains why
...
- **Invariant #9** (selection is not re-run after a rewrite) is stated as a documented design
  decision in a `remarks` comment (`packages/hooks/src/capability.ts:120-124`) but the existing test suite proves only
...
- **Whether `denied_by_exact`/`denied_by_shape` counts, or any other field of the
  ...
  counts (`packages/hooks/src/capability.ts:325-331`), so the invariant holds today, but there is no architecture-level
...
- **The production values `HookRunnerDeps` composes into a `HookRunner`'s `killGraceMs` /
  ...
  `logger`, `sessionId` in `packages/hooks/src/capability.ts:333-338`, leaving `killGraceMs`/`maxStdoutBytes`/
