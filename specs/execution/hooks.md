...
(`packages/hooks/src/index.ts`). The package owns four mechanical concerns end to end:
...
| Symbol | Kind | File |
| --- | --- | --- |
| `createHookRunner` | fn | `packages/hooks/src/runner.ts` |
| `HOOK_PROTOCOL_VERSION` (`= 1`) | const | `packages/hooks/src/runner.ts` |
| `MAX_STDIN_BYTES` (`= 256 * 1024`) | const | `packages/hooks/src/runner.ts` |
| `HookRunner` | interface | `packages/hooks/src/runner.ts` |
| `HookRunnerDeps` | interface | `packages/hooks/src/runner.ts` |
| `filterHookEnv` | fn | `packages/hooks/src/env.ts` |
| `interpolatedNames` | fn | `packages/hooks/src/env.ts` |
| `EnvFilterOptions`, `FilteredHookEnv` | interface | `packages/hooks/src/env.ts` |
| `argText`, `compileMatch`, `matchesCandidate` | fn | `packages/hooks/src/match.ts` |
| `globToRegExp` | fn (re-export) | `packages/capability/src/glob.ts`, via `packages/hooks/src/match.ts` |
| `ARG_MATCH_MAX_CHARS` (`= 8192`) | const | `packages/hooks/src/match.ts` |
| `CompiledMatch` | interface | `packages/hooks/src/match.ts` |
| `parseHookStdout` | fn | `packages/hooks/src/parse.ts` |
| `HOOK_MESSAGE_MAX_CHARS` (`= 4_000`) | const | `packages/hooks/src/parse.ts` |
| `ParsedHookOutput`, `ParseOptions` | type/interface | `packages/hooks/src/parse.ts` |
| `runHookCommand` | fn | `packages/hooks/src/subprocess.ts` |
| `DEFAULT_KILL_GRACE_MS` (`= 1_500`), `DEFAULT_MAX_STDOUT_BYTES` (`= 64*1024`), `DEFAULT_MAX_STDERR_BYTES` (`= 8*1024`) | const | `packages/hooks/src/subprocess.ts` |
| `HookChildProcess`, `HookReadable`, `HookWritable`, `HookSpawnOptions`, `SpawnFn`, `SubprocessDeps`, `SubprocessRequest`, `SubprocessResult`, `TimerDeps` | interface/type | `packages/hooks/src/subprocess.ts` |
| `NOOP_HOOK_LOGGER` | const | `packages/hooks/src/types.ts` |
| `HookFailure`, `HookFailureKind`, `HookInvocation`, `HookLogger`, `HookMatch`, `HookOutcome`, `HookResult`, `HookSpec`, `ToolCandidate` | interface/type | `packages/hooks/src/types.ts` |

Not re-exported from `.` though defined in `src/`: `HOOK_BLOCKING_EXIT_CODE` (`= 2`,
`packages/hooks/src/runner.ts`) and `ignoreRejection` (`packages/hooks/src/subprocess.ts`).
...
| Symbol | Kind | File |
| --- | --- | --- |
| `HOOKS_SEED_MARKER` (`= "<workspace-hooks>"`) | const | `packages/hooks/src/capability.ts` |
| `compileWorkspaceHooks` | fn | `packages/hooks/src/capability.ts` |
| `buildSeedBlock` | fn | `packages/hooks/src/capability.ts` |
| `runCredentialNames` | fn | `packages/hooks/src/capability.ts` |
| `WorkspaceHooksOptions` | interface | `packages/hooks/src/capability.ts` |
| `createWorkspaceHooksCapability` | fn | `packages/hooks/src/capability.ts` |
| `createHooksCapability` | fn | `packages/hooks/src/capability.ts` |
...
| `PROMPT_HOOK_EVENTS` | `["user_prompt_expansion"]` |
| `EXTERNAL_HOOK_EVENT_NAMES` | Clarvis→foreign event-name table (§3.4) |
| `EXTERNAL_TOOL_NAMES` | foreign-normalized-name→Clarvis-tool-name table (§3.4) |
| `EXTERNAL_HOOK_TOOL_NAMES` | Clarvis built-in→preferred foreign stdin name |
| `EXTERNAL_TOOLS_WITHOUT_COUNTERPART` | `Set` of 5 foreign names with no Clarvis tool |
| `normalizeToolName` | fn: strip non-alnum, lower-case |
| `HOOK_DEFAULT_TIMEOUT_MS` | per-class default timeout table |
| `MAX_HOOKS_PER_SOURCE` (`64`), `MAX_HOOKS_PER_RUN` (`128`), `MAX_HOOK_COMMAND_CHARS` (`8192`), `MAX_HOOK_MATCH_PATTERNS` (`64`), `MAX_HOOK_PATTERN_CHARS` (`2048`), `MAX_HOOK_TIMEOUT_MS` (`60000`) | consts |
| `hookSchema` | zod schema, one validated hook entry |
| `HookConfig` | `z.infer<typeof hookSchema>` |

### 2.4 Settings/plugin surface — `packages/loop/src/runtime/capabilities/hooks.ts`

| Symbol | Shape |
| --- | --- |
| `HOOKS_SETTINGS_FIELDS.hooks` | `z.array(hookSchema).max(MAX_HOOKS_PER_SOURCE)`, optional |
| `HOOKS_REQUEST_PARAMS.hook_user_prompt_expansion` | strict, optional host-derived `{ command_name }` context |
| `HOOKS_PLUGIN_FIELDS.hooks` | same schema, plugin-manifest field |
| `hooksSettingsSpec` | `CapabilitySettingsSpec` — `key: "hooks"`, merges scopes as **all operator hooks then all plugin hooks**, capped at `MAX_HOOKS_PER_RUN`, `pluginContributable: true` |
...
```
(`packages/hooks/src/runner.ts`, remark : "the runner is the whole public surface of
...
`packages/kernel/src/plugins/plugin-manifest.ts`, and
`packages/hooks/src/runner.ts`.

### 3.2 The stdin payload — flat, foreign-dialect identity fields

Built by `stdinPayload` (`packages/hooks/src/runner.ts`) from `identityFields`
:
...
```
Example drawn from `packages/hooks/tests/component/runner.test.ts` and the real-shell
integration test at `packages/hooks/tests/integration/real-subprocess.test.ts` — neither
...
(`packages/hooks/src/runner.ts`, proven by
`packages/hooks/tests/component/runner.test.ts`). `session_id` is omitted when the host
supplies none (`packages/hooks/src/runner.ts`, test at `packages/hooks/tests/component/runner.test.ts`).

The model-facing wire identity is retained in `CLARVIS_HOOK_TOOL`; matching also sees the optional
canonical dotted identity (`ToolCandidate.aliases`, `packages/hooks/src/types.ts`, consumed at
`packages/hooks/src/match.ts`). Stdin uses the external built-in spelling, or
...
(`packages/hooks/src/event-serialization.ts`). The separate
`CLARVIS_HOOK_TOOL_FULL_NAME` retains the canonical identity for Clarvis-native commands and
diagnostics (`packages/hooks/src/runner.ts`; pinned at
...
(`packages/hooks/src/runner.ts`; tests `packages/hooks/tests/component/runner.test.ts`).

### 3.3 Per-event serialization table (`event-serialization.ts`)

`SERIALIZE` (`packages/hooks/src/event-serialization.ts`) is a **total, statically checked**
record over `HookConfig["event"]` (`satisfies { [Event in HookEvent]:... }`) — a new
...
(`tool_name`, `tool_input`, `tool_response`) so a hook authored elsewhere reads them unmodified. Free-text fields are clamped to `TEXT_CLAMP = 8_000` chars
(`clampText`); structured values are clamped to `VALUE_CLAMP = 32_000` bytes of JSON,
replaced by `{ truncated: true, bytes }` when they exceed it, or `{ truncated: true }` when they
fail to serialize at all (`clampValue`). `post_tool_use`'s `tool_response` drops raw
image bytes and reports `image_count` instead (test at
...
| `subagent_start` | `subagent_instance_id`, `profile`, `model`, `task` (clamped text) | `packages/hooks/tests/component/capability.test.ts` |
| `subagent_complete` | `subagent_instance_id`, `status`, `result` (clamped text) | `packages/hooks/tests/component/capability.test.ts` |
| `pre_compact` | `agent`, `subagent_instance_id`, `estimated_tokens` | `packages/hooks/tests/component/capability.test.ts` |
| `model_call_error` | `agent`, `subagent_instance_id`, `iteration`, `model`, `message` (clamped) | `packages/hooks/tests/component/capability.test.ts` |
| `budget_exhausted` | `agent`, `reason`, `tokens_used`, `iterations_used` | `packages/hooks/tests/component/capability.test.ts` |
| `user_steer` | `agent`, `subagent_instance_id`, `iteration`, `message` (clamped), `id` | `packages/hooks/tests/component/capability.test.ts` |
...
An `EnvFilterOptions.add` record (`packages/hooks/src/env.ts`) supplies variables applied **last**, after
...
(`packages/hooks/tests/unit/env.test.ts`). The one production caller,
`createWorkspaceHooksCapability`, never supplies it (`packages/hooks/src/capability.ts`
...
1. The host builds a `HookInvocation` for the event (data payload, optional tool candidate,
   ...
   `compileWorkspaceHooks` path this is `hookInvocationFor` (`packages/hooks/src/event-serialization.ts`).
...
4. `runner.resolve(result, inv)` (`packages/hooks/src/runner.ts`) folds the result into the `HookOutcome` the
...
`argText(args, key)` (`packages/hooks/src/match.ts`) reads one argument field as the text a pattern is tested
against, with four rules each pinned by a dedicated test (`packages/hooks/tests/unit/match.test.ts`):
...
- **A non-object `arguments` value never matches** — there is no fallback to stringifying the whole
  ...
  direction ("every rule here is a narrowing", `packages/hooks/src/match.ts`).
...
`matchesCandidate(compiled, candidate)` (`packages/hooks/src/match.ts`) combines a compiled filter and a
...
combination rule is individually pinned in `packages/hooks/tests/unit/match.test.ts`.

`compileMatch` (`packages/hooks/src/match.ts`) compiles `tool` patterns via `globToRegExp` and `args` patterns as
...
non-matching, never ignored) and logs a warning naming the field and pattern (pinned by
`packages/hooks/tests/unit/match.test.ts`). The doc comment states this path is reachable only for an embedder that
skips validation: "the loop's schema already rejects an uncompilable `args` pattern at config read
time, so `broken` is reachable only for an embedder that skipped validation" (`packages/hooks/src/match.ts`) —
`hookSchema`'s own `superRefine` is what performs that rejection (`packages/capability/src/hooks-config.ts`).

### 4.3 `compileWorkspaceHooks` — folding a list of results into one `LifecycleHook` verdict

For a **gate** event (`packages/hooks/src/capability.ts`): iterates every selected spec in
order; a `deny` short-circuits the whole loop immediately (test:
`packages/hooks/tests/component/capability.test.ts`, "short-circuits on the first deny, so
...
later rewrite does not re-run selection" (`packages/hooks/src/capability.ts`) — but every *subsequent* hook in
the same pass now sees the invocation rebuilt from the replaced arguments (test:
`packages/hooks/tests/component/capability.test.ts`, "threads the replacement forward so the last writer wins"). At the
...
accumulated, else `{ kind: "pass" }`.

For an **observer** event : all selected hooks run concurrently via
...
For a **compaction** event (`pre_compact`): every selected hook runs in sequence; each
...
`EVENT_METHOD` is a **total** record over gate+observer+compaction events mapping each to
...
object, and the function returns `undefined` when nothing gate/observer/compaction-shaped
is configured (test: "returns undefined when nothing gate- or observer-shaped is
configured", `packages/hooks/tests/component/capability.test.ts`).
...
(`packages/hooks/src/subprocess.ts`; pinned by `describe("hooks.spawn")`,
`packages/hooks/tests/component/observability.test.ts`).
...
`finishAfterProcessEvents` settles immediately (via `setImmediate`) once `exit`,
`close` **and** `stdinDone` have all landed; if only one of `exit`/`close` has landed it waits up to
`EXIT_DRAIN_MS = 100` before forcing settlement and destroying the stdout/stderr streams
(tests: "exit without close settles after the drain window",
`packages/hooks/tests/unit/subprocess.test.ts`). A spawn that throws synchronously settles
immediately with `spawnError` set and never reaches the child machinery (tests
`packages/hooks/tests/unit/subprocess.test.ts`). An already-aborted signal short-circuits **before** any spawn at all
(test `packages/hooks/tests/unit/subprocess.test.ts`).

`killAll`, used by both `startKill`'s SIGTERM and its SIGKILL escalation, is a
...
(`packages/hooks/tests/integration/bun-spawn.test.ts`). Once the child has settled, if the
deadline (not abort) fired, `hooks.timeout_kill` is logged at warn with `hook_event`, `timeout_ms`,
`escalated_to_sigkill` and `tree_killed` (`packages/hooks/src/subprocess.ts`; pinned by `describe("hooks.timeout_kill")`,
`packages/hooks/tests/component/observability.test.ts`).

`stdin` is fed via `stdin.end(req.stdin, "utf8")` with an `error` listener registered
...
(test: "a hook that never reads stdin does not take the host down",
`packages/hooks/tests/unit/subprocess.test.ts`). **This is a seam guarding the injectable `SpawnFn`, not something
the production spawn can hit.** `stdinDone` is initialized `child.stdin === null`
(`packages/hooks/src/subprocess.ts`), and the real Bun spawn (`bunSpawn`) always returns `stdin: null`
 because the payload is supplied at spawn time as `stdin: new Blob([options.input])`
 — the doc comment on `HookSpawnOptions.input` states this plainly: "Payload supplied at
spawn time on Bun, whose child-process stdin drops writes". So in production the
`stdin.on("error")`/`.end()` branch never runs at all; it exists to protect a
test double (`FakeChild`) that models a live writable stream, which is what
`packages/hooks/tests/integration/real-subprocess.test.ts` exercises through `bunSpawn` — that test proves a large payload
...
1. `opts.truncated` ⇒ immediate failure, "stdout exceeded the capture limit" (`packages/hooks/src/parse.ts`).
2. Empty/whitespace-only stdout ⇒ `pass`.
3. Non-JSON or non-object JSON ⇒ failure.
4. `body.kind === undefined` ⇒ delegate to `translateForeignOutcome` (§4.6); its result (or `pass`
   as fallback) is returned.
5. `body.kind` present but not a string ⇒ failure.
6. Otherwise switch on `"pass"|"deny"|"advise"|"rewrite"|"context"`, each with its own field
   validation, defaulting/degradation rules, and `opts.allowContext`/`opts.allowRewrite` gating; unknown `kind` ⇒ failure naming the (clamped) value.

Every `kind`'s degradation rule, individually tested:

| `kind` | Input | Outcome | Test |
| --- | --- | --- | --- |
| _(none)_ | Bare `{}` or `{"other":1}` — no recognized field at all | `pass` | `packages/hooks/tests/unit/parse.test.ts` |
| `deny` | `message` absent, empty, or **not a string** (e.g. `7`) | `deny` with placeholder `"denied by hook (no reason given)"` | `packages/hooks/tests/unit/parse.test.ts` |
| `advise` | `message` absent | degrades to `pass` | `packages/hooks/tests/unit/parse.test.ts` |
| `context` | `text` absent or empty/whitespace, at a `CONTEXTUAL` fire point | degrades to `pass` | `packages/hooks/tests/unit/parse.test.ts` |
| `context` | any non-empty `text`, at a gate (`opts.allowContext === false`) | `bad_output`, `"a 'context' outcome is not valid at this event"` | `packages/hooks/tests/unit/parse.test.ts` |
| `rewrite` | `opts.allowRewrite === false`, or `arguments` not a plain object (string, array, absent) | `bad_output`, `"arguments cannot be replaced at this event"` / `"replacement arguments are not an object"` | `packages/hooks/tests/unit/parse.test.ts` |
...
Order of checks, and why each precedes the next (`packages/hooks/src/parse.ts`):
...
1. **Blocking check first.** `permissionDecision ∈ {deny, block}` (nested under
   ...
   regardless of anything else in the body. This precedes replacement-argument
   ...
   would turn a block into an allow".
...
3. `decision === "ask"` ⇒ `advise` if it has a reason, else `pass` — "ask" has no
   Clarvis gate counterpart.
4. `additionalContext` read **before** honoring a plain allow, because the dialect lets one body
   carry both an `allow` and context. The field is read under **three**
   spellings — `hookSpecificOutput.additionalContext`, top-level `additionalContext`, and a third,
   snake_case `additional_context` — none of which is documented anywhere else in this
   file; pinned by "every context spelling … becomes a context outcome"
   (`packages/hooks/tests/unit/parse.test.ts`).
5. `updatedInput` present ⇒ rewrite if `opts.allowRewrite`, else bad output; the replacement must be
   a plain object or it is bad output. This branch fires **even with no decision field
   at all** — "replacement arguments carry their own intent".
6. Non-empty offered context ⇒ `context` if `opts.allowContext`, else `advise` (never dropped).
7. `allow`/`approve` with nothing else offered ⇒ `pass`.
8. Otherwise `undefined`, which the caller treats as `pass` (`packages/hooks/src/parse.ts`).
...
logs a warning instead of propagating (`packages/hooks/src/capability.ts`; test:
`packages/hooks/tests/component/capability.test.ts`, "swallows a throwing context hook
...
1. `opts.resolveHooks(ctx) ?? []`; if empty, returns `null` — the capability does not activate
   (`packages/hooks/src/capability.ts`; test: "does not activate for a run with no hooks
   configured", `packages/hooks/tests/component/capability.test.ts`). Re-read on **every** call (`WorkspaceHooksOptions.resolveHooks`
   remark; test: "re-reads the configuration on every run", `packages/hooks/tests/component/capability.test.ts`).
...
4. Builds a `HookRunner` bound to `ctx.workspaceRoot`, the filtered env, the normalized logger
   (`ctx.logger ?? NOOP_LOGGER`), and `ctx.executionId` as `sessionId`.
5. `compileWorkspaceHooks` builds the `LifecycleHook` (or `undefined` if nothing gate/observer/
   ...
   when present, is read once into a prompt context (`packages/hooks/src/capability.ts`).
   The returned `RunCapability` carries `lifecycle: [hook]` only when defined, and `seedBlock`
   first runs the prompt observers and then `buildSeedBlock`, so a
   context-only config still contributes; a hook-only config contributes `undefined` from
   `seedBlock`, per test `packages/hooks/tests/component/capability.test.ts`). `forAgent` always returns `null` — no
   per-agent surface (test `packages/hooks/tests/component/capability.test.ts`).

`runUserPromptExpansionHooks` (`packages/hooks/src/capability.ts`) returns immediately when
...
Collects, per run (`packages/hooks/src/capability.ts`):
...
- every name `${VAR}`-interpolated into a provider's own `headers` and into each of its `models`
  ...
  drop every model-level header in silence"; pinned by the two "T9" tests, one per
  ...
  `packages/hooks/tests/unit/credential-names.test.ts`);
...
- plus `extra` (the caller-supplied `credentialNames` list) — because "the kernel narrows a run's
  ...
  inherited environment (test: "keeps host-managed names even when the run narrowed
  every server away", `packages/hooks/tests/unit/credential-names.test.ts`).
...
(`packages/hooks/src/capability.ts`). It takes `hooks: readonly LifecycleHook[]` that are
already built — not a `HookConfig[]` to resolve from settings — and an overridable `name` that
defaults to `HOOKS_CAPABILITY_NAME`. Its `forRun` returns `null` only when the supplied
list is empty; otherwise it returns a `RunCapability` whose `lifecycle` is the list **verbatim**
and whose `forAgent` always returns `null`. Three asymmetries from
`createWorkspaceHooksCapability`:
- **No `seedMarker` at all** — the returned `Capability` object carries only `name` and `forRun`, unlike `createWorkspaceHooksCapability`'s `Capability`, which always declares
  `HOOKS_SEED_MARKER`.
...
accepts an override" (`packages/hooks/tests/component/capability.test.ts`).
...
1. **A hook's runner surface never throws.** `runHookCommand` documents "**Never rejects**"
   (`packages/hooks/src/subprocess.ts`) and `HookRunner.run` documents "Never rejects"
   (`packages/hooks/src/runner.ts`). Pinned by the spawn-throw tests
   (`packages/hooks/tests/unit/subprocess.test.ts`) and by `compileWorkspaceHooks`'s "gate
...
2. **An observer event can never deny, however the hook resolves.** `runner.resolve`: a successful
   non-gate `deny` downgrades to `pass` (`packages/hooks/src/runner.ts`); a non-gate failure resolves to `pass`
   regardless of `on_failure`. Pinned by `packages/hooks/tests/component/runner.test.ts`.
3. **Cancellation never denies, even for a fail-closed hook.** `runner.resolve`: `aborted` resolves
   to `pass` before `on_failure` is even consulted (`packages/hooks/src/runner.ts`). Pinned by
   `packages/hooks/tests/component/runner.test.ts` and by `classify`'s precedence
   (`aborted` outranks `timeout`, `packages/hooks/src/runner.ts`).
4. **Exit code 2 at a gate is a verdict (`deny`), not an ordinary failure**, and is honoured even
   ...
   `packages/hooks/src/runner.ts`. Pinned by
   `packages/hooks/tests/component/runner.test.ts` and the real-shell integration path is
   exercised indirectly (the same classify function). At a non-gate (observer) event the same exit
   code 2 is an ordinary `exit_nonzero` failure (`packages/hooks/src/runner.ts`; test
   `packages/hooks/tests/component/runner.test.ts`).
5. **A command that does not exist is `exit_nonzero` (code 127), never `spawn_failed`** — "what
   gets spawned is the shell, and the shell exists" (`packages/hooks/src/types.ts`, `packages/hooks/src/runner.ts`).
   Pinned end-to-end by `packages/hooks/tests/integration/real-subprocess.test.ts`.
6. **A rewrite outcome is representable only where `HookInvocation.rewritable` is true**, and
   ...
   allowed. Production: `packages/hooks/src/types.ts`, enforced in `packages/hooks/src/parse.ts`. Pinned by
   `packages/hooks/tests/unit/parse.test.ts` (our own `rewrite` kind), by
   `packages/hooks/tests/unit/parse.test.ts` (the foreign dialect's `updatedInput`, including the `GATE`-scoped case
...
7. **`REWRITABLE_EVENTS` contains only `pre_tool_use`, not `post_tool_use`.** A rewrite fired after
   ...
   (`packages/hooks/src/event-serialization.ts`). Unpinned directly by a dedicated `post_tool_use`-rewrite test in
...
8. **A rewrite replaces arguments wholesale, never merges them.** Stated at the `HookOutcome` type
   (`packages/hooks/src/types.ts`) and at the loop's own `HookVerdict` (`packages/capability/src/api.ts`,
   "The replacement is total, not a merge"). Within `compileWorkspaceHooks`, later hooks see the
   replaced arguments and "the last writer wins" (`packages/hooks/src/capability.ts`); pinned by
   `packages/hooks/tests/component/capability.test.ts`.
9. **Selection is decided once per fire point, against the model's original arguments, and is not
   ...
   once, before the loop over selected specs (`packages/hooks/src/capability.ts`); a later replacement changes
   `current`/`inv` for subsequent hooks' *invocation* but does not re-select which specs fire. Unpinned by a test that would prove re-selection does *not* happen (the existing
...
10. **A `deny` short-circuits the gate fold; hooks after it never run.** `packages/hooks/src/capability.ts`
...
11. **Configuration order is load-bearing: operator hooks always precede plugin hooks**, and the
    ...
    (`packages/hooks/src/runner.ts`). The *production* of that order (operator-then-plugin) is
    `hooksSettingsSpec.merge` in `packages/loop/src/runtime/capabilities/hooks.ts`, outside
    `@clarvis/hooks` itself but consumed by it; the ordering guarantee "an operator always gets
    the first verdict" is stated at `packages/hooks/src/runner.ts` and at `packages/loop/src/runtime/capabilities/hooks.ts`, and demonstrated by
...
12. **The `session_start`/`pre_compact` seed/contribution channels never fail or block a run.**
    ...
    (`packages/capability/src/hooks-config.ts`); `buildSeedBlock` additionally swallows a throwing runner in
    production code, not merely by convention (`packages/hooks/src/capability.ts`; pinned by
    `packages/hooks/tests/component/capability.test.ts`).
13. **The credential-environment denylist is derived per run, never a static list, and is applied
    ...
    (`packages/hooks/src/env.ts`), and `opts.add` is applied last, exempt from every rule
    (`packages/hooks/src/env.ts`); pinned by "an exact denylist entry cannot strip a keep-listed variable"
    (`packages/hooks/tests/unit/env.test.ts`), "a keep-listed variable counts as neither"
    (`packages/hooks/tests/unit/env.test.ts`), and "added variables are applied last and are never filtered"
...
14. **Withheld environment variables are counted, never named**, anywhere in this package's own
    logging. `FilteredHookEnv.denied` carries counts only (`packages/hooks/src/env.ts`);
    `createWorkspaceHooksCapability` logs only `denied_count`/`denied_by_exact`/`denied_by_shape`
    (`packages/hooks/src/capability.ts`). Unpinned by a direct assertion that the log line's JSON never
    contains a credential's name (the `hooks.spawn` log test does assert the *command* is absent
    from its own JSON, `packages/hooks/tests/component/observability.test.ts`, but no test asserts the same of a denied
...
15. **A hook that never reads stdin cannot bring down the host process — by two distinct
    ...
    (`packages/hooks/src/subprocess.ts`); pinned by `packages/hooks/tests/unit/subprocess.test.ts`.
    Production's real spawn (`bunSpawn`) never reaches that branch at all: it always returns
    `stdin: null`, because the payload was already supplied at spawn time as a `Blob`, so `stdinDone` starts `true` and the guard code is structurally unreachable.
    `packages/hooks/tests/integration/real-subprocess.test.ts` ("a hook that never reads a large payload neither crashes nor
...
16. **A hook's whole process tree dies with it on timeout or abort**, via `killTree`, not merely the
    immediate child. Production: `runner.ts` (via `packages/hooks/src/subprocess.ts`, `killAll`/`startKill`).
    Pinned end-to-end by `packages/hooks/tests/integration/real-subprocess.test.ts`, "a
...
17. **Truncation sticks for the remainder of a stream once the cap is hit**, rather than marking
    ...
    (`packages/hooks/src/subprocess.ts`). Pinned by
    `packages/hooks/tests/unit/subprocess.test.ts`.
18. **Truncated stdout is never parsed, even if the truncated prefix happens to look like valid
    ...
    (`packages/hooks/src/parse.ts`). Pinned by `packages/hooks/tests/unit/parse.test.ts`.
19. **Argument-match patterns are tested only against a clamped prefix** (`ARG_MATCH_MAX_CHARS =
    ...
    (`packages/hooks/src/match.ts`). Pinned by
    `packages/hooks/tests/unit/match.test.ts`.
20. **A broken `match.args` regex makes the whole filter never fire, rather than being ignored.**
    ...
    filter as never matching (`packages/hooks/src/match.ts`). Pinned by
    `packages/hooks/tests/unit/match.test.ts` and
    `packages/hooks/tests/component/observability.test.ts` (the `broken_patterns` count in
    `hooks.selected`).
21. **A hook's compiled `match` is cached by object identity (`WeakMap`) across repeated fire
    ...
    `packages/hooks/src/runner.ts`. Pinned by
    `packages/hooks/tests/component/runner.test.ts`, "filters are compiled once per spec".
22. **`hooks[].event` is a closed, exhaustive enum of exactly the five vocabulary groups
    ...
    `HOOK_EVENTS` composition and `z.enum(HOOK_EVENTS, …)` (`packages/capability/src/hooks-config.ts`).
    ...
    string per `packages/hooks/src/types.ts`, so it has no independent test of the enum's closure).
...
24. **Tool hook matching keeps both wire and canonical identities, while compatible stdin uses the
    ...
    Pinned: `packages/hooks/tests/component/capability.test.ts`,
    `packages/hooks/tests/unit/match.test.ts`, and
...
| --- | --- | --- | --- |
| Signal aborted before spawn | `aborted: true`, no spawn attempted | `packages/hooks/src/subprocess.ts` | `classify` → `aborted` failure → always resolves `pass` |
| `spawn()` throws | `spawnError` set | `packages/hooks/src/subprocess.ts` | `classify` → `spawn_failed` failure |
| Child emits `error` | `spawnError` set from the error message | `packages/hooks/src/subprocess.ts` | same as above |
| Deadline elapses | `timedOut = true`; SIGTERM then SIGKILL after `killGraceMs` | `packages/hooks/src/subprocess.ts` | `classify` → `timeout` failure (unless `aborted` also true, which outranks it) |
| Abort mid-flight | `aborted = true`; SIGTERM then SIGKILL | `packages/hooks/src/subprocess.ts` | `classify` → `aborted` failure, outranks `timedOut` |
...
| stdout is not JSON at all (bare text, JSON with trailing garbage) | — | `packages/hooks/src/parse.ts` | `bad_output`, `"stdout is not JSON"` |
...
(`packages/hooks/src/subprocess.ts`; `packages/hooks/src/types.ts`).
...
(`cleanHookMessage`, `packages/hooks/src/parse.ts`), including in the *failure reason string* itself, which is
also length-guarded to under 200 chars for an oversized `kind` value (`packages/hooks/src/parse.ts`; pinned by
`packages/hooks/tests/unit/parse.test.ts`).

## 7. Coupling

**Depends on** (production, `packages/hooks/package.json` — exactly two entries):
...
`packages/hooks/src/index.ts` and at `packages/hooks/src/types.ts`: "the loop depends on this package, so this
...
(`packages/hooks/src/types.ts`). Nothing under `packages/hooks/src` imports `@clarvis/loop`
statically or dynamically — the name occurs there only in TSDoc prose
(`packages/hooks/src/index.ts`, `packages/hooks/src/types.ts`).
...
(`packages/loop/src/runtime/build-run-deps.ts`). The bare `.` entrypoint has **no
...
(`packages/capability/src/hooks-config.ts`). Moving the vocabulary into the optional package would make
...
(`packages/hooks/src/capability.ts`; `packages/loop/src/runtime/build-run-deps.ts`).
...
(`packages/loop/src/runtime/loop/lifecycle-hooks.ts`, `!hooks.some(h => h.preFinalize)`) and an
...
keys would silently disable both fast paths for the whole run (`packages/hooks/src/capability.ts`). This is
...
- **Invariant #9** (selection is not re-run after a rewrite) is stated as a documented design
  decision in a `remarks` comment (`packages/hooks/src/capability.ts`) but the existing test suite proves only
