...
(`packages/tools/src/tools/shell.ts`). `monitor_start`/`monitor_poll`/`monitor_stop`/`monitor_list`
...
(`packages/tools/src/tools/registry.ts`), including `monitor_poll` and `monitor_list`, which only
read state — the registry does not distinguish "reads state" from "read-only surface eligible". A
separate, cross-package test in `@clarvis/loop` (`packages/loop/tests/architecture/agent-tool-wire-names.test.ts`)
...
| --- | --- | --- |
| `resolveShell(deps?)` | `packages/tools/src/shell.ts` | resolves this host's `ShellSpec` (memoized when called with no args) |
| `shellArgs(shell, command)` | `packages/tools/src/shell.ts` | builds argv to run `command` under `shell` |
| `encodePowerShellCommand(command)` | `packages/tools/src/shell.ts` | base64/UTF-16LE `-EncodedCommand` payload |
| `exitCaptureWrapper(command, flavor)` | `packages/tools/src/shell.ts` | wraps `command` so its exit status lands in `$MON_EXIT` |
| `currentShellFlavor(platform?)` | `packages/tools/src/lib/platform.ts` | `"posix"` or `"powershell"` for a platform |
| `ShellSpec` | `packages/tools/src/shell.ts` | `{ flavor, file }` |
| `ShellDeps` | `packages/tools/src/shell.ts` | injectable seams for `resolveShell` |
| `killTree(pid, signal, deps?)` | `packages/tools/src/lib/process.ts` | kills a whole process tree |
| `ownProcessGroup(platform?)` | `packages/tools/src/lib/process.ts` | whether `spawn` should set `detached` |
| `KillDeps`, `TaskkillRunner` | `packages/tools/src/lib/process.ts` | injectable seams for `killTree` |
| `sweepMonitors(workspaceRoot, options?)` | `packages/tools/src/lib/monitor.ts` | GC's finished monitors' sidecar/log/exit files |
| `warn(message, warning?)` | `packages/tools/src/lib/log.ts` | emits through the process-wide `WarnSink` |
| `setWarnSink(fn \| null)` | `packages/tools/src/lib/log.ts` | installs/clears that sink |
| `NOOP_TOOLS_LOGGER` | `packages/tools/src/lib/log.ts` | a `ToolsLogger` that discards every call |
| `ToolsLogger`, `WarnSink`, `ToolsWarning` | `packages/tools/src/lib/log.ts` | the two diagnostics seams |
| `bound(text, maxBytes)` | `packages/tools/src/lib/output.ts` | head-truncate with a marker |
| `boundOrSpill(text, maxBytes, spill, logger?, stream?)` | `packages/tools/src/lib/output.ts` | tail-truncate, spilling the full text to a file first |
| `createCaptureSink(opts)` | `packages/tools/src/lib/output.ts` | bounded-resident, spill-on-overflow stream accumulator |
| `createOutputCoalescer(emit, intervalMs?)` | `packages/tools/src/lib/output.ts` | batches a live stream to at most one emit per interval |
| `allocateBudget(aBytes, bBytes, total)` | `packages/tools/src/lib/output.ts` | splits a shared byte budget between two streams |
| `CAPTURE_INLINE_FLOOR` | `packages/tools/src/lib/output.ts` | `64 * 1024` — smallest resident tail once a sink spills |
| `readLogSlice(logPath, offset, maxBytes)` | `packages/tools/src/lib/logslice.ts` | paginated, UTF-8-safe read of a log file |
| `uniqueToken()` | `packages/tools/src/lib/token.ts` | pid + epoch-ms + counter + 6 random bytes |
| `bestEffort(operation, run)` | `packages/tools/src/lib/tasks.ts` | swallow-and-warn wrapper for cleanup work |
...
(`packages/tools/tests/integration/monitor-lib.test.ts` imports the lib functions directly;
`packages/tools/tests/component/monitor-spawn.test.ts` and
`packages/tools/tests/component/monitor-stop-kill.test.ts` import the `create*` factories directly).

### Relevant `RuntimeConfig` fields (`packages/tools/src/config.ts`)

| Field | Default constant | Default value | File |
| --- | --- | --- | --- |
| `maxOutputBytes` | `DEFAULT_MAX_OUTPUT_BYTES` | `131072` | `packages/tools/src/config.ts` |
| `maxShellOutputBytes` | `DEFAULT_MAX_SHELL_OUTPUT_BYTES` | `16384` | `packages/tools/src/config.ts` |
| `shellTimeoutMs` | `DEFAULT_SHELL_TIMEOUT_MS` | `120000` | `packages/tools/src/config.ts` |
| `shellTimeoutMaxMs` | `DEFAULT_SHELL_TIMEOUT_MAX_MS` | `600000` | `packages/tools/src/config.ts` |
| `monitorReadyTimeoutMs` | `DEFAULT_MONITOR_READY_TIMEOUT_MS` | `30000` | `packages/tools/src/config.ts` |
| `maxMonitors` | `DEFAULT_MAX_MONITORS` | `32` | `packages/tools/src/config.ts` |
| `stateRoot` | — | (derived) | resolved |
...
| `gitMetadataPaths` | — | discovered once from a validated linked worktree | resolved |
| `registerTemporaryRoot` | — | closure over the live `temporaryRoots` list | built |
| `logger` | — | `NOOP_TOOLS_LOGGER` when unset | resolved |
| `sandbox` | — | `undefined` | `packages/tools/src/config.ts` |
| `secretEnvNames` | — | `undefined` | `packages/tools/src/config.ts` |

`ServerConfig` (`packages/tools/src/config.ts`) is a type alias of `RuntimeConfig` and is
**not** re-exported from `index.ts`, whose type re-export at `packages/tools/src/index.ts` names
only `RuntimeConfig` and `AgentToolsOptions` — tests import it directly from `../../src/config.ts`.

## 3. Data and formats

### Monitor sidecar (`MonitorMeta`, `packages/tools/src/lib/monitor.ts`)
...
`packages/tools/src/lib/monitor.ts`, using `writeAtomic` from `lib/atomic.ts`). `id` is minted
by `mintId()` as `mon_` + 8 lowercase hex characters (`packages/tools/src/lib/monitor.ts`;
pinned by `packages/tools/tests/integration/monitor-lib.test.ts`, which also asserts two calls
never collide).

### On-disk layout (all under the workspace's machine-local state dir, never the working tree)

| Path builder | Produces | File |
| --- | --- | --- |
| `monitorSidecar(id)` | `<state>/local/monitor-<id>.json` | `packages/paths/src/workspace-state.ts` |
| `monitorLog(id)` | `<state>/local/monitor-<id>.log` | `packages/paths/src/workspace-state.ts` |
| `monitorExit(id)` | `<state>/local/monitor-<id>.exit` | `packages/paths/src/workspace-state.ts` |
| `spillFile(token, stream)` | `<state>/local/shell-<token>.<stream>.log` | `packages/paths/src/workspace-state.ts` |
| `toolOutputSpill(token)` | `<state>/local/toolout-<token>.txt` | `packages/paths/src/workspace-state.ts` |
...
(`packages/tools/src/lib/monitor.ts`); `sidecarPath`/`logPath`/`exitPath` are thin wrappers over
the same builders. `isMonitorSidecar` recognizes exactly the `monitor-*.json` shape
(`packages/paths/src/workspace-state.ts`) and is what `listSidecars` filters directory entries
by (`packages/tools/src/lib/monitor.ts`). None of the three monitor files, nor a shell spill,
ever lands inside the workspace root — pinned directly:
`packages/tools/tests/integration/monitor-lib.test.ts` asserts none of the three built paths
`startsWith(workspacePaths(root).root)`, and
`packages/tools/tests/integration/shell.test.ts` asserts a spill on a virgin workspace creates
...
`MAX_MONITOR_EXIT_BYTES = 64` bytes (`packages/tools/src/lib/monitor.ts`); the sidecar is capped at
`MAX_MONITOR_SIDECAR_BYTES = 256 * 1024` bytes.

### Tool result shapes (all JSON-stringified text)

| Tool | Shape |
| --- | --- |
| `shell` (success) | `{ exit_code, stdout, stderr, signal, timed_out: false }` (`packages/tools/src/tools/shell.ts`) |
| `shell` (error) | `ToolError` codes `timeout`/`aborted`/`output_limit`/`io_error`, each carrying `stdout`/`stderr` |
| `monitor_start` | `{ id, running, ready, output, next_offset }` — `ready: null` when no `ready_when` given (`packages/tools/src/tools/monitor.ts`) |
| `monitor_poll` | `{ running, output, next_offset, exit_code }` |
| `monitor_stop` | `{ stopped: true, id }` |
| `monitor_list` | `{ monitors: [{ id, command, running, started_at, cwd }...] }`, newest-first |

### Truncation markers (exact strings the model sees)

- Head-truncated (`bound`): `\n[... output truncated: {end} of {total} bytes shown ...]`
  (`packages/tools/src/lib/output.ts`).
...
- `monitor_poll`'s buffered-continuation marker: `\n[... more output buffered; continue with
  offset={nextOffset} ...]` (`packages/tools/src/tools/monitor.ts`).
...
### `uniqueToken()` format (`packages/tools/src/lib/token.ts`)

`${pid}-${epochMs}-${counter++}-${6 random bytes as hex}` — used both for `shell`'s spill filenames
(`spillTarget`, `packages/tools/src/tools/shell.ts`) and anywhere else a collision-proof suffix
is needed.
...
1. Resolve `cwd` (confined via `resolvePath`) and clamp `timeout_ms` to
   `min(requested, config.shellTimeoutMaxMs, MAX_TIMER_DELAY_MS)`, where
   `MAX_TIMER_DELAY_MS = 2_147_483_647` is `setTimeout`'s int32 ceiling.
2. `statDirectory` confirms `cwd` exists and is a directory before ever spawning.
3. `runCommand` builds the sandbox-resolved spec (`sandboxCommand`, delegated — see §7), spawns with
   ...
   spec. Production: `packages/tools/src/config.ts`,
...
4. Two `CaptureSink`s are created, one per stream, sharing `captureCap = max(config.maxOutputBytes,
   MAX_CAPTURE_FLOOR)` (`MAX_CAPTURE_FLOOR = 8 * 1024 * 1024`) and `inlineLimit =
   max(config.maxShellOutputBytes, CAPTURE_INLINE_FLOOR)`. A sink accumulates in an in-memory
   ...
   file (a sequential drain queue, `packages/tools/src/lib/output.ts`) and appends it to a
   bounded `tail` capped at `inlineLimit` bytes (the mechanism behind invariant 17); the
   write queue itself is bounded only by `captureCap` under a synchronous flood
   (`packages/tools/tests/integration/capture-sink.test.ts`, "queues rather than drops").
5. Each `data` event first returns early if the sink is already `capped`; otherwise it pushes into the
   ...
   that same push crossed `captureCap` (`packages/tools/src/tools/shell.ts`) — so the chunk
...
6. A `setTimeout(timeoutMs)` kills the tree and sets `timedOut` on expiry; an
   `AbortSignal` (if given) does the same on abort.
7. On the child's `exit` event, settlement is deferred `STDIO_DRAIN_MS = 100`ms to catch trailing
   output before `close`/`finish` runs; `close` finishes immediately if it fires
...
8. `finalize` (the injectable `finalizeOutput`) splits the shared `maxShellOutputBytes` budget between
   ...
   each. `allocateBudget(aBytes, bBytes, total)` (`packages/tools/src/lib/output.ts`)
   ...
   `packages/tools/tests/integration/output.test.ts` ("keeps both when they fit within total",
   ...
   `boundOrSpill(inline, budgetBytes, opts.spill()...)` (`packages/tools/src/lib/output.ts`),
...
9. In priority order, a settled call rejects `aborted`, then `timeout`, then `output_limit`, and only
   otherwise resolves with the JSON success envelope — so an abort that also happened to
   overflow the cap is reported as `aborted`, not `output_limit`. `exit_code` in the success envelope
   comes from `computeExit(code, signal)` (`packages/tools/src/tools/shell.ts`): a normal exit
   ...
   always delivers an exit code.

`createOutputCoalescer(emit, intervalMs?)` (`packages/tools/src/lib/output.ts`) is the
...
pinned by name in `packages/tools/tests/integration/output.test.ts` ("batches pushes within one
...
### `monitor_start` — background, non-blocking (`createMonitorStart`, `packages/tools/src/tools/monitor.ts`)

1. Resolve/confine `cwd`; compile `ready_when` into a `RegExp` if given, converting a parse failure
   into `invalid_input` (`compileRegex`; handler).
2. Count currently-live monitors (`listSidecars` + `monitorRunning` per entry); refuse with
   `too_many_monitors` at or above `config.maxMonitors`.
3. Mint an id, resolve the shell once (`shell()`, threaded into both the wrapper and
   `sandboxCommand` so they can never disagree —), wrap the command with
   `exitCaptureWrapper(command, host.flavor)`.
...
4. Open the log file for append (`openSync(lp, "a")`), spawn with
   ...
   ownProcessGroup()`. The fd is closed in the parent immediately after spawn;
   the child inherits its own copy.
5. A spawn error or a child with no `pid` removes the just-created log file and throws `io_error`; `child.unref()` lets the parent process exit without waiting on it.
6. The sidecar is written. If `ready_when` was given, `waitForReady` polls
   `readLogSlice(logPath, 0, config.maxOutputBytes)` every `READY_POLL_MS = 75`ms
    until the regex matches, the process dies, or `ready_timeout_ms`/abort fires;
   ...
   window is never observed" (`packages/tools/src/tools/monitor.ts`).

`isAlive(pid)` (`packages/tools/src/lib/monitor.ts`) is the liveness probe every check in this
...
(the function's own remarks). Pinned by
`packages/tools/tests/integration/monitor-lib.test.ts` ("current process alive, bogus pid
dead") ("isAlive treats EPERM as alive"), ("isAlive treats ESRCH as dead").

### `monitor_poll` (`packages/tools/src/tools/monitor.ts`)

1. Read the sidecar (throws `monitor_not_found` if missing/corrupt) and the exit state; `running =
   !exited && isAlive(pid)`.
2. Read a `readLogSlice` window from `offset` up to `config.maxOutputBytes`.
...
4. An optional `match` regex (compiled the same way as `ready_when`) filters to matching lines only,
   applied to the (already hold-back-adjusted) text.
5. If the slice reported `more`, the buffered-continuation marker is appended — **after**
...
### `monitor_stop` (`createMonitorStop`, `packages/tools/src/tools/monitor.ts`)
...
| alive | `monitor_stop` called | `SIGTERM` to the tree, wait `STOP_GRACE_MS = 400`ms, re-check |
| alive after grace | (escalation) | `SIGKILL` to the tree |
| any | after signalling (or not) | `removeMonitorFiles` unconditionally |
...
`packages/tools/tests/integration/monitor.test.ts`). On Windows, `killTree`'s first call
already force-kills (`taskkill /T /F`), so the grace period becomes latency rather than a second real
signal (`packages/tools/src/tools/monitor.ts`).
...
cached flag), and sorts `started_at` descending.
...
| exited (natural) | child process exits, `exitCaptureWrapper`'s trap/`finally` writes `$MON_EXIT` | `monitor_stop` (files removed) *or* `sweepMonitors` after `MONITOR_MAX_AGE_MS = 24h` past the exit file's mtime (`packages/tools/src/lib/monitor.ts`) |
| killed (no exit file) | `monitor_stop`'s `SIGKILL`, or an external kill | `sweepMonitors` reaps immediately — no completion timestamp to preserve |
| gone (files removed) | `monitor_stop` or `sweepMonitors` | terminal |

`sweepMonitors(workspaceRoot, options?)` (`packages/tools/src/lib/monitor.ts`) lists sidecars
...
`currentShellFlavor(platform)` (`packages/tools/src/lib/platform.ts`) is the single source both
...
(`packages/tools/src/lib/platform.ts`). `currentShellFlavor`'s own remarks give the reason it is
...
than merely discouraged" (`packages/tools/src/lib/platform.ts`).
`computeShell` (`packages/tools/src/shell.ts`): non-`win32` always yields `{flavor:"posix",
...
it is recomputed and never cached, so an injected call can never poison the shared memo
(pinned by `packages/tools/tests/unit/shell.test.ts`).

`shellArgs` : POSIX gets `["-c", command]` verbatim; PowerShell gets `["-NoProfile",
"-NonInteractive", "-EncodedCommand", encodePowerShellCommand(command)]` — the whole command is
base64, so it never meets the Windows command-line tokenizer at all.
`encodePowerShellCommand` prepends a fixed UTF-8-forcing `POWERSHELL_PREAMBLE`
 ahead of the raw command, then encodes `preamble\ncommand` as UTF-16LE→base64. Three design
...
- The preamble's encoder is built as `New-Object System.Text.UTF8Encoding $false` rather than taken
  ...
  byte-order mark to stdout, breaking every assertion on exact output. Pinned by
  `packages/tools/tests/unit/shell.test.ts`, "forces BOM-less UTF-8 on both stream directions
...
- `cmd.exe` is unsupported on the same reasoning that rules out a configurable analyzer flavor: it has
  ...
  and it offers no dependable exit status across a chained command.
- Neither `-ExecutionPolicy Bypass` nor `$ErrorActionPreference = 'Stop'` is set on the PowerShell
  ...
  `sh -c` where only the last command's status matters.

`exitCaptureWrapper` : POSIX installs an `EXIT` trap that writes `$?` to a `.tmp` sibling
...
file, per the function's own remarks). PowerShell wraps the command in `try {…} finally
{…}`, testing `$?` *before* `$LASTEXITCODE` because `$LASTEXITCODE` is sticky across the whole
payload once any native command has run — reversing the order would make an unrelated earlier
command's status leak into the reported exit code (pinned by
`packages/tools/tests/unit/shell.test.ts`, "tests $? before $LASTEXITCODE").
...
| Event | File | Remark |
| --- | --- | --- |
| `tools.shell_spawn` | `packages/tools/src/tools/shell.ts` | "a shell command is being spawned; everything it does from here is attributed to this process group" |
| `tools.shell_exit` | `packages/tools/src/tools/shell.ts` | "a shell command settled; the trace keeps its output as opaque text and indexes none of these" |
| `tools.monitor_spawn` | `packages/tools/src/tools/monitor.ts` | "a background monitor is being spawned with both streams redirected into one log; this is the write side of the pair that decides whether output can be captured at all" |
| `tools.monitor_poll` | `packages/tools/src/tools/monitor.ts` | "a monitor was polled; this is the read side of the pair — zero bytes against a running monitor means the capture never happened, not that the command is quiet" |
...
1. **The `shell`/`monitor_*` tools are never advertised or dispatchable on the read-only surface.**
   Production: `packages/tools/src/tools/registry.ts` (`readOnly: false` for all five). Test:
   `packages/loop/tests/architecture/agent-tool-wire-names.test.ts` ("excludes shell and the
...
2. **No production module in this package's `src/` writes to `process.stderr`, `process.stdout`, or
   ...
   Production: `packages/tools/src/lib/log.ts`.
   Test: `packages/tools/tests/architecture/logging-channel.test.ts` (forbidden-pattern sweep) (exact-count check on the sanctioned module).
...
3. **The default `WarnSink` writes to `process.stderr`; installing a custom sink via `setWarnSink`
   ...
   Production: `packages/tools/src/lib/log.ts`.
   Test: `packages/tools/tests/unit/log.test.ts`.
...
4. **`ownProcessGroup` returns `true` on every non-`win32` platform and `false` on `win32`, and
   ...
   Production: `packages/tools/src/lib/process.ts`; call sites
   `packages/tools/src/tools/shell.ts`, `packages/tools/src/tools/monitor.ts`.
   Test: `packages/tools/tests/unit/process.test.ts`.
...
5. **`killTree` on POSIX addresses the process group as `-pid`, falling back to the bare `pid` only
   ...
   Production: `packages/tools/src/lib/process.ts`.
   Test: `packages/tools/tests/unit/process.test.ts` (POSIX group-first-then-fallback; Windows always-force; both-fail → `false`).
...
6. **`resolveShell()` called with no arguments memoizes its answer process-wide; called with any
   ...
   Production: `packages/tools/src/shell.ts`.
...
7. **`exitCaptureWrapper`'s PowerShell branch tests `$?` strictly before `$LASTEXITCODE` in the
   ...
   Production: `packages/tools/src/shell.ts` (the ordering in the emitted lines).
   Test: `packages/tools/tests/unit/shell.test.ts`.
...
8. **`shellArgs`'s PowerShell branch never lets any character of the raw command reach the Windows
   ...
   Production: `packages/tools/src/shell.ts`.
   Test: `packages/tools/tests/unit/shell.test.ts` (`payload` matches only base64 alphabet even
...
9. **A monitor's exit file, sidecar, and log never resolve inside the workspace root.**
   Production: `packages/paths/src/workspace-state.ts` (`localDir` under the global state
   root, not the workspace).
   Test: `packages/tools/tests/integration/monitor-lib.test.ts`; also
   `packages/tools/tests/integration/shell.test.ts` for a shell spill.
...
10. **`monitorRunning` trusts the exit sentinel over a live pid check** — once the exit file exists,
    ...
    Production: `packages/tools/src/lib/monitor.ts` (exit-file check first, `isAlive` only as
    the else branch — see §4's `isAlive` paragraph for what that probe itself checks).
    Test: `packages/tools/tests/integration/monitor-lib.test.ts` ("trusts the exit sentinel
...
11. **`sweepMonitors` reaps a monitor with no exit file immediately (no grace period), and reaps a
    ...
    Production: `packages/tools/src/lib/monitor.ts`.
    Test: `packages/tools/tests/integration/monitor-lib.test.ts` (reaps a stale exit past 24h, preserves a fresh exit, removes a dead/no-exit monitor while keeping
    a live one).
...
12. **`readLogSlice` never returns a slice with a trailing incomplete UTF-8 sequence, except when the
    ...
    Production: `packages/tools/src/lib/logslice.ts`.
    Test: `packages/tools/tests/integration/logslice.test.ts` (multi-byte cut-boundary cases;
    "still makes progress when a single char is larger than the budget").
...
13. **`readLogSlice` reads size and bytes from the same open file descriptor**, so a rename/replace
    ...
    Production: `packages/tools/src/lib/logslice.ts` (one `handle`, `stat()` then
    `handle.read()` on the same handle).
    Test: `packages/tools/tests/integration/logslice.test.ts` ("takes size and bytes from the
...
14. **`monitor_poll` holds back a trailing partial line (no final newline) while the monitored process
    ...
    Production: `packages/tools/src/tools/monitor.ts`.
    Test: `packages/tools/tests/integration/monitor.test.ts` (`.skipIf(!monitorCapturesOutput)`)
    and the complementary "emits a final partial line once the process is dead".
...
15. **`monitor_poll`'s `match` filter is applied after the partial-line hold-back and before the
    ...
    Production: `packages/tools/src/tools/monitor.ts` (order of operations).
    Test: `packages/tools/tests/integration/monitor.test.ts` ("keeps matching lines and pages
...
16. **`shell`'s settlement priority is `aborted` > `timeout` > `output_limit` > success** — an abort
    ...
    Production: `packages/tools/src/tools/shell.ts` (the `if` chain's order).
    Test: unpinned directly for the triple-overlap case; `packages/tools/tests/integration/shell.test.ts`
    exercises `aborted`, `timeout` and `output_limit`
...
17. **A `CaptureSink` never holds more than `inlineLimit` bytes resident once it has started spilling**
    ...
    synchronous flood.
    Production: `packages/tools/src/lib/output.ts` (the function's own remarks state the queue
    caveat explicitly) (`appendTail` truncates `tail` to `inlineLimit`).
    Test: `packages/tools/tests/integration/capture-sink.test.ts` ("writes every observed byte
    to the spill file while holding only a bounded tail" — tracks peak `residentBytes` across 40
    pushes and asserts `peak <= INLINE * 2` and `peak < 40_000`) pins the bound directly
    ...
    above. `CaptureSink.residentBytes` is exposed "for tests" (`packages/tools/src/lib/output.ts`),
...
18. **A failed spill degrades to a bare tail with no file reference, never an exception** — both
    ...
    Production: `packages/tools/src/lib/output.ts` (`boundOrSpill`'s catch branch) + (`createCaptureSink`'s `spillFailed`/`drain` catch).
    Test: `packages/tools/tests/integration/output.test.ts` ("falls back to the plain
...
19. **`monitor_start` refuses to start once live monitors reach `config.maxMonitors`, and a slot frees
    ...
    Production: `packages/tools/src/tools/monitor.ts` (liveness computed fresh per call via
    `monitorRunning`, not from any cached count).
    Test: `packages/tools/tests/integration/monitor.test.ts` (refusal at the limit) ("frees a monitor slot once a monitor exits").
...
20. **No model-facing refusal message in this package's source tells the model how to lift the
    ...
    Production: `packages/tools/src/tools/monitor.ts` (the actual message: "stop some with
    monitor_stop first" — an instruction to use another tool, not to lift a restriction).
    Test: `packages/tools/tests/architecture/no-bypass-hints.test.ts` (package-wide sweep; not
...
21. **The dispatcher never re-clamps a `bounded: true` tool's text output to `maxOutputBytes`** — all
    ...
    Production: `packages/tools/src/core.ts` (`boundParts` returns `parts` unchanged when
    `bounded` is truthy); tool declarations at `packages/tools/src/tools/shell.ts`,
    `packages/tools/src/tools/monitor.ts`.
    Test: unpinned by a dedicated bounded-vs-unbounded comparison test in this document's scope; the truncation
    behavior itself is exercised in `packages/tools/tests/integration/output.test.ts`, but
...
| --- | --- | --- |
| Spawn throws synchronously (`shell`) | `packages/tools/src/tools/shell.ts` | `ToolError("io_error", "Failed to spawn command: …")` |
| Spawn throws synchronously (`monitor_start`) | `packages/tools/src/tools/monitor.ts` | log fd closed, log file removed, `ToolError("io_error", "Failed to spawn monitor: …")` |
| Spawned child never gets a pid (`monitor_start`) | `packages/tools/src/tools/monitor.ts` | log file removed, `ToolError("io_error", "… process has no pid")` |
| A monitor child emits `error` after a successful spawn (`monitor_start`) | `packages/tools/src/tools/monitor.ts` | `child.on("error", () => {})` — a completely silent no-op; no log call, no sidecar update, nothing surfaced to the caller. Not exercised by any test in this document's scope |
| Child `error` event (`shell`) | `packages/tools/src/tools/shell.ts` | both sinks disposed (best-effort, errors swallowed), `ToolError("io_error", "Failed to run command: …")` |
| `finalizeOutput` rejects (`shell`) | `packages/tools/src/tools/shell.ts` | `ToolError("io_error", "Failed to finalize output: …")` |
| Wall-clock timeout | `packages/tools/src/tools/shell.ts` | tree `SIGKILL`ed, `ToolError("timeout", …)` carrying partial `stdout`/`stderr` |
| `AbortSignal` fires | `packages/tools/src/tools/shell.ts` | tree `SIGKILL`ed, `ToolError("aborted", …)` carrying partial `stdout`/`stderr` |
| One stream exceeds `captureCap` | `packages/tools/src/tools/shell.ts` | tree `SIGKILL`ed, `ToolError("output_limit", …)` naming `max_capture_bytes` |
| Spill file write fails (either `boundOrSpill` or `CaptureSink`) | `packages/tools/src/lib/output.ts` | degrades to a bare tail marker with no file reference; logged via `logger.warn` at event `tools.spill_failed` |
| Monitor sidecar missing/corrupt/wrong-shape/oversized | `packages/tools/src/lib/monitor.ts` | `ToolError("monitor_not_found", …)` — indistinguishable from a monitor that never existed |
| Monitor exit sentinel unreadable/non-numeric/out-of-range | `packages/tools/src/lib/monitor.ts` | `{exited: true, code: null}` — logged at `tools.monitor_exit_unreadable`; "indistinguishable from a killed process" per the function's own remark |
| Monitor exit sentinel too-large or not-a-file | `packages/tools/src/lib/monitor.ts` | same as above (treated as present-but-unreadable, not absent) |
| Invalid `ready_when`/`match` regex | `packages/tools/src/tools/monitor.ts` | `ToolError("invalid_input", …)` naming the offending field |
| `maxMonitors` reached | `packages/tools/src/tools/monitor.ts` | `ToolError("too_many_monitors", …)` |
| `killTree` cannot signal anything | `packages/tools/src/lib/process.ts` | returns `false`; logged at `tools.kill_tree_failed`; every caller (`shell`'s `killAll`, `monitor_stop`) falls back to a direct `child.kill`/no further action rather than throwing |
| `taskkill` binary absent/fails on a non-Windows test host | `packages/tools/src/lib/process.ts` | falls back to `process.kill(pid, "SIGKILL")`; if that also throws, `false` |
| Monitor process ignores `SIGTERM` | `packages/tools/src/tools/monitor.ts` | after `STOP_GRACE_MS`, escalates to `SIGKILL` |
| **Windows: `monitor_start`'s redirected stdio never captures output at all** | not handled — an open defect | `packages/tools/tests/helpers/fixtures.ts` (`monitorCapturesOutput`) documents this as the write side failing silently: "the log is empty, not merely differently encoded"; `shell` is unaffected because it captures over pipes, not an inherited descriptor. The tool's own doc comment (`packages/tools/src/tools/monitor.ts`) independently records that establishing which side (write vs. read) fails took "an abandoned two-handle experiment" and that a `running` monitor whose poll reports zero bytes is this defect surfacing, not a quiet command |
| Non-`ToolError` throw anywhere in a handler | `packages/tools/src/errors.ts` | collapsed to a generic `{error:"internal", message:"internal error"}`; the real detail (stack or message) goes only to the warn sink at `tools.internal_error` |
| A descendant is re-parented before `killTree` walks the tree | `packages/tools/src/lib/process.ts` | escapes the kill on both platforms, from opposite directions — acknowledged, unmitigated, by the function's own remark: "Both have the same hole from opposite directions: a descendant whose intermediate parent has already exited is re-parented and escapes the walk, exactly as `setsid` detaches a child from its group on POSIX. Closing it on Windows would take a Job Object, which is deliberately out of scope." Not handled by any timeout/abort/output_limit/`monitor_stop` path, all of which rely on `killTree` |
...
- `@clarvis/paths` — `ensureWorkspaceLocalDir`, `workspaceStatePaths`, `isMonitorSidecar` from
  `packages/tools/src/lib/monitor.ts`; `executableOnPath` from `packages/tools/src/shell.ts`.
  Forced by the paths package being this package's *only* internal `dependencies` entry
  (`packages/tools/package.json`); nothing in `lib/monitor.ts` or `shell.ts`
...
- `../sandbox.ts` (`sandboxCommand`) — both `packages/tools/src/tools/shell.ts` and
  `packages/tools/src/tools/monitor.ts` import it to build the actual spawn spec (file/args/env,
  ...
  disagree on shell flavor (`packages/tools/src/tools/monitor.ts`).
- `../guard/context.ts` (`buildGuardContext`) is used by `core.ts`'s `applyGuard`
  (`packages/tools/src/core.ts`), not by `shell.ts`/`monitor.ts` directly — whether a `shell`
  ...
  (`packages/tools/src/config.ts`) are the seam; this document's handlers never reference them.
- `../core.ts`'s `dispatch`/`boundParts` — the dispatcher (not the handler) is what makes `bounded:
  true` mean "do not re-clamp" (`packages/tools/src/core.ts`); this document's tools only declare
  the flag.

**What depends on this subsystem:**

- `@clarvis/loop` (`packages/loop/src/runtime/build-run-deps.ts`) dynamically imports
  ...
  (`packages/tools/src/lib/log.ts`). This is a **separate seam** from `spillFailed`
...
- `@clarvis/kernel` (`packages/kernel/src/file-kernel.ts`, `createFileKernel`'s
  ...
  `sweepSpillDir`) inside a `WorkspaceHousekeeping` instance — a static, direct call, not
...
- `packages/tools/src/index.ts` re-exports nearly everything in the "Surface" tables above; any host
  ...
  without pulling in the whole tool registry: `packages/hooks/src/subprocess.ts` imports
  `killTree`, `ownProcessGroup`, `resolveShell` and `shellArgs` from `@clarvis/tools/shell` and
  nothing else from this package, and `packages/tools/src/shell-entry.ts` names that consumer
...
- **Whether invariant 16's priority order (`aborted` > `timeout` > `output_limit`) is actually
  ...
  `if` chain's order (`packages/tools/src/tools/shell.ts`) is the only evidence for the
...
- **A discrepancy between a comment and the code it sits beside**: `lib/tasks.ts`'s `bestEffort` doc
  ...
  (`packages/tools/src/lib/tasks.ts`), but `lib/log.ts`'s own default `WarnSink` writes to
  `process.stderr` (`packages/tools/src/lib/log.ts`), and
  `packages/tools/tests/unit/log.test.ts` explicitly asserts that restoring the default sink
...
- **Why `STOP_GRACE_MS` is exactly `400`, `READY_POLL_MS` is exactly `75`, `MONITOR_MAX_AGE_MS` is
  ...
  (`packages/tools/src/tools/monitor.ts`, `packages/tools/src/lib/monitor.ts`,
  `packages/tools/src/tools/shell.ts`) are used consistently and some have comments explaining
