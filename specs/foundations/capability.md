...
> is anchored to a file and a named symbol or test. Open questions are collected in the final section.
...
`packages/capability/src/contract.ts`) plus the machinery that folds a list of such
contributions into the single tool/handler/gate bundle a loop consumes
(`packages/capability/src/compose.ts`). Alongside it sit the registries that make a capability
declarable from outside the engine: a settings-block registry
(`packages/capability/src/registry.ts`), a per-run typed port registry
(`packages/capability/src/services.ts`), and the `PortKey` mechanism that lets one capability
reach another without importing it (`packages/capability/src/services.ts`).
...
| Field | Value | Source |
| --- | --- | --- |
| `name` | `@clarvis/capability` | `packages/capability/package.json` |
...
| `dependencies` | `zod: ^4.4.3` only | `packages/capability/package.json` |
| `engines.bun` | `>=1.4.0` | `packages/capability/package.json` |
| Test layout | `test` = unit, then integration, then architecture | `packages/capability/package.json` |
...
`packages/capability/package.json`. `./trace` exists so a capability that only records events
does not pull the whole contract in, and so the open/closed split is visible at the import site
(`packages/capability/src/trace.ts`). `./ports` is the narrow set of interfaces a capability is
written against, deliberately the measured minimum
(`packages/capability/src/ports.ts`). Trace kinds, events and the `Logger` port are delegated
...
| --- | --- | --- | --- |
| Registration | `Capability` (`packages/capability/src/contract.ts`) | the host | process-lifetime, or per-run for a session-bound host (`packages/capability/src/contract.ts`) |
| Run | `RunCapability \| null` (`packages/capability/src/contract.ts`) | `Capability.forRun(ctx)` (`packages/capability/src/contract.ts`) | one per run, `null` = not used this run |
| Agent | `AgentCapability \| null` (`packages/capability/src/contract.ts`) | `RunCapability.forAgent(scope)` (`packages/capability/src/contract.ts`) | one per agent, `null` = this agent does not get it |
| Loop | `AgentLoopContribution` (`packages/capability/src/contract.ts`) | `AgentCapability.attach(bc)` (`packages/capability/src/contract.ts`) | one per agent loop |
...
| `reservedWireNames?` | `readonly string[]` | same registered-not-active rule (`packages/capability/src/contract.ts`) |
| `toolEffects?` | `Readonly<Record<string, ToolEffect>>` | keyed by wire name; deliberately separate from reservation (`packages/capability/src/contract.ts`) |
| `requiresUserInput?(view)` | `boolean` | consulted *before* any `forRun` (`packages/capability/src/contract.ts`) |
| `forRun(ctx)` | `Promise<RunCapability \| null> \| RunCapability \| null` | `packages/capability/src/contract.ts` |

A `Capability` declares itself with a `CapabilityGrantDeclaration` (`packages/capability/src/contract.ts`): `name` (the
...
`Capability.forRun(ctx)` receives a `RunCapabilityContext` (`packages/capability/src/contract.ts`), which extends
`CapabilityRequestView` (`request`, `requestParam(key)`, `packages/capability/src/contract.ts`) with:

| Field | Type | Source / note |
| --- | --- | --- |
| `owner` | `string` | `packages/capability/src/contract.ts` |
| `entryGrants` | `readonly string[]` | `packages/capability/src/contract.ts` |
...
| `signal?` | `AbortSignal` | long work in `forRun`/`seedBlock` should observe it |
| `priorState?` | `Record<string, unknown>` | the prior run's `capability_state`, keyed by capability name; absent for a fresh run or a capability that wrote nothing |
...
| --- | --- | --- |
| `projected` | `(event: CapabilityEvent) => CapabilityEvent` | `packages/capability/src/contract.ts` |
| `capabilitiesForScope` | `(caps \| undefined, scope) => AgentCapability[]` | `packages/capability/src/compose.ts` |
| `systemSectionsFor` | `(caps \| undefined, id) => string[]` | `packages/capability/src/compose.ts` |
| `activationForScope` | `(caps \| undefined, scope) => AgentActivation` | `packages/capability/src/compose.ts` |
| `foldContributions` | `(readonly AgentLoopContribution[]) => FoldedContributions` | `packages/capability/src/compose.ts` |
| `createCapabilityRegistry` | `(seed?: CapabilityRegistrySeed) => CapabilityRegistry` | `packages/capability/src/registry.ts` |
| `composeCapabilityRegistry` | `(base \| undefined, declarations) => CapabilityRegistry` | `packages/capability/src/registry.ts` |
| `createCapabilityRequestView` | `(request: RunRequest) => CapabilityRequestView` | `packages/capability/src/services.ts` |
| `portKey` | `<T>(id: string) => PortKey<T>` | `packages/capability/src/services.ts` |
| `createCapabilityServices` | `() => CapabilityServices` | `packages/capability/src/services.ts` |
| `requestParamKeys` | `(specs) => string[]` | `packages/capability/src/settings-spec.ts` |
| `handlerBaseOf` | `(bc: AgentBuildContext) => HandlerBase` | `packages/capability/src/handler-base.ts` |
| `openCallEnvelope` | `(a: CallEnvelopeArgs) => CallEnvelope` | `packages/capability/src/call-envelope.ts` |
...
`CapabilitySettingsSpec` (`packages/capability/src/settings-spec.ts`) is what a capability living in its own package hands
a host:

| Field | Type | Meaning |
| --- | --- | --- |
| `key` | `string` | the block key in `settings.json` and, when allowed, in a plugin manifest |
| `schema` | `z.ZodType` | the unwrapped block schema; composition applies optional/describe |
...
| Port | Members | File |
| --- | --- | --- |
| `ContextPort` | `appendNote`, `setStableBlock(kind, content)`, `setCanonicalState(content)` | `packages/capability/src/ports.ts` |
| `TracePort` | `record<K>(kind, detail)`, `signal<K>(kind, detail)`, `now(): number` | `packages/capability/src/ports.ts` |
| `Logger`, `LogFn` | four levels + optional `child`/`level` | `packages/capability/src/ports.ts` (delegated) |

`./ports` additionally re-exports `Elicit` and friends, `AgentRegistryPort`, `LLMProvider` and
`ToolChoice` (`packages/capability/src/ports.ts`). The stated rule is that the loop's `LiveContext` (~50 members) and
`TraceHandle` satisfy `ContextPort`/`TracePort` **structurally**, with no adapter and no cast
(`packages/capability/src/ports.ts`).
...
**Messages and content.** `MessageRole` (`packages/capability/src/api.ts`) is `system | user | assistant`; a `TextPart`
 or `ImagePart` (`image` + optional `mediaType`) makes up a `ContentPart`, and
`MessageContent` is a bare string or an array of them. `Message` pairs a role with
content. `AssistantMessagePhase` is `commentary | final_answer`, and `AssistantTextPart`
 retains one assistant text block, its optional public phase, and opaque provider replay
...
compaction prompt") is drained from a `CompactionSource` the same way. `ToolCallRef`
 ...
 carries opaque `providerOptions` a provider adapter alone interprets, so replay stays
provider-neutral at the loop.

**Providers and models.** `ToolTransport` is `stdio | http | sse`; `AgentRole` is
...
`ProviderKind`
 is `openai-compatible | openai | anthropic |
...
over the native OpenAI Responses adapter. `PromptCacheMode`
 is `explicit | implicit | off`; its doc-comment states absence is deliberately not `off` — "it
means nobody has decided," and a host with a pricing catalog is expected to resolve it.
`ModelConfig` (`packages/capability/src/api.ts`) carries `context_window_tokens`,
`max_output_tokens`, `capabilities`, provider-published `reasoning_efforts`, `prompt_cache`, and
per-model `headers`/`body` overrides; `ProviderConfig` carries the
...
contradicts the resolved call (`model`, `stream`, `tool_choice`, `stream_options`). The
...
**Budget and grants.** `BudgetMode` is `stop | escalate`; `BudgetConfig` is
`on_exceed` plus optional `total_token_limit`/`timeout_ms`/`max_escalations`. `BuiltinGrant`
 is the engine's own four grants (`ask_user`, `read_workspace`, `edit_workspace`,
`run_commands`); `Grant` opens it the same way `ErrorCode` opens, because its doc-comment
states "external capability grant names are registered before request validation rather than added to
this union". `ReasoningSummary`
...
**`RunRequest`.** The complete input to one loop run:
...
at 2x, a read at 0.1x either way. A capability shipped in its own package takes its
...
| `SubagentCompleteContext` | `subagentInstanceId`, `status`, `result` | `packages/capability/src/api.ts` |
| `PreCompactContext` | `agent`, `subagentInstanceId?`, `estimatedTokens` | `packages/capability/src/api.ts` |
| `PostCompactContext` | `agent`, `subagentInstanceId?`, `operation`, `freedChars?`, `keptChars?` | `packages/capability/src/api.ts` |
| `ModelCallErrorContext` | `agent`, `subagentInstanceId?`, `iteration`, `model`, `message` | `packages/capability/src/api.ts` |
| `BudgetExhaustedContext` | `agent`, `reason: "exhausted" \| "declined"`, `tokensUsed`, `iterationsUsed` | `packages/capability/src/api.ts` |
| `UserSteerContext` | `agent`, `subagentInstanceId?`, `iteration`, `message`, `id?` | `packages/capability/src/api.ts` |

`onPreCompact` may return `CompactionContribution[]` (method): a `source` (attribution
...
field through which "instead of" could be expressed. A throw from `onPreCompact` is
swallowed by the loop's collector, since compaction fires because the context is already over budget
(`packages/loop/src/runtime/loop/lifecycle-hooks.ts`; API rationale at
...
`ToolArgValidate` is a function type — `(schema, args) => string | null` — rather than a shared
helper, because the implementation is an ajv instance and this package deliberately carries no JSON
Schema dependency. `AgentRunState` is the mutable per-agent state threaded through
...
the next iteration top — the probe buffers instead of consuming. The doc-comment on the
...
via a never-read `_t` field (`packages/capability/src/services.ts`). Ids are namespaced by the publishing capability
because two capabilities minting the same id would silently overwrite each other
(`packages/capability/src/services.ts`). The two ids this package owns are pinned by test:
...
`tool_call_started` (`packages/capability/src/call-envelope.ts`):

```
{ agent, [subagent_instance_id], iteration_ref, call_id, started_at, name, arguments }
```

`tool_call` (`packages/capability/src/call-envelope.ts`), emitted by both `ok` and `fail`:
...
safe projection instead of the raw model payload (`packages/capability/src/call-envelope.ts`).
`started_at` is stamped at envelope construction and `ended_at` at record time, both from
`trace.now()` (`packages/capability/src/call-envelope.ts`). On `fail`, the same string is written to `result` and
to `error` unless a redacted `traceMessage` is supplied (`packages/capability/src/call-envelope.ts`), which
the test pins at `packages/capability/tests/unit/call-envelope.test.ts`.

The model-facing return strings are fixed:

| Method | Returned text |
| --- | --- |
| `ok(result)` | `` Tool '<name>' result: <result> `` (`packages/capability/src/call-envelope.ts`) |
| `fail(message)` | `` Tool '<name>' result (error): <message> `` (`packages/capability/src/call-envelope.ts`) |
...
values are opaque to the engine (`packages/capability/src/run.ts`). It is read back into
`RunCapabilityContext.priorState`, where a capability reads only its own slot and the value is
exactly what its own `finalizeRun` returned (`packages/capability/src/contract.ts`). `finalizeRun` returns the value
to file under the capability's name in `ExecutionRecord.capability_state`, or `undefined` to write
nothing (`packages/capability/src/contract.ts`).
...
by `@clarvis/trace`'s `TraceStore.recoverOrphans` (`packages/capability/src/execution-status.ts`), a claim echoed on
`RunResponse` at `packages/capability/src/run.ts`.

### 3.5 `ToolEffect`

`type ToolEffect = "read" | "mutate" | "control" | "spawn_run" | "unknown"`
(`packages/capability/src/tool-effect.ts`). `unknown` is described as the load-bearing member: a gate that must refuse
...
`LLMToolCall` (`packages/capability/src/llm-port.ts`) is `id`, `name`, `arguments: unknown`, plus two signal fields:
...
context keeps the model's own arguments). `LLMUsage` is `input_tokens`/`output_tokens`/
`cached_tokens`/`cache_write_tokens`.

`LLMCallResult` normalizes one model call: `text?`, `toolCalls?`, `usage` (always present),
...
`ResolvedProviderConfig` is `kind`, `baseUrl?`, `apiKeyEnv?`, `headers?`, `body?`,
...
(`packages/capability/src/llm-port.ts`). `headers` values are raw `${VAR}` templates, never resolved — resolved at
client construction, not per message, which is why `ResolvedProviderConfig` rides beside
`LLMCallParams` rather than inside it.

`LLMCallParams` is the full call input: `model`, `messages`, `tools`, `provider`, optional
...
the two newest usable ones and use provider-specific fallback rules), `maxRetries?`/
...
marks the first slice of a retried call), and `onToolInputDelta?` (a separate, `call_id`-keyed
...
`LLMProvider` is the single-method port — `call(params) => Promise<LLMCallResult>` — every
backend implements and every decorator (`withTransportRetry`, `withCallLogging`,
`withPromptCacheDefaults`) wraps.

`ProviderErrorInit` is the optional constructor input for `ProviderError` (§2.4): `kind?`
...
counts input tokens written into it, the two disjoint from each other and from plain `input`.
`TokenAccumulator` is a type alias of `TokenCounts` used as a mutable running total.
`SubagentAggregate` extends `TokenCounts` with `iterations` (summed across every instance of
...
never an error" — plus an optional `undrained?()` reporting steers queued and never drained.
`AgentRegistration` is what a producer declares at registration: `kind`, `nativeId`, `title`,
optional `profile`, and `control`. `AgentSettlement` is a child's terminal outcome:
`status`, optional `result`/`iterations`/`tokens`.

`AgentHandle` is a producer's handle on one registered child: `ingest(event)` (feeds one
...
`AgentRegistryPort` is the producer-facing slice, deliberately write-only ("reading the tree
is the supervision tools' job, not a producer's,"): `register(registration)` returns an
`AgentHandle` or `null` when the registry is sealed or at its live-children ceiling — a producer must
treat `null` as "do not spawn" and answer with a plain refusal; `adopt(id, task)` hands the
...
the spawn stops being background"; `liveCount()` reports children neither settled nor
evicted.
...
only ever holds the combined handle (`packages/capability/src/convergence-guards.ts`). `GuardTrip` is `code:
"tool_failure_loop" | "stagnation_detected"` plus a `message`. `GuardWarning` is the
soft-tier counterpart, same two codes. `ConvergenceGuards` is four methods: `record(signature,
...
single failure re-trips and the escalation was theatre". This is the concrete type behind
`AgentBuildContext.guards` (§2.8).

### 3.11 Compaction anchor (`compaction-anchor.ts`)

`CompactionAnchor` (`packages/capability/src/compaction-anchor.ts`) is a `label` and a `body` — a stable reference block
...
`inputSchema`/`kind`. `NamespacedRegistry` is `tools` + `resolve(name)` (nullable) +
`allUnavailable()`.

`FailureKind` is `transient | context_overflow | client | auth | quota | content_policy`;
...
looked like a malformed payload. `ProviderErrorDetails` is `kind` + optional
`status`/`retry_after_ms`. `ErrorBody` is `code: ErrorCode`, `message`, optional `details`.
`StructuredResult`/`ResultValue` are both `unknown`, the second an alias of the first.

`RunResponse` is discriminated by `status`: five variants (`completed`, `budget_exhausted`,
...
failure. `MCPConnection` is one live server connection: `name`, `transport`, `status`,
bounded initialize `instructions?`, `callTool`/`listResources?`/`readResource?`/`close`.
`NamespacedTool` is one MCP tool under
...
`AgentResult` (`packages/capability/src/agent-result.ts`) is the terminal outcome of one agent's loop, and the concrete
...
(`packages/capability/src/compose.ts`):

| Step | Code | Rule |
| --- | --- | --- |
| 1 | `packages/capability/src/compose.ts` | for each tool, reject if `wireName` already seen — throws ``foldContributions: duplicate tool wire name '<n>' across contributions`` |
...
dispatching handler disagree (`packages/capability/src/compose.ts`). The same docstring records that the core appends
the submit handler and the MCP catch-all **after** the fold (`packages/capability/src/compose.ts`) — the engine side of
that is delegated to [loop-capability-composition](../engine/capability-composition.md).

`foldHooks` (`packages/capability/src/compose.ts`) builds each field only when at least one contribution set it, so a
caller's `folded.hooks.beforeIteration ? ...` presence check keeps working (`packages/capability/src/compose.ts`);
the empty case is pinned at `packages/capability/tests/unit/compose.test.ts`. Fan-out semantics:
...
(`packages/capability/src/compose.ts`); order and dropping pinned at `packages/capability/tests/unit/compose-activation.test.ts`.
`activationForScope` bundles both into `{ capabilities, systemSections }` (`packages/capability/src/compose.ts`).
...
(`packages/capability/src/contract.ts`). `AgentScope` is `AgentIdentity` plus `clock`, `signal` and `elicit`
(`packages/capability/src/contract.ts`).

### 4.3 Settings registry

`createCapabilityRegistry(seed)` builds two maps and applies the seed through the same public methods
(`packages/capability/src/registry.ts`):

| (state, event) | -> (state, effect) |
| --- | --- |
| key absent, `register(spec)` | key stored; `specs()` order = insertion order |
| key present, `register(spec)` | **throws** ``capability settings key '<k>' is already registered`` |
| `specs()` | returns `[...byKey.values()]` — a fresh array each call |
| blank name, `registerGrant` | **throws** `capability grant name must be a non-empty string` |
| name present, same `entryCanSpawn` | silent no-op, returns |
| name present, different `entryCanSpawn` | **throws** ``capability grant '<n>' is already registered differently`` |
| name absent | stored |
...
(`packages/capability/src/registry.ts`); pinned at `packages/capability/tests/unit/registry.test.ts`.

`requestParamKeys` flat-maps `Object.keys(spec.requestParams ?? {})` across specs in spec order
(`packages/capability/src/settings-spec.ts`), pinned at `packages/capability/tests/unit/registry.test.ts`.
...
are parsed or the block reads as an unrecognized key (`packages/capability/src/registry.ts`).

### 4.4 Inter-capability ports

`createCapabilityServices()` returns a `Map`-backed registry (`packages/capability/src/services.ts`):

| (state, event) | -> (state, effect) |
| --- | --- |
| key free, `provide(key, value)` | stored |
| key taken, `provide(key, value)` | **throws** ``capability port '<id>' is already provided``; the original value stays (pinned `packages/capability/tests/unit/capability-ports.test.ts`) |
| any, `get(key)` | the stored value cast to `T`, or `undefined` |
...
would be a missing tool rather than an error (`packages/capability/src/services.ts`). The engine's own sequencing is
consistent with that: `AGENT_REGISTRY_PORT` is provided before any `forRun`
(`packages/loop/src/runtime/orchestrator.ts`) while `TOOL_EFFECT_PORT` is provided *after*
every `forRun` has resolved (`packages/loop/src/runtime/orchestrator.ts`), so a capability that
...
is an untyped index into the same request object (`packages/capability/src/services.ts`); pinned against an
open, capability-declared key at `packages/capability/tests/unit/capability-ports.test.ts`.

### 4.5 Tool call envelope

`openCallEnvelope(a)` runs in this order (`packages/capability/src/call-envelope.ts`):

1. If `schema` is present and `validate` is absent, **throw** immediately. The docstring gives the reason: this package carries no JSON Schema implementation, so
   the alternative to throwing is reporting every malformed call as valid — a validation boundary
   failing open in silence.
2. Stamp `startedAt = trace.now()`.
3. Resolve `callId = call.id` when non-empty, else `randomUUID()`.
4. Compute `invalid = schema === undefined ? null : (validate?.(schema, call.arguments) ?? null)`
    — a validator returning `undefined` therefore reads as valid, pinned at
   `packages/capability/tests/unit/call-envelope.test.ts`.
...
`start`; the test asserts that constructing an envelope with a failing validator records
nothing at all (`packages/capability/tests/unit/call-envelope.test.ts`).
...
`undefined` (`packages/capability/src/handler-base.ts`); pinned at `packages/capability/tests/unit/call-envelope.test.ts`.

### 4.6 Tool-argument normalization

`normalizeToolArguments(raw)` (`packages/capability/src/tool-arguments.ts`):

| Input | Outcome | File |
| --- | --- | --- |
| `undefined` / `null` | `{ ok: true, args: {} }` | — |
| plain non-array object | `{ ok: true, args: raw }` — the same object by identity | (identity pinned `packages/capability/tests/unit/tool-arguments.test.ts`) |
| `""` or whitespace-only string | `{ ok: true, args: {} }` | `packages/capability/tests/unit/tool-arguments.test.ts` |
| string parsing to an object | `{ ok: true, args: parsed }` | `packages/capability/tests/unit/tool-arguments.test.ts` |
| string failing `JSON.parse` | `{ ok: false, reason: "unparsable", preview }` | `packages/capability/tests/unit/tool-arguments.test.ts` |
| string parsing to a non-object | `{ ok: false, reason: "not_an_object", preview }` | `packages/capability/tests/unit/tool-arguments.test.ts` |
| any other type (number, boolean, array) | `{ ok: false, reason: "not_an_object", preview }` | `packages/capability/tests/unit/tool-arguments.test.ts` |
...
(`packages/capability/src/tool-arguments.ts`). Two properties are pinned by test: the message contains the preview and
the word "truncated" (`packages/capability/tests/unit/tool-arguments.test.ts`), and it never contains
"required property" while it does contain "not run" (`packages/capability/tests/unit/tool-arguments.test.ts`).

### 4.7 Title and brief validation

`parseTaskTitle(value)` (`packages/capability/src/task-title.ts`), in order: non-string -> reject; a line break
matching the `LINE_BREAK` pattern at `packages/capability/src/task-title.ts` (CR, LF and the two Unicode line/paragraph
separators, written there as escapes) -> reject; trim and collapse `[\t ]+` runs to one space; empty after normalization -> reject; more than `TASK_TITLE_MAX = 60` **Unicode**
...
`splitFrontmatterFence(raw)` (`packages/capability/src/frontmatter-fence.ts`):

1. Strip a leading BOM and then `trimStart()`.
2. Match `/^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/` (executed).
3. No match and the normalized text starts with `---` -> `{ kind: "unterminated", body: raw }`.
4. No match otherwise -> `{ kind: "absent", body: raw }`.
5. Match -> `{ kind: "fenced", frontmatter: match[1] ?? "", body: match[2] ?? "" }`.

`body` on the two non-fenced outcomes is `raw` exactly as given, with the BOM and leading whitespace
still on it (pinned `packages/capability/tests/unit/frontmatter-fence.test.ts`). Two
adjacent `---` lines report `unterminated`, not an empty block, because the closing fence must be
preceded by a newline the opening fence did not already consume (pinned
`packages/capability/tests/unit/frontmatter-fence.test.ts`). The frontmatter group is lazy, so the first `---` line
closes the fence and a later one is body text (pinned
`packages/capability/tests/unit/frontmatter-fence.test.ts`). The function never throws — every input maps to one
...
joins with `.*` inside `^...$` (`packages/capability/src/glob.ts`). The module docstring says both callers are
security-relevant and that an unanchored match would let `github.*` also select
`mygithub.internal` (`packages/capability/src/glob.ts`); the anchoring is pinned at `packages/capability/tests/unit/glob.test.ts`.
...
would not observe (`packages/capability/src/per-owner.ts`, implementation). `sharedFallback(build)` builds at
most once total and ignores its owner argument (`packages/capability/src/per-owner.ts`); it is meant to be composed as
`memoizeByOwner`'s fallback build function, never as a replacement, because building directly inside
`memoizeByOwner` would build one instance *per* owner (`packages/capability/src/per-owner.ts`). All three
compositions are pinned at `packages/capability/tests/unit/per-owner.test.ts`.
...
7. Call `observer` inside a bare `try/catch`, then `logger.warn(failure,
   "best_effort_failed")` inside another.

`detachObserved` is `void bestEffort(...)` (`packages/capability/src/tasks.ts`).
`suppressSecondaryRejection(promise, observedBy)` **throws** when `observedBy` is blank and otherwise
attaches an empty catch (`packages/capability/src/tasks.ts`). `observedBy` is validated and otherwise never read: its
only function is to force a caller to name the primary observation channel in the source, at the call
site, rather than to drive any behavior (`packages/capability/src/tasks.ts`).

### 4.12 Event projection

`projected(event)` returns the same event with `wire = { type: event.kind }`, adding `detail` only
when `event.detail !== undefined` (`packages/capability/src/contract.ts`). The stated rule is that a host projects an
...
to know about (`packages/capability/src/contract.ts`).
...
specifiers. Production: every module under `packages/capability/src/` (e.g. `packages/capability/src/contract.ts`, all
relative). Test: `packages/capability/tests/architecture/self-import.test.ts`, which scans
...
(`packages/capability/tests/integration/sanitize-runtime.test.ts`) and is outside the scan root.
...
`advertisedTools`; the default is advertised. Production: `packages/capability/src/compose.ts`. Test:
`packages/capability/tests/unit/compose.test.ts`.

**INV-C5.** A folded hook field exists only when some contribution supplied it, so presence checks
downstream stay meaningful. Production: `packages/capability/src/compose.ts`. Test:
`packages/capability/tests/unit/compose.test.ts` (asserts `folded.hooks` equals `{}`).

**INV-C6.** Folded hooks fire in contribution order; `contributesProgress` is a logical OR; and
`onTeardown` is awaited sequentially. Production: `packages/capability/src/compose.ts`. Test:
`packages/capability/tests/unit/compose.test.ts`.
...
`packages/capability/src/compose.ts`. Test: `packages/capability/tests/unit/compose.test.ts`,
`packages/capability/tests/unit/compose-activation.test.ts`.

**INV-C8.** Registering a second settings spec under an existing key throws rather than silently
dropping the registration. Production: `packages/capability/src/registry.ts`. Test:
`packages/capability/tests/unit/registry.test.ts`.

**INV-C9.** `specs()` and `grants()` return fresh arrays, so a caller cannot mutate the registry
through a returned list. Production: `packages/capability/src/registry.ts`. Test:
`packages/capability/tests/unit/registry.test.ts` (specs only; the same property for `grants()` is unpinned).

**INV-C10.** A grant name must be non-empty; an identical re-declaration is a no-op; a re-declaration
that changes `entryCanSpawn` throws. Production: `packages/capability/src/registry.ts`. Test:
`packages/capability/tests/unit/registry.test.ts`.

**INV-C11.** `composeCapabilityRegistry` produces an isolated registry and never mutates the base.
Production: `packages/capability/src/registry.ts`. Test: `packages/capability/tests/unit/registry.test.ts`.

**INV-C12.** `requestParamKeys` collects every declared param key across specs, in spec order.
Production: `packages/capability/src/settings-spec.ts`. Test: `packages/capability/tests/unit/registry.test.ts`.

**INV-C13.** Publishing a port under an already-taken key throws, and the rejected write does not
take effect. Production: `packages/capability/src/services.ts`. Test:
...
Production: `packages/capability/src/call-envelope.ts`. Test: `packages/capability/tests/unit/call-envelope.test.ts`.

**INV-C17.** A failed argument validation produces a non-null `invalid` and records **no** trace
entry. Production: `packages/capability/src/call-envelope.ts` (validation happens before any `record`). Test:
`packages/capability/tests/unit/call-envelope.test.ts`.

**INV-C18.** A call whose provider supplied no id gets a freshly minted UUID, and absent arguments are
recorded as `{}`. Production: `packages/capability/src/call-envelope.ts`. Test:
`packages/capability/tests/unit/call-envelope.test.ts`.

**INV-C19.** `handlerBaseOf` omits `subagentInstanceId`, `signal` and `validateArgs` rather than
setting them to `undefined`. Production: `packages/capability/src/handler-base.ts`. Test:
`packages/capability/tests/unit/call-envelope.test.ts`.

**INV-C20.** `projected` sets `wire.type` from `kind` and omits the `detail` **key** entirely when the
event has no detail. Production: `packages/capability/src/contract.ts`. Test:
`packages/capability/tests/unit/capability-ports.test.ts`.

**INV-C21.** `memoizeByOwner` builds each distinct owner at most once and never evicts; `sharedFallback`
builds at most once in total regardless of owner. Production: `packages/capability/src/per-owner.ts`. Test:
`packages/capability/tests/unit/per-owner.test.ts`.
...
`packages/capability/src/tool-arguments.ts`. Test: `packages/capability/tests/unit/tool-arguments.test.ts`.

**INV-C23.** The malformed-arguments message never claims the model omitted a required property, and
states that the call was not run. Production: `packages/capability/src/tool-arguments.ts`. Test:
...
plus the ellipsis this module appends. Production: `packages/capability/src/tool-arguments.ts`. Test:
`packages/capability/tests/unit/tool-arguments.test.ts`.
...
`packages/capability/tests/unit/task-title.test.ts`.

**INV-C27.** `globToRegExp` anchors the whole candidate string; only `*` is a wildcard and every other
regex metacharacter is literal. Production: `packages/capability/src/glob.ts`. Test:
`packages/capability/tests/unit/glob.test.ts`.
...
`packages/capability/src/frontmatter-fence.ts`. Test: `packages/capability/tests/unit/frontmatter-fence.test.ts`.

**INV-C29.** Two adjacent `---` lines are `unterminated`, not an empty frontmatter block; an empty
block is written as `---`, blank line, `---`. Production: `packages/capability/src/frontmatter-fence.ts` (the pattern's
mandatory `\r?\n` before the closing fence). Test: `packages/capability/tests/unit/frontmatter-fence.test.ts`.

**INV-C30.** Every engine-declared agent error code is also an engine-declared error code, checked
both at compile time and at runtime. Production: `packages/capability/src/agent-result.ts` (a conditional-type
...
`packages/capability/src/agent-result.ts`. Test: `packages/capability/tests/unit/open-vocabularies.test.ts`.
...
`packages/capability/src/execution-status.ts`. Test: `packages/capability/tests/unit/vocabulary.test.ts`.

**INV-C35.** A `ProviderError` constructed without a `kind` classifies as `transient` — the one kind
that is retried — and `streamStarted` defaults to `false`. Production: `packages/capability/src/llm-port.ts`.
Test: `packages/capability/tests/unit/vocabulary.test.ts`.
...
`packages/capability/src/errors.ts`. Test: `packages/capability/tests/unit/errors.test.ts`.
...
Production: `packages/capability/src/errors.ts`. Test: `packages/capability/tests/unit/errors.test.ts`.

**INV-C38.** `ContinuationUnavailableError` echoes the unresolvable id into `details.continue_from`
and directs the caller to retry with the full message history. Production: `packages/capability/src/errors.ts`. Test:
`packages/capability/tests/unit/errors.test.ts`.
...
`[type]` placeholder joined by newlines. Production: `packages/capability/src/message-content.ts`. Test:
`packages/capability/tests/unit/message-content.test.ts`.

**INV-C44.** `partialStructOf` returns a spread-ready empty object when nothing was submitted, not an
object with an undefined key. Production: `packages/capability/src/agent-result.ts`. Test:
`packages/capability/tests/unit/capability-ports.test.ts`.
...
activation filter. Production (declaration): `packages/capability/src/contract.ts`. Production (engine):
`packages/loop/src/runtime/orchestrator.ts` both read `allCapabilities`, while
`runCapabilities` is the post-`forRun` filtered list. Unpinned in this package; the
...
a capability cannot reclassify an engine tool. Production (declaration): `packages/capability/src/contract.ts`.
Production (engine): `packages/loop/src/runtime/tools/tool-effect.ts`. Unpinned in this
package.
...
stop the spawn from being background. Production (declaration): `packages/capability/src/agents-port.ts`. Unpinned in
this package.
...
(declaration): `packages/capability/src/llm-port.ts`. Production (engine): `@clarvis/llm`'s `packages/llm/src/retry-llm-provider.ts`
attaches it; `@clarvis/loop`'s `packages/loop/src/runtime/loop/iteration-metrics.ts` and
`packages/loop/src/runtime/loop/output-budget.ts` are two independent callers that add it in. Test (engine side):
`packages/llm/tests/unit/retry-llm-provider.test.ts`.
...
`packages/capability/src/llm-port.ts`, and only an explicit `"off"` withholds them. Production: `packages/capability/src/llm-port.ts`
(the doc-comment) and `provider-resolver.ts` (the resolver this package owns). Test (absence
preserved rather than defaulted to off): `packages/capability/tests/unit/provider-resolver.test.ts`. The adapter's
...
after-tool hook contexts for that dispatch. Production:
`packages/capability/src/loop-contract.ts` and `packages/capability/src/api.ts`.
Production (engine): `packages/loop/src/runtime/loop/loop.ts`. Test:
...
| Port key already provided | **throws**; the first value survives | `packages/capability/src/services.ts` |
| Port key never provided | `get` returns `undefined`; consumers must treat it as "feature off" | `packages/capability/src/services.ts` |
| `schema` without `validate` on a call envelope | **throws**, explicitly to avoid failing open | `packages/capability/src/call-envelope.ts`, rationale |
| Validator returns `undefined` instead of `null` | Treated as valid (`?? null`) | `packages/capability/src/call-envelope.ts` |
| Provider omitted the tool call id | A UUID is minted; the call proceeds | `packages/capability/src/call-envelope.ts` |
...
| A capability event carries no `wire` | The host drops it; an unwrapped event stays internal by design | `packages/capability/src/contract.ts`; kernel handler `packages/kernel/src/runs/map-events.ts` |
| `bestEffort` work throws; observer throws; logger throws | Each is caught independently; the returned promise still resolves | `packages/capability/src/tasks.ts` |
| Failure repeated inside the rate-limit window | Silently dropped — no observer call, no log line | `packages/capability/src/tasks.ts` |
| Unterminated or absent frontmatter fence | Reported as a variant, never thrown; the two callers disagree about severity and that disagreement is policy, not splitting | `packages/capability/src/frontmatter-fence.ts` |
| `unref` on a handle with no `unref` method | No-op | `packages/capability/src/unref.ts` |
...
capability can live in its own package (`packages/capability/src/index.ts`).
...
| Consumer | Edge | Cite |
| --- | --- | --- |
| `@clarvis/loop` | value import of `createCapabilityRequestView` / `createCapabilityServices` | `packages/loop/src/runtime/orchestrator.ts` |
| `@clarvis/loop` | value import of `composeCapabilityRegistry` / `createCapabilityRequestView` | `packages/loop/src/runtime/execute-run.ts` |
...
`TraceHandle` are assignable with no adapter and no cast (`packages/capability/src/ports.ts`). Likewise `Logger` is
shaped so pino's `LogFn` satisfies it without this package depending on pino (`packages/capability/src/ports.ts`), and
`ToolArgValidate` is a function type rather than a shared helper precisely so the JSON Schema
implementation stays in the engine (`packages/capability/src/loop-contract.ts`). `AgentBuildContext` is described as a
...
- **A `projected()` event with no detail is dropped by the kernel.** `projected` deliberately omits
  ...
  `event.wire.detail === undefined` (`packages/kernel/src/runs/map-events.ts`). Whether that
...
- **The self-import scan has two blind spots.** `packages/capability/tests/architecture/self-import.test.ts`
  matches only `from "..."` clauses, so a bare side-effect `import "@clarvis/capability"` or a dynamic
  `await import("@clarvis/capability")` would not be caught; and its preprocessing strips only `/**... */`
