...
Semantic formulation is also a first-class presentation state: the host emits transient
`preparing` and `reviewing_definition` invalidations, which drive the Lead activity line and the
Goal section of the shared activity sidebar until the host returns a receipt. They never become
durable Goal state or authority. Creation keeps the
