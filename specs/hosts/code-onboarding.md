...
1. **Onboarding** (`src/onboarding/**`): a fixed ladder of readiness gates
   (`packages/code/src/onboarding/doctor.ts`) that decide whether the TUI can proceed straight to
...
The `PROFILE_SHAPED_ISSUES` doc comment at `packages/code/src/onboarding/doctor.ts` states the
concrete failure it closes: "every run in the workspace was rejected before its first model call while
Doctor reported `Ready`" (`packages/code/src/onboarding/doctor.ts`). The diagnostic channel exists because ordinary logging is
forbidden from touching the terminal
(`packages/code/src/adapters/terminal-guard.ts`) and because a user who hits a defect needs a record they can attach to a
...
| `startupRoute(ctx, report): StartupRoute` | fn | routes a blocked boot to `setup` or `repair` from settings state, not from the diagnostic screen | `packages/code/src/onboarding/doctor.ts` |
...
| Symbol | Shape | Cite |
| --- | --- | --- |
| `DoctorViewDeps` | `{ ctx, report, recheck, openFix, startAnyway, keys, notify, openKeyboard?, boot? }` | `packages/code/src/views/config/DoctorView.tsx` |
...
`DoctorView`'s row classification and its three fix flows (repair-settings, set-key, set-default) are
in §4.9; `packages/code/tests/integration/doctor-render.test.tsx` is the dedicated integration suite
behind every claim there.

### 2.4 `adapters/platform.ts`

| Symbol | Shape | Cite |
| --- | --- | --- |
| `PlatformCapabilities` | `{revision, keyboard, remote, runtimePlatform, terminal, mouse, clipboard:{osc52}, multiplexer, plain, themeBg, colorDepth}` | `packages/code/src/adapters/platform.ts` |
| `Platform` | `{capabilities, onShutdown, shutdown, suspend, resume, copyText, readClipboardImage}` | `packages/code/src/adapters/platform.ts` |
...
| Symbol | Shape | Cite |
| --- | --- | --- |
| `ClipboardProcessRequest` | `{command, args, stdin?, signal?, timeoutMs?, maxStdoutBytes?}` | `packages/code/src/adapters/clipboard-process.ts` |
| `ClipboardProcessResult` | `{exitCode, stdout: Buffer, stderr: string, timedOut, cancelled, outputExceeded, error?}` | `packages/code/src/adapters/clipboard-process.ts` |
| `ClipboardProcessDependencies` | `{spawn?, killTree?, ownProcessGroup?, killGraceMs?}` | `packages/code/src/adapters/clipboard-process.ts` |
| `runClipboardProcess(request, deps?): Promise<ClipboardProcessResult>` | bounded, cancellable, byte-capped process runner | `packages/code/src/adapters/clipboard-process.ts` |

### 2.6 `adapters/terminal-guard.ts`

| Symbol | Shape | Cite |
| --- | --- | --- |
| `installTerminalGuard(): () => void` | intercepts `process.stderr.write` and every text-emitting `console.*` method; returns a restore function that flushes what was withheld | `packages/code/src/adapters/terminal-guard.ts` |

### 2.7 `adapters/debug-session.ts`

| Symbol | Shape | Cite |
| --- | --- | --- |
| `DebugSessionStatus` | `{open: boolean; path?: string; level?: DiagnosticLevel}` | `packages/code/src/adapters/debug-session.ts` |
| `DebugSessionController` | `{status, open(level?), close(), dispose()}` | `packages/code/src/adapters/debug-session.ts` |
| `DebugSessionControllerDeps` | `{workspace?: string; create?: (level) => DiagnosticSession}` | `packages/code/src/adapters/debug-session.ts` |
| `createDebugSessionController(deps?): DebugSessionController` | owns the `/debug` lifecycle: open/retune/close/status | `packages/code/src/adapters/debug-session.ts` |

### 2.8 `adapters/diagnostic-session.ts`

| Symbol | Shape | Cite |
| --- | --- | --- |
| `DiagnosticSessionOptions` | `{workspace?, directory?, maxBytes?, keepFiles?, now?, pid?, level?}` | `packages/code/src/adapters/diagnostic-session.ts` |
| `createDiagnosticSession(options?): DiagnosticSession` | opens one bounded, owner-only JSONL session file, with a `DiagnosticLogger` the kernel can be handed | `packages/code/src/adapters/diagnostic-session.ts` |

### 2.9 `core/diagnostic-events.ts`

| Symbol | Shape | Cite |
| --- | --- | --- |
| `DiagnosticLevel` | `"debug" \| "info" \| "warn" \| "error"` | `packages/code/src/core/diagnostic-events.ts` |
| `DIAGNOSTIC_LEVELS` | `["debug","info","warn","error"]`, least to most severe | `packages/code/src/core/diagnostic-events.ts` |
| `DEFAULT_DIAGNOSTIC_LEVEL` | `"debug"` | `packages/code/src/core/diagnostic-events.ts` |
| `isDiagnosticLevel(value): value is DiagnosticLevel` |  | `packages/code/src/core/diagnostic-events.ts` |
| `DiagnosticLogger` | `{debug,info,warn,error: LogFn; child?(bindings, {level?}); readonly level?}` — structural port the kernel's `Logger` satisfies | `packages/code/src/core/diagnostic-events.ts` |
| `DiagnosticSession` | `{path, logger, level, setLevel, event, count, bind, close}` | `packages/code/src/core/diagnostic-events.ts` |
| `installDiagnosticSession(session): () => void` | installs the process-wide singleton; returned fn uninstalls only if it is still the active one | `packages/code/src/core/diagnostic-events.ts` |
| `activeDiagnosticLogger(): DiagnosticLogger \| undefined` | for handing the kernel its logger at construction | `packages/code/src/core/diagnostic-events.ts` |
| `activeDiagnosticSession(): DiagnosticSession \| undefined` | for a surface reporting on diagnostics (Doctor's `diagnostics` gate, `/debug`) | `packages/code/src/core/diagnostic-events.ts` |
| `diagnosticBind(fields)`, `diagnosticEvent(event, details?, level?)`, `diagnosticCount(event, details?, counterKey?)` | fire-and-forget free functions, inert with no session installed | `packages/code/src/core/diagnostic-events.ts` |
| `diagnosticAsync<T>(operation, run, options?): Promise<T>` | wraps one async op with `async.started`/`async.pending`/`async.settled`/`async.failed`; is a no-op wrapper when no session and no `onSlow` | `packages/code/src/core/diagnostic-events.ts` |

### 2.10 CLI surface (`cli-args.ts`, cited for the `--debug` contract only — see delegation note below)

| Flag / env var | Effect | Cite |
| --- | --- | --- |
| `--debug` | enables the session for the current normal application invocation; help/version remain fast paths, usage errors do not open it, and update refuses it | `packages/code/src/cli-args.ts` |
| `--debug=<error\|warn\|info\|debug>` | enables and sets the floor in one flag | `packages/code/src/cli-args.ts` |
| `CLARVIS_CODE_DEBUG` | env equivalent of `--debug`; any value not in `{"", "0", "off", "false", "no"}` enables it, and if it also names a level it sets the floor | `packages/code/src/cli-args.ts` |
| `CLARVIS_CODE_DEBUG_LEVEL` | env equivalent of `--debug=<level>`, read before falling back to `CLARVIS_CODE_DEBUG`'s own value as a level | `packages/code/src/cli-args.ts` |
| `resolveDebugRequest(mode, env): DebugRequest` | folds flag + env, flag wins in both directions | `packages/code/src/cli-args.ts` |
...
`RecoveryView` (`packages/code/src/views/onboarding/RecoveryView.tsx`) presents one blocking
`StartupIssue { label; detail; hint? }` — always the *first* blocking gate, computed by its
caller (`recoveryIssue` in `packages/code/src/app/commands.tsx`, outside this document). Its `deps`
add `ready(): boolean` and `onReady(): void` to the same shape of callbacks: a `createEffect`
calls `deps.onReady()` via `queueMicrotask` the moment `deps.ready()` turns true, which is how the screen
closes itself as soon as the last blocking gate clears without any user action. Its `LevelSpec`
...
(`packages/code/src/adapters/diagnostic-session.ts`; path composition in `packages/paths/src/workspace-state.ts`).
Opened with `wx` (exclusive create) and mode `0o600`; a same-second collision from the same pid retries
with a numeric suffix up to 100 times (`packages/code/src/adapters/diagnostic-session.ts`), and the newest `keepFiles - 1`
files are kept before opening a new one (`retainNewest`, `packages/code/src/adapters/diagnostic-session.ts`).
...
- `event` is either the caller's own name, the value of a declared `event` field on a pino-style bindings
  object (`loggerEvent`, `packages/code/src/adapters/diagnostic-session.ts`), or the fallback `kernel.<level>`; an
...
- Reserved envelope keys `v, at, seq, level, source, event, pid, memory, details` cannot be overwritten
  by `session.bind()` (`RESERVED_ENVELOPE_KEYS`).

**Sanitization** (`sanitize()`, `packages/code/src/adapters/diagnostic-session.ts`) is applied to every logged detail and
...
`packages/code/src/adapters/diagnostic-session.ts`). `path` is deliberately **not** in `CONTENT_KEY` — including it made the session's own
`diagnostics.start` record redact the very file path it announces. Independently of key-based
...
sequences stripped (`safeString`).

**Counters** (`session.count`) are recorded only at counts 1–8 and at powers of two thereafter
(`isPowerOfTwo`), and at most 256 distinct counter keys are tracked before new ones
collapse into `diagnostics.counter-overflow` (`MAX_COUNTERS`).
...
can always be written even after the regular budget is exhausted. Once the
...
- **`diagnostics.stop`** (`close()`, `packages/code/src/adapters/diagnostic-session.ts`, forced): `{bytes, saturated,
  counterKeys, topCounters}`, where `topCounters` is the top 32 counters by count, descending
  (`[...counters].sort(...).slice(0, 32)`).
- **`runtime.heartbeat`** (`packages/code/src/adapters/diagnostic-session.ts`): fired every `HEARTBEAT_MS` (5000 ms) via a
  `setInterval` the session `unref()`s so it cannot keep the process alive; payload is
...
`requestRender()` runs into; the doc comment above the function notes it never binds today
...
runGates(ctx) -> DoctorReport{gates, results, blocked}
  blocked = any(hard gate).status == "fail"  OR  any(soft gate).status != "pass"
             (ui/comms gates never block)                              [packages/code/src/onboarding/doctor.ts]

bootGate(report)   = report.blocked ? "doctor" : "shell"                [packages/code/src/onboarding/doctor.ts]
...
startupRoute(ctx, report):
  ...
  else                                      -> "repair"                [packages/code/src/onboarding/doctor.ts]
```

This means: a fresh machine with nothing written yet routes to `setup`
(`packages/code/tests/integration/doctor.test.ts`); an unset credential on an otherwise-configured provider
routes to `repair`, not `setup` (`packages/code/tests/integration/doctor.test.ts`); a corrupt settings file
always routes to `repair` regardless of provider state (`packages/code/tests/integration/doctor.test.ts`).

### 4.2 Per-gate evaluation, and failure containment

`runGates` calls `checkGate(gate, ctx)` for every entry in `GATES`, in array order
(`packages/code/src/onboarding/doctor.ts`). `checkGate` times the call and, on a throw, calls `reportGateFailure`, which (a) emits
...
hint:"...run with --debug..."}` result for that one gate only (`packages/code/src/onboarding/doctor.ts`, pinned by
`packages/code/tests/integration/doctor.test.ts`). A throwing gate never aborts `runGates` or corrupts another
gate's result.
...
- **`agents`** normally sees the shipped five even when no agent file exists on disk. If its effective
  ...
  coexists with the other kind (`packages/code/tests/integration/doctor.test.ts`), and an unrecognized
  frontmatter key alone does not count as invalid.
- **`agents`**'s second phase, run only once the fleet is conflict/invalid/refusal-free, computes
  ...
  which the `providers`/`default_model` gates already own (`packages/code/src/onboarding/doctor.ts`).
...
- **`diagnostics`** reports `warn` (never `fail`) while a session is recording, deliberately — `ui`
  ...
  session's `path` travels as the `hint` (`packages/code/src/onboarding/doctor.ts`).
...
`restore()` + `process.exit` immediately (pinned by
`packages/code/tests/integration/platform-lifecycle.test.ts`). Otherwise: abort every in-flight clipboard
...
`installTerminalGuard()` (`packages/code/src/adapters/terminal-guard.ts`): replaces `process.stderr.write` and all eight
text-emitting `console.*` methods with interceptors that append to a shared byte-capped ring buffer
(`MAX_DEFERRED_BYTES` = 64 KiB, oldest chunks dropped first); a write's callback still fires
synchronously so a caller awaiting drain is never stranded (pinned by
`packages/code/tests/unit/terminal-guard.test.ts`). The returned restore function is idempotent (a second call is
a no-op) and, on first call, puts every original back and flushes the buffered bytes through
...
| --- | --- | --- | --- |
| no session installed | `open(level?)` | this controller owns a session | creates via `create()`, installs it, `retuned:false` | `packages/code/src/adapters/debug-session.ts` |
| this controller's session installed | `open(level?)` | same session, retuned | `setLevel(level)` on the existing session; `retuned:true`, same `path` | `packages/code/src/adapters/debug-session.ts` |
...
distinguish the two. But `setLevel`'s own no-op short-circuit (`packages/code/src/adapters/diagnostic-session.ts`, invariant
...
**Row classification** (`packages/code/src/views/config/DoctorView.tsx`): every gate not already resolved (`isResolved`: `status
...
`packages/code/tests/integration/doctor-render.test.tsx` ("only unresolved actionable rows participate in Doctor
navigation") ("a gate with no action is status-only and Enter does not pretend it can fix
it").

**`doRepairSettings` fix flow** (`packages/code/src/views/config/DoctorView.tsx`), reached from the `repair-settings` fix kind:
...
alike. Pinned by `packages/code/tests/integration/doctor-render.test.tsx` (corrupt config gate) (valid
config: nothing to repair, rechecks anyway) (decline leaves settings untouched)
(unparsable file offers reset, failed repair notifies).

**`doSetKey` fix flow** (`packages/code/src/views/config/DoctorView.tsx`), reached from the `set-key` fix kind: collects every
...
`backend.reconnect`. Pinned by `packages/code/tests/integration/doctor-render.test.tsx` (no missing credentials) (one missing: opens editor directly, save failure notifies) (one missing: saves,
rechecks, reconnects) (several missing: picker first).

**`doSetDefault` fix flow** (`packages/code/src/views/config/DoctorView.tsx`), reached from the `set-default` fix kind: picks the
...
and notifies the chosen name and scope. Pinned by `packages/code/tests/integration/doctor-render.test.tsx` (picks
the first runnable agent) (no agents at all) (write failure notifies).

### 4.10 Diagnostic child-logger floor composition

`DiagnosticSession.logger.child(bindings, options?)` (`packages/code/src/adapters/diagnostic-session.ts`) composes a derived
logger's floor rather than replacing it: the new `ownFloor` is `Math.max(ownFloor, childFloor(options?.level))`, so a child can only ever narrow (raise) what its parent already admits, never widen it. The
derived logger's own `level` getter reports `DIAGNOSTIC_LEVELS[Math.max(floor, ownFloor)] ?? "silent"`
 — the session's current floor and the chain's composed `ownFloor`, whichever is stricter.
`childFloor(requested)` reads a `CLARVIS_LOG`-style level: `undefined` yields `0` (emit
...
its `createEffect` (`packages/code/src/views/onboarding/RecoveryView.tsx`) re-runs on every change to
...
`packages/code/tests/integration/onboarding-render.test.tsx` ("recovery closes itself once
`ready()` turns true").

## 5. Invariants

1. **A doctor gate's throw degrades only that gate**, never the whole report. — `packages/code/src/onboarding/doctor.ts` —
   pinned by `packages/code/tests/integration/doctor.test.ts`.
2. **Boot is blocked exactly when a `hard` gate fails or a `soft` gate is not passing**; `ui`/`comms`
   gates never block. — `packages/code/src/onboarding/doctor.ts` — pinned by
   `packages/code/tests/integration/doctor.test.ts` (all-pass → not blocked) (soft `warn` →
   blocked).
...
4. **`startupRoute` returns `repair` for any corrupt scope, even before checking for missing settings or
   providers.** — `packages/code/src/onboarding/doctor.ts` — pinned by `packages/code/tests/integration/doctor.test.ts`.
...
20. **The terminal guard withholds `console.log`/`console.info` (stdout-bound methods) as well as the
    ...
    `packages/code/src/adapters/terminal-guard.ts` — pinned by `packages/code/tests/unit/terminal-guard.test.ts` (asserts `console.log`
...
| --- | --- | --- |
| A gate's `check()` throws | Caught by `checkGate`; logged as `doctor.check.failed` (error-level diagnostic with `check_id`/`duration_ms`); substitutes a synthetic `fail` result naming the thrown message and hinting `--debug` | `packages/code/src/onboarding/doctor.ts` |
| A seeder finds no settings file in either scope | Declines with `reason:"no-settings-file"`, writes nothing | `packages/code/src/onboarding/seed-block-once.ts` |
| A seeder finds a corrupt scope | Declines with `reason:"corrupt"`, writes nothing (would fail anyway) | `packages/code/src/onboarding/seed-block-once.ts` |
| Native clipboard tool spawn fails (`ENOENT`, `EPERM`, ...) | Reported as `result.error`, treated as a miss — the next candidate (or OSC-52 fallback) is tried, never a thrown exception reaching `Platform.copyText` | `packages/code/src/adapters/platform.ts`, pinned `packages/code/tests/integration/platform-lifecycle.test.ts` |
| Clipboard helper hangs | Killed after `timeoutMs` (default 2000 ms); `child.kill` itself throwing is swallowed | `packages/code/src/adapters/clipboard-process.ts` |
| `renderer.destroy()`/`suspend()`/`resume()` throws | Swallowed with an empty `catch {}` | `packages/code/src/adapters/platform.ts` (`createPlatform`), pinned `packages/code/tests/integration/platform-lifecycle.test.ts` |
...
- `DiagnosticLogger` is a **structural** port (`packages/code/src/core/diagnostic-events.ts`) deliberately shaped so a
...
- `installDiagnosticSession`'s uninstall guard (`if (activeSession === session) activeSession =
  undefined`, `packages/code/src/core/diagnostic-events.ts`) is what makes two independently-owned sessions (a boot-time
...
- `packages/code/src/app/commands.tsx`'s `/debug` slash command (`applyDebugCommand`,
  `packages/code/src/app/commands.tsx`) is the sole caller of
  `DebugSessionController.open`/`close`/`status`. Its final branch is the "kernel records
