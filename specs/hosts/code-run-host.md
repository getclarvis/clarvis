...
A yielded `shell` result carries its original control token while the process remains live. The loop retains a stop callback bound to that run's `ExecutionSessionManager` and agent, publishes the control on the terminal tool event, and emits live-only `tool_control_released` when physical completion removes the callback. The TUI clears the matching control without changing the result; run settlement also clears any remaining controls in restored history. Production: `createToolInterruptRegistry` in `packages/loop/src/runtime/tools/tool-interrupt.ts`, `openToolInvocation` in `packages/loop/src/runtime/loop/loop.ts`, `mapEntry` in `packages/trace/src/trace-mapper.ts`, `engineEventToProto` in `packages/kernel/src/runs/map-events.ts`, and `createTranscriptStore` in `packages/code/src/adapters/store.ts`. Test: `packages/loop/tests/integration/selective-shell-interrupt.test.ts`, `packages/loop/tests/unit/tool-interrupt.test.ts`, `packages/code/tests/unit/store-status.test.ts` (`physical completion removes a yielded shell's stop control`), and `packages/code/tests/component/run-host.test.ts` (`a yielded shell keeps its interrupt control through the hosted run`).
...
Authority and mutation review are owned by [self-configuration.md](self-configuration.md).
...
| run fails with an envelope error | `store.appendRunFailure` (impl `packages/code/src/adapters/store.ts`) | one error node per distinct `(execId, code)`, suppressed if the same rendered text is already present; the node reads `<code>: <message>` and collapses to the code alone when the message *is* the code |
| a restored `run_ended` records a failure | `appendRunFailure` from the persisted `code` and `message` (`packages/code/src/adapters/store.ts`) | the restored transcript names the cause the live path named, instead of the run's category (`guard_trip`); a trace with no recorded message falls back to its code |
...
| backend rebuild exceeds 10 s | timeout branch (`packages/code/src/adapters/memory-pressure.ts`) | `memory.maintain.timeout` is recorded at `warn`; the step keeps its attempt identity, so a late result still finishes the episode's pass, and the physical attempt continues |
