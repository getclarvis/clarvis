...
(`packages/code/src/views/config/create-view-host.ts`). `createFieldEditor` is a single-slot modal
editor covering text, masked secret, number, pick, enum and multiline inputs
(`packages/code/src/views/config/field-editor.tsx`). `HubMenu` is the flat Settings menu
(`packages/code/src/views/config/hub-menu.tsx`), over the same item list used by the
...
controller in `packages/code/src/features/providers/controller.ts`, which stages a draft, tracks
...
(`packages/code/src/adapters/settings.ts`) is the UI's read/write/validate view over the kernel's
...
| Export | Kind | Source |
| --- | --- | --- |
| `createViewHost`, `ViewHostBundle`, `ViewHostControls` | re-export | `./create-view-host.ts` (`packages/code/src/views/config/view-host.tsx`) |
| `createFieldEditor`, `FieldEditState`, `FieldEditor`, `PickItem` | re-export | `./field-editor.tsx` (`packages/code/src/views/config/view-host.tsx`) |
| `SourceBadge`, `SelectableRow`, `EmptyHint`, `LoadingHint`, `ErrorBanner`, `FieldRow`, `ToggleRow`, `SectionHeader`, `StatusRow`, `SettingRow`, `DetailLines`, `Dash`, `DetailRow` | re-export | `../../ui/primitives/index.ts` (`packages/code/src/views/config/view-host.tsx`) |
| `bindLevelKeys`, `SelectableList`, `ViewFrame` | re-export | `../../ui/patterns/index.ts` (`packages/code/src/views/config/view-host.tsx`) |
| `LevelView` | type re-export | `ui/patterns/level-host.tsx` (`packages/code/src/views/config/view-host.tsx`) |
...
function createViewHost(opts: {
  ...
  initialScope?: Scope;
}): ViewHostBundle              // packages/code/src/views/config/create-view-host.ts
```

`ViewHostBundle` = `{ host: ViewHost; controls: ViewHostControls }` (`packages/code/src/views/config/create-view-host.ts`).

| `ViewHostControls` member | Contract | File |
| --- | --- | --- |
| `runSave(): Promise<void>` | runs the registered save handler; single-flight (a second call while in flight returns the same promise) | `create-view-host.ts` |
| `scopeBound: Accessor<boolean>` | whether the mounted view ever called `host.bindScope` | set |
| `escape(): void` | dirty-checked close: pop a level, else confirm a discard, else cancel + back/close | `create-view-host.ts` |
| `dispose(): void` | forced teardown: settles a pending confirm as `false`, runs the cancel handler, drops the save handler, **no dirty check** | `create-view-host.ts` |

The `ViewHost` the screen receives is defined at `packages/code/src/keys/commands.ts` and carries
...
(`packages/code/src/views/config/field-editor.tsx`). `FieldEditor` exposes:

| Method | Signature | Notes |
| --- | --- | --- |
| `start` | `(label, current, commit, opts?: { alwaysCommit?: boolean })` | commits only on change unless `alwaysCommit` |
| `startSecret` | `(label, commit)` | masked; raw value never rendered |
| `startNumber` | `(label, current, { min?, max?, commit, notify })` | rejects non-finite and out-of-range, floors the value |
| `startPick` | `(label, items, commit, opts?: { onManual?: () => void })` | opens `CatalogPicker` over an arbitrary list |
| `startEnum` | `(label, options: readonly (string \| PickItem)[], current, commit)` | pre-selects `current` |
| `startMultiline` | `(label, current, commit)` | textarea; applied with `ctrl+s` |
| `editing` | `Accessor<FieldEditState \| null>` | render `EditInput` while non-null |
| `EditInput` | `() => JSX.Element` | `packages/code/src/views/config/field-editor.tsx` |

`PickItem` = `{ label: string; value: string; detail?: string }`.

### 2.4 `key-entry.ts`

```ts
function promptForApiKey(fe: FieldEditor, envVar: string, deps: KeyEntryDeps): void  // packages/code/src/views/config/key-entry.ts
...
`packages/code/src/features/issues.ts` (`packages/code/src/views/config/validation.ts`); the implementations are at
`packages/code/src/features/issues.ts`.

### 2.6 Settings hub and extension child routes

```ts
interface HubMenuItem { id: string; label: string; desc: string; cmd: string }   // packages/code/src/views/config/hub-menu.tsx
...
| `DefaultsPanel(host, deps)` | `DefaultsDeps { settings, env, notify }` | `packages/code/src/views/config/DefaultsPanel.tsx` |
| `McpBrowser(host, deps)` | `McpBrowserDeps { nodes, refresh, editConfig, notify }` | `packages/code/src/views/config/McpBrowser.tsx` |
...
| Factory | Returns | File |
| --- | --- | --- |
| `createProviderListLevel(ctx)` | `{ title, body, spec, openAdd }` | `packages/code/src/views/config/providers/list-level.tsx` |
| `createProviderDetailLevel(ctx)` | `{ title, body, spec, modelIds }` | `packages/code/src/views/config/providers/detail-level.tsx` |
| `createProviderModelLevel(ctx)` | `{ body, spec }` | `packages/code/src/views/config/providers/model-level.tsx` |

Shared state passes through `ProvidersViewContext` (`packages/code/src/views/config/providers/context.ts`), which
...
```ts
PROVIDER_DETAIL_FIELDS = ["name","kind","base_url","env var","API key","source","headers","body"]  // packages/code/src/views/config/providers/detail-level.tsx
PROVIDER_ISSUE_DETAIL_FIELD = { name:"name", base_url:"base_url", api_key_env:"env var" }          // packages/code/src/views/config/providers/detail-level.tsx
MODEL_FIELDS = ["context_window_tokens","max_output_tokens","prompt_cache","headers","body"]        // packages/code/src/views/config/providers/model-level.tsx
```
...
| Export | Kind | File |
| --- | --- | --- |
| `ProvidersCommandDeps` | host services passed to the Providers view, including optional subscription auth/catalog and copy/open integrations | `packages/code/src/features/providers/commands.ts` |
| `registerProvidersCommands(commands, deps)` | registers internal `providers.open` as the `settings` hub child, with no standalone slash token | `packages/code/src/features/providers/commands.ts` |
| `ProvidersEvent` | 7-arm union of controller events | `packages/code/src/features/providers/events.ts` |
| `presentProvidersEvent(event): Notice` | event → terminal text + tone | `packages/code/src/features/providers/events.ts` |
| `createProvidersController(deps): ProvidersController` | headless provider draft/save controller | `packages/code/src/features/providers/controller.ts` |
| `ProvidersControllerDeps` | `createProvidersController`'s dependency-injection shape (`settings`, `keys`, `code`, `catalog`, `scope`, `markDirty`, `emit`, `onReconnect?`, `manageDefaultModel?`) | `packages/code/src/features/providers/controller.ts` |
| `ProvidersController` | reactive provider/model/key draft surface, including `addSubscriptionProvider` for an entitled provider | `packages/code/src/features/providers/controller.ts` |
| `ProviderKind` (re-export) | re-exported from wherever it is truly declared, for callers that only need the kind literal | `packages/code/src/features/providers/controller.ts` |
| `KEY_SOURCES`, `SOURCE_MEANING` | `["auto","env","keyfile"]` and their one-line meanings | `packages/code/src/features/providers/controller.ts` |
| `ProviderKeyStatus` | `"not-required" \| "staged" \| "environment" \| "keyfile" \| "missing"` | `packages/code/src/features/providers/controller.ts` |
| `ModelRemovalBlock` | `{kind:"default-model"} \| {kind:"agent-references"}` | `packages/code/src/features/providers/controller.ts` |
| `ProviderMapField` | `"headers" \| "body"` | `packages/code/src/features/providers/controller.ts` |
| `headerSuggestions`, `bodySuggestions`, `headerKeyProblem`, `headerValueProblem`, `bodyKeyProblem`, `headersFootnote`, `bodyFootnote` | request-map authoring rules | `packages/code/src/features/providers/request-params.ts` |

### 2.10 `adapters/settings.ts`

`SettingsAdapter` (`packages/code/src/adapters/settings.ts`) is the boundary every settings screen writes through:

| Member | Purpose |
| --- | --- |
| `version` | signal bumped on every successful write / reload / trust change / repair |
| `read(scope)` | that scope's file as loaded |
| `corrupt(scope)` | the scope's parse error, or `null` |
| `planRepair` / `applyRepair` | destructive repair preview and application |
| `effective()` | the merged view |
| `withheldWorkspaceFields()` | workspace fields the kernel refused to merge |
| `workspaceTrust()` / `setWorkspaceTrust(approve)` | `"inert" \| "trusted" \| "unapproved" \| "changed"` |
| `origin(key)` | which scope supplies `key`; a withheld workspace field does not count |
| `effectiveProviders()` | per-provider origin `global` / `workspace` / `shadow` |
| `knownGrants()` | kernel-reported grant list or `undefined` |
| `sources()` | the on-disk `settings.json` path for each scope, `workspace` optional |
| `write(scope, patch)` | CAS write, schema-validated before it leaves |
| `validateProviders(s, resolveAgainst?)` | provider/model/default_model field validation |
...
| Export | File |
| --- | --- |
| `ServerStatus = MCPStatus \| "declared"` | `adapters/mcp-capabilities.ts` |
| `LiveTool`, `LivePrompt`, `PromptMessage`, `McpServerDecl` | `adapters/mcp-capabilities.ts` |
| `parseMcpServers(record)` | `adapters/mcp-capabilities.ts` |
| `classifyCapability(name, kind, profiles?)` → `Classified` | type |
| `ServerNode` | `adapters/mcp-capabilities.ts` |
| `BACKEND_NAME = "kernel"` | `adapters/mcp-capabilities.ts` |
| `reconcile(decls, tools, prompts, backend, profiles?)` | `adapters/mcp-capabilities.ts` |
| `promptMessagesToContent(messages)` | `adapters/mcp-capabilities.ts` |
| `SchemaArgRow`, `schemaArgRows(inputSchema)` | `adapters/mcp-capabilities.ts` |
| `synthSampleArgs(inputSchema)` | `adapters/mcp-capabilities.ts` |
| `mcpServerSettingsSchema` (re-export of `@clarvis/kernel/config`) | `adapters/mcp-capabilities.ts` |
...
| Export | Kind |
| --- | --- |
| `McpClientCaps` | interface: `listTools`, `listPrompts`, `getPrompt(name, args)`, `connectionStatus()` — the subset of an MCP client the bridge needs |
| `McpEffects` | interface: `submitPromptTurn`, `submitSkillRun`, `activeProfile`, `openMcpServers`, `collectArgs` — host callbacks a triggered prompt/skill invokes |
| `McpCapabilities` | interface: `nodes` (an `Accessor<ServerNode[]>`), `refresh()`, `skillAgent(name)`, `dispose()` |
| `McpCapabilitiesDeps` | interface: `client`, `commands` (`Pick<Commands, "promptCommand" \| "skillCommand">`), `effects`, `declared()`, `profiles()`, optional `refreshSlowMs` |
| `createMcpCapabilities(deps)` → `McpCapabilities` | function |

`reportListFailure<T>(surface, error)` is module-private (not exported): it logs
...
`createKernelCapabilitiesClient` (`packages/code/src/adapters/kernel-capabilities-client.ts`),
whose own comment states that "downstream MCP tools were never surfaced at catalog time
...
— `default_model` (`packages/code/src/features/providers/controller.ts`):
...
pinning that is `packages/code/tests/unit/providers-controller.test.ts`, which asserts the written
file is exactly `{ providers: [...] }`.

A newly seeded model entry has this shape (`packages/code/src/features/providers/controller.ts`):

```ts
{
  context_window_tokens: hit?.context_window_tokens ?? 128000,   // DEFAULT_WINDOW, packages/code/src/features/providers/controller.ts
...
`derivePromptCacheMode` comes from `@clarvis/kernel/config` (`packages/code/src/features/providers/controller.ts`); the derivation itself
belongs to [hosts/model-catalog.md](model-catalog.md). What is in scope here is *where* it is called:
`addModelFromCatalog` (`packages/code/src/features/providers/controller.ts`) copies
`reasoning_efforts` with the other catalog facts and derives `prompt_cache` once, at authoring time in this
controller, and writes it into `settings.json` alongside the model. Its TSDoc states
...
`providers().length + 1` and is incremented until unused (`packages/code/src/features/providers/controller.ts`). A blank model is
`{ context_window_tokens: 128000 }` (`packages/code/src/features/providers/controller.ts`).
...
(`packages/code/src/views/config/providers/model-level.tsx`); `undefined` **deletes** the key, which is how "(auto)" is expressed
...
`{}` (`packages/code/src/features/providers/controller.ts`). For `headers` the editor's `unknown`
values are cast to `Record<string, string>` in
`packages/code/src/features/providers/controller.ts`, safe only because the map is
...
| Table | Contents |
| --- | --- |
| `AUTH_HEADER` | `openai-compatible`/`openai` → `Authorization`; `anthropic` → `x-api-key`; `google` → `x-goog-api-key` |
| `COMMON_HEADER_SUGGESTIONS` | `HTTP-Referer`, `X-Title`, `OpenAI-Organization`, `OpenAI-Project`, `anthropic-beta` |
| `BODY_ROOT_SUGGESTIONS` | 17 keys, `provider` first (staged as `{ order: [], allow_fallbacks: false }`) |
| `BODY_PROVIDER_SUGGESTIONS` | `order`, `allow_fallbacks`, `only`, `ignore`, `require_parameters`, `data_collection`, `quantizations`, `sort`, `max_price` |
| `BODY_REASONING_SUGGESTIONS` | `effort`, `max_tokens`, `exclude`, `enabled` |

`bodySuggestions(path)` is path-routed, not one flat table: `path.length === 0`
...
`headerKeyProblem` accepts only `/^[A-Za-z0-9!#$%&'*+.^_\`|~-]+$/`. `headerValueProblem`
strips every well-formed `${VAR}` with `envRefPattern()` from `@clarvis/kernel/policy` and rejects a
residual `${`. `bodyKeyProblem` guards **only** `path.length === 0` against
`FORBIDDEN_PROVIDER_BODY_KEYS`.

### 3.3 MCP reconciliation output

`reconcile` returns `[backendNode, ...downstream]` (`packages/code/src/adapters/mcp-capabilities.ts`):

- `backendNode` is `{ name: "kernel", origin: "control-plane", status: <backend>, tools: [], prompts: <skill prompts> }`. Its `tools` array is always empty — control-plane tools are classified out and
  never bucketed.
- one `downstream` node per name in the union of live-bucket names and declared names, sorted by name, with `status: live ? "connected" : "declared"`.

Name classification (`classifyCapability`): tools split on `.`, prompts on `:`; a bare prompt
...
description}`. `synthSampleArgs` walks `inputSchema.required` — falling back to *all*
properties when the schema declares none required — and emits `0` for number/integer, `false`
for boolean, `[]` for array, `{}` for object, `"<key>"` otherwise.
...
`revision` field of the matching `view.sources` entry (`packages/code/src/adapters/settings.ts`) — inside the queued
operation and passes it to `config.updateSettings(scope, patch, expectedRevision)`. The revision is a 64-hex string in practice
(`packages/code/tests/integration/settings.test.ts` asserts `/^[a-f0-9]{64}$/`).

Diagnostic events emitted here (field names are a contract):

| Event | Fields |
| --- | --- |
| `settings.save.rejected` | `scope`, `issue_count`, `fields`, `reason` (`unparsable` \| `invalid`) |
| `settings.save.refused` | `scope`, `keys`, `error` |
| `settings.resync.failed` | `scope`, `error` |
| `settings.save.applied` | `scope`, `keys` |
| `settings.model_ref.unparsed` | `site`, `error` (sampled) |

`issueFields` joins at most 12 distinct issue paths, `"(root)"` for an empty path. `zodIssueSummary` renders the first two issues and a `(+N more)` suffix
(`packages/code/src/adapters/zod-summary.ts`).

### 3.5 `SettingsRepair` — the destructive-repair persisted shape

`SettingsRepair` (`packages/code/src/adapters/settings.ts`) is a two-variant type over the kernel's
...
Both readings come from the type's own TSDoc.

`planRepair(scope)` asks the kernel for a preview via
...
comes back, or returns `null` when the kernel reports nothing to repair. `applyRepair(repair)`
 is queued through `publishInOrder` like every other publishing operation (§3.4) and
...
(`packages/code/src/views/overlay-host.ts`):

| Command id | Binding | Enabled when | Runs |
| --- | --- | --- | --- |
| `view.save` | `ctrl+s` | `host.dirty() && !host.pendingConfirm()` | `controls.runSave()`, notifying on rejection |
| `view.scope.toggle` | `ctrl+t` | `controls.scopeBound() && !host.pendingConfirm()` | `host.toggleScope()` |
| `view.escape` | `escape` at `LAYER.OVERLAY` | always | `controls.escape()`; Ctrl+C remains global |

`controls.dispose()` runs only from the frame's own `dispose` (`packages/code/src/views/overlay-host.ts`).

### 4.2 `createViewHost` state machine

Scope toggle (`packages/code/src/views/config/create-view-host.ts`):

| State | Event | Next | Effect |
| --- | --- | --- | --- |
| `scopeMode = "retarget"` (default) | `toggleScope` | scope flips | `scopeLoad?.()` |
| `scopeMode = "reload"`, `dirty() === false` | `toggleScope` | scope flips | `scopeLoad?.()` |
| `scopeMode = "reload"`, `dirty() === true` | `toggleScope` | unchanged | arms confirm "Unsaved changes — switching scope discards them. Switch?"; on `yes` flips + loads |

Pinned by `packages/code/tests/unit/view-host-scope.test.ts` (retarget keeps a dirty draft, no
confirm) (reload+clean re-reads) (reload+dirty confirms; `n` keeps, `y` discards and
reloads).

Escape (`packages/code/src/views/config/create-view-host.ts`), in order: a pending confirm swallows it → `level.pop()` if depth
> 0 → if dirty, arm "Discard unsaved changes?" and only on yes run cancel + `back ?? close` → else run
cancel + `back ?? close`. `runCancel` nulls the handler before invoking it, so escape-then-
dispose cannot double-invoke — pinned at `packages/code/tests/unit/view-host-scope.test.ts`.

`runSave` is single-flight: a second call while in flight returns the identical promise, and a retry
after settlement runs the handler again (`packages/code/src/views/config/create-view-host.ts`; pinned at
`packages/code/tests/unit/view-host-scope.test.ts`, which asserts `duplicate === first`).

`confirm` refuses to arm a second prompt over an armed one, resolving `false` immediately
(`packages/code/src/views/config/create-view-host.ts`). Disarming without a `yes` settles `false` via a deferred effect on
`armedConfirm.armed`.

### 4.3 `createFieldEditor`

One slot: `editing` is a single signal, and every `start*` overwrites it (`packages/code/src/views/config/field-editor.tsx`).
Each modal mode registers its own `LAYER.MODAL` layer through `setLayer`, and `clearLayer` releases it
on commit or cancel. Two guard bindings — `ctrl+s` and `ctrl+t` bound to a no-op —
are spread into the text, secret and number layers, so the
shell's save/scope keys cannot fire mid-edit; the multiline layer claims `ctrl+s` for its own apply
command and no-ops `ctrl+t`.

| Mode | Commit key | Cancel keys | Commit rule |
| --- | --- | --- | --- |
| text | `return` → `editor.commit` | `escape` | commits only if `value !== current`, unless `alwaysCommit` |
| secret | `return` → `editor.secret.save` | `escape` | always commits the accumulated plaintext; `left/right/up/down/home/end` bound to no-op |
| number | `return` | `escape` | blank → `commit(undefined)` only if `current !== undefined`; non-finite → notify and stay open; `< min` / `> max` → notify and stay open; else `Math.floor` and commit on change |
| multiline | `ctrl+s` → `editor.multiline.apply` | `escape` | commits `multiline.plainText ?? current`; textarea layer via `registerManagedTextareaLayer` |
| pick / enum | handled by `CatalogPicker` | its own close | `done(picked)` commits only when `picked !== st.initialValue` |
...
`mask.repeat(value.length)` under a re-entrancy `guard`. `onCursorChange` pins the
cursor to the end and clears any selection. Behaviour pinned in
`packages/code/tests/integration/secret-input-render.test.tsx` (bullets render, Enter commits
plaintext) (a key containing `*` commits intact in ASCII mode) (cursor keys pinned)
(backspace shortens, Esc does not commit).

`promptForApiKey` composes on top: the label is `API key → <envVar>` plus an optional
`(usageNote)` (`packages/code/src/views/config/key-entry.ts`), and a blank submission notifies `"empty — key unchanged"` without
calling `commit`. Pinned at `packages/code/tests/unit/key-entry.test.ts`.

### 4.4 Hub navigation

`HubMenu` registers one level whose `nav.activate` calls `deps.openChild(item.cmd)`
(`packages/code/src/views/config/hub-menu.tsx`). The app wires Settings `openChild` to
...
`when: () => !maps.active() && host.level.depth() >= 2`. `packages/code/src/ui/patterns/level-host.tsx` selects the
...
(prefix computed). Credential status text/colour comes from `presentKeyStatus`
: `not-required` → em dash, `staged` → "Unsaved key" (warn), `environment` → "Shell
...
Level spec : `nav.count = providers().length`, `activate = "open"` → `activate()` which sets
`drill`, zeroes `detailRow` and pushes the provider name as a breadcrumb. Verbs: `add`
(`openPicker`) and `delete` (guarded by `sel() < providers().length`).

A trailing line shows the selected provider's own validation issue, if any: `issueMessage()`
 reads `ctrl.validation()`, finds the first issue whose `provider` matches the currently
...
| Condition | Behaviour |
| --- | --- |
| `!catalogReady(catalog)` | show the two subscription rows plus manual entry; absence of the public catalog does not hide subscription login |
| bootstrap and no providers yet | picker titled `Set up Clarvis ⟩ Step 1 of 2 ⟩ Provider` |
...
| pick `__all_providers__` | `setShowAll(true)` and stay open |
| pick a provider id | `seedProviderFromCatalog`, clear `manualBootstrapProvider`, drill in, push breadcrumb, then chain into `openModelPicker(cp, config)` |
| `onManual` | close picker, `addBlank()` |

`addBlank` sets `manualBootstrapProvider` when in bootstrap so the detail level can show the
manual-setup copy.
...
The title is `"Providers"` when empty, else `Providers ⟩ <keyedCount> of <n> ready`.
...
| Row | Action |
| --- | --- |
| 0 | `editProviderField("name")` — commits trimmed; on a non-empty name also `host.level.retitle` |
...
| 2 on an ordinary provider | `editProviderField("base_url")` — empty trims to `undefined` |
| 3 (`ENV_ROW`) | `renameEnvVar()` — text editor seeded with `api_key_env ?? suggestedEnvVar` |
| 4 (`API_KEY_ROW`) | `setKeyValue()` — with an env var, straight to the secret prompt; without one, name the variable first and chain into the secret |
| 5 (`SOURCE_ROW`) | `editSource()` — refuses with "name the env var first — set the key" when `api_key_env` is unset; else enum over `KEY_SOURCES` with `SOURCE_MEANING` details |
| 6 / 7 | `ctx.openMap("headers" \| "body", "provider")` |
| ≥ `fieldCount()` | drill into `modelIds()[row - fieldCount()]`: set `modelId`, zero `modelRow`, push the id |

Pinned: `packages/code/tests/integration/providers-key-render.test.tsx` (Enter on a saved API-key
row opens the *secret* editor, not the env-var editor) (Enter on the env-var row renames) (named-but-empty goes straight to the secret) (unset names the var first, chained) (source row opens the enum).

Credential-value copy : staged → `••••• staged — saves with Save changes`; `set` → `✓ from
shell env`; `keyfile` → `✓ saved in keys.json`; `unset` → `✗ no value yet`; no env var → `— not
configured`. The selected-row hint varies with the same state.

Three of the eight rows carry inline validation markers: `issues = issueSet(() =>
mapProviderIssues(ctrl.validation(), current()?.name))` is wired onto the `FieldRow`s for
`name`, `base_url` and `api_key_env` via `issue={issues.for("name" | "base_url" | "api_key_env")}`. Row 1 (`kind`) receives no `issue=` prop at all — see §8 item 4 on why a
...
`manualBootstrapProvider() && modelIds().length === 0`.

### 4.8 L2 — model detail (`model-level.tsx`)

`nav.count = MODEL_FIELDS.length`. `activate()` clamps the row into range, then:

| Field | Action |
| --- | --- |
| `headers` / `body` | `ctx.openMap(field, "model")` |
| `prompt_cache` | advance one step in `PROMPT_CACHE_CYCLE`, stage it, and notify `Prompt cache: <next ?? "automatic"> (<scope> settings, unsaved)` |
| `context_window_tokens` / `max_output_tokens` | `editor.startNumber` with `min: 1` |

One verb, `f` → `fillFromCatalog`, shown only while `fillHit() !== undefined`.
`fillFromCatalog` writes, in this order: context window (if present), max output (if present),
...
equality in `packages/code/tests/component/providers-level-contracts.test.ts`:
...
`ctrl.fillModelFromCatalog` (kind-then-anything fallback, `packages/code/src/features/providers/controller.ts`), while `catalogHit`
uses `ctrl.catalogModelFor` — the model under **this provider's own name only**
(`packages/code/src/features/providers/controller.ts`). `promptCacheNote()` reads `catalogHit` (`packages/code/src/views/config/providers/model-level.tsx`) and prints four
distinct sentences, including the special case "catalog: explicit ⟩ not derived on this kind" for an
`openai-compatible` provider. Pinned at
...
(`packages/code/src/views/config/providers/model-level.tsx`).
...
`mapCell` renders the count as `—` / `N entry` / `N entries`.

The map editor has no `close`: it derives its state from the host's level depth
(`packages/code/src/ui/patterns/map-editor.tsx` states the mechanism). The Providers tests exercise this from both
sides — one escape unwinds one drill inside `body`, and a further escape returns to the provider
detail (`packages/code/tests/integration/providers-key-render.test.tsx`); escaping a
*model's* headers map returns to the model level, not the provider's.

Two edge behaviours of the shared map editor bound directly on what a Providers save actually writes:
`persist()` (`packages/code/src/ui/patterns/map-editor.tsx`) compares a `JSON.stringify` of the rebuilt root against the
current value and writes nothing when they are equal, so opening `headers`/`body` and escaping
without a change never marks the panel dirty; `persistPruned()` additionally strips
...
| 1 | pick a catalog provider | seed it, drill in, chain into `openModelPicker` (`packages/code/src/views/config/providers/list-level.tsx`) |
...
| 1' | pick manual entry | `addBlank()` with `manualBootstrapProvider = true` (`packages/code/src/views/config/providers/list-level.tsx`) |
...
at `packages/code/tests/integration/providers-key-render.test.tsx` (catalog path: writes `default_model: "alpha/m1"` plus the
...
`load()` (`packages/code/src/features/providers/controller.ts`) copies the scope's providers, reads `default_model`, clears both
pending maps, records `savedSnapshot = snapshot()`, and clears the dirty mark.

`snapshot()` serializes `{ providers...(manageDefaultModel ? { default_model } : {}) }`.
`refreshDirty()` sets dirty when the snapshot differs **or** either pending map is non-empty.
Two tests pin the pair: `packages/code/tests/unit/providers-controller.test.ts` (a value edited back to its original clears
the mark) (a staged credential keeps it dirty anyway, because a key never lands in
`settings.json`).
...
| 2 | `!validation().ok` | emit `validation_failed` with the first issue, return `"validation"`; nothing is written |
| 3 | — | `await settings.write(scope, next)` |
...
`packages/code/tests/unit/providers-controller.test.ts`. Error paths pinned (key-error, staged key retained) (source-error after keys already saved).

`keysRev` exists because `deps.keys` / `deps.code` are mutated outside Solid's reactivity; bumping it
forces `envStatusOf` and `sourceOf` to re-derive (read).
...
contributes the draft.

`resolveCatalogProvider` matches by name **and** kind first, then falls back to the first
catalog provider of the same kind whose `base_url` or `api_key_env` matches; `suggestedEnvVar`
...
`keyUsageNote` returns `"source=env - keys.json is ignored"` for `env`, nothing for
`keyfile`, and `"shell env overrides this (source=auto)"` for `auto` when the env var is set.

### 4.12 Event presentation

`ProvidersEvent` (`packages/code/src/features/providers/events.ts`) is a 7-arm union; `presentProvidersEvent` (`packages/code/src/features/providers/events.ts`) maps
...
`createMcpCapabilities(deps)` (`packages/code/src/adapters/mcp-capabilities-bridge.ts`) holds one
Solid signal, `nodes`, and two mutable maps: `registered` (per slash-command key → `{fingerprint,
off}`) and `skillAgents` (per bare skill name → the agent it runs on). It is constructed
...
lookup (consumed at `packages/code/src/views/input/autocomplete.ts`).

**`refreshOnce()`** reads `deps.client.connectionStatus()`; only when it is `"connected"` does
it `Promise.all` the client's `listTools()`/`listPrompts()`, each `.catch`-guarded by
`reportListFailure` (§2.12) so one rejecting half never sinks the other. Immediately
after that await, `if (disposed || refreshQueued) return;` discards the pass without ever
...
runs `syncPromptCommands(prompts, profiles)`.

**`refresh()`** is single-flight with trailing coalescing. A call while `refreshActive` is
set only flips `refreshQueued = true`, counts `mcp.refresh.coalesced`, and returns the *same* in-flight
promise; it never starts a second physical listing. The owning call runs a `do…while`
loop — clear `refreshQueued`, `await diagnosticAsync("mcp.refresh", refreshOnce, {slowMs:
deps.refreshSlowMs})`, repeat while `refreshQueued && !disposed` — so however many
...
fires on every call; `deps.refreshSlowMs` is the seam that shortens `diagnosticAsync`'s
pending-operation warning (`async.pending`, `core/diagnostic-events.ts`) for a test that cannot
wait out the real default — no production registration site passes it (`app/commands.tsx`
...
**`syncPromptCommands(prompts, profiles)`** walks the live prompt list and, per entry, calls
...
prompt names one, deleted otherwise; a `spec: LivePrompt` is assembled carrying only
the fields that are actually defined (`displayName`, `agent`, `plansMode` are each spread in only when
present), and `JSON.stringify(spec)` is its fingerprint. If a command is already
registered under that key with the *same* fingerprint, nothing happens — re-listing an
...
`deps.commands.skillCommand(local, spec, handler)` registers the new one. The handler
dispatches on `spec.agent`: when set, it calls `deps.effects.submitSkillRun(local, args, spec.agent)`
and never touches `getPrompt`; otherwise it awaits `deps.client.getPrompt(local,
{task: args})`, bails if `disposed` became true while awaiting, and calls
...
0`, and `plansMode` only when the spec has one.
...
For a `"downstream"` origin with a `server`, the key is `` `${server}:${local}` ``
(`promptKey`); the spec carries only `name`/`description`/`arguments`; the same
fingerprint-dedupe applies; the registered handler collects arguments through
`deps.effects.collectArgs(server, spec)` only when `spec.arguments` is non-empty, bails on `disposed`
or a `null` (cancelled) collection, then calls `deps.client.getPrompt(promptKey(server,
local), args)` and — unless disposed meanwhile — `submitPromptTurn(messages, `/${server}:${local}`)`
with no skill metadata.
...
its entry is dropped from `skillAgents` too. This is the deregistration half of the
per-refresh lifecycle the module's own TSDoc names: it "registers a slash command for each downstream
prompt and skill (deregistering ones that disappear on the next `refresh`)".

**`dispose()`** is idempotent (`if (disposed) return`). It flips `disposed`, clears
`refreshQueued`, calls every registered `off()` in **reverse** registration order inside its own
`try {} catch {}` so one throwing teardown does not skip the rest, then clears both
maps and resets `nodes` to `[]`. Because `refreshOnce`'s post-await check and
...
Three read-only levels (`packages/code/src/views/config/McpBrowser.tsx`, each `readOnly: true`). `refresh()` runs once at
construction, setting `loading` and clearing or setting `listError`.

| Depth | nav | verbs |
| --- | --- | --- |
| 0 servers | count `nodes().length`, activate "open" | `e` "where to edit" (passes the server name only for a `downstream` node), `refresh` (`ctrl+r`) |
| 1 tools+prompts | count `tools.length + prompts.length`, activate "detail" | `i` "invoke prompt" guarded by `onPromptRow`, `e` "where to edit" |
| 2 item | — | `i` "invoke" guarded by `itemKind() === "prompt"` |

`onPromptRow` requires `prompts.length > 0` in addition to `detailSel() >= tools.length`; its
...
control-plane node shows `"control-plane hidden"` instead of counts, a `shared` badge appears
when the declaration says so, and a fixed legend line is always trailing. Section
labels `backend` and `downstream (aggregated by the kernel)` are emitted inside the row renderer
 and counted separately into `contentRows` by `headerCount()`.

Tool detail renders the `schemaArgRows` table, falls back to `"no arguments"` or a raw JSON dump when
the table is empty, and then renders a live `renderToolPreview(server, tool,
synthSampleArgs(inputSchema))` block. Pinned at
`packages/code/tests/integration/mcp-browser-render.test.tsx` (sections, legend, read-only badge,
both verbs) (argument table not a JSON dump)/ (`[e]` naming)/
(invoke) (control-plane explanation) (declared-but-empty explanation) ("no
arguments").
...
(`packages/code/src/adapters/settings.ts`): `write`, `reload`, `setWorkspaceTrust` and `applyRepair` all queue on
`publicationTail`, and a failure is the caller's error while the queue continues. Pinned at
`packages/code/tests/integration/settings.test.ts` ("state-publishing settings operations share one invocation-ordered queue") ("a slow agent reload cannot overwrite a later trust refresh").

`write(scope, patch)` in order:

1. `structuredClone(patch)` and compute the sorted `keys` string for diagnostics.
2. Inside the queued operation: refuse if `corrupt(scope)` — record `settings.save.rejected` with reason `unparsable` and throw `"<scope> settings.json is invalid (…) — fix it by hand before saving"`.
3. Merge `{...current...patch}` and validate against `kernelSettingsSchema`; on failure record `reason: "invalid"` with the issue paths and throw `refusing to save invalid settings: <zodIssueSummary>`.
4. Capture `expectedRevision = sourceRevision(scope)` — after the awaits above, i.e. after the previous queued save has landed.
5. `config.updateSettings(...)`; on throw, `resyncAfterRefusedWrite` re-reads settings **and** agents and bumps the version, then **rethrows the original error**.
6. On success record `settings.save.applied` and bump the version.

`setWorkspaceTrust` re-reads the agent list alongside the settings view, because the same
verdict gates both.

`validateProviders` issues, in order: `providers` empty; per provider `name` missing /
...
accepted. The inline comment states the defect that rule fixes.
...
to `ProvidersPanel`.
Production: `packages/code/package.json` (no `exports` key at all today).
Test: `packages/code/tests/architecture/providers-panel-boundary.test.ts`.
...
Test: `packages/code/tests/component/providers-panel-surface.test.ts`.
...
Production: `packages/code/src/views/config/providers/detail-level.tsx`, `packages/code/src/views/config/providers/model-level.tsx`, `packages/code/src/views/config/providers/list-level.tsx`.
Test: `packages/code/tests/component/providers-level-contracts.test.ts` (L0 counts 0 and 2)
(the eight detail fields) (`nav.count === PROVIDER_DETAIL_FIELDS.length + 2`) (the five
model fields). additionally pins the issue-field correspondence that reads the array. Reinforced from the render side by
`packages/code/tests/integration/providers-key-render.test.tsx` ("the provider list has no
default_model editor").
...
Test: `packages/code/tests/component/providers-level-contracts.test.ts` ("L1 keeps credential/map
rows before the provider's model rows") — a provider with two models yields
...
Production: `packages/code/src/views/config/providers/model-level.tsx` (`fillFromCatalog`) (the verb and its `when`).
Test: `packages/code/tests/component/providers-level-contracts.test.ts`, assertions.
...
Production: `packages/code/src/views/config/create-view-host.ts`. Test: `packages/code/tests/unit/view-host-scope.test.ts` (asserts
`duplicate === first`).
...
Production: `packages/code/src/views/config/create-view-host.ts`, `runCancel`.
Test: `packages/code/tests/unit/view-host-scope.test.ts` (dispose runs cancel once, twice is a no-op)
(escape then dispose does not double-invoke) (dirty escape after the confirm still runs it
once).
...
Production: `packages/code/src/views/config/create-view-host.ts`.
Test: `packages/code/tests/unit/view-host-scope.test.ts`.

**INV-P4.** `view.scope.toggle` is enabled only for a view that actually bound a scope.
Production: `packages/code/src/views/overlay-host.ts` reading `controls.scopeBound()`, set at `packages/code/src/views/config/create-view-host.ts`.
...
Production: `packages/code/src/features/providers/controller.ts` gated by `deps.manageDefaultModel`, itself `bootstrap`
(`ProvidersPanel` in `packages/code/src/views/config/ProvidersPanel.tsx`).
Test: `packages/code/tests/unit/providers-controller.test.ts` asserts the written file is exactly
...
Production: `packages/code/src/features/providers/controller.ts` (`refreshDirty`), snapshot.
...
Production: the `disposed()` re-checks at `packages/code/src/features/providers/controller.ts`, and the
`createDisposeGuard` gate at `packages/code/src/features/dispose-guard.ts`.
Test: `packages/code/tests/unit/providers-controller.test.ts`.
...
Test: `packages/code/tests/unit/providers-controller.test.ts` (key-error retains the staged key)
...
Production: `load` copies each provider shallowly (`packages/code/src/features/providers/controller.ts`) and `mutate` re-copies.
Test: `packages/code/tests/unit/providers-controller.test.ts` ("T10").
...
Production: `packages/code/src/features/providers/controller.ts`; consumers `packages/code/src/views/config/providers/model-level.tsx`.
Test: `packages/code/tests/unit/providers-controller.test.ts`; render-side at
`packages/code/tests/integration/providers-key-render.test.tsx`.
...
Production: `packages/code/src/views/config/providers/model-level.tsx`.
Test: `packages/code/tests/component/providers-level-contracts.test.ts`.

**INV-P12.** Setting a model's `prompt_cache` to `undefined` deletes the key rather than storing a
sentinel — `"(auto)"` is an *absent* key.
Production: `packages/code/src/features/providers/controller.ts`. Test: `packages/code/tests/unit/providers-controller.test.ts`.

**INV-P13.** An empty `headers` / `body` map deletes its key rather than persisting `{}`.
Production: `packages/code/src/features/providers/controller.ts` (`applyMap`).
Test: `packages/code/tests/unit/providers-controller.test.ts`.

**INV-P14.** A blank API-key submission never overwrites an existing key.
Production: `packages/code/src/views/config/key-entry.ts`. Test: `packages/code/tests/unit/key-entry.test.ts`.
...
Production: `packages/code/src/views/config/field-editor.tsx` (cursor pin and mask diff) (arrow/home/end bound to
no-op).
Test: `packages/code/tests/integration/secret-input-render.test.tsx`.
...
Production: `packages/code/src/views/config/field-editor.tsx` (initial index) (`picked !== st.initialValue`).
Test: `packages/code/tests/integration/field-editor-pick-render.test.tsx`.

**INV-P17.** A text or number field commits only on change unless `alwaysCommit` is passed.
Production: `packages/code/src/views/config/field-editor.tsx`.
Test: `packages/code/tests/integration/field-editor-pick-render.test.tsx`.
...
Production: `packages/code/src/views/config/field-editor.tsx` (`GUARDS`), spread; multiline.
Test: unpinned.
...
Production: `packages/code/src/ui/patterns/bind-level-keys.ts`; providers pass
...
`packages/code/tests/integration/providers-key-render.test.tsx`, which
...
Production: `packages/code/src/features/providers/controller.ts` (`modelRemovalBlocked`), presented by
`presentModelRemovalBlock` in `packages/code/src/views/config/ProvidersPanel.tsx`.
Test: `packages/code/tests/unit/providers-controller.test.ts`; render-side
`packages/code/tests/integration/providers-key-render.test.tsx`.
...
Production: `packages/code/src/adapters/mcp-capabilities.ts`.
Test: `packages/code/tests/unit/mcp-capabilities.test.ts`.

**INV-P23.** A configured MCP server that has never connected still appears, with status `declared`.
Production: `packages/code/src/adapters/mcp-capabilities.ts`.
Test: `packages/code/tests/unit/mcp-capabilities.test.ts`; render-side `packages/code/tests/integration/mcp-browser-render.test.tsx`.
...
either way.
Production: `packages/code/src/adapters/settings.ts`.
...
Production: `packages/code/src/adapters/settings.ts`.
Test: `packages/code/tests/integration/settings.test.ts` (conflict re-syncs and the external edit stands) (a failing re-read still reports the original refusal).
...
Production: `packages/code/src/adapters/settings.ts`.
Test: `packages/code/tests/integration/settings.test.ts` cover the ordinary cases; the withheld
branch is unpinned.
...
Production: `packages/code/src/features/providers/request-params.ts` (`AUTH_HEADER`).
Test: `packages/code/tests/unit/request-params.test.ts`.

**INV-P31.** `bodyKeyProblem` guards only the root of `body`; a `messages` key nested under a
provider-owned object is accepted.
Production: `packages/code/src/features/providers/request-params.ts`. Test: `packages/code/tests/unit/request-params.test.ts`.
...
Production: `packages/code/src/views/config/McpBrowser.tsx` (`prompts.length > 0 &&` …).
Test: unpinned as such; `packages/code/tests/integration/mcp-browser-render.test.tsx` exercises the positive case only.
...
Production: `packages/code/src/adapters/mcp-capabilities-bridge.ts` (the `refreshActive`
guard and `do…while refreshQueued` loop).
Test: `packages/code/tests/component/mcp-bridge.test.ts` ("overlapping refreshes are single-flight
and coalesce into one trailing response") ("a never-settling MCP refresh does not accumulate
physical requests").
...
Production: `packages/code/src/adapters/mcp-capabilities-bridge.ts` (`dispose`), the
post-await `disposed || refreshQueued` check in `refreshOnce` and the `!disposed` loop
condition in `refresh`.
Test: `packages/code/tests/component/mcp-bridge.test.ts` ("dispose unregisters commands and
...
Production: `packages/code/src/adapters/mcp-capabilities-bridge.ts` (skill branch) (downstream branch).
Test: unpinned — `packages/code/tests/component/mcp-bridge.test.ts` ("changed metadata for the
...
| --- | --- | --- |
| No models.dev catalog when adding a provider | open the reduced picker with subscription rows and manual entry; do not pretend the subscription catalog is the public catalog | `packages/code/src/views/config/providers/list-level.tsx` |
...
| No catalog when adding models | fall straight through to manual model-id entry | `packages/code/src/views/config/providers/detail-level.tsx` |
| Catalog has no entry matching the provider | offer a "No catalog match — pick a source provider" picker whose manual row still reaches manual entry | `packages/code/src/views/config/providers/detail-level.tsx` |
| Model not in the catalog on L2 | the `f` verb is hidden (`when: () => fillHit() !== undefined`) and the body says so | `packages/code/src/views/config/providers/model-level.tsx` |
| Save blocked by validation | `save()` returns `"validation"`, emits `validation_failed` (tone `error`), writes nothing; the panel then moves the cursor to the first issue's field | `packages/code/src/features/providers/controller.ts`, the `host.onSave` handler in `packages/code/src/views/config/ProvidersPanel.tsx` |
| Secret store throws mid-save | `"key-error"`, notice `key save failed for <VAR>: <text>`, remaining keys and all sources are skipped, staged value retained | `packages/code/src/features/providers/controller.ts` |
| Key-source write throws | `"source-error"`, notice `source save failed for <VAR>: <text>`, no reconnect | `packages/code/src/features/providers/controller.ts` |
| Controller disposed mid-save | returns `"ok"` silently and stops before writing secrets or emitting | `packages/code/src/features/providers/controller.ts` |
...
| Header/body key or value rejected | the map editor's `rejectKey`/`rejectValue` returns a sentence and the entry is not staged | `packages/code/src/features/providers/request-params.ts`; render-side `packages/code/tests/integration/providers-key-render.test.tsx` |
| Blank API-key submission | notify "empty — key unchanged"; `commit` never runs | `packages/code/src/views/config/key-entry.ts` |
| Non-finite / out-of-range number | notify and keep the editor open (no `clearLayer`, no `setEditing(null)`) | `packages/code/src/views/config/field-editor.tsx` |
| MCP refresh rejects | the error is stored in `listError` and rendered as a persistent banner *in place of* the empty state | `packages/code/src/views/config/McpBrowser.tsx`; test `packages/code/tests/integration/mcp-browser-render.test.tsx` |
| MCP refresh never settles | `loading` stays true and the loading hint shows, not "no MCP servers" | `packages/code/src/views/config/McpBrowser.tsx`; test |
| MCP `listTools`/`listPrompts` rejects | `reportListFailure` logs `mcp.list.failed` and substitutes `[]`, so one failing half does not crash the refresh | `packages/code/src/adapters/mcp-capabilities-bridge.ts` |
| An `mcpServers` entry fails its schema | dropped silently from the parsed list | `packages/code/src/adapters/mcp-capabilities.ts` |
...
| Target scope's `settings.json` unparsable | `write` throws "…is invalid (…) — fix it by hand before saving" after recording `settings.save.rejected` with `reason: "unparsable"` | `packages/code/src/adapters/settings.ts` |
| Merged document fails the kernel schema | `write` throws `refusing to save invalid settings: <first two issues> (+N more)` | `packages/code/src/adapters/settings.ts`, `packages/code/src/adapters/zod-summary.ts` |
| CAS conflict / kernel refusal | `resyncAfterRefusedWrite` logs, re-reads settings + agents, bumps `version`, and rethrows the original error | `packages/code/src/adapters/settings.ts` |
| The re-read itself fails | `settings.resync.failed` is logged; the original refusal is still what the caller sees | `packages/code/src/adapters/settings.ts`; test `packages/code/tests/integration/settings.test.ts` |
| A model reference cannot be parsed | `noteUnparsedModelRef` samples a diagnostic and the caller falls back (context window → fallback; `default_model` → not known; `refs` → no match) | `packages/code/src/adapters/settings.ts`, used |
...
- `ProvidersPanel` → the three level modules (the level-factory imports in
  ...
  modules import only `context.ts` back (`packages/code/src/views/config/providers/list-level.tsx`, `packages/code/src/views/config/providers/detail-level.tsx`,
  `packages/code/src/views/config/providers/model-level.tsx`), and `context.ts` imports no level.
- Every config screen reaches the UI toolkit through `views/config/view-host.tsx`, which is the only
  file that names `ui/primitives/index.ts` and `ui/patterns/index.ts` for them (`packages/code/src/views/config/view-host.tsx`). `packages/code/src/views/config/validation.ts` plays the same role for `features/issues.ts`.
- `ProvidersPanel` is loaded and constructed by `packages/code/src/features/providers/commands.ts`; that module is the seam that
  decides `bootstrap` from the effective provider count. The panel itself never reads settings
...
- `features/providers/controller.ts` has **no** import of anything under `views/`; the view→controller
  ...
  `packages/code/tests/unit/providers-controller.test.ts`.
- `create-view-host.ts` depends on `views/confirm.ts` (`useArmedConfirm`) and on
  `core/tasks.ts`'s `detachObserved`; the shell that drives it is `packages/code/src/views/overlay-host.ts`.
- `McpBrowser` renders through `views/tools/registry.tsx`'s `renderToolPreview`
  (`packages/code/src/views/config/McpBrowser.tsx`) — the browser reuses the transcript's own renderers rather than a second set.
- `adapters/settings.ts` imports `adapters/mcp-capabilities.ts` for `parseMcpServers`
  (`packages/code/src/adapters/settings.ts`), not the reverse.
- `adapters/mcp-capabilities-bridge.ts` imports `classifyCapability`/`reconcile` and their supporting
  types from `adapters/mcp-capabilities.ts` (`packages/code/src/adapters/mcp-capabilities-bridge.ts`); the
  ...
  `packages/code/src/runtime.tsx` constructs it as `capabilities`, whose value
  `AppBackend.client`'s getter re-exposes (`packages/code/src/views/App.tsx`,
  `packages/code/src/runtime.tsx`), which `packages/code/src/views/App.tsx` passes on
  as `mcpClient` and `packages/code/src/app/commands.tsx` finally hands to
