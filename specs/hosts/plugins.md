...
`PluginView` for the operator's panel (`packages/kernel/src/plugins/plugin-service.ts:353`) and the
run-time contributions a run consumes (`packages/kernel/src/plugins/plugin-contributions.ts:74`).
...
(`packages/loop/src/settings/marketplace-schema.ts:745`). A Git install uses a shallow validated
...
`rename` into the global install root (`packages/kernel/src/adapters/git/plugin-fetcher.ts:198-334`,
`packages/kernel/src/plugins/plugin-service.ts:464-497`,
`packages/kernel/src/adapters/filesystem/plugin-repository.ts:287-300`).
...
`packages/code/src/adapters/marketplace.ts:28-32`). This changes discovery only: every plugin in that
...
anything but its hooks**" (`packages/kernel/src/plugins/plugin-manifest.ts:729-732`) — and
`readMarketplaceDocument`'s as its mirror — "Tolerance degrades one listing, never the collection"
(`packages/loop/src/settings/marketplace-schema.ts:681`). Three admitted surfaces are atomic rather
...
(`packages/kernel/src/plugins/plugin-contributions.ts:331-383`).
...
|---|---|---|
| `pluginManifestSchema` | value (**not** re-exported from `host.ts`) | `packages/loop/src/settings/plugin-schema.ts:59` |
| `PluginManifest` | type | `packages/loop/src/settings/plugin-schema.ts:108` |
| `unknownManifestKeys(document): string[]` | value | `packages/loop/src/settings/plugin-schema.ts:122` |
| `suspectedManifestTypos(document): SuspectedManifestTypo[]` | value | `packages/loop/src/settings/plugin-schema.ts:151` |
| `pluginSettingsFragment(manifest): SettingsFile` | value | `packages/loop/src/settings/plugin-schema.ts:176` |
...
| `marketplaceSchema` | value | `packages/loop/src/settings/marketplace-schema.ts:747` |
| `Marketplace`, `MarketplaceEntry` | types | `packages/loop/src/settings/marketplace-schema.ts:518`, `:484` |

The `host.ts` re-export list is `packages/loop/src/host.ts:15-40`. `pluginManifestSchema` itself is
...
|---|---|---|
| `readPluginManifestSource` | `(dir) => PluginManifestSource \| { error }` | `packages/kernel/src/plugins/plugin-manifest.ts:354` |
| `resolvePluginManifest` | `(dir, raw, manifestLocation?, effectivePluginName?, runtime?) => ResolvedPluginManifest` | `packages/kernel/src/plugins/plugin-manifest.ts:1698` |
| `pluginSkillRoots` | `(dir, declared, manifestLocation?) => { roots, notes }` | `packages/kernel/src/plugins/plugin-manifest.ts:168` |
| `hooksDocumentSchema` | zod union | `packages/kernel/src/plugins/hook-dialects.ts:175` |
| `convertHooksDocument` | `(document, pluginRoot, options?) => { hooks, notes }` | `packages/kernel/src/plugins/hook-dialects.ts:559` |
...
| `effectivePluginMcpName` | `(plugin, server) => \`${plugin}:${server}\`` | `packages/kernel/src/plugins/plugin-contributions.ts:138` |
| `createPluginService` | `(PluginServiceOptions) => PluginService` | `packages/kernel/src/plugins/plugin-service.ts:323` |
| `validateGitUrl` | `(raw) => string` (throws) | `packages/kernel/src/plugins/plugin-service.ts:70` |

`PluginContributions` (the run-time interface, `packages/kernel/src/plugins/plugin-contributions.ts:74-113`)
...
| `skillRoots` | `(selection) => SkillRootInput[]` | `:82` |
| `pinnedSkillRoots` | `(selection) => SkillRootInput[]` | `PluginContributions` |
| `skillBootstraps` | `(selection) => PluginBootstrapSkill[]` | `:92` |
| `settingsScopes` | `(selection) => SettingsScope[]` | `:94` |
| `mcpServers` | `(selection) => ResolvedPluginMcpContribution[]` | `:96` |
| `agents` | `(selection) => AgentRecord[]` | `:98` |
| `readAgent` | `(selection, qualifiedName) => AgentRecord \| null` | `:100` |
| `locateCapabilityExecutable` | `(selection, capability, plugin) => { root, declaration } \| { error }` | `:102-106` |
| `skillPlansMode` | `(selection, plugin, skill) => CapabilitySkillPlansMode \| undefined` | `:108-112` |

### 2.3 Protocol / wire

`PluginService` (`packages/protocol/src/plugins.ts:143-176`) has five methods, exact `PluginRef`/
...
`PluginInstallSource` (`:25-42`) is the normalized union consumed by `installSource`: Git carries
...
`PluginCapabilityExecutable` (`:44-50`, 4 fields):
...
`PluginContributions` (`:52-73`, 8 fields):
...
`packages/kernel/src/ports/plugin-repository.ts:62-80` and `:83-101`, over the value shapes
...
implementations are
`createFilePluginRepository` (`packages/kernel/src/adapters/filesystem/plugin-repository.ts:259`)
and `createGitPluginFetcher` (`packages/kernel/src/adapters/git/plugin-fetcher.ts:168`); both are
injectable overrides on `PluginServiceOptions` (`packages/kernel/src/plugins/plugin-service.ts:294-317`).
...
(`packages/kernel/src/plugins/plugin-service.ts:323-325`: `opts.processRunner ?? createNodeProcessRunner(logger)`),
and `createGitPluginFetcher` takes the same instance by injection rather than calling
`node:child_process` itself (§7.1). The adapter wraps `spawn` (`:71-75`, stdio `["ignore","pipe","pipe"]`,
argv-only — no shell) in a `Promise` that settles exactly once through a shared `finish` closure
(`:81-88`): a `close` event resolves with `{ exitCode, stdout, stderr }`, an `error` event and an
already-aborted or later-aborted `request.signal` both reject with a `cancellationError` (`:50-55`,
`:67-69`, `:89-92`, `:102-108`), and a `request.timeoutMs` — when given — `SIGTERM`s the child and rejects with `"<command>
timed out after <ms>ms"` (`:109-117`); the timer is `unref()`'d so a pending timeout never keeps the
...
exit logs nothing. The runner takes `NOOP_LOGGER` as its default (`:62`), so a caller that supplies no
...
| Symbol | Line |
|---|---|
| `toPluginView`, `loadPlugins`, `createPluginsStore`, `PluginsStore` | `packages/code/src/adapters/plugins.ts:74,142,153,132` |
| `OFFICIAL_MARKETPLACE_URL`, `MarketplaceListing`, `MarketplaceSource`, `MarketplaceAdapter`, `createMarketplaceAdapter`, `addMarketplaceSource` | `packages/code/src/adapters/marketplace.ts:29,35,43,50,270,375` |
| `validateGitUrl`, `gitCloneAsync` (client-side duplicates) | `packages/code/src/adapters/plugin-install.ts:78,141` |
| `MarketplaceBrowser` | `packages/code/src/views/config/MarketplaceBrowser.tsx:151` |
...
(`platform_override`→`platformOverride`, `:111-116`). `capability_run_policies?.plans` is **not**
carried across verbatim: it is flattened from `{ skills: Record<name, mode> }` into a sorted
`skillPlanPolicies: { skill, mode }[]` array (`Object.entries(...).map(...).sort(...)`, `:118-123`),
...
| `<plugin>/plugin.json` | root manifest candidate | `packages/kernel/src/plugins/plugin-manifest.ts:32` |
| `<plugin>/.clarvis-plugin/plugin.json` | authoritative Clarvis manifest when present | `:37`, `:375-382` |
| `<plugin>/.<host>-plugin/plugin.json` | another host's candidate, matched by regex `^\.[A-Za-z0-9_-]+-plugin$` | `:49` |
| `<plugin>/.mcp.json` | first conventional MCP companion | `:107` |
| `<plugin>/mcp.json` | fallback conventional MCP companion | `:107` |
| `<plugin>/hooks/hooks.json` | conventional hooks document | `:104` |
| `<plugin>/skills/` | default skills root | `:110` |
...
`pluginManifestSchema` (`packages/loop/src/settings/plugin-schema.ts:49`) is `.loose()` (`:99`).

| Key | Shape | Line |
|---|---|---|
| `name` | `pluginNameField` — **the only required key** | `:51` |
| `version` | non-empty string, optional | `pluginManifestSchema` in `packages/loop/src/settings/plugin-schema.ts` |
| `description` | non-empty string, optional | `:60` |
...
`pluginSkillRoots` (`packages/kernel/src/plugins/plugin-manifest.ts:1775`) and then excluded from the
"keys Clarvis does not act on" note (`:1777-1782`).
...
(`packages/code/src/adapters/marketplace.ts:26`).
...
(`packages/kernel/src/plugins/plugin-contributions.ts:204`, `MAX_SKILL_ROOTS = 32` at
...
(`packages/kernel/src/plugins/plugin-contributions.ts:188-204`). The reserve is double the four
...
(`packages/loop/src/settings/marketplace-schema.ts:747`), so **the only refusal is a non-object**
...
(`packages/loop/src/settings/marketplace-schema.ts:195-200`). Listing keys acted on: `name`, `source`,
...
3. Otherwise the root `plugin.json` and every name-sorted `.<host>-plugin/plugin.json` candidate are
   ...
   `capabilityRunPolicies` (`MANIFEST_CONTRIBUTION_KEYS` and `manifestContributionScore`, `:277-311`).
   Root-then-sorted-host order breaks ties, and documents are never merged (`:314-330`, `:403-413`).
...
document to hide it (`:354-402`). With no candidate, the error names every searched location
(`:415-417`). The discovery scan is bounded at
`manifestLocationEntries`; overflow returns an error instead of a partial list (`:61-100`).
...
that describe a mistake rather than a difference" (`:560-562`); production order is `:1770-1783`,
pinned at `packages/kernel/tests/integration/plugin-manifest.test.ts:407-413`, which asserts `notes[0]` is the "did you mean" line and that the
...
`companionPath` (`packages/kernel/src/plugins/plugin-manifest.ts:439-448`) resolves a declared path
against `dirs.base` first — but only if the result both stays inside `dirs.root` **and already
exists** (`:443-447`) — otherwise against `dirs.root`. `confined` compares the *resolved* path
against `root` or `root + sep` (`:439-447`), so `a/../../b` is refused on the same rule as `../b` and
an absolute path never escapes the root.

`pluginDirsFor` (`:480-486`) sets `base` to the manifest's own directory, but falls back to `root`
...
workspace-confined writes" (`:434-437`).

Pinned: `packages/kernel/tests/integration/plugin-manifest.test.ts:1676-1738` (four refusal cases + one accepted nested case),
`:1773-1785` (a `.alpha-plugin` manifest's `../skills/` resolves to the plugin's `skills/` with **no**
"outside the plugin" note), `:1803-1806` (`../../elsewhere` is still refused from either base).
...
`sanitizeMcpServers` (`packages/kernel/src/plugins/plugin-manifest.ts:1061-1084`) then validates each
entry against `mcpServerPluginSchema` individually, keeps the ones that pass, notes each one that
does not, and deletes the key entirely if nothing survives (`:1065-1083`). Its docstring names the
blast radius it exists to bound: the schema "reports that per *record*, so one such entry used to
fail the whole manifest and take the plugin's agents, hooks and skills with it" (`:1043-1059`). Pinned:
`packages/kernel/tests/integration/plugin-manifest.test.ts:1012-1063`.
...
(`packages/loop/src/settings/settings-schema.ts:334` vs `:358-360`). Its docstring records the measured
cost of strictness: "Measured on a public catalog of 196 plugins, that single rule broke 24 of them
and cost 82 skills that had nothing to do with MCP" (`:336-357`). That the two schemas genuinely
differ is pinned at `packages/kernel/tests/integration/plugin-manifest.test.ts:1052-1063`. Unknown keys are dropped, not carried
(`packages/loop/src/settings/settings-schema.ts:353-360`).

### 4.6 `hooks` — source selection and translation

`resolveHooks` (`packages/kernel/src/plugins/plugin-manifest.ts:746-795`) runs `harvestDeclared`
first, then `harvestConvention` only if the first yielded **zero** hooks.

| `document.hooks` | Read as | Line |
|---|---|---|
| non-empty array of all strings | a list of files to read and concatenate | `:678-685` |
| any other array | Clarvis's own hook array, left for the schema | `:686` |
| a string | one named file | `:689` |
| an object | an inline event map | `:691-693` |
| anything else | nothing | `:694` |

State table for source selection:

| `harvestDeclared` result | Convention file present? | Outcome | Note emitted | Line |
|---|---|---|---|---|
| ≥1 hook | yes, and not the file the manifest named | manifest wins | "`hooks/hooks.json` not read — the manifest declares its own hooks, which take precedence" | `:769-780` |
| ≥1 hook | yes, and *is* the file the manifest named | manifest wins | none | `:771-774` via `isConventionPath` |
| ≥1 hook | no | manifest wins | none | `:769-780` |
| 0 hooks, key present | yields ≥1 | convention wins | "the manifest declares none, so `hooks/hooks.json` was read instead" | `:783-790` |
| 0 hooks, key absent | yields ≥1 | convention wins | none | `:783-790` |
| 0 hooks | 0 hooks | key deleted | any harvest notes | `:793-794` |

The "empty declaration falls through" rule is stated with its trigger: "a real plugin was found
carrying exactly that (`"hooks": {}`) while its commands lived in the convention file" (`:740-744`).
Pinned for both `{}` and `[]` at `packages/kernel/tests/integration/plugin-manifest.test.ts:1323-1336`.

The convention file is looked for **beside the manifest first** when the manifest lives in a
dot-directory (`conventionHooksPath`, `:501-507`), pinned at
`packages/kernel/tests/integration/plugin-manifest.test.ts:1787-1807`. A read, parse or conversion
failure in either source returns only empty hooks plus notes, so the rest of the plugin remains
loadable (`harvestDocument`, `harvestFile`, and `harvestConvention`, `:588-654`, `:698-717`).

#### 4.6.1 `convertHooksDocument`

`packages/kernel/src/plugins/hook-dialects.ts:559-673`. Per event key:
...
1. Normalize the event name (strip non-alphanumerics, lowercase — `:66-68`) and look it up in
   ...
   `EXTERNAL_HOOK_EVENT_NAMES` (`:52-54`). An unmapped name yields a note and no hooks (`:567-572`).
   The round trip over every entry of the table is pinned at
   `packages/kernel/tests/integration/plugin-manifest.test.ts:1554-1568`.
2. If the target event is in `OBSERVER_HOOK_EVENTS` and any group carries commands, note that the
   commands run but "a verdict of theirs cannot block anything" (`:573-585`).
3. Per group: if the event is `pre_tool_use`/`post_tool_use` (`TOOL_SCOPED`, `:110`), translate the
   matcher; otherwise no filter, plus a note if a matcher was written anyway (`:586-613`).
4. Per entry: accept `"command"`/omitted or `"mcp_tool"`; skip a command without `command`, an MCP
   entry without `server`/`tool`, and an MCP entry on `SessionEnd` (`:614-630`). Convert `timeout`
   ...
   `server`/`tool`/`input` fields (`:632-667`). Pinned by the complete projection cases at
...
`packages/kernel/tests/integration/plugin-manifest.test.ts:1480-1494`.
...
refusal **took the entire plugin down**" (`:87-90`). Pinned at `packages/kernel/tests/integration/plugin-manifest.test.ts:1570-1605`,
where the second test asserts every surviving hook satisfies `hookSchema`.

#### 4.6.2 Matcher translation

`translateMatcher` (`packages/kernel/src/plugins/hook-dialects.ts:508-534`) strips `^`/`$` from the
**whole** matcher first, then tests the catch-all (`:512-513`). A catch-all alternative still refuses
the group because keeping it would widen the filter (`:498-506`, `:520`). An inexpressible or
counterpart-free alternative is dropped when another exact alternative survives; only a matcher
with no survivors is refused (`:516-533`). Pinned at
`packages/kernel/tests/integration/plugin-manifest.test.ts:1207-1230`, `:1237-1265`.

`translateToolName` (`:418-471`), in order:

| Input shape | Result | Line |
|---|---|---|
| `mcp__<server>__<tool>` (plain halves) | `<server>.<tool>`, or `<plugin>:<server>.<tool>` when this plugin owns the server | `:433-449` |
| `mcp__.*` or `mcp__*` | `*.*` | `:422-424` |
| `mcp__<server>__.*` | dotted server wildcard, qualified when plugin-owned | `:433-449` |
| `mcp__plugin_.*<server>.*` | force-qualified `<plugin>:<server>.*` | `:425-431` |
| `mcp__<server-prefix>.*` | `<server-prefix>*`, qualified when it identifies a plugin-owned server | `:451-458` |
| any other `mcp__…` | `inexpressible` | `:459` |
| plain `^[A-Za-z0-9_-]+$` in `EXTERNAL_TOOLS_WITHOUT_COUNTERPART` | `no_counterpart` | `:462-465` |
| plain name | `EXTERNAL_TOOL_NAMES[normalized] ?? part` | `:462-465` |
| `<a>.<b>` (already namespaced) | passes through | `:468` |
| anything else | `inexpressible` | `:470` |
...
would normalize to `edit` and "silently become the single exact name `edit_file`" (`:404-416`).
...
on, which on a gate event turns a narrow rule into one that judges every call" (`:482-486`).
...
`packages/kernel/src/plugins/hook-dialects.ts:270-319`. A plugin root is any variable name matching `^[A-Za-z0-9_]*PLUGIN_ROOT$`
(`:192`); the bare form uses a negative lookahead so `$PLUGIN_ROOTS` and `$MY_PLUGIN_ROOT_DIR` are
left alone (`:202`, pinned `packages/kernel/tests/integration/plugin-manifest.test.ts:1090-1133`).
...
function missed was the difference between a gate and nothing at all" (`:242-247`). The supported
substitutions and deliberately preserved forms in the table are pinned at
`packages/kernel/tests/integration/plugin-manifest.test.ts:1090-1133`.
...
ordinary Clarvis hook array (`packages/kernel/src/plugins/hook-dialects.ts:634-667,683-700`). Hook
execution still keeps the workspace as its working directory (`packages/hooks/src/runner.ts:504-529`),
...
| 2 | read the manifest source | `manifest` | `:339-343` |
| 3 | create the persistent source-qualified runtime-data directory | — | `:344-348` |
| 4 | resolve the manifest with that runtime directory | `manifest` | `:349-353` |
| 5 | read the whole `agents/` surface | `agents` | `:355-359` |
| 6 | read the install record | `install_record` | `:360-364` |
...
agents, hooks, MCP servers and skills are all absent" (`:240`). The docstring records what it
replaced: "a manifest that did not parse, an agents tree over its limit and an absent install were
all the same silent `undefined`" (`:221-224`). The manifest/dir/skills diagnostic cases and the
...
- **`skillRoots`** (`:606-666`) filters declared roots to those that `statSync` says are directories,
  ...
  `"user"` (`:662`); `source` is `plugin:<name>` (`:663`). It also carries the host approval marker
...
- **`mcpServers`** (`:694-697`) attaches `pluginVersion` (from the manifest) and `resolvedRevision`
  (from the install record, or `git rev-parse HEAD` as a fallback for unmanaged plugins —
  `revisionOf`, `:300-314`).
- **`agents`** (`:699-706`) parses each file's frontmatter leniently and names it `<plugin>:<agent>`
  with `scope: "plugin"` (`toAgentRecord`).
- **`readAgent`** (`:708-721`) requires the plugin to be in `enabled` and matches the file by exact
  `<agent>.md` name.
- **`locateCapabilityExecutable`** (`:723-736`) checks enablement, then installation, then the
  manifest, then the named capability — four distinct error strings.
- **`skillPlansMode`** (`:738-744`) returns `manifest.capabilityRunPolicies?.plans?.skills[skill]`
...
(`packages/loop/src/runtime/capabilities/hooks.ts:75-83`; limit at
`packages/capability/src/hooks-config.ts:251`).

`pluginSettingsFragment` (`packages/loop/src/settings/plugin-schema.ts:176-186`) carries `mcpServers`
...
(`packages/loop/src/runtime/capabilities/hooks.ts:75-84`). The built-in `agentTools`, `sandbox` and
...
1. `checkedInstallTarget(target)` and `validateGitUrl(url)` (`:518-521`).
2. `installPrepared` registers an `AbortController` with the kernel lifecycle (`:464-470`).
3. `fetcher.fetch(safe, subdir, signal)` — `mkdtemp` under the global dir, `git clone --depth 1
   ...
   subdir (`packages/kernel/src/adapters/git/plugin-fetcher.ts:198-251`).
4. `repository.inspect(prepared.root)` then `readManifest`; no manifest or a fallback-only name on a
   ...
   (`packages/kernel/src/plugins/plugin-service.ts:473-488`).
...
6. `finally`: `prepared.dispose()` (removes the staging tree) and release the lifecycle handle
   (`packages/kernel/src/plugins/plugin-service.ts:493-495`).
...
in place and no `PreparedPlugin` is returned (`packages/kernel/src/adapters/git/plugin-fetcher.ts:352-355`); when it is not a checkout
but has a recorded Git `source`, the plugin is re-fetched and atomically **replaced** (`:342-350`,
`packages/kernel/src/adapters/filesystem/plugin-repository.ts`); when it is neither, `invalid_request` `'<name>' was not installed
from git` (`packages/kernel/src/adapters/git/plugin-fetcher.ts:348`). An update whose new manifest names a different plugin is refused
(`packages/kernel/src/plugins/plugin-service.ts:610-647`).
...
checkout is untouched" (`packages/kernel/src/adapters/git/plugin-fetcher.ts:70-79`). For a failed `clone` this is trivially true (nothing
...
(`packages/kernel/src/plugins/plugin-service.ts:70-75`, `packages/loop/src/settings/marketplace-schema.ts:90-113`),
...
`skillNamesOf` (`:227-267`) runs the **same** `createAgentSkills` scan a run would, over the same
...
(`:210-215`), and that silencing warnings made a plugin shipping twenty skills read as "18 skills"
(`:221-225`). At most `MAX_SKILL_REJECTION_NOTES = 3` rejections are named, then a count (`:169`,
`:257-261`).

`partitionAgents` (`:109-119`) splits agent files into `agents` and `broken_agents` by whether the
leniently-parsed frontmatter satisfies `agentFrontmatterSchema` (`:101-105`). `executablesOf`
(`:124-146`) renders one `$ ` line per hook command, per MCP server (stdio argv or URL) and per
...
`capabilityExecutablesOf` (`packages/kernel/src/plugins/plugin-service.ts:149-163`) is the machine-readable twin of the
...
(`.sort((a, b) => a.capability.localeCompare(b.capability))`, `:162`), independent of manifest
authoring order. `executablesOf` performs the identical override selection for its `$` lines
(`:137-144`) rather than reusing `capabilityExecutablesOf`'s result. `contributionsOf` (`:272-290`)
...
pinned directly: `packages/kernel/tests/integration/plugin-service.test.ts:130-181`, "projects every
...
`createMarketplaceAdapter` (`packages/code/src/adapters/marketplace.ts:270-335`):

- Sources are `[...defaults, ...deps.urls(), ...discovered]`, de-duplicated in first-seen order
  (`:276-281`). `defaults` is the official catalog unless a test supplies an override; `discovered`
  comes from `discoverAgentsCatalogs` (`:254-256`), which reads `agentsMarketplaceFiles()` in
...
- `read(id)` dispatches on `isAgentsMarketplaceFile(id)`: a local document is read in place, anything
  else is cloned (`:283-292`).
- `fetchMarketplace` (`:221-244`) validates the URL, `mkdtemp` in the OS temp dir, `gitCloneAsync`,
  then takes the **first** existing document of `[<root>/marketplace.json,
  <root>/.agents/plugins/marketplace.json]` (`documentsIn`, `:65-67`), and always removes the
  checkout in `finally`.
- `load()` skips a source that already has a cached `marketplace`, but retries one that errored
  (`:317-328`); `refresh()` clears both the cache and the discovered list (`:330-333`). Pinned at
  `packages/code/tests/integration/marketplace.test.ts:237-251`.
- `listings()` de-duplicates on `url \0 name`, so two marketplaces offering the same plugin name both
  appear (`:296-315`, pinned `packages/code/tests/integration/marketplace.test.ts:253-278`), and sorts by name (`:315`).
...
`packages/code/tests/integration/marketplace.test.ts:399-464`, including an `ELOOP` symlink cycle asserting exactly one diagnostic.

### 4.14 Marketplace document reading (`readMarketplaceDocument`)

`packages/loop/src/settings/marketplace-schema.ts:685-728`. Root: `name` (defaulted + noted),
...
`readEntry` (`:552-650`) drops a listing for exactly two reasons — no usable `name` (`:559-562`) and
an unresolvable `source` (`:565-568`). Everything else is supplied and noted: "dropping a whole
...
still gets the bare default (`:609-618`). Pinned:
...
`readSource` (`:364-475`) state table:
...
(`:573-581`), pinned at `packages/code/tests/integration/marketplace-schema.test.ts:93-110` and `:351-363`.

`authoredTextVia` (`:319-336`) looks for the first non-empty string under a list of key spellings,
first at the top level, then inside any of `interface`/`presentation`/`display`/`metadata`/`meta`
(`:291-298`), and returns the key it read from. The two spelling lists are load-bearing constants:
`SUMMARY_KEYS = ["description", "shortDescription", "short_description", "summary", "tagline"]`
(`:282-289`) and `DISPLAY_NAME_KEYS = ["displayName", "display_name", "title"]` (`:301`). `actedKeys` (`:659-665`) widens the known-key set by
whatever was actually consumed, so a key a value was read out of stops being reported as unacted-on —
the docstring calls the alternative "a note that contradicts the behaviour beside it" (`:314-317`).
...
1. **The install directory is the host-owned runtime identity.** A native Git install creates that
   ...
   Production: `packages/kernel/src/plugins/plugin-service.ts:464-497`, `:610-647`;
   `packages/kernel/src/plugins/plugin-contributions.ts:279-313`, `:331-383`;
   `packages/kernel/src/plugins/plugin-manifest.ts:1142-1202`, `:1215-1267`. Test:
   `packages/kernel/tests/integration/plugin-manifest.test.ts:773-794` and
   `packages/kernel/tests/integration/plugin-service.test.ts:431-456`, `:474-484`.

2. **`name` is the only required manifest key.** `packages/loop/src/settings/plugin-schema.ts:50-74`.
...
5. **`hooks` is the only `pluginContributable` built-in settings block.**
   `packages/loop/src/runtime/capabilities/hooks.ts:75-84`; every other spec in the repository declares
   `pluginContributable: false`. Consumed by `pluginSettingsFragment`
   (`packages/loop/src/settings/plugin-schema.ts:175-179`). Pinned indirectly:
   `packages/loop/tests/unit/plugin-schema.test.ts:163-180` asserts the fragment carries only
...
7. **`bootstrapSkill` never reaches merged settings.** It is not `pluginContributable` and the
   ...
   field at `:39-41`). Pinned: `packages/loop/tests/unit/plugin-schema.test.ts:178-180`.
...
8. **A plugin name is lowercase, starts and ends with an alphanumeric character, uses only `.`, `_`
   ...
   `packages/loop/src/settings/settings-schema.ts:112-131`. Refusal cases are pinned by
   `packages/loop/tests/unit/plugin-schema.test.ts:31-35` and, for marketplace listings,
   `packages/loop/tests/unit/marketplace-schema.test.ts:208-213`; the positive dotted-name surface is
...
9. **A Clarvis-specific manifest is authoritative; otherwise exactly one richest compatible
   ...
   `packages/kernel/src/plugins/plugin-manifest.ts:293-418`). Pinned:
...
10. **Outside the higher-priority Clarvis-specific override, a manifest candidate that exists but
    ...
    `packages/kernel/src/plugins/plugin-manifest.ts:354-402`. Pinned by `does not let an unusable root
...
11. **Both readers of a manifest go through `resolvePluginManifest`.** The install/panel path
    (`packages/kernel/src/plugins/plugin-service.ts:81-99`) and the run path
    (`packages/kernel/src/plugins/plugin-contributions.ts:331-365`). Unpinned as a structural rule; the
...
12. **Every path a manifest names is confined to the plugin root, decided on the *resolved* path.**
    `companionPath` (`packages/kernel/src/plugins/plugin-manifest.ts:439-448`). Pinned:
    `packages/kernel/tests/integration/plugin-manifest.test.ts:1358-1404` (relative climb, climb through a subdirectory, absolute path,
    and an accepted nested path).

13. **A relative path in a dot-directory manifest resolves from that directory first, and still
    cannot escape the root.** `pluginDirsFor` (`:480-486`) + `companionPath`'s two-base attempt
    (`:439-448`). Pinned: `packages/kernel/tests/integration/plugin-manifest.test.ts:1439-1491`.
...
14. **No hooks source can cost a plugin anything but its hooks.** Every failure path in
    ...
    (`packages/kernel/src/plugins/plugin-manifest.ts:588-717`). Pinned by the declared/conventional
...
15. **One hooks source wins outright; two are never merged, and the loser is named.**
    `resolveHooks` (`:746-795`). Pinned by the `precedence — one source wins, the two are never
...
16. **An empty `hooks` declaration (`{}` or `[]`) falls through to the convention file.**
    `:769` tests `fromManifest.hooks.length > 0` before the convention fallback at `:783`. Pinned by
...
17. **A declared or conventional `mcpServers` document that cannot be used costs only the
    ...
    (`packages/kernel/src/plugins/plugin-manifest.ts:823-906`). Pinned by the companion-document
...
18. **A companion is never inlined if doing so pushes the manifest past `manifestBytes`.**
    `packages/kernel/src/plugins/plugin-manifest.ts:883-885`. Pinned by `withholds servers that would
...
19. **An unusable MCP server entry is dropped alone; the key is removed only if nothing survives.**
    `sanitizeMcpServers` (`packages/kernel/src/plugins/plugin-manifest.ts:1061-1084`). Pinned:
    `packages/kernel/tests/integration/plugin-manifest.test.ts:1024-1049`.

20. **`mcpServerPluginSchema` is tolerant and `mcpServerSettingsSchema` is strict; the difference is
    load-bearing.** `packages/loop/src/settings/settings-schema.ts:334` vs `:358-360`. Pinned:
    `packages/kernel/tests/integration/plugin-manifest.test.ts:1052-1063`.
...
21. **A `skills` declaration never leaves a plugin with nowhere to look.** Every failure path in
    ...
    (`packages/kernel/src/plugins/plugin-manifest.ts:168-227`). Pinned by the foreign-manifest skill
...
23. **Presentation metadata is display data only and never widens what a plugin may do.**
    `PluginPresentation`'s docstring (`packages/kernel/src/plugins/plugin-manifest.ts:509-541`); the same statement is repeated on
    the wire DTO (`packages/protocol/src/plugins.ts:100-106`). Nothing in
...
24. **Suspected misspellings are reported ahead of merely-foreign keys, and a key reported as a
    misspelling is not also reported as foreign.** `packages/kernel/src/plugins/plugin-manifest.ts:1770-1783`. Pinned:
    `packages/kernel/tests/integration/plugin-manifest.test.ts:407-413`.
...
25. **A short key gets a tighter typo budget than a long one.** `typoBudget` returns 1 for ≤4
    ...
    `packages/loop/tests/unit/plugin-schema.test.ts:94-102`.

26. **An external hook event with no Clarvis counterpart is reported, never approximated.**
    `packages/kernel/src/plugins/hook-dialects.ts:567-572`; the event table is inverted from `EXTERNAL_HOOK_EVENT_NAMES` rather
    than written out (`:52-54`). Pinned: `packages/kernel/tests/integration/plugin-manifest.test.ts:1496-1503` (`Notification`) and
    `:1554-1568` (every named event round-trips).

27. **The external-event correspondence is one-to-one, so inverting it loses nothing.** Pinned
    directly: `packages/kernel/tests/integration/plugin-manifest.test.ts:1554-1568`.

28. **A hook whose Clarvis event cannot act on its verdict is noted.** `OBSERVER_ONLY` check at
    `packages/kernel/src/plugins/hook-dialects.ts:573-585`. Pinned:
    `packages/kernel/tests/integration/plugin-manifest.test.ts:1505-1512`.

29. **An entirely untranslatable matcher drops the group; a mixed matcher keeps its exact surviving
    alternatives and reports the rest.** `packages/kernel/src/plugins/hook-dialects.ts:508-534`.
    Pinned: `packages/kernel/tests/integration/plugin-manifest.test.ts:1207-1265`.

30. **A catch-all matcher means no filter; a catch-all inside an alternation is refused.**
    `packages/kernel/src/plugins/hook-dialects.ts:512-520`. Pinned:
    `packages/kernel/tests/integration/plugin-manifest.test.ts:1255-1284`.

31. **The foreign tool-alias table applies only to a name carrying no pattern syntax.**
    `packages/kernel/src/plugins/hook-dialects.ts:422-465` gates the alias branch on
...
33. **A timeout past the ceiling is clamped, never emitted as a schema-invalid hook.**
    `translateTimeout` (`packages/kernel/src/plugins/hook-dialects.ts:94-107`). Pinned: `packages/kernel/tests/integration/plugin-manifest.test.ts:1570-1605`, the
...
34. **A plugin-root placeholder is expanded, not pattern-matched, and a suffix-collision is left
    alone.** `substituteRoot` (`packages/kernel/src/plugins/hook-dialects.ts:270-319`), `BARE_PLUGIN_ROOT`'s lookahead (`:202`).
    Pinned: `packages/kernel/tests/integration/plugin-manifest.test.ts:1090-1133`.
...
40. **An unusable `agents/` surface or install record omits the plugin's entire executable surface.**
    `loadableOf` returns `undefined` at both steps (`packages/kernel/src/plugins/plugin-contributions.ts:331-380`), and the
    agent walk is explicitly atomic — "callers must never execute a prefix returned before the
    failure" (`packages/loop/src/settings/plugin-agents.ts:26-28`). Pinned by `omits every executable
...
49. **A plugin's MCP servers are namespaced `<plugin>:<server>` by one host-owned function.**
    ...
    `mcpServers` (`:694-697`) and `contributionsOf` (`packages/kernel/src/plugins/plugin-service.ts:272-290`). Pinned:
...
50. **Plugin MCP provenance carries version and resolved revision, never the manifest version alone
    ...
    (`packages/kernel/src/plugins/plugin-contributions.ts:300-314`). Consumed as the Tasks provider's plugin identity
    (`packages/kernel/src/tasks/task-provider-factory.ts:214-221`). Pinned by `qualifies same-named MCP
    ...
    **This precedence is the reverse of the panel's.** `revisionOf` above is
    ...
    (`packages/kernel/src/adapters/filesystem/plugin-repository.ts:143-160`), so live Git state wins
...
54. **A subdir must resolve inside the checkout and be a directory.** `pluginRoot`
    (`packages/kernel/src/adapters/git/plugin-fetcher.ts:83-97`). Pinned by the accepted/refused subdir
    cases in `packages/kernel/tests/integration/plugin-service.test.ts`.

55. **A prepared staging tree is always disposed, on success and on failure.** The shared install
    `finally` and update `finally` are at `packages/kernel/src/plugins/plugin-service.ts:493-495` and
    `:638-640`; the Git/local/npm acquisition catches remove their own incomplete staging trees at
    `packages/kernel/src/adapters/git/plugin-fetcher.ts:248-250`, `:274-276`, and `:330-332`.
...
56. **Plugin Git selects its own repository and cannot inherit repository-local routing from the
    ...
    `GIT_CONFIG_NOSYSTEM=1` for network operations (`packages/kernel/src/adapters/git/plugin-fetcher.ts:168-189`,
    mirrored client-side at `packages/code/src/adapters/plugin-install.ts:9-15`, `:117-121`; metadata at
    `packages/kernel/src/adapters/filesystem/plugin-repository.ts:52-59`; unmanaged revision at
    `packages/kernel/src/plugins/plugin-contributions.ts:300-314`). The fetch/clone timeout is 120 s
    (`packages/kernel/src/adapters/git/plugin-fetcher.ts:24`, `packages/code/src/adapters/plugin-install.ts:5`).
...
57. **A git remote is reduced to its host before anything is logged.** `repoHost`
    (`packages/kernel/src/adapters/git/plugin-fetcher.ts:52-59`) and `reportGitFailure` (`:70-79`); the docstring notes
    `https://user:token@host/repo` is a supported form that redaction of key-*shaped* strings does not
    cover (`:47-50`). Unpinned.
...
59. **`validateGitUrl` refuses a flag-shaped URL, `ext::`, cleartext `http`/`git`, and a local path.**
    `packages/kernel/src/plugins/plugin-service.ts:70-75`, delegating to
    `packages/loop/src/settings/marketplace-schema.ts:90-113`. Pinned twice:
    `packages/kernel/tests/integration/plugin-service.test.ts:253-269` and `packages/code/tests/integration/plugin-install.test.ts:13-45`.
...
60. **An install-root fanout past `installRootEntries` throws rather than returning a partial
    ...
    (`packages/kernel/src/adapters/filesystem/plugin-repository.ts:76-122`, `:235-240`). Pinned by
...
61. **A marketplace document is refused only when it is not an object.**
    `packages/loop/src/settings/marketplace-schema.ts:747`. Pinned:
...
62. **A listing is dropped only for a missing name or an unresolvable source; every other missing
    field is supplied and noted.** `readEntry` (`packages/loop/src/settings/marketplace-schema.ts:552-568` versus `:573-649`).
...
68. **An oversized catalog is truncated with a note, never refused.**
    `packages/loop/src/settings/marketplace-schema.ts:705-710`. Pinned:
...
69. **A source's own `marketplace.json` outranks the `.agents` one it also publishes.** `documentsIn`
    order (`packages/code/src/adapters/marketplace.ts:65-67`). Pinned:
    `packages/code/tests/integration/marketplace.test.ts:358-377`.

70. **A local source whose containment cannot be decided is treated as escaping, and the failure is
    logged.** `staysInside` (`packages/code/src/adapters/marketplace.ts:111-121`) + `reportContainmentUnknown` (`:132-137`).
    Pinned: `packages/code/tests/integration/marketplace.test.ts:399-464`.
...
71. **The panel lists exactly the skills a run would serve, by running the same catalog scan.**
    ...
    (`packages/kernel/src/plugins/plugin-service.ts:227-267`).
...
74. **`load()` caches a success and retries an error; `refresh()` clears everything.**
    `packages/code/src/adapters/marketplace.ts:317-333`. Pinned:
    `packages/code/tests/integration/marketplace.test.ts:237-251`.

75. **`addMarketplaceSource` writes to the `global` scope only for a new additional URL, and writes
    nothing for a duplicate.** `packages/code/src/adapters/marketplace.ts:375-388`. Pinned:
    `packages/code/tests/integration/marketplace.test.ts:480-507`.
...
76. **The official marketplace is a built-in first source, is de-duplicated against settings, and is
    ...
    `every` (`packages/code/src/adapters/marketplace.ts:28-32`, `:281`), plus the official-source
    refusal in `addMarketplaceSource` (`:380-384`). Pinned:
    `packages/code/tests/integration/marketplace.test.ts:74-82`, `:509-515`.
...
78. **A plugin-owned MCP hook matcher uses the same host-owned install namespace as runtime
    ...
    (`packages/kernel/src/plugins/plugin-service.ts:81-99`,
    `packages/kernel/src/plugins/plugin-contributions.ts:331-365`); MCP resolution precedes hook
    conversion (`packages/kernel/src/plugins/plugin-manifest.ts:1734-1762`), and translation qualifies
    only an owned server (`packages/kernel/src/plugins/hook-dialects.ts:351-383`, `:418-471`). Pinned:
    `packages/kernel/tests/integration/plugin-manifest.test.ts:1165-1205`.
...
79. **External `UserPromptExpansion` and `Skill` names map to exact Clarvis concepts, never an
    ...
    it (`packages/kernel/src/plugins/hook-dialects.ts:38-76`, `:462-465`). Pinned:
    `packages/kernel/tests/integration/plugin-manifest.test.ts:1480-1494`.
...
80. **A translated external MCP hook receives the external server/tool spelling on stdin without
    ...
    (`packages/hooks/src/event-serialization.ts:110-124`, `:230-237`). Pinned by
    `packages/hooks/tests/component/capability.test.ts:473-495`.
...
81. **A leading relative executable in a translated external hook is anchored to the plugin root;
    ...
    `packages/kernel/src/plugins/hook-dialects.ts:634-667,683-700`; test:
    `packages/kernel/tests/integration/plugin-manifest.test.ts:1080-1087,1354-1379,1815-1833`.
...
|---|---|---|
| raw text over `manifestBytes` | "plugin manifest exceeds the …-byte resource limit" | `:1705-1709` |
| not JSON | "invalid JSON: …" | `:1711-1715` |
| JSON but not an object | "a plugin manifest must be a JSON object" | `:1717-1718` |
...
`loadableOf` (`packages/kernel/src/plugins/plugin-contributions.ts:331-383`) drops the plugin
...
`kernel.plugin.skipped { plugin, scope, source, phase, cause }` at `warn` (`:214-241`). A plugin whose skill
roots do not exist, or whose skill-root budget is spent, is reported at `phase: "skills"` and
contributes no roots — but still contributes agents, hooks and MCP servers (`:606-666`).
...
| `resource_exhausted` | install-root fanout | `packages/kernel/src/adapters/filesystem/plugin-repository.ts:235-240` |
| plain `Error` | `validateGitUrl` refusals; `git <cmd> failed: …` | `packages/kernel/src/plugins/plugin-service.ts:70-75`; `packages/kernel/src/adapters/git/plugin-fetcher.ts:100-103` |

### 6.4 Timeouts, retries, cancellation

- Git and npm acquisition subprocesses: 120 s (`packages/kernel/src/adapters/git/plugin-fetcher.ts:24-25`; client-side Git at `packages/code/src/adapters/plugin-install.ts:5`). The client-side variant
...
- Synchronous `git` reads inside `revisionOf` and `gitValue`: 5 s, output capped at
  `installRecordBytes` (`packages/kernel/src/plugins/plugin-contributions.ts:300-314`, `packages/kernel/src/adapters/filesystem/plugin-repository.ts:52-59`). Both treat
  any non-zero exit as "no value".
- An in-flight `install`/`update` registers an `AbortController` with the kernel lifecycle and is
  aborted on `close()` (`packages/kernel/src/plugins/plugin-service.ts:469-495`, `:620-640`). Pinned by
...
- **There is no retry anywhere in this subsystem.** A failed clone, fetch or reset propagates.
  ...
  which is a re-attempt rather than a retry policy (`packages/code/src/adapters/marketplace.ts:318`).
...
(`packages/kernel/src/plugins/plugin-service.ts:70-75`,
`packages/loop/src/settings/marketplace-schema.ts:90-113`). The client-side marketplace clone keeps
an independent implementation with the same accepted transports and refusal messages at
`packages/code/src/adapters/plugin-install.ts:59-104`. `@clarvis/code` uses its own copy in
`fetchMarketplace` (`packages/code/src/adapters/marketplace.ts:14`, `:221-226`) because the marketplace
clone is a client-side seam that never goes through the kernel. Both copies are separately pinned
(`packages/kernel/tests/integration/plugin-service.test.ts:253-269`, `packages/code/tests/integration/plugin-install.test.ts:13-45`), so the duplication is guarded by
...
- ~~**Why the plugin-skill-root budget is `MAX_SKILL_ROOTS - 8` specifically.**~~ **Resolved by
  ...
  (`packages/kernel/src/plugins/plugin-contributions.ts:188`–`:204`). Still true: no test pins the
...
- ~~**The panel's displayed revision and a run's MCP-provenance revision can disagree, with no test
  ...
  **Resolved: each side's own local rationale is on record, and together they account for the
  asymmetry.** `inspectPlugin` (`packages/kernel/src/adapters/filesystem/plugin-repository.ts:127-160`) prefers live
  `git` state over the install record; `revisionOf` (`packages/kernel/src/plugins/plugin-contributions.ts:300-314`) prefers the
  install record over `git`, falling to a live `git rev-parse HEAD` only when "unmanaged local
  plugins have no install record" (`:302-303`'s comment). The protocol field `inspectPlugin` populates
  is itself documented as "Resolved Git revision of the **installed checkout**"
  (`packages/protocol/src/plugins.ts:124-125`, emphasis added) — i.e. the panel's job is to show what is
  *actually on disk right now*, so a plugin directory a user `git pull`-ed by hand outside Clarvis's
  ...
  manifest version as source identity**" (`:300`) — its caller needs a value that stays **pinned** to
  what Clarvis itself installed/recorded, because it becomes the run's MCP-provenance identity
  (`packages/kernel/src/tasks/task-provider-factory.ts:300-318`, threading `resolvedRevision` into a
...
- **`code`'s marketplace clone bypasses the kernel entirely.** `fetchMarketplace`
  (`packages/code/src/adapters/marketplace.ts:221-244`) calls the client-side `gitCloneAsync`, whose
  `Bun.spawn` lives at `packages/code/src/adapters/plugin-install.ts:112-137`, while plugin
