...
if [[ "$engine" == docker ]]; then
  ...
  cd packages/kernel
  bun test tests/integration/local-docker-runtime.e2e.test.ts tests/integration/runtime-docker-identity.e2e.test.ts --timeout 60000
  ...
  cd packages/kernel
  bun test tests/integration/local-docker-runtime.e2e.test.ts tests/integration/runtime-podman-isolation.e2e.test.ts --timeout 60000
else
